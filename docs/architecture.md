...
- **検証ゲートは2段ある**。どちらも「静かに壊れる」を防ぐためのもので、守備範囲が違う:
  - **① `pipeline:validate`（parsed の自己整合）**: 予算・決算データは「合計 = 内訳の和」
    「歳入 ≧ 歳出（決算）」等で自己検証できる。error が1件でもあれば `needs_review` となり
    normalize は通らない（人が確認して解消するか、明示的に `--force`）
  - **② `pipeline:derive` の出口（生成物どうしの整合）**: **validate は gen 同士を見ない**ので、
    「収録済みなのに実態とズレる」型は誰も守っていなかった（/coverage の件数と配信シャードの件数の
    食い違い・事業報告の収録漏れ・URL スラグ・年度間クロスチェーン・骨格予算の翌年度の説明・
    表示専用フィールドの汚染・系列色）。**問題があれば throw して derive を失敗させる**
- **出口ゲートは両方向に張る**（2026-09-12）。①②を足しても、そこまでのゲートは全部
  **「載っているものが正しいか」**しか見ておらず、**「載っていたものが消えたか」**を見る網が無かった。
  収録済みの座間が `BUDGET_SOURCES` の行ごと消えても、registry・parsed・validation・スラグは全部
  残るので **typecheck も build も出口ゲートも素通りする**（画面は「収録不可」と誤って断言した。handoff §4）。
  ⇒ **新しい台帳やレジストリを出口ゲートに繋ぐときは、順方向（台帳にあるものが実装されているか）だけでなく
  逆方向（実装から消えていないか）も張る**。`unrecordable.ts`（収録できたのに記録が残っていれば throw）・
  `skeleton-budgets.ts`（§11k の逆方向）・当初予算の配信漏れ（⑥）・スラグの逆方向（③'）が今その形。
  ⚠ **逆方向のゲートに除外リストを持たせない** — 除外リストは腐るし、腐っても静かなので同じ型を呼ぶ。
  除外が要るように見えたら、まず「その資料はなぜ配信されないのか」を設計で消す（`fixture: true` に寄せる）
