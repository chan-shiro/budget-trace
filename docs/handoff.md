...
- **データ整備状況ページ `/coverage`（2026-07-14）**: 「どこまで収録できたか（＝ToDo）」「原本を
  ...
  **レジストリ（sources.ts）と魚拓台帳（archives.json）・raw-meta から derive** するので手で更新不要・
  常に実態と一致する（`coverage.gen.ts`）。収録13団体×7データセットのマトリクス（空欄＝ToDo）、
  全62資料の sha256・取得日・魚拓/照合状態・③リンク、47都道府県の網羅、ライセンス区分
  （要許可2／未確認50／再配布可10）を出す。**scope から対象を解決できない資料は「未分類」として
  可視化**する（黙って落とさない — 実際この仕組みで山口市の 団体コード 誤記を検出した）。
