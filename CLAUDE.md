...
現状は**サーバーレスの UI プロトタイプ**。画面遷移はコンポーネント内 state（`st.screen`）で管理しつつ、URL クエリに同期（ディープリンク・共有・戻る/進む対応）する SPA。ディレクトリは [docs/architecture.md](docs/architecture.md) の `src/` レイアウトへ移行済み（`src/server/` `src/shared/` `src/test/` はスケルトンのみ）。

**カバレッジ階層**: 自治体は2階層。**full**（甲府市のみ・予算ベースの詳細画面）と **decision**（全1,741市町村・総務省決算ベース）。判定は団体コード（`D.tierOf`、`FULL_MUNIS`）。decision は款別歳出（→項）・歳入科目（→内訳）・1人あたり・財政指標・類似比較・決算経年まで。full 専用画面（主な事業・執行・評価・補正・前年当初比較）は decision では出さずリクエストチップにする。詳細・配信モデルは [docs/data-strategy.md](docs/data-strategy.md)。
