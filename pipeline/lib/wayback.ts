 ...
 *   **私たちの raw が正しい**。実ファイルが 1 MiB の倍数ちょうどになる確率は100万分の1程度で、
 *   かつ「raw より小さい」「sha 不一致」も同時に満たす必要があるので、**コピーが切れていること**は
 *   断定してよい。ただし**スナップショット自体の恒久欠損とは断定できない**（2026-08-12・#214）—
 *   打ち切りはクロール捕捉に固有で **SPN の保存は切られない**（大田 R4 は 5 MiB 超が無傷）ため、
 *   --force で SPN 再保存 → 実体化後に再実行すると解消できる。また**配信の切断が MiB 境界ちょうどに
 *   乗ることもある**（京都 R4・新宿 H29 — 完全なスナップショットのダウンロードが 5MiB/1MiB で切れ、
 *   再取得で全量一致した）。
