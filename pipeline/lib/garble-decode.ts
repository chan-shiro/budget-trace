...
export const GARBLE_CHAR_MAP: Record<string, string> = {
  ...
  "\u4811": "５",
  "\u4816": "：",
  // ---- 飯塚 R6「当初予算資料」の款別比較表から追加（2026-09-25・§13-39）。数字は三重 p.3 と同じ +0x3EAC 帯
  //      （`decodeGarbleBand`）で、帯の外に落ちる4字だけをここに置く。真の字は次で確定した:
  //   △ … 款3 利子割交付金 `500 / 4,000 / 䕧3,500` の算術（500 − 4,000 ＝ △3,500）。⚠ 三重の ▲ U+4566 の**隣**。
  //   ゴ・ル・フ … 款8「ゴルフ場利用税交付金」。同一様式で健全な R5・R7 の同じ款と1字ずつ突合。
  "\u4567": "△",
  "\u4742": "ゴ",
  "\u4779": "ル",
  "\u4763": "フ",
  ...
  "\u3735": "防",
  "\u203B": "※",
  // 2026-09-10（第28巡・野田 R8／川西 H30〜H28）。**2市が同じ7字で throw** した（同じ組版ソフトの帯）。
  // 真の字は同じ表の健全な年度（野田 R7）と突き合わせて確定した:
  //   表題「歳入歳出予算事項別明細書」の 項・明・細・書、歳入款8「ゴルフ場利用税交付金」の ゴ・ル・フ
  "\u386F": "項",
  "\u1AC2": "明",
  "\u28FD": "細",
  "\u1B69": "書",
