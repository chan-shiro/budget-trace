...
//   ⚠ **まだ拾えない原文がある**（2026-09-12 時点で unverified 196件）。**型に名前を付けない** —
//   「帰属表示」は①権利が誰に属するかの表示 ②引用元を明記せよという要求 の二義で、
//   群の名前にすると読む人ごとに切り方が変わる（レビューで実際に割れた）。
//   **「何が無いか」で書く**: **禁止・不許可を述べる述語が1つも無い**原文が残っている —
//   安城・盛岡・豊橋・米子（問い合わせ先だけ）／松阪・ひたちなか・福岡県・山口県（権利の所在だけ）／
//   四日市（引用元明記・改変不可・配布時は要確認）／岸和田・西尾・刈谷・野田・入間（`All Rights Reserved` だけ）。
//   **これらは語彙では拾えない**（拾おうとすると意訳になる）。
const licenseClassOf = (lic: string): "open" | "permission-required" | "unverified" =>
  /要許可|非営利|無断|複製・転用|転載を禁止|使用を禁止|(?:転載|複製|二次利用|引用)[^。]{0,20}(?:禁じ|禁止)|(?:許可|承諾|同意)(?:・(?:許可|承諾|同意))?(?:なく(?!ても|とも)|を得(?:ず|ない|ないまま))[^。]{0,30}(?:できません|禁じ|禁止|お断り)/.test(lic)
    ? "permission-required"
