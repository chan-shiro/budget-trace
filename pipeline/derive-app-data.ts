...
//   ⇒ **実測**（2026-09-12・ライセンス原文の異なり266種）: 新語彙が当たるのは23種で、
//   **open の語彙（政府標準利用規約・CC BY 等）と競合するものは0**。区分が動くのは
//   **小牧10・春日井6 の計16ソースで、いずれも unverified → permission-required**（閉じる側）。
//   **次に語彙を触るときも同じ実測をしてから入れること**（§9g）。
const licenseClassOf = (lic: string): "open" | "permission-required" | "unverified" =>
  /要許可|非営利|無断|複製・転用|転載を禁止|使用を禁止|(?:転載|複製|二次利用|引用)[^。]{0,20}(?:禁じ|禁止)|(?:許可|承諾|同意)なく[^。]{0,30}(?:できません|禁じ|禁止)/.test(lic)
    ? "permission-required"
