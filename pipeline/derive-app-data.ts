...
//   ⚠ **まだ拾えない型がある** — 「問い合わせ／届け出／連絡」だけの型（安城・盛岡・豊橋・米子）、
//   帰属表示だけの型（松阪・ひたちなか・福岡県・山口県）。⚠ **四日市は帰属表示と問い合わせの複合型**（「引用元を明記してください」と「印刷物などの形で配布する場合などは、個別にご確認下さい」の両方がある）、`All Rights Reserved` だけの型
//   （岸和田・西尾・刈谷・野田・入間）。**これらは語彙で拾える型ではない**（意訳になる）。
