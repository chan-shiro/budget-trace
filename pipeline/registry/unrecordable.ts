...
export const UNRECORDABLE: UnrecordableRecord[] = [
  ...
  },
  {
    ...
    categories: ["broken-text-layer"],
    reason:
      "この年度だけ当初予算の説明資料が掲載されておらず、款別の表は予算書のなかにしかない。" +
      "その予算書は表のページから文字が取り出せない。総括の2ページで取り出せるのは印刷されたページ番号の6字だけで、款や項が並ぶページも同じように読めない。" +
      "紙のスキャンではなく、書体は埋め込まれているのに表の文字だけが読み出せない状態になっている。",
