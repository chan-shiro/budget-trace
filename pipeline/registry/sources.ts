...
export const SOURCES: SourceEntry[] = [
  ...
  ...([
    // 郡山市（福島県・中核市・団体コード 072036。⚠ 収録済みの福島県 070001 とは別法人・別サイト・別条項）。
  ...
  ...([
    // いわき市（福島県・中核市・団体コード 072044。⚠ 収録済みの福島県 070001・郡山市 072036 とは別サイト・別条項）。
  ...
  ...([
    ...
    // ⚠ **H29 以前は収録不可**（`unrecordable.ts`）— テキスト層が金額のカンマ・小数点の直後に空白を入れる
    //    （`38, 896, 003`）ため桁ごとに割れて Σ が桁違いにずれる（**大声で落ちる型**）。
  ...
  ...([
    ...
    // ⚠ **R5〜H25 の総括表 PDF には「盛岡市」の字が1つも無い**（川越・高槻 §13-9 型）。同一性は取得元 URL と
    ...
    //    議会修正なし・骨格予算の年度なし。⚠ **H24 以前は款別が無い**（HTML 本文の9区分集約＋「億万円」表記）。
  ...
  ...([
    ...
    // ⚠⚠ **R5 以前は収録不可**（`unrecordable.ts`）— 予算書が H23〜R5 の16年度とも**全ページ スキャン画像**で
