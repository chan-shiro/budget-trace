...
export const SOURCES: SourceEntry[] = [
  ...
  },
  {
    ...
    //   リンクを拒むものではありません」としつつ、**①リンクしたら pr@pref.shizuoka.lg.jp へリンク元・
    //   リンク先の URL を連絡することを求め**、**②注意事項で「最初に読むべきページを飛ばし、記載内容が
    //   十分に伝わらないページへの直接リンクは避けてください」と明記**している。①は新潟（事前に電子メールで
    //   連絡＝立てる）、②は広島・宮城（トップページ以外は相談＝立てる）と同じ側で、しかも②は
    //   **PDF 直リンクをまさに名指し**しているので、掲載ページへ振り替える。
    noDeepLink: true,
    license:
