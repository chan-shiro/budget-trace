...
export const SOURCES: SourceEntry[] = [
  ...
  ...([
    ...
    // ⚠⚠ **R8・R4 は骨格予算**（原典 p.4 の脚注「※H22、H26、H30、R4、R8は骨格予算編成」）。**当年度側が骨格**なので
    //   prevNote には書かない（§11k・長崎 R8 型）。R8 の6月補正（肉付け）は一般会計 +1,025,271千円。
  ...
  ...([
    ...
    // 骨格予算: 収録した**予算書6ファイル**を `骨格|肉付` で全文検索して **0件**（自分で実測）。
    //   ⚠ `市長選` は R8 に10件・R7 に19件・R5 に1件・R4 に4件出るが、**すべて款項目の `4 市長選挙費`
    //   `162 市長選挙執行経費` そのもの**で骨格の記述ではない。
    //   ⚠ この検索は予算書だけに当てたもので、他市の前例（弘前=5月補正概要・大垣=当初予算の概要・
    //   今治=記者発表・朝霞=当初予算資料）では**骨格の明記は予算書の外にある**。多摩の「予算の概要」は未確認。
