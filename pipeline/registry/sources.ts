  ...
  ...([
    ...
    parserOptions: {
      ...
      ...(fy === "R5"
        ? {
            prevNote:
              "前年度（令和4年度）の当初予算は骨格予算（令和4年度5月補正予算（第1号）の概要に「令和４年度当初予算は、義務的経費、経常的経費及び総合計画に位置付けられている事業を中心とした骨格予算」「骨格予算に政策的経費を肉付け」と明記）。ここでの前年度額はその骨格予算の当初額（78,820,000千円）で、肉付け後の額（80,062,157千円）ではない。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
