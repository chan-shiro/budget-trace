...
export const SOURCES: SourceEntry[] = [
  ...
  ...([
    ...
    //   物理ページは年度で動く（R8 25/26・R7〜R4 21/22・R3 19/20・自分で実測）。印字ノンブル＝物理。
    ...
    parserOptions: {
      ...
      ...(fy === "R8"
        ? { prevNote: "前年度（令和7年度）の当初予算は骨格予算（令和7年度当初予算案の概要の表紙に〔骨格予算〕と明記。令和8年度の概要も「令和７年度当初予算は骨格予算であったため、６月補正後予算額と比較」と注記）。ここでの前年度額はその骨格予算の当初額で、6月補正後の額ではない。" }
        : fy === "R4"
          ? { prevNote: "前年度（令和3年度）の当初予算は骨格予算（令和4年度当初予算案の概要に「令和３年度当初予算は骨格予算であったため」と明記）。ここでの前年度額はその骨格予算の当初額で、6月補正後の額ではない。" }
          : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    ...
    // ⚠ 款名は原典どおりで寄せない: H24 以前 `寄付金`（H25〜 `寄附金`）、H30 以前 `交通安全特別交付金`（R7〜 `交通安全対策特別交付金`）。
