...
const FUCHU_TOKYO_LICENSE =
  "府中市ホームページ自体及び府中市ホームページに掲載されている情報は、オープンデータとして公開されている項目を除いて、すべて著作権の対象となります。著作権は日本国著作権法および国際条約により保護されています。府中市ホームページ内のすべての情報について、無断転載はご遠慮ください。";

// 川西市の前年度列は「当初」だが毎年**組替後**で、歳出の一部の款が前年度資料の当年度額と食い違う（総額と歳入は一致）。
// 原典に組替の注記が無い年（R7・R6・H30）なので、**自分で款単位に突合して測った事実だけ**を書く（§13-28・高岡 H26 と同じ扱い）。
const KAWANISHI_PREV_NOTES: Record<string, string> = {
  R7: "前年度（令和6年度）の額は令和6年度当初予算を組み替えたもので、民生費 30,056,550千円・教育費 5,878,339千円は令和6年度の当初予算額（30,050,114千円・5,884,775千円）と各 6,436千円ずれる（歳入の全款と総額 63,349,000千円は一致）。原典に組替の注記は無い。",
  R6: "前年度（令和5年度）の額は令和5年度当初予算を組み替えたもので、総務費・民生費・商工費・土木費・教育費の5款が令和5年度の当初予算額とずれる（最大は土木費の 16,508千円。歳入の全款と総額 56,851,000千円は一致）。原典に組替の注記は無い。",
  H30: "前年度（平成29年度）の額は平成29年度当初予算を組み替えたもので、総務費 6,441,561千円・商工費 472,019千円は平成29年度の当初予算額（6,621,142千円・292,438千円）と各 179,581千円ずれる（歳入の全款と総額 54,569,000千円は一致）。原典に組替の注記は無い。",
};

const KOMAKI_LICENSE =
  "小牧市ホームページに掲載されている文章、写真、イラスト、画像等の著作権は、小牧市または著作権者の方にあります。著作権法上認められた場合を除き、著作権者の許可なく文章、写真、イラスト、画像等の全部または一部を利用することは禁止します。";
...
export const SOURCES: SourceEntry[] = [
  ...
  ...([
    // つくば市（茨城県・団体コード 082201。⚠ 収録済みの水戸市 082015・つくばみらい市 082325 とは別団体）。
    // 「当初予算（案）概要」の**３ 一般会計予算（案）（１）歳入 款別 /（２）歳出 款別**（歳入21〜23款・歳出14款）。
    // ⚠⚠ **同じ PDF に一般会計以外の総括表が7つ同居する**（R8 の物理ページ）— p.4 会計別の規模／
    //    p.12「（３）歳出 性質別」（**列構成も合計 122,710,000 も歳出款別と同一**）／p.15 国保・p.16 後期高齢・
    //    p.17 作岡財産区＋公平委員会・p.18 介護（**節ラベルが素の `歳入` `歳出`・合計ラベルも `歳入合計` `歳出合計` で
    //    一般会計と1文字も違わない**）／p.19〜22 水道・下水道。
    //    ⚠⚠ 偵察が実測: **見出しを素の `歳入` にして p.15 を当てると国保が Σ 差0 で完走する**。
    //    唯一の網が **`（１）歳入` / `（２）歳出` という一般会計にしか無い見出し**なので、**短くしないこと**。
    //    ⚠ **登録済みの見出しのままページを取り違えた場合は、どのページでも「見出しがありません」で throw する**
    //    （p.12 の実見出しは `（３）歳出 性質別` なので `（２）歳出` にも当たらない。収録レビューが実測）。
    //    ⚠ 「款行が1件も抽出できませんでした」で落ちるのは**見出しを素の `歳入` 等に縮めたとき**の話で、
    //    条件が違う。**どちらでも throw する＝安全側**だが、ページずれを診断するときにメッセージで判断を誤らないこと。
    // ⚠ **歳入合計＝歳出合計**なので、歳入/歳出を取り違えても Σ では捕まらない（見出しが網）。
    // ⚠ 物理ページは 5/9 と 6/10 の2系統で**年度により入れ替わる**（規則性なし）。ファイル名も毎年バラバラ。
    // ⚠ 年度ページの ID が**2系統ある**（`/shisei/joho/zaisei/26559.html` と `/soshikikarasagasu/…/26479.html`）。
    //    **ID を系統間で流用すると 404**（偵察が実測）。
    // ⚠ 款番号は `1.` のピリオド付き（松戸型・パーサが除去済み）。款名は上下段折返しだが名前欄が空になる型なので
    //    既存の awaitTail が拾う。
    // ⚠⚠ **R2 だけ廃止款の款番号欄が `×.`**（U+00D7 ＋ ASCII ピリオド）で、この巡でパーサの廃止マーカーに
    //    `×` 類と直後のピリオドを足すまでは**款名が `自動車取得税×.` になり下段 `交付金` が落ちていた**
    //    （金額は当年度0／前年度121,046 で正しく **Σ は4系統とも差0**＝目視でしか気づけない型）。
    // ⚠ 歳入の款構成が動く: H30〜H25=21款／H31=22款（`環境性能割交付金` が皆増で新設）／
    //    R2=23款（`法人事業税交付金` 新設＋廃止款 `自動車取得税交付金`）／R3 以降=22款。
    // ⚠ 歳出款13・14 は **`諸支出金`・`予備費`**（独自款）。寄せない。
    // 前年度列は**当初**（H26↔H25 から R8↔R7 まで**13ペア全部・全款**を突合して不一致0・偵察が実測。列順の反転なし）。
    // ライセンス「著作権」（/about/1002981.html・更新 2023-03-01・確認日 2026-08-23）。
    //    ⚠⚠ **市のオープンデータ（CC BY 4.0）は規約自身が「本規約は、つくば市公式ホームページ掲載の全ての情報に
    //    該当するものではありません」と範囲を限る**。予算ページの CC BY 表示は同ページの**別セクション**にある
    //    款項目節 CSV（R3〜R7）に付いており、**この概要 PDF は対象外**（§9g）。
    //    ⚠ 決算書 PDF と予算 CSV には CC BY が及ぶので、**それらを別ソースで収録するときは license を混ぜない**
    //    （同一自治体で区分が分かれる）。
    //    ⚠ リンク条項は「サイト内のどのコンテンツにリンクいただいても結構です」＝**トップページ指定は無い**ので
    //    `noDeepLink` は立てない。ただし**フレーム内表示を断っている**ので発行元ページを iframe に入れる導線は作らない。
    // [年度, ファイル名, 歳入ページ, 歳出ページ, landing ID]
    ["R8", "R8yosan_gaiyou.pdf", 5, 9, "26559"],
    ["R7", "R7yosan_gaiyou.pdf", 6, 10, "22193"],
    ["R6", "R6yosan_gaiyou.pdf", 6, 10, "16931"],
    ["R5", "R5toushoyosan_gaiyou.pdf", 6, 10, "1018857"],
    ["R4", "R4_toshoyosan_gaiyo.pdf", 6, 10, "1015990"],
    ["R3", "R3_tousyoyosann_gaiyou.pdf", 6, 10, "1013446"],
    ["R2", "gaiyou.pdf", 6, 10, "1009169"],
    ["H31", "H31_gaiyou.pdf", 5, 9, "1005993"],
    ["H30", "H30_gaiyou.pdf", 5, 9, "1003389"],
    ["H29", "H29tousyoyosangaiyou.pdf", 5, 9, "1002377"],
    ["H28", "h28gaiyou.pdf", 5, 9, "1002378"],
    ["H27", "h27gaiyou.pdf", 5, 9, "1002379"],
    ["H26", "H26_gaiyou.pdf", 5, 9, "1002380"],
    ["H25", "H25_gaiyou.pdf", 6, 10, "1002381"],
  ] as const).map(([fy, file, rev, exp, page]) => ({
    id: `tsukuba-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 つくば市当初予算（案）概要（一般会計 歳入・歳出 款別）`,
    publisher: "つくば市",
    url: `https://www.city.tsukuba.lg.jp/material/files/group/22/${file}`,
    landingPage: `https://www.city.tsukuba.lg.jp/shisei/joho/zaisei/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "つくば市（一般会計・団体コード082201）",
    license:
      "本サイト上の各ファイル、及びその内容に関しての諸権利は、特記事項がない限り、原則としてつくば市に帰属します。本サイト上の情報を個人的に利用する場合、営利目的で利用されないという前提において複製(端末からのプリントアウト、ダウンロード等)が認められるものとしますが、改変等は認められません。また個人的な利用であっても著作権等に関する表示を削除してはいけません。市章等、本サイト上の画像を許可なく使用・転載することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
  ...
  ...([
    // 平塚市（神奈川県・団体コード 142034。⚠ 収録済みの横浜141003・川崎141305・相模原141500・
    //   横須賀142012・藤沢142051 とは別物）。
    // 「当初予算の概要」の**「令和N年度 一般会計予算と前年度予算との比較表」**（歳入22〜23款・歳出12〜13款）。
    // ⚠ 物理ページは R7〜R3 が 5/6、**R2 だけ 4/5**（印字ノンブルは全年度 －3－/－4－ で、ずれ幅が年度で違う）。
    // ⚠⚠ **同じ PDF の p.8（R2 は p.7）が「財政構造（一般会計）」で、`（歳入）`『（歳出）』の小見出しも
    //    合計ラベル `合　計` も款別表と1文字も違わない**（歳入＝自主/依存財源・歳出＝性質別を1ページに縦積み）。
    //    ⚠ **ただし款番号が無いので当てても throw する**（偵察が実測。八尾 §13-13 の「Σ差0 のまま完走」型ではない）。
    //    それでも**物理ページは年度ごとに固定する**。
    // ⚠ 歳入側の見出しに `一般会計予算と前年度予算との比較表`（款別表のページにしか無い語）を使うと、
    //    ページを1つずらしたときも見出し不一致で落ちる。歳出側は `（歳出）` しか無いので代替が効かない。
    // ⚠ 合計ラベルは `合　計`（既定の「歳入合計/歳出合計」では当たらない）。
    // ⚠ 廃止款は R3 歳出 `○諸支出金`（U+25CB）・R2 歳入 `〇自動車取得税交付金`（U+3007）。
    //    **同じ市の隣り合う年度で記号が違う**（葛飾・中央と同型）。どちらもパーサが kanNo=null で拾う。
    // ⚠ 歳出款の並びが R2 と R3 以降で違う（R2 は 12=`諸支出金` / 13=`予備費`、R3 以降は 12=`予備費`）。寄せない。
    // ⚠⚠ **R2 の前年度（令和元年度当初）は骨格予算** — H31 概要 p.1 に「＊平成31年度は骨格的予算」と明記。
    //    ⚠ **R2 の概要には「骨格」の語が0件**＝原典が何も断っていないので、こちらで `prevNote` を付ける。
    // 前年度列は**当初**（列見出しが `当初予算額` と明記。加えて R7→R2 の**5ペア全款**を突合して不一致0）。
    // ⚠⚠ **R8 と H31〜H27 は収録不可**（`unrecordable.ts` に記録・どちらも収録時に自分で実測）。
    // ライセンス「平塚市ウェブサイトの考え方」（/koho/page-c_00374.html・確認日 2026-08-23）。
    //    ⚠ オープンデータライブラリは CC BY 4.0 だが「当ライセンスは、平塚市オープンデータライブラリの
    //    データセット一覧に記載のデータのみに適用されます」と自ら範囲を限り、**6カテゴリ全27ファイルに
    //    予算・決算・財政は0件**＝及ばない（§9g）。
    //    ⚠ リンク条項は「リンクはご自由に」「**トップページへのリンクを推奨します**」＝**推奨どまり**なので
    //    `noDeepLink` は立てない（水戸・八尾 §13-13 の「トップページとしてください」とは文言が違う）。
    //    ⚠ フレーム内表示を断る条項も無い。
    // [年度, ファイルID, 歳入ページ, 歳出ページ]
    ["R7", "200172091", 5, 6],
    ["R6", "200149782", 5, 6],
    ["R5", "200149779", 5, 6],
    ["R4", "200101452", 5, 6],
    ["R3", "200081415", 5, 6],
    ["R2", "200064292", 4, 5],
  ] as const).map(([fy, fid, rev, exp]) => ({
    id: `hiratsuka-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 平塚市当初予算の概要（一般会計予算と前年度予算との比較表）`,
    publisher: "平塚市",
    url: `https://www.city.hiratsuka.kanagawa.jp/common/${fid}.pdf`,
    landingPage: "https://www.city.hiratsuka.kanagawa.jp/keikaku/page-c_02405.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "平塚市（一般会計・団体コード142034）",
    license:
      "平塚市ウェブサイトに掲載されている文書、写真、画像、イラスト等の各ファイル及びその内容に関する諸権利は、原則として平塚市に帰属します。また、一部の画像等の著作権は、原著作者が所有しています。これらの情報は、「私的使用のための複製」や「引用」などの著作権法上認められた場合を除き、無断の転用・引用を禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "一般会計予算と前年度予算との比較表",
      expenditureHeading: "（歳出）",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(fy === "R2"
        ? {
            prevNote:
              "前年度（令和元年度＝平成31年度）の当初予算は、市長選挙のため骨格予算として編成されている（原典「平成31年度当初予算の概要」p.1 に「＊平成31年度は骨格的予算」と明記）。ここでの前年度額はその骨格予算の当初額。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 福井市（福井県・中核市・県庁所在地・団体コード 182010）。
    // ⚠ 収録済みの**福井県 180009 とも 福島市 072010（`fukushima-shi-*`）とも別物**。
    // 「当初予算（案）の概要」の**巻頭「一般会計歳入歳出予算総括表」**（歳入22〜23款・歳出13款・千円）。
    // ⚠ **同じ年度ページに並ぶ「予算資料」PDF は使えない** — R8 は歳出が性質別しか無く、R7・R5・R2 は**百万円**
    //    （R7 は原典が「表示単位未満を端数処理しているため内訳の計と一致しない場合があります」と自ら断っており
    //    Σ が ±1〜2 ずれて error）、R6・R4・R3 は**スキャン画像**（全文の抽出が 301 / 208 / 349 字）。
    // ⚠ 年度ページ URL に規則が無い（`p071963` / `p070783` / `p026947` / `p025604` / `yosan-r4` / `yosan-r3` / `p021700`）。
    //    ファイル名も R8 だけ `R8yosannogaiyou.pdf`、**R7 だけ `R7syuyouzigyou.pdf`（`zi`）**で不揃い。外挿しない。
    // ⚠ 総括表の物理ページは **R7 だけ 5/6**（他年度は 6/7）。印字ノンブルは全年度 -4-/-5-。
    // ⚠ `一般会計歳入歳出予算総括表` の語は**各年度の PDF 全体で1ページにしか出ない**（全ページ走査で偵察が実測）。
    //    ⚠ 隣接ページを当てた場合は throw する（p.5 会計別＝款行0件・p.8 性質別＝歳出合計が無い）。
    // ⚠⚠ **別ルートの「予算に関する説明書」（R8 のみ公開・533p）には八尾 §13-13 型の罠がある** —
    //    一般会計の `歳入歳出予算事項別明細書 １ 総括`（物理 p.11）と**国民健康保険特別会計の同名・同列構成の総括**
    //    （物理 p.226）ほか特別会計9本が同居する。**このルートは使わない**。
    // ⚠ 歳出は **13款で `諸支出金` を持たず 14 が `予備費`**（款番号が 13 を飛ばす）。原典どおり・寄せない。
    // ⚠ 廃止款の接頭辞は `○`（U+25CB）＋`皆減`（R8 の環境性能割交付金・R2 の自動車取得税交付金）。
    // 前年度列は**当初**（R8→R2 の6ペア＋H31 を全款突合して不一致0。差は廃止款だけ。列反転なし）。
    // ライセンス「福井市サイトポリシー」§1 著作権について＋【CCライセンス】
    //    （https://www.city.fukui.lg.jp/sisei/kohou/hp/site-p.html・最終更新 2017-10-23・確認日 2026-08-23）。
    //    ⚠ 冒頭が「福井市公式ホームページ（ドメイン city.fukui.lg.jp）のご利用に当たっては」＝**サイト全体**が対象。
    // ⚠⚠ **ライセンスの扱いに論点がある** — 福井市は珍しく**サイト全体の基本ライセンスを CC－BY－SA と宣言している**
    //    のに、**同じ条項に「無断で利用することはできません」が同居する**ため `licenseClassOf` は
    //    `permission-required` に落ちる。⚠ CC の段落だけを切り出すと `unverified` になる
    //    （`CC－BY－SA` は**全角ハイフン**なので `CC[ -]?BY` に当たらない）。
    //    **原文を切り出さずに丸ごと引き、保守側（要許可）で登録する**（§9g の「意訳・切り出しをしない」）。
    //    ⚠ `/coverage` 上は実態より制限的に見えるが、**こちらの都合で発行元の文面を編集しない**。
    // ⚠ オープンデータパーク（`/sisei/tokei/opendata/`・全9カテゴリ **124データセット**）の CC-BY 2.1 JP は
    //    「公開しているデータ」に自ら範囲を限り、**予算・決算・財政は0件**（`財政` はカテゴリ名「行財政」だけ）＝
    //    本 PDF に及ばない（§9g）。
    // ⚠ リンク条項は「トップページへのリンクは、原則として自由です」＋リンク元の内容による拒否のみで、
    //    **深リンクを断る文言もフレーム内表示の禁止も無い**ので `noDeepLink` は立てない（前橋 §13-10 と同型）。
    // ⚠ H31〜H22 は年度ページごと 404 で Wayback にしかない（H31 は同一様式・try-parse 差0 を偵察が実測）。
    //    **横展開優先の方針で今回は現行サイトの7年度に絞った**（`unrecordable.ts` には載せない＝収録できないのではない）。
    // [年度, 年度ページID, ファイル名, 歳入ページ, 歳出ページ]
    ["R8", "p071963", "R8yosannogaiyou.pdf", 6, 7],
    ["R7", "p070783", "R7syuyouzigyou.pdf", 5, 6],
    ["R6", "p026947", "R6syuyoujigyou.pdf", 6, 7],
    ["R5", "p025604", "R5syuyoujigyou.pdf", 6, 7],
    ["R4", "yosan-r4", "R4syuyoujigyou.pdf", 6, 7],
    ["R3", "yosan-r3", "R3syuyoujigyou.pdf", 6, 7],
    ["R2", "p021700", "R2syuyoujigyou.pdf", 6, 7],
  ] as const).map(([fy, page, file, rev, exp]) => ({
    id: `fukui-shi-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 福井市当初予算（案）の概要（一般会計歳入歳出予算総括表）`,
    publisher: "福井市",
    url: `https://www.city.fukui.lg.jp/sisei/zaisei/yosan/${page}_d/fil/${file}`,
    landingPage: `https://www.city.fukui.lg.jp/sisei/zaisei/yosan/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "福井市（一般会計・団体コード182010）",
    license:
      "福井市公式ホームページに掲載されている文書、写真、画像及び映像に関する著作権は、福井市又は原著作者に帰属しています。私的使用のための複製、引用等著作権法上、国際条約上認められた範囲内及びクリエイティブ・コモンズ・ライセンス（以下「CCライセンス」という。）により表示するライセンス条件の範囲内で使用する場合を除き、これらを無断で利用することはできません。著作権法上、国際条約上認められた範囲及びCCライセンスにより表示するライセンス条件の範囲を超える利用を希望する場合には、各ページの担当所属に直接ご相談ください。【福井市公式ホームページのCCライセンス】福井市公式ホームページのCCライセンスの基本ライセンスは、写真、画像、映像及び個別に設定する場合を除き、CC－BY－SA（表示－継承）とします。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "一般会計歳入歳出予算総括表",
      expenditureHeading: "歳出",
    },
  })),

  ...([
    // 長岡市（新潟県・特例市→中核市・団体コード 152021）。
    // ⚠ 収録済みの**新潟市 151009**、および**長野市 202011（`nagano-shi-*`）とは別物**。id は `nagaoka-`。
    // 「当初予算（案）【計数資料編】」の**歳入款別予算の状況 / 歳出目的別予算の状況（一般会計）**（歳入22〜23款・歳出12〜13款）。
    // ⚠⚠ **`dashAsZero` の要否が年度ごとに逆転する**（偵察が全年度で実測）—
    //    **R7・R2 は必須**（皆増の款の前年度セルが `-` で、指定しないと**右隣の現計予算額を掴む**。
    //    R7 は歳出前年度Σ +150,159・R2 は歳入前年度Σ +340,000）。
    //    **R8・R4 は立ててはいけない**（廃止款の款番号欄が `-` なので 0 に化けると廃止款の分岐に乗らず
    //    **行ごと消える**。R8 は歳出前年度Σ −61,314・R4 は歳入前年度Σ −1）。
    //    ⚠⚠ **前年度Σ の不一致は validate では warning 止まり**なので、**年度ごとに明示指定する**（外挿しない）。
    // ⚠ **R8 だけ節の並びが違う**（R8 は 3 歳入款別・5 歳出目的別、R7 以前は 2・4）。物理ページも R8 が 5/7、他は 4/6。
    // ⚠ 列は `[当年度A, 前年度当初B, 前年度現計C, A-B, A-C]` で、**原典が当初と現計を別列で持っている**ため
    //    前年度が補正後になる余地が構造的に無い（R8 款13 は B=1,194,081／C=1,170,519 で B を取れている）。
    // ⚠ 款番号の付き方が年度で動く（R7 は災害復旧費が款11・R3/R2 は自動車取得税交付金が款9 で以降が1つずれる）。寄せない。
    // ⚠ 各表に「（借換債を除く）」等の補助行があるが款番号が無く、Σ 差0＝款として拾われないことを実測済み。
    // ⚠⚠ **予算書ルート（`r0Nj-03.pdf` 予算に関する説明書）には八尾 §13-13 型の罠がある** —
    //    「歳入歳出予算事項別明細書 １ 総括」が**一般会計＋特別会計で5組**あり見出しが1文字も違わない。
    //    **計数資料編を使えばこの罠に入らない**（款別の表は一般会計の歳入・歳出それぞれ1つずつしか無い）。
    // ⚠ 計数資料編で隣接ページを当てた場合は throw する（p.3 会計別＝款行0件、p.6/p.8 は合計ラベルが `合計` で不一致）。
    // ⚠ **R3・R2 は発行元から消えており Wayback の `id_` 実体 URL から取る**（`pipeline:archive` の対象外になるのは正常）。
    // ⚠ **R6 は収録不可**（`unrecordable.ts`）。⚠ H31・H30 はスキャン画像・H29 以前は未調査なので台帳に載せない。
    // ⚠ **R8 の年度ページはまだ更新中**（最終更新 2026-06-23）。`-01a` がさらに `-01b` へ差し替わる可能性がある
    //    （R6 に `-01` → `-01a` の履歴がある）。再取得時はリンク先のファイル名を確認すること。
    // 前年度列は**当初**（R8→R2 の隣接ペアを全款突合して不一致0。さらに**別系統の CC BY の CSV**とも
    //    R7/R5/R4/R3/R2 の当年度が全款一致＝2系統で裏取り）。
    // ライセンス「免責事項について」（/other/menseki.html・確認日 2026-08-23）。
    //    ⚠⚠ **長岡は他市と違い、オープンデータに予算・決算が実際に登載されている**（財政カテゴリ6データセット＝
    //    歳入款別/歳出目的別の**当初予算額 H24〜R7 の14年度**と決算額・CC BY 4.0）。**ただし登載は CSV だけで、
    //    この計数資料編 PDF は含まれない**（規約も「『長岡市オープンデータ』と明示して掲載する情報」に範囲を限る）。
    //    **PDF の license 欄に CC BY を書くと §9g の逆方向の事故になる**。
    //    ⚠ リンク条項は「各コンテンツへの直接リンクを設定していただいてかまいません」＝**深リンクを明示的に許容**
    //    （フレーム内表示の禁止も無い）ので `noDeepLink` は立てない。ただし「長岡市提供のものであると
    //    正しく認識できるように」という条件があるので**出典表示は必須**（画面は出典チップで満たしている）。
    // [年度, url, landing, 歳入ページ, 歳出ページ, dashAsZero]
    ["R8", "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r08j-01a.pdf",
      "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r08yosan.html", 5, 7, false],
    ["R7", "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r07j-01.pdf",
      "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r07yosan.html", 4, 6, true],
    ["R5", "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r05j.pdf",
      "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r05yosan.html", 4, 6, false],
    ["R4", "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r04j.pdf",
      "https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r04yosan.html", 4, 6, false],
    ["R3", "https://web.archive.org/web/20211207150033id_/https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r03j.pdf",
      "https://web.archive.org/web/20251220035045/https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r03yosan.html", 4, 6, false],
    ["R2", "https://web.archive.org/web/20210123192055id_/https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/file/r02j.pdf",
      "https://web.archive.org/web/20220810083432/https://www.city.nagaoka.niigata.jp/shisei/cate03/zaisei/r02yosan.html", 4, 6, true],
  ] as const).map(([fy, url, landing, rev, exp, dash]) => ({
    id: `nagaoka-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 長岡市当初予算（案）計数資料編（一般会計 歳入款別・歳出目的別）`,
    publisher: "長岡市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "長岡市（一般会計・団体コード152021）",
    license:
      "当サイト上に掲載している内容（文章、写真、図、イラスト等）に関する著作権は、原則として長岡市が所有しています。当サイトの内容について、「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。引用・転用・転載などをご希望の場合には、コンテンツ内に記載された担当課へ直接ご連絡ください。「長岡市オープンデータ」のコンテンツについては、「長岡市オープンデータ利用規約」により取扱を定めます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "歳入款別予算の状況",
      expenditureHeading: "歳出目的別予算の状況",
      ...(dash ? { dashAsZero: true } : {}),
    },
  })),

  ...([
    // 加古川市（兵庫県・団体コード 282103）。⚠ 同県の神戸281000・姫路282014・尼崎282022・
    //   **明石282031（id が `akashi-` で紛らわしい）**・西宮282049 とは別条項・別資料。id は `kakogawa-`。
    // 「予算のポイント」（企画部）の**p.4「２－① 一般会計款別集計（歳入）」/ p.6「３－① …（歳出）」**
    //   （歳入21〜22款・歳出12款・千円）。⚠ 物理ページは**7年度とも 4/6 で動かない**（印字ノンブルとも一致）。
    // ⚠⚠ **表の右端に「主 な 増 減 理 由」欄がある** — `-layout` でも `-raw` でも款行と同じ行に入り、
    //    理由文に `現年度分` のような **`年度`** を含む行が `KAN_HEADER_RE` に当たって**款ごと消える**
    //    （R5 歳入の款14 が落ちて Σ −459,268。＝ゲートは捕まえる）。⇒ `CropX` で理由欄を切り落とす。
    // ⚠⚠ **crop の右端は版面幅で決まり、R5 だけ違う** — R8/R7/R6/R4/R3/R2 は A4 横 841.9pt で **405**、
    //    **R5 だけ版面が 708.72pt なので 340**。⚠ **年度を外挿しない** — R5 に 405 を当てると
    //    **歳出だけ**理由文の折返しが款名に食い込み `庁舎維持総務費` `妊娠出産尿終末処衛生費` になるが、
    //    **Σ は差0 のまま**なので款名の目視でしか気づけない（過去の教訓 と同型＝右で境界を引く）。
    // ⚠⚠ **同じ PDF の p.8「４ 一般会計性質別集計」は 13 区分で合計も同額** — 見出しを `一般会計` などと
    //    短く書くと**当年度Σ 差0 のまま通り、款名が `人件費` `扶助費` になる**（偵察が実測）。
    //    **見出しは必ず `一般会計款別集計` と書く**（前橋 §13-10・明石 §13-11・久留米 §13-12 と同型の4例目）。
    //    ⚠ 見出しが crop に入らないため歳入・歳出で分けられない。**側の取り違えは合計ラベルが捕まえる**
    //    （p.6 を歳入に当てると「歳入合計 行が見つかりません」で throw する＝実測）。
    // ⚠ 空セルの `-` があるので `dashAsZero`（R8 の環境性能割交付金＝皆減／R2 の法人事業税交付金＝新設・
    //    自動車取得税交付金＝廃止）。R2 は指定しないと当年度 +120,000・前年度 −29,000 でずれる。
    // ⚠ 歳入の款番号に**欠番がある**（9・21。R2 だけ 9 に `自動車取得税交付金` が入る）。原典どおり・詰めない。
    // ⚠ 歳出款14 は **`予備費`**（独自款）・款11/13 は欠番。寄せない。
    // ⚠ **「予算概要」PDF は R5 以前がスキャン画像**（29p で全文202字 等）なので、年度を通して使えるのは
    //    「予算のポイント」の方。R8〜R6 の概要はテキストがあるが**ルートを混ぜない**。
    // ⚠ **R3・R2 は発行元から削除済み（404）で Wayback から取る**。⚠⚠ **タイムスタンプを他へ振り替えないこと** —
    //    同 URL の 2021/2023/2025 の捕捉は**1 MiB ちょうどの打ち切り**（§9b）で pdftotext が開けず、
    //    **2024-07-11 の捕捉だけが完全**（偵察が実測）。
    // ⚠ **R1（H31）・H30・H29 は収録不可**（`unrecordable.ts`）— 原本が 404 で、魚拓が全捕捉とも 1 MiB 打ち切り。
    //    **原本が消えているので SPN で取り直すこともできない**。
    // 前年度列は**当初**（R8→R2 の6ペアで歳入21款・歳出12款が1円単位で完全一致。市税1款だけの照合ではない）。
    // ライセンス「著作権・免責事項」（更新 2020-01-31・確認日 2026-08-23）。
    //    ⚠ オープンデータ規約（CC BY 4.0）は「加古川市オープンデータカタログサイトで公開している情報」に
    //    自ら範囲を限り、**カタログ80データセットに予算0件・財政0件**（CKAN API で実検索）＝及ばない（§9g）。
    //    ⚠ **リンクに関する条項のページ自体が存在しない**（フッターの「リンク」は関連機関へのリンク集）ので
    //    `noDeepLink` は立てない。
    // [年度, url, crop の右端]
    ["R8", "https://www.city.kakogawa.lg.jp/material/files/group/9/R8point.pdf", 405],
    ["R7", "https://www.city.kakogawa.lg.jp/material/files/group/9/20250226.pdf", 405],
    ["R6", "https://www.city.kakogawa.lg.jp/material/files/group/9/rokunendoyosannpointo.pdf", 405],
    ["R5", "https://www.city.kakogawa.lg.jp/material/files/group/9/jifuisdhaskf.pdf", 340],
    ["R4", "https://www.city.kakogawa.lg.jp/material/files/group/9/r4yosannopointo.pdf", 405],
    ["R3", "https://web.archive.org/web/20240711215337id_/https://www.city.kakogawa.lg.jp/material/files/group/9/R03-yosanpoint.pdf", 405],
    ["R2", "https://web.archive.org/web/20240711215439id_/https://www.city.kakogawa.lg.jp/material/files/group/9/R2_yosanpoint.pdf", 405],
  ] as const).map(([fy, url, cropTo]) => ({
    id: `kakogawa-yosanpoint-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 加古川市予算のポイント（一般会計款別集計）`,
    publisher: "加古川市",
    url,
    landingPage:
      fy === "R3" || fy === "R2"
        ? "https://web.archive.org/web/20210922170359/https://www.city.kakogawa.lg.jp/soshikikarasagasu/kikakubu/zaiseika/yosan/1529392713015.html"
        : "https://www.city.kakogawa.lg.jp/soshikikarasagasu/kikakubu/zaiseika/yosan/1529392713015.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "加古川市（一般会計・団体コード282103）",
    license:
      "加古川市ホームページに掲載している著作物（文章、写真、イラストなど）にかかる著作権は、加古川市もしくは、加古川市に情報提供していただいた提供元に帰属します。権利者の許可なく使用・転載することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 6,
      revenueHeading: "一般会計款別集計",
      expenditureHeading: "一般会計款別集計",
      revenueTotalLabel: "歳入合計",
      expenditureTotalLabel: "歳出合計",
      revenueCropX: { from: 0, to: cropTo },
      expenditureCropX: { from: 0, to: cropTo },
      dashAsZero: true,
    },
  })),

  ...([
    // 徳島市（徳島県・県庁所在地・団体コード 362018）。⚠ **収録済みの徳島県 360007 とは別団体**
    //   （市は city.tokushima.tokushima.jp、県は pref.tokushima.lg.jp）。id は `tokushima-shi-`。
    // 「各会計別予算規模・款別内訳表」の**一般会計予算款別歳入内訳表 / 歳出内訳表**（歳入20〜22款・歳出13款）。
    // ⚠⚠ **p.1「各会計予算規模」が一般会計＋特別会計7＋企業会計5 の13会計一覧で、行の形が款行と区別できない** —
    //    偵察が実測: `revenuePage: 1` を当てると **当年度・前年度とも Σ 差0 のまま完走し**、
    //    総額 216,547,005 千円（13会計合計）・款名が `一般会計` `国民健康保険事業特別会計` … になる。
    //    八尾 §13-13・つくば §13-14 と同型。**唯一の網はページの固定と、見出しを縮めないこと**。
    // ⚠ 性質別表は別ファイルだが**合計が款別と同額**で、見出しを `一般会計予算` と縮めると
    //    **当年度Σ 差0 のまま款名が `人件費` `扶助費` になる**（前年度Σ だけ +4 で warning ＝ゲートは半分しか守らない）。
    //    **見出しは `一般会計予算款別歳入内訳表` とフルで書く**。
    // ⚠⚠ **市長選が4年ごと（2012・2016・2020・2024年4月＝H24・H28・R2・R6）で、その年度が骨格予算**。
    //    **翌年度（H25・H29・R3・R7）の前年度列は「肉付補正後」**で前年度当初ではない
    //    （列が3年度分あり `[当年度, 前年度(N月肉付補正後), 前年度(当初)]`。既定の ints[1] は補正後側を掴む）。
    //    ⚠⚠ **`prevBasis` の自動判定は「当初」と誤る**（原典の表記が `（９月肉付補正後）` で判定語に当たらない）。
    //    ⚠⚠ **しかも原典の `※` 注記が自動で `prevNote` に入るため、derive の年度間クロスチェーンゲートは通ってしまう**
    //    （ゲートは `prevBasis !== "当初" || prevNote` を説明とみなす）。**ゲートは止めてくれないので明示指定が必須**。
    //    ⚠ 前年度当初の列（3列目）を採る案は **R3 だけ成立し R7 は throw する**（R7 の款22 繰越金は当年度が空欄で
    //    整数が2個しかない）ので、**4年度とも既定列＋`prevBasis: "補正後"` に揃える**。
    // ⚠ **H31 以前は歳入・歳出が別ファイル**（`revenueFile` / `expenditureFile`）。R8〜R2 は1ファイル3ページ。
    // ⚠ **H25・H24 は款番号が原典に無く**（列見出しが `款` でなく `区 分`）`kanNoless` が必須、
    //    **合計ラベルも `計` の一字**（他年度は `合計`）。⚠ その結果 kanNo は全件 null になる。
    // ⚠ 年度ディレクトリの規則が**完全に破れている**（`r8`〜`r2`・`h31` のあと **`2020`=H30 / `2019`=H29 /
    //    `2016`=H28 / `2015`=H27 / `2014`=H26 / `2013`=H25 / `2012`=H24**。西暦が年度と一致しない）。外挿しない。
    // ⚠ **PDF 自体に「徳島市」の文字列が1度も出ない**（表題が `令和８年度一般会計予算款別歳入内訳表` だけ）。
    //    ⚠ **H25・H24 のファイルには識別語が一つも無い**ので、取り違えの検出は raw のパス名に頼るしかない。
    // ⚠ R2 の廃止款 `自動車取得税交付金` は `○` 接頭辞（既知）。⚠ 独自款は無し（`予備費` は款として持つ）。
    // ライセンス「ホームページの使い方＞著作権について」（/about/use.html・最終更新 2016-04-01・確認日 2026-08-24）。
    //    ⚠ 徳島市は**独自のオープンデータカタログを持たず県のポータルに相乗り**しており、
    //    そのポータルの CC BY は規約自身が `opendata.pref.tokushima.lg.jp` 配下に範囲を限る。
    //    ⚠ **徳島市エリアの87データセットに予算・決算・財政は0件**（`決算` の1件は「徳島県企業局決算書」＝県の資料）＝
    //    及ばない（§9g）。
    //    ⚠ リンク条項は「原則として徳島市ホームページのトップページへのリンクをお願いします」＝§11h 第2群なので
    //    `noDeepLink` を立てる。加えて「フレーム内に表示させるリンク設定は行わないでください」＝第3群もあるので、
    //    **発行元ページを iframe に入れる導線は作らない**。
    // [年度, landing パス, 歳入ファイル, 歳出ファイル（単一ファイルなら null）, 歳入p, 歳出p, 補正後か, 旧様式か]
    ["R8", "r8/r8yosanannogaiyoutou", "r8_kaikeibetuyosankibokanbetuutiwakehyou.pdf", null, 2, 3, false, false],
    ["R7", "r7/r7yosanannogaiyoutou", "reiwa7yosankibo.pdf", null, 2, 3, true, false],
    ["R6", "r6/2024022602", "reiwa6yosankibo.pdf", null, 2, 3, false, false],
    ["R5", "r5/2023022401", "reiwa5yosankibo.pdf", null, 2, 3, false, false],
    ["R4", "r4/R4tousyo", "003R4.pdf", null, 2, 3, false, false],
    ["R3", "r3/r3tousyoyosangaiyo", "003.pdf", null, 2, 3, true, false],
    ["R2", "r2/0228165931777", "003.pdf", null, 2, 3, false, false],
    ["H31", "h31/h31_tousho_gaiyo", "h31_tousho_04sain.pdf", "h31_tousho_05saisA.pdf", 1, 1, false, false],
  ] as const).map(([fy, page, revFile, expFile, rev, exp, hosei, kyu]) => ({
    id: `tokushima-shi-kanbetsu-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 徳島市一般会計予算款別内訳表（歳入・歳出）`,
    publisher: "徳島市",
    url: `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${page}.files/${revFile}`,
    ...(expFile
      ? {
          urls: [
            `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${page}.files/${revFile}`,
            `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${page}.files/${expFile}`,
          ],
        }
      : {}),
    landingPage: `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "徳島市（一般会計・団体コード362018）",
    license:
      "徳島市ホームページから発信するコンテンツ（文章・写真・動画等の情報）の著作権は、原則として徳島市に属します（一部の写真やイラストなどは、外部に著作権のある場合があります）。このため、本サイト上のコンテンツについて、無断で複製、転載、改変、編集、頒布、販売等を行うことは禁止します。コンテンツの引用、転載等につきましては、担当課にご相談ください。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "一般会計予算款別歳入内訳表",
      expenditureHeading: "一般会計予算款別歳出内訳表",
      revenueTotalLabel: kyu ? "計" : "合計",
      expenditureTotalLabel: kyu ? "計" : "合計",
      ...(expFile ? { revenueFile: revFile, expenditureFile: expFile } : {}),
      ...(kyu ? { kanNoless: true } : {}),
      ...(hosei ? { prevBasis: "補正後" as const } : {}),
    },
  })),

  ...([
    // 徳島市 H30〜H24（上のブロックの続き）。⚠ **この年度群は歳入・歳出が別ディレクトリの別ページ**で、
    //   landing も歳入用・歳出用に分かれる（registry は歳入側を landing にしてある）。
    // ⚠ 年度ディレクトリが西暦だが**年度と一致しない**（`2020`=H30 / `2019`=H29 / `2016`=H28 /
    //   `2015`=H27 / `2014`=H26 / `2013`=H25 / `2012`=H24）。**外挿しない**。
    // ⚠ **H29 は前年度が「H28 9月補正後」**（H28 が市長選の骨格予算）。**H25 は「H24 6月補正後」**。
    //   どちらも原典の列見出しが `平成２８年度（９月補正後）` `平成２４年度（６月補正後予算）` と明示している。
    // ⚠ **H25・H24 は款番号が無く合計ラベルが `計`**（`kanNoless` ＋ ラベル指定）。
    // ⚠ landing の表題は「一般会計予算目的別歳出内訳表」だが**PDF 内の見出しは「款別歳出内訳表」**。見出しは PDF 側を使う。
    // [年度, 歳入 landing, 歳入ファイル, 歳出 landing, 歳出ファイル, 補正後か, 旧様式か]
    ["H30", "2020/saunyu", "sainyuuutiwake.pdf", "2020/mokuteki", "mokuteki.pdf", false, false],
    ["H29", "2019/29ippannsainyuu", "29ippannsainyuuutiwake.pdf", "2019/29mokutekisaisyutu", "29mokutekibetusaisyutu.pdf", true, false],
    ["H28", "2016/100100a201604071337", "02-2.pdf", "2016/100100a201604071339", "02-3.pdf", false, false],
    ["H27", "2015/sainyu_uchiwake", "sainyuu.pdf", "2015/mokuteki", "mokuteki.pdf", false, false],
    ["H26", "2014/sainyu_uchiwake", "02-2.pdf", "2014/mokuteki", "02-3.pdf", false, false],
    ["H25", "2013/sainyu_uchiwake", "02-02.pdf", "2013/mokuteki", "02-03.pdf", true, true],
    ["H24", "2012/sainyu_uchiwake", "02-2.pdf", "2012/mokuteki", "02-3.pdf", false, true],
  ] as const).map(([fy, revPage, revFile, expPage, expFile, hosei, kyu]) => ({
    id: `tokushima-shi-kanbetsu-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 徳島市一般会計予算款別内訳表（歳入・歳出）`,
    publisher: "徳島市",
    url: null,
    urls: [
      `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${revPage}.files/${revFile}`,
      `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${expPage}.files/${expFile}`,
    ],
    landingPage: `https://www.city.tokushima.tokushima.jp/shisei/zaisei/yosan/yosan_jokyou/${revPage}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "徳島市（一般会計・団体コード362018）",
    license:
  ...
  ...([
    // 草加市（埼玉県・団体コード 112216）。⚠ 収録済みのさいたま市・川越市・川口市・所沢市・越谷市とは別団体。
    // 本命は**市議会サイトが議案附随資料として載せている「一般会計予算参考資料」**の
    //   「令和N年度・令和(N-1)年度一般会計予算歳入款別比較表 / 歳出款別比較表」（歳入21款・歳出13款）。
    // ⚠⚠ **発行元は草加市だが、配信ホストは市議会サイト**（市の予算ページにこの資料は無い）。
    //    市の「著作権等」は自ら「**本サイト上の**文書…」と範囲を限っており、**厳密には市議会サイト上の
    //    ファイルには及ばない**。市議会サイト側には著作権・リンクの条項が**1つも無い**（サイトマップの
    //    全ページを走査して確認）ので、その文言だけを採ると `unverified`＝自サーバー配信になる。
    //    ⚠ **同じ発行元が自サイトで「無断使用、転載、二次利用を禁止」と明示している以上、
    //    開ける側へ倒さない** — 市の著作権等の原文をそのまま license に置き、要許可として扱う。
    //    **これは規約の射程の判断であって、原文の意訳ではない**（§9g は「意訳・切り出しをしない」であり、
    //    「どの規約が及ぶかの判断をしない」ではない）。
    // ⚠⚠ **市議会サイトは http のみ**（`https://www.soka-shigikai.jp/` は接続できない・偵察が実測）。
    // ⚠⚠ **`CropX` の要否が年度で逆転する** — R8・R6 は **A3 見開き**（1190.6pt）で `{from:560,to:1191}` が
    //    **必須**（外すと歳入1款・歳出4款しか取れず款名が `ＩＥ地方交付税ＧＧ市債地方譲与税` に化ける）。
    //    **R7 は A4（595.3pt）なので書いてはいけない**（当てると throw する）。
    //    ⚠ **どちらも大声で落ちる**ので静かには壊れないが、**年度ごとに実測して決める**（加古川 §13-14 と同型）。
    // ⚠⚠ **`HeaderExtra` を外すと款1 が `歳入款別市税` になる** — 列見出し `歳 入 款 別` が単独行で残って
    //    款1 に連結する。**Σ は4系統とも差0 のまま**なので**款名の目視だけが網**。
    // ⚠⚠ **隣のページが性質別表**（R8 は p.5・R7 は p.9・R6 は p.5）で、見出しを `一般会計予算` と縮めると
    //    **Σ4系統とも差0 のまま完走し款名が `人件費` `物件費` … になる**（偵察が実測）。
    //    **見出しは `…歳入款別比較表` / `…歳出款別比較表` までフルで書く**。
    // ⚠ **歳入合計＝歳出合計**なので、歳入/歳出のページを取り違えても Σ では捕まらない。
    // ⚠ 物理ページが年度で違う（R8・R6 は 3/4、**R7 は 5/7**）。
    // ⚠ **参考資料は R8・R7・R6 の3年度しか無い**（R5 以前の2月定例会ページには載っていない。Wayback にも無い）。
    //    市サイトの予算書「総括」は R8〜R3・H31 の7年度あるが、**款行の行頭が罫線 `┃`（U+2503）**で
    //    款番号検出に当たらず、**歳入側を単独で当てると静かに1款だけ返す**（`CropX` でも救えない＝
    //    款行1行が1つの語として組まれているため）。⚠ さらに構成比が1%未満の款で素の `0` になり
    //    整数トークンの位置が款ごとに動くので、**列指定でも直らない**。開くには
    //    「`│`/`┃` をセル区切りにしてセル番号で列を取る」新モードが要る。**この巡では見送った**
    //    （`unrecordable.ts` には載せない＝収録できないのではなく、パーサ側の宿題）。
    // ⚠ 市サイトの「予算の概要」は金額が `412億4,211万` 表記なので使えない。
    // 前年度列は**当初**（R8→R7→R6 の全34件（歳入21＋歳出13）で不一致0。さらに R6 の前年度列は
    //    R5 予算書総括の当年度列とも全件一致＝別系統で裏取り）。
    // ⚠ リンク条項は「リンクをする場合はできるだけトップページにお願いします」＝**要望形**で、
    //    水戸・八尾の指示形とは違う。フレーム内表示の禁止も無い。⇒ `noDeepLink` は立てない。
    // ⚠ 草加市は独自のオープンデータページを持たず埼玉県ポータル経由。**県ポータルの草加市データセットは10件で
    //    予算・決算・財政は0件**（実検索）＝及ばない（§9g）。
    // [年度, KaigiID, ファイル名, 歳入p, 歳出p, A3 か]
    ["R8", "131", "Kg560_dai7sannkou.pdf", 3, 4, true],
    ["R7", "124", "Kg463_9%2015%20sanko.pdf", 5, 7, false],
    ["R6", "119", "Kg373_07giansannkou.pdf", 3, 4, true],
  ] as const).map(([fy, kaigi, file, rev, exp, a3]) => ({
    id: `soka-yosan-sankou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 草加市一般会計予算参考資料（歳入・歳出款別比較表）`,
    publisher: "草加市",
    url: `http://www.soka-shigikai.jp/voices/GikaiDoc/attach/Congress/${file}`,
    landingPage: `http://www.soka-shigikai.jp/g07_gian_shosai.asp?KaigiID=${kaigi}`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "草加市（一般会計・団体コード112216）",
    // 「著作権等」（https://www.city.soka.saitama.jp/cont/s1002/070/a02/04.html・更新 2022-02-01・確認日 2026-08-24）
    license:
      "本サイト上の文書や画像等の各ファイル及びその内容に関する諸権利は、原則として草加市に帰属します。また、一部の画像・イラスト等の著作権は、原著作者が所有していますので、これらの無断使用、転載、二次利用を禁止します。著作権法上の「私的使用」や「引用」の範囲を越え、本ページの情報の使用を希望する場合は事前に市長室広報課までお問い合わせください。事前の許可がない限り、転載、変更、発行、配布、掲示などは一切できません。なお、掲載されている情報は、個人的かつ非営利的な使用目的だけのために利用する場合に限り、複製、使用、ダウンロードができます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      // ⚠ 年号を含めず**年度に依存しない部分だけ**を書く（原典は `令和８年度・令和７年度…` だが、
      //   `一般会計予算歳入款別比較表` の時点で性質別表（`…一般会計予算性質別経費比較表`）と区別できる）。
      revenueHeading: "一般会計予算歳入款別比較表",
      expenditureHeading: "一般会計予算歳出款別比較表",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      revenueHeaderExtra: "歳入款別",
      expenditureHeaderExtra: "歳出款別",
      ...(a3
        ? { revenueCropX: { from: 560, to: 1191 }, expenditureCropX: { from: 560, to: 1191 } }
        : {}),
    },
  })),

  ...([
    // 大和市（神奈川県・施行時特例市・団体コード 142131）。⚠ 収録済みの横浜・川崎・相模原・横須賀・藤沢・
    //   平塚とは別団体。⚠ **東大和市（132209・東京都）とも別**。
    // 「予算書付属説明資料」の**(1) 歳入予算内訳 / (1) 目的別歳出予算内訳**（歳入22〜24款・歳出14款）。
    // ⚠ 物理ページは **歳入 p.8 / 歳出 p.12 が R8〜H28 の11年度すべてで不動**（印字ノンブルは R8 だけ物理と
    //   一致し、R7 以前は物理−4）。
    // ⚠⚠ **見出しを縮めてはいけない** — 同じ PDF に**特別会計の概要**（R8 は p.32 国保・p.33 介護・p.34 後期高齢）が
    //   `［ 歳 入 ］`・合計ラベル `合 計` で同居し、**見出しを素の `歳入`/`歳出` にして samePage で当てると
    //   Σ4系統すべて差0 で完走する**（款名が `国民健康保険税` `保険給付費` になるだけ）。
    //   ⚠ 国保は歳入合計＝歳出合計なので**側の取り違えも Σ では捕まらない**。
    //   ⚠ **推奨見出しなら p.32 は「見出しがありません」で throw する**＝見出しが唯一の網（偵察が実測）。
    // ⚠ 資料編の「２．歳入予算内訳」（R8 p.21）は**見出し語が本表と1文字も違わない**財源別表で合計も同額だが、
    //   款番号が無いので throw する。性質別（p.14）も同様。
    // ⚠⚠ **予算書本体は代替に使えない** — 総括ページで款10 が `国有提供施設等所在市町村助成` に切れ、
    //   こぼれた `交付金等` が款11 に食い込んで `交付金等地方特例交付金` になる（**Σ は差0 のまま**＝最も危険な型）。
    // ⚠⚠ **H31・R2〜R6 は合計行だけ数字の字間に空白が入る**（`8 7,18 0,0 00`）ため、この巡で足した
    //   `totalRowJoinSpacedDigits` が要る。⚠ **款行は無傷**なので、この6年度だけ Σ が桁違いにずれて
    //   大声で落ちていた（静かには壊れない型）。R8・R7・H30〜H28 では no-op。
    // ⚠ **H30〜H28 は発行元から消えており Wayback の `id_` 実体 URL から取る**（原本 URL は 404 を実測）。
    //   ⚠ WARP にも同一ファイルがあり **R3 で sha256 の一致を偵察が実測**している。
    // ⚠ 歳出款6 は **`農林費`**（`農林水産業費` ではない）。`諸支出金`・`予備費` も独自款。寄せない。
    //   ⚠ **R8 だけ `災害復旧費` が皆減で款番号を落とし最下段に来る**（R7 以前は款11）。
    // ⚠ 歳入款10 は `国有提供施設等所在市町村助成交付金等`（**末尾に `等` が付く**）。原典どおり。
    // ⚠ **H27 以前は歳入の款別表が無い**（自主財源/依存財源の2階層＋小計行）ので budget 要件を満たさない。
    //   偵察が H27・H24・H22 で実測。⚠ 歳出だけは H21 まであるが**片側では収録しない**ので、
    //   `unrecordable.ts` には載せない（＝資料の形が違うだけで、この巡の対象外）。
    // 前年度列は**当初**（R8→H28 の隣接10ペアで歳入23・歳出14の全款を突合し不一致0。列反転なし）。
    // ⚠ 列指定・`dashAsZero`・`CropX` は**11年度とも不要**（廃止・皆増のセルはダッシュでなく `0` と `―`）。
    // ライセンス「ホームページの運用方針」（/gyosei/21692.html・更新 2026-01-07・確認日 2026-08-24）。
    //   ⚠ オープンデータ一覧は CC BY 4.0 を宣言するが「本ページに掲載しているデータ」に範囲を限り、
    //   **登載12種に予算・決算・財政は0件**＝及ばない（§9g）。
    //   ⚠ リンク条項は「**可能な限り**、次のトップページの URL を使用するよう**お願いいたします**」＝
    //   水戸・八尾の指示形より弱い依頼形で、フレーム内表示の禁止も無い。⇒ `noDeepLink` は立てない。
    // [年度, url, Wayback か, 字間空白の合計行か]
    ["R8", "https://www.city.yamato.lg.jp/material/files/group/8/R8fuzoku0217pdf.pdf", false, false],
    ["R7", "https://www.city.yamato.lg.jp/material/files/group/8/R7huzoku_0328.pdf", false, false],
    ["R6", "https://www.city.yamato.lg.jp/material/files/group/8/R6fuzoku.pdf", false, true],
    ["R5", "https://www.city.yamato.lg.jp/material/files/group/8/R5huzoku.pdf", false, true],
    ["R4", "https://www.city.yamato.lg.jp/material/files/group/8/R4yosanfuzoku.pdf", false, true],
    ["R3", "https://web.archive.org/web/20211130055422id_/http://www.city.yamato.lg.jp/web/content/000169011.pdf", true, true],
    ["R2", "https://web.archive.org/web/20211130042940id_/http://www.city.yamato.lg.jp/web/content/000158074.pdf", true, true],
    ["H31", "https://web.archive.org/web/20211130035449id_/http://www.city.yamato.lg.jp/web/content/000146078.pdf", true, true],
    ["H30", "https://web.archive.org/web/20211130040629id_/http://www.city.yamato.lg.jp/web/content/000134813.pdf", true, false],
    ["H29", "https://web.archive.org/web/20211130070021id_/http://www.city.yamato.lg.jp/web/content/000124610.pdf", true, false],
    ["H28", "https://web.archive.org/web/20211130042324id_/http://www.city.yamato.lg.jp/web/content/000113132.pdf", true, false],
  ] as const).map(([fy, url, isWayback, spaced]) => ({
    id: `yamato-yosan-fuzoku-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 大和市予算書付属説明資料（歳入予算内訳・目的別歳出予算内訳）`,
    publisher: "大和市",
    url,
    landingPage: isWayback
      ? "https://warp.ndl.go.jp/20210812/20210810202428/http://www.city.yamato.lg.jp/web/zaisei/zaisei.html"
      : "https://www.city.yamato.lg.jp/gyosei/soshik/39/shinoshokai/zaisei/5283.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "大和市（一般会計・団体コード142131）",
    license:
      "本市ホームページに掲載している個々の情報は、著作権の対象となっています。また、本市ホームページ全体も編集著作権の対象となっており、ともに著作権法により保護されています。私的利用のための複製や引用など著作権法上認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 8,
      expenditurePage: 12,
      revenueHeading: "歳入予算内訳",
      expenditureHeading: "目的別歳出予算内訳",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(spaced ? { totalRowJoinSpacedDigits: true } : {}),
    },
  })),

  ...([
    // 富士市（静岡県・団体コード 222101）。⚠ 収録済みの静岡市・浜松市・沼津市とは別団体。
    //   ⚠ **富士吉田市（山梨県 192023・`fujiyoshida-`）とも別**。id は `fuji-shi-`。
    // 「当初予算説明附属資料」の**２ 一般会計款別予算額調**（歳入21〜23款・歳出14款）。
    // ⚠⚠ **`CropX` が必須で、右端は全年度 388 に固定する** — 表の右端に「主な増減事由」欄があり、
    //   (a) **R8 は理由文の `給与費（会計年度）` が `KAN_HEADER_RE` の `年度` に当たって歳出款10 教育費が行ごと消える**
    //       （Σ −12,402,939＝ゲートは捕まえる）、
    //   (b) ⚠⚠ **H30/H29 は理由欄の左端 x が他年度より 9pt 左**（391.1／他は 399.4〜400.6）で、
    //       `to: 398` にすると理由文の1文字目が款名に前置され **`市市税` `情分担金及び負担金` `青国施民生費` になるのに
    //       Σ は4系統とも差0**（偵察が実測）。**目視でしか気づけない型**。
    //   ⇒ **388 なら10年度とも差0・款名クリーン**（対比列の右端は 382〜394 で重なる語は残るので落ちない）。
    //   ⚠ crop を外しても R7〜H31 は差0 で通ってしまう＝**要否が年度で変わるので全年度に固定する**。
    // ⚠ 歳出ページには表題が無く節ラベル `歳出` だけ（歳入は表題 `２ 一般会計款別予算額調`）。
    // ⚠ 同じ PDF の p.3「各種会計予算総括表」を当てると Σ −149,611,100 で落ち、
    //   「歳出性質別の推移」は款番号が無く throw する。**どちらも静かには通らない**（加古川 §13-14 型の事故は起きない）。
    //   ⚠ **一般会計と同型の特別会計総括はこの資料に入っていない**（目次13項目を偵察が実確認）。
    // ⚠ **R4 以前は発行元から削除済み**（旧 CMS の URL は全て 404）。
    //   ⚠⚠ **R3 は Wayback の 2023-09-21 捕捉がちょうど 1 MiB で打ち切り**（§9b）なので **2024-08-04 の捕捉**を使う。
    //   ⚠⚠ **R2・H31 は Wayback の唯一の捕捉が 1 MiB 打ち切りで、原本が 404 なので取り直せない** → **WARP を使う**。
    //   ⚠ H30・H29 は原典が5〜6分冊で、款別は**第1分冊**にある。
    // ⚠ 歳出款13・14 は `諸支出金`・`予備費`（独自款）。⚠ R4 の歳入末尾は廃止款 `自動車取得税交付金`（既定処理で拾える）。
    // 前年度列は**当初**（R8→H29 の9ペアで歳入・歳出の全款を突合し一致。差は皆増の2件だけ）。
    // ライセンス「著作権・免責事項・リンク」（/info/p006864.html・更新 2025-05-15・確認日 2026-08-24）。
    //   ⚠ 「富士市オープンデータ利用規約」は CC BY 4.0 を宣言するが規約自身が「本ページ（＝富士市オープンデータ）」に
    //   範囲を限り、**県カタログの富士市113件に予算・決算・財政は0件**（実検索）＝及ばない（§9g）。
    // ⚠ **`noDeepLink` を立てた** — リンク条項が「**できるだけトップページ…にしてください**」＝§11h 第2群。
    //   ⚠ 判断の分かれ目は**動詞の形**にした: `としてください`（水戸・八尾）と同じ**指示形**なので立てる側、
    //   `推奨します`（平塚）・`お願いいたします`（大和）は立てない側。「できるだけ」で緩めてはいるが
    //   発行元が宛先を能動的に指定していることに変わりはない。
    //   ⚠ 加えて「フレームの中で表示させることはご遠慮ください」＝第3群もあるので、
    //   **発行元ページを iframe に入れる導線は作らない**。
    // [年度, url, landing, 歳入p, 歳出p]
    ["R8", "https://www.city.fuji.shizuoka.jp/documents/16234/08_r8_fuzokusetumei.pdf",
      "https://www.city.fuji.shizuoka.jp/1010050000/p007744.html", 4, 5],
    ["R7", "https://www.city.fuji.shizuoka.jp/documents/7866/hngtkl000000jjas_1.pdf",
      "https://www.city.fuji.shizuoka.jp/1010050000/p005153.html", 4, 5],
    ["R6", "https://www.city.fuji.shizuoka.jp/documents/5118/f1e59900000018go.pdf",
      "https://www.city.fuji.shizuoka.jp/1010050000/p005158.html", 6, 7],
    ["R5", "https://www.city.fuji.shizuoka.jp/documents/5125/rn2ola000004dic7.pdf",
      "https://www.city.fuji.shizuoka.jp/1010050000/p005165.html", 6, 7],
    ["R4", "https://web.archive.org/web/20230921090831id_/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000003onxp-att/rn2ola000003onzw.pdf",
      "https://web.archive.org/web/20240521170641/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000003onxp.html", 6, 7],
    ["R3", "https://web.archive.org/web/20240804170616id_/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000002xvh5-att/rn2ola000002xvjc.pdf",
      "https://web.archive.org/web/20230926051408/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000002xvh5.html", 6, 7],
    ["R2", "https://warp.ndl.go.jp/20200808/20200806080543/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000002ahcz-att/rn2ola000002ahf6.pdf",
      "https://warp.ndl.go.jp/20200808/20200806080543/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000002ahcz.html", 6, 7],
    ["H31", "https://warp.ndl.go.jp/20190806/20190805031658/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000001suf8-att/rn2ola000001suhp.pdf",
      "https://warp.ndl.go.jp/20190806/20190805031658/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000001suf8.html", 5, 6],
    ["H30", "https://web.archive.org/web/20230925171400id_/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000001e5c0-att/rn2ola000001e5eh.pdf",
      "https://web.archive.org/web/20230609182931/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000001e5c0.html", 4, 5],
    ["H29", "https://web.archive.org/web/20230925162451id_/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000000r0of-att/rn2ola000000r0qw.pdf",
      "https://web.archive.org/web/20230609181847/https://www.city.fuji.shizuoka.jp/shisei/c1601/rn2ola000000r0of.html", 4, 5],
  ] as const).map(([fy, url, landing, rev, exp]) => ({
    id: `fuji-shi-yosan-fuzoku-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 富士市当初予算説明附属資料（一般会計款別予算額調）`,
    publisher: "富士市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "富士市（一般会計・団体コード222101）",
    license:
      "富士市ウェブサイト内に掲載されている文書や画像などの各ファイル及びその内容に関する諸権利は、原則として富士市に帰属し、国際条約・法律等によって保護されています（一部の画像等の著作権は、原著作者が所有しています）。「私的使用のための複製」や「引用」など、著作権法上認められる場合を除き、富士市の許可なくウェブサイト内に掲載されている文書・画像などを無断使用・複製・転載・販売・改変・印刷配布することを禁止します。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "一般会計款別予算額調",
      expenditureHeading: "歳出",
      revenueTotalLabel: "歳入合計",
      expenditureTotalLabel: "歳出合計",
      revenueCropX: { from: 0, to: 388 },
      expenditureCropX: { from: 0, to: 388 },
    },
  })),

  ...([
    // 茅ヶ崎市（神奈川県・団体コード 142077）。⚠ 収録済みの横浜・川崎・相模原・横須賀・藤沢・平塚・大和とは別団体。
    // 本命は**記者発表資料「当初予算案の概要」**（16p・Excel 出力の born-digital）の
    //   **物理 p.3 一般会計歳入（21〜23款）/ p.7 歳出 目的別（13款）**。印字ノンブルと物理ページが一致する。
    // ⚠⚠ **「予算書及び予算説明書」（479p・27MB）は使ってはいけない** — Producer が
    ...
    //   ⚠ try-parse では「歳入合計 行が見つかりません」で throw するので**誤って当てる事故は起きない**が、
    //   **OCR の資料は Σ が合っても数字を信用できない**という一般則として記録しておく。
    // ⚠⚠ **`expenditureHeading` を `歳出` と短く書いてはいけない** — 同じ PDF の p.11 が **`歳出(性質別）`** で、
    //   どちらも `歳出` を含むため**見出しゲートを素通りする**。⚠ ただし性質別に款番号は無いので
    //   `kanNoless` 無しなら throw、有りでも Σ が +7,689,764 割れる（＝静かには通らない）。
    //   **`歳出(目的別）` とフルで書く**。⚠ **開き括弧は半角 U+0028・閉じ括弧は全角 U+FF09**（原文どおり・全16年度で不変）。
    // ⚠ **`kanNoless` は全年度に一律で付ける** — R3 の廃止款 `自動車取得税交付金` が
    //   **廃止マーカーも `皆減` の語も持たない**（`0 0.0% 1 0.0% △1`）ため、無いと**前年度Σ が −1 の warning**で
    //   derive まで流れる。⚠ 他年度では款数・Σ とも結果が変わらないことを偵察が実測している。
    // ⚠ **R1 以前は歳入・歳出が別ファイル**（`revenueFile`/`expenditureFile`・物理 p.2。p.1 はグラフと文章）。
    // ⚠⚠ **R2 の総括表だけ数値列が4本**（`当年度 | 前年度(B)【骨格予算】| 6月補正(C)【肉付け】| (D)=(B)+(C)`）。
    //   既定の ints[1] は **(B)＝前年度当初**を掴むので**前年当初比としては正しい**が、
    //   **原典が印字している増減額は (D)＝肉付後との比較**なので画面の増減と一致しない → `prevNote` を付ける。
    // ⚠⚠ **R1 自身が骨格予算なのに、総括表 PDF には「骨格」の語が1文字も無い**
    //   （根拠は同年度の別ファイル「予算の概要」）。**総括表だけ見て収録すると事実がどこにも残らない**ので `prevNote` を付ける。
    // ⚠ **発行元が過年度を消す** — 記者発表のインデックスは**直近17か月分しか残らない**。
    //   R6・R5・R3・R2 は **Wayback に捕捉が無く WARP が唯一の写し**、R4 のみ Wayback。
    // ⚠ **R7 の直リンクは2系統ある**（`001/061/665/…` は live 200・`001/061/659/…` は 404 で WARP のみ）。
    //   **sha256 が一致する**ことを偵察が確認済みなので **live の 665 を採る**。
    // ⚠ **H30・H21 の総括表 PDF には表題レベルの市名が無い**（`平成N年度一般会計歳入` としか書いていない）ので、
    //   **他市の同種資料と取り違えても字面では気づけない**。取得元 URL と合計額・年度間チェーンで確かめること。
    //   ⚠ 「『茅ヶ崎』が0件」なのは**歳入側だけ**で、H30 の歳出側には事業名（`（仮称）茅ヶ崎公園体験学習施設整備事業費`）で
    //   2件ある（収録レビューが実測）。**表題で識別できないという主旨は変わらない**。
    // ⚠ **H25・H23 は収録不可**（`unrecordable.ts`）。⚠ H20・H19 は PDF が無く HTML 表のみ（別パーサが要る）。
    // 前年度列は**当初**（隣接ペアを全款突合して不一致0。差は新設款の皆増と廃止款の消滅だけ）。
    // ⚠ `CropX`・列指定・`dashAsZero` はいずれも不要（構成比・伸び率が `%` 付きで `stripPercents` が落とす）。
    // ライセンス「リンクについて」「ホームページの考え方について」（/about/1001981.html・/about/1001980.html・
    //   更新 2023-12-08 / 2025-05-26・確認日 2026-08-24）。
    //   ⚠ オープンデータライブラリは CC BY 4.0 だが規約が「本サイト（＝ライブラリ）」に範囲を限り、
    //   **カタログ1,267リンクに予算・決算・財政は0件**（実検索）＝及ばない（§9g）。
    //   ⚠ リンク条項は「リンク先の URL は**原則トップページとしてください**」＋**事前連絡の依頼**＋
    //   フレーム内表示の禁止＝§11h 第2群なので `noDeepLink` を立てる。
    // [年度, url, landing, 歳入ファイル, 歳出ファイル, 歳入p, 歳出p, prevNote]
    ["R8", "https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/066/693/202602203a.pdf",
      "https://www.city.chigasaki.kanagawa.jp/koho/1030702/1002784/1066560/1066693.html", null, null, 3, 7, null],
    ["R7", "https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/061/665/202502173.pdf",
      "https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1061665.html", null, null, 3, 7, null],
    ["R6", "https://warp.ndl.go.jp/20240511/20240510192121/https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/057/170/03.pdf",
      "https://warp.ndl.go.jp/20240510/20240510105105/https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1057170.html", null, null, 3, 7, null],
    ["R5", "https://warp.ndl.go.jp/20230511/20230510043452/https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/051/892/0224_tousyo.pdf",
      "https://warp.ndl.go.jp/20230511/20230510043452/https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1051892.html", null, null, 3, 7, null],
    ["R4", "https://web.archive.org/web/20220708102513id_/https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/046/761/20220221gaiyou.pdf",
      "https://web.archive.org/web/20220307170410/https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1046761.html", null, null, 3, 7, null],
    ["R3", "https://warp.ndl.go.jp/20210512/20210511154637/https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/042/323/20210219kishahappyou1.pdf",
      "https://warp.ndl.go.jp/20210512/20210511154637/https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1042323.html", null, null, 3, 7, null],
    ["R2", "https://warp.ndl.go.jp/20200510/20200510000000/https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_/001/038/075/202002192.pdf",
      "https://warp.ndl.go.jp/20200510/20200510000000/https://www.city.chigasaki.kanagawa.jp/koho/1030702/1020324/1038075.html", null, null, 3, 7,
      "前年度（令和元年度）の当初予算は骨格予算として編成されており、原典の総括表は「令和元年度当初予算額(B)【骨格予算】」「6月補正(C)【肉付け予算】」「(B)+(C)」の3列を並べている。ここでの前年度額は(B)＝骨格予算の当初額で、原典が印字している増減額は(B)+(C)（肉付け後）との比較なので一致しない。"],
    ["R1", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1032911/1034901.html",
      "sainyu.pdf", "mokuteki.pdf", 2, 2,
      "この年度の当初予算は骨格予算として編成されている（市長選のため。根拠は同じ年度ページの別資料「予算の概要」で、この総括表そのものには骨格の記載が無い）。"],
    ["H30", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1026775/1030356.html",
      "02H30sainyuu.pdf", "03H30saisyutumokuteki.pdf", 2, 2, null],
    ["H29", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1020094/1024480.html",
      "02H29sainyuu.pdf", "03H29saisyutumokuteki.pdf", 2, 2, null],
    ["H28", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1015076/1017856.html",
      "sainyuu.pdf", "saisyutumokuteki.pdf", 2, 2, null],
    ["H27", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1010199/1012903.html",
      "27sainyuu.pdf", "27saisyutumokuteki.pdf", 2, 2, null],
    ["H26", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1008513/1008525.html",
      "H26sainyuusoukatu.pdf", "H26saisyutumokuteki.pdf", 2, 2, null],
    ["H24", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/1008515/1008541.html",
      "H24ippannkaikeisainyuu.pdf", "H24ippannkaikeisaisyutumokuteki.pdf", 2, 2, null],
    ["H22", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/h22/1008562.html",
      "22tousho_soukatsu2.pdf", "22tousho_soukatsu3.pdf", 2, 2, null],
    ["H21", null, "https://www.city.chigasaki.kanagawa.jp/zaisei/1008506/h21/1008572.html",
      "sainyu.pdf", "saisyutumokuteki.pdf", 2, 2, null],
  ] as const).map(([fy, singleUrl, landing, revFile, expFile, rev, exp, note]) => {
    const dir = {
      R1: "001/034/901", H30: "001/030/356", H29: "001/024/480", H28: "001/017/856",
      H27: "001/012/903", H26: "001/008/525", H24: "001/008/541", H22: "001/008/562", H21: "001/008/572",
    }[fy as string];
    const base = "https://www.city.chigasaki.kanagawa.jp/_res/projects/default_project/_page_";
    return {
      id: `chigasaki-yosan-soukatsu-${fy.toLowerCase()}`,
      title: `${eraYear(fy)}年度 茅ヶ崎市一般会計予算総括表（歳入・歳出（目的別））`,
      publisher: "茅ヶ崎市",
      url: singleUrl ?? `${base}/${dir}/${revFile}`,
      ...(revFile && expFile ? { urls: [`${base}/${dir}/${revFile}`, `${base}/${dir}/${expFile}`] } : {}),
      landingPage: landing,
      kind: "pdf" as const,
      fiscalYear: fy,
      scope: "茅ヶ崎市（一般会計・団体コード142077）",
      license:
        "茅ヶ崎市公式ホームページに掲載している文書や画像ファイル及び内容に関する権利は、茅ヶ崎市に帰属します。また、一部の画像等の著作権は原著作者が所有しています。著作権法上の「私的使用のための複製」や「引用」などの範囲を超えて、無断で複製・転用することはできません。",
      noDeepLink: true,
      parser: "kofu-yosansho" as const,
      parserOptions: {
        revenuePage: rev,
        ...
        expenditureHeading: "歳出(目的別）",
        kanNoless: true,
        ...(revFile && expFile ? { revenueFile: revFile, expenditureFile: expFile } : {}),
        ...(note ? { prevNote: note } : {}),
      },
    };
  }),
  ...
  ...([
    // 調布市（東京都・団体コード 132080）。⚠ **特別区ではなく市**なので標準の款体系（都区財政調整の款は無い）。
    //   ⚠ 収録済みの東京都・23区・八王子市・町田市とは別団体。
    // 「市政経営の概要（予算参考資料）」巻末の**令和N年度一般会計歳入歳出予算**（歳入22〜23款・歳出13款）。
    // ⚠ 款番号は **5,10,15,16,…,90 の飛び番**（連番ではない）。原典どおり・詰めない。
    // ⚠⚠ **同じ PDF に特別会計4本（国保・用地・介護・後期高齢）が1文字も違わない様式で同居する** —
    //   節ラベルは `歳 入`/`歳 出`・合計ラベルも `歳入合計`/`歳出合計`。偵察が実測: 見出しを素の `歳入` にして
    //   国保のページに当てると **Σ4系統とも差0 で完走し、款名が `国民健康保険税` `保険給付費` になる**。
    //   ⚠ **国保は歳入合計＝歳出合計なので側の取り違えも Σ では捕まらない**。
    //   ⇒ **唯一の網が `一般会計歳入歳出予算` という見出し**（特別会計の表題は `国民健康保険事業特別会計歳入歳出予算`
    //   なのでこの部分文字列を含まない）。**短くしない**。八尾 §13-13・つくば §13-14・徳島/大和 §13-15 に続く5例目。
    // ⚠ **款と項が同一表**（款は字下げ0〜1・項は39〜45）なので `kanIndentMax` が必須。
    //   外すと歳入64款・歳出44款になり Σ +119,134,999 の error（＝大声で落ちる）。
    // ⚠ **1側が2ページにまたがり合計行は2ページ目にしかない**ので `revenuePages`/`expenditurePages` の範囲指定が要る
    //   （単ページ指定は「歳入合計 行が見つかりません」で throw）。
    // ⚠⚠ **分冊がどれかは年度で変わる**（R8/R7/R6＝単一・R5/R4＝分冊2・R3＝分冊10・R2＝分冊9・H31＝分冊5）。
    //   **物理ページも毎年変わる**ので年度を外挿せず毎回中身を開いて確認する。
    // ⚠ **R3 の原典に乱丁がある** — 分冊10 の印字 -208-/-209-（物理32-33）が一般会計歳入の完全な重複で、
    //   本来そこに来るはずの国民健康保険事業特別会計が資料全体に1件も無い。物理28-29 と32-33 は差分ゼロなので
    //   どちらでも同じ結果になるが、**先に来る物理28-29 を採る**（旧ドメインの同名ファイルと sha256 一致＝原典の版そのもの）。
    // ⚠ 歳出款30 は **`農業費`**（`農林水産業費` ではない）。⚠ 歳出款65 は `諸支出金`・款90 は `予備費`。寄せない。
    // ⚠ 資料は**OCR ではない**（Producer は iText / SkyPDF 系で `Paper Capture` は0件）。
    // 前年度列は**当初**（R8→H31 の7リンクで歳入・歳出の全款が一致。列反転も無し）。
    //   ⚠ 原典の列見出しは `７ 年 度 (B)` だけで、**`（N月肉付補正後）` の表記も `※` 注記も款別ページに1件も無い**。
    // ⚠⚠ **R9 を収録するときは必ず R8 当初と全款突合する** — **調布市長選は 2026-07-05（4月ではない）**で
    //   新市長が年度途中に就任しているため、**R9 資料の前年度列が「肉付補正後」になる可能性がある**。
    //   ⚠ 自動判定は当たらず、注記が `prevNote` に入るとゲートも通ってしまう（徳島 §13-15 の型）。
    //   ⚠ R8 当初そのものは骨格予算ではない（2026-03-26 議決の通常予算）。
    // ⚠ **H30 以前（H30〜H21 の10年度）は旧ドメインの予算インデックス（Wayback）にページがあるが未調査**。
    //   「収録できない」ではないので `unrecordable.ts` には載せない。
    // ライセンス「ホームページの著作権・リンク」（/010040/p003478.html・更新 2021-05-25・確認日 2026-08-24）。
    //   ⚠ オープンデータは CC BY 4.0 を宣言するが規約自身が「オープンデータとして掲載しているページ以外に
    //   掲載されているデータの利用については、本市ホームページにおける著作権の取り扱いに準じてください」と範囲を限り、
    //   **カタログ1,464件をキーワード「予算」で検索して0件**（実測）＝及ばない（§9g）。
    //   ⚠ リンク条項は「トップページに対するリンクを**お願いします**」＝**依頼形**なので `noDeepLink` は立てない
    //   （§13-15 の動詞基準。平塚・大和・草加と同群）。
    //   ⚠ ただしフレーム内表示は「**お断りします**」＋「必ず、別ウィンドウで開くように設定してください」と
    //   **拒否＋指示形**なので、**発行元ページを iframe に入れる導線は作らない**。
    // [年度, url, landing, 歳入from, 歳入to, 歳出from, 歳出to]
    ["R8", "https://www.city.chofu.lg.jp/documents/16867/r8siseikeieinogaiyou.pdf",
      "https://www.city.chofu.lg.jp/010020/p003005.html", 200, 201, 202, 203],
    ["R7", "https://www.city.chofu.lg.jp/documents/14046/r7siseikeinogaiyou.pdf",
      "https://www.city.chofu.lg.jp/010020/p001126.html", 224, 225, 226, 227],
    ["R6", "https://www.city.chofu.lg.jp/documents/11292/6.pdf",
      "https://www.city.chofu.lg.jp/010020/p001117.html", 218, 219, 220, 221],
    ["R5", "https://www.city.chofu.lg.jp/documents/4873/r5_gaiyou2.pdf",
      "https://www.city.chofu.lg.jp/010020/p001007.html", 102, 103, 104, 105],
    ["R4", "https://www.city.chofu.lg.jp/documents/4872/siseikeieinogaiyou2.pdf",
      "https://www.city.chofu.lg.jp/010020/p001008.html", 134, 135, 136, 137],
    ["R3", "https://www.city.chofu.lg.jp/documents/4871/gaiyou10_3.pdf",
      "https://www.city.chofu.lg.jp/010020/p001009.html", 28, 29, 30, 31],
    ["R2", "https://web.archive.org/web/20220308001452id_/https://www.city.chofu.tokyo.jp/www/contents/1585292543775/files/gaiyou9.pdf",
      "https://web.archive.org/web/20210301000000/https://www.city.chofu.tokyo.jp/www/genre/0000000000000/1000000010276/index.html", 2, 3, 4, 5],
    ["H31", "https://web.archive.org/web/20220308001459id_/https://www.city.chofu.tokyo.jp/www/contents/1553505242501/files/gaiyou5.pdf",
      "https://web.archive.org/web/20210301000000/https://www.city.chofu.tokyo.jp/www/genre/0000000000000/1000000010276/index.html", 2, 3, 4, 5],
  ] as const).map(([fy, url, landing, rf, rt, ef, et]) => ({
    id: `chofu-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 調布市市政経営の概要（一般会計歳入歳出予算）`,
    publisher: "調布市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "調布市（一般会計・団体コード132080）",
    license:
      "調布市ホームページに掲載されている情報(文字、写真、イラストなど)は著作権の対象となっています。また、ホームページ全体も編集著作権の対象となっています。これらの著作権は調布市および写真撮影者、イラスト作成者などに帰属しており著作権法および国際条約により保護されています。これらの情報について、著作権法上認められた「私的使用のための複製」や「引用」などの場合を除き、無断で複製、転用等をすることはできません。また、本ホームページの内容の全部または一部について無断で改変することもできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePages: { from: rf, to: rt },
      expenditurePages: { from: ef, to: et },
      revenueHeading: "一般会計歳入歳出予算",
      expenditureHeading: "一般会計歳入歳出予算",
      kanIndentMax: 4,
    },
  })),

  ...([
    // 下関市（山口県・中核市・団体コード 352012）。⚠ 収録済みの山口県 350001・山口市 352039 とは別団体。
    // **主経路 = 予算書「(1) 総括」**（歳入歳出予算事項別明細書 総括）の2ページ単体 PDF（p.1 歳入・p.2 歳出）。
    // ⚠⚠ **この経路を本命にする理由は「特別会計が構造的に入っていない」こと** — 概要ルートは同じ PDF に
    //   特別会計12＋企業会計5 の同型表と、**単位が百万円の目的別表**が同居する（後述）。
    // ⚠⚠ **`HeaderExtra` を外すと歳出款1 が `一般財源国県支出金市債その他議会費` になる**（H21〜R4 の全年度）。
    //   さらに **H30〜R4 だけ**は歳入款1 が `一般会計市税`（右上のページ見出しが連結）になる。
    //   **どちらも Σ は4系統とも差0 のまま**＝款名の目視だけが網。⚠ **要否が年度で変わるので全年度に固定する**
    //   （富士 §13-15 の教訓）。
    // ⚠ `dashAsZero` は **R2 の廃止款「自動車取得税交付金」の当年度セルが `-`** なので必須（外すと Σ +109,740 で落ちる）。
    //   他年度は no-op だが**全年度に固定**する。
    // ⚠ **H31 だけ歳入が見開き2ページ型**（p.1 に款名＋本年度・p.2 に前年度＋対比）なので `revenueSpread` が要る。
    //   歳出は p.3。⚠ 外すと「見開きの行数が合いません」で throw する。
    // ⚠ **H27 だけ歳入款10 が `地方特例交付金特例`**（原典で次行に `特例` が落ちる）。**Σ 差0** なので
    //   `kanNameSuffixStrip` で剥がす。
    // ⚠ **R5〜R8 の総括 PDF は Producer が `JUST PDF 5` でテキストがアウトライン化**されており使えない
    //   （フォント0・画像0・抽出2バイト）。この4年度は概要ルートで入れる（別ブロック）。
    // ⚠ 歳入款9 は `国有提供施設等所在市助成交付金`（**`市町村` ではなく `市`**）。原典どおり。
    // ⚠ **当年度側が骨格予算の年度**は H21・H25・H29・R3（市長選が4年ごと・2009/2013/2017/2021/2025年3月）。
    //   ⚠ ただし**総括ルートの前年度列は全年度「当初」**（H21→R4 の当年度額の鎖が完全に一致することを偵察が実測）。
    //   骨格の影響が前年度列に出るのは概要ルートだけ。
    // ⚠ **資料は OCR ではない**（Producer は JUST PDF 4 / Word / Excel / SkyPDF / Distiller 系）。
    //   ⚠ ただし**概要ルートの H31 は `Paper Capture Plug-in`＝OCR で金額が1桁も抽出されない**ので採らない
    //   （同年度は総括で読める）。H27 の概要も ToUnicode 欠落で使えない。
    // ライセンス「著作権・リンク」（/site/userguide/51553.html・更新 2022-02-01・確認日 2026-08-24）。
    //   ⚠⚠ **この原文は「出所を明示することにより、複製・引用・転載できます」と明示的に許諾しており、
    //   「無断転載禁止」は個別注記がある場合の例外を指す語として現れる**。偵察が実ファイルを見た限り
    //   本資料の表紙・奥付・本文に「無断転載禁止」の注記は無い。⚠ `licenseClassOf` は `無断` の語で
    //   `permission-required` に落とす（**語の位置による偽陽性の可能性**）が、**原文をそのまま置けば安全側に倒れる**ので
    //   既定はそれでよい。③自サーバー配信を開けたい場合だけ人が判断する。**意訳・切り出しはしない**。
    //   ⚠ リンク条項は「リンクは原則として自由」＋「下関市ホームページへのリンクであることを明記してください」で、
    //   **指示の対象は出所の明示であってリンク先の限定ではない**。⇒ `noDeepLink` は立てない。フレーム条項も無い。
    //   ⚠ 山口県オープンデータカタログの下関市53件に**予算・決算・財政は0件**＝CC BY は及ばない（§9g）。
    // [年度, ファイル番号, landing ID]
    ["R4", "55057", "61348"],
    ["R3", "51210", "50173"],
    ["R2", "51128", "50167"],
    ["H31", "51067", "50159"],
    ...
    id: `shimonoseki-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 下関市一般会計予算 歳入歳出予算事項別明細書（1）総括`,
    publisher: "下関市",
    url: `https://www.city.shimonoseki.lg.jp/uploaded/attachment/${file}.pdf`,
    landingPage: `https://www.city.shimonoseki.lg.jp/soshiki/14/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "下関市（一般会計・団体コード352012）",
    license:
    ...
    parserOptions: {
      ...(fy === "H31"
        ? { revenueSpread: { namePage: 1, amountPage: 2 }, expenditurePage: 3 }
        : { revenuePage: 1, expenditurePage: 2 }),
      revenueHeading: "歳 入",
      expenditureHeading: "歳 出",
      dashAsZero: true,
      revenueHeaderExtra: "^一般会計$|一般財源|国県支出金|特定財",
      expenditureHeaderExtra: "^一般会計$|一般財源|国県支出金|特定財",
      ...(fy === "H27" ? { kanNameSuffixStrip: { revenue: "特例$" } } : {}),
    },
  })),

  ...([
    // 下関市 R5〜R8（総括 PDF が `JUST PDF 5` でアウトライン化されている年度）— 「当初予算(案)の概要」の
    //   **（５）会計別前年度対比表**（`会 計・款 別` の1表に 一般会計・歳入 → 一般会計・歳出 → `一般会計 計` が縦積み）。
    // ⚠⚠ **`一般会計計` という合計ラベルが、同じ PDF の特別会計12＋企業会計5 への誤爆を止める唯一の網**。
    //   ラベルを `計` に縮めると危険（偵察が p.14 に当てて throw を確認）。
    // ⚠⚠ **同じ PDF の物理 p.10 は「一般会計歳出の状況（目的別）」で、同じ13款・同じ前年度列だが単位が百万円** —
    //   `expenditurePage: 10` にすると **Σ4系統とも差0 のまま完走する**（偵察が実測）。
    //   歳入だけ p.13（千円）に残すと**1000倍のズレが Σ を素通りする**。**ページを固定することだけが網**。
    // ⚠ 款番号が無い表なので `kanNoless` が必須。⚠ `うち 市民税`『うち 固定資産税』の内訳行を落とさないと Σ +30,891,989。
    // ⚠ 歳入合計・歳出合計の行が無く合計は `一般会計 計` の1行だけ（歳入＝歳出＝同額）なので `samePage` は使えない。
    // ⚠ **縦書きラベル「一般会計・歳入／歳出」の1文字が款名に前置される**（H19〜R2 で必発・R4 以降は独立行）。
    //   `kanNamePrefixStrip` と `HeaderExtra` の両方で落とす。**Σ は差0 のまま**なので目視だけが網。
    // ⚠⚠ **R8 の前年度列は「肉付補正後」** — R7 は市長選（2025年3月）の骨格予算で、
    //   前年度列 136,217,300 = R7当初(骨格) 135,020,000 ＋ 6月肉付補正 1,197,300。
    //   根拠は概要 p.7 の ※1「令和７年度数値は、当初予算（骨格）と６月（肉付け補正）の合計値」。
    //   ⚠⚠ **款別表（p.13）自体には注記が無いので `prevNote` も空になり、derive の年度間クロスチェーンも通る**
    //   （徳島 §13-15 と同型）。**`prevBasis` の明示が唯一の防波堤**。⚠ 次に同じことが起きるのは **R12**。
    // ⚠ **R5 の p.13 には表題も列見出しもテキストとして存在しない**ため、内訳行 `うち 固定資産税` を見出しに流用している
    //   （見出し行は読み飛ばされるので実害は無く、`^うち` で落とすのと二重に効く）。⚠ 見出しゲートとしては弱い。
    // ⚠ **H31・H27・R3 の概要は使えない**（H31 は `Paper Capture`＝OCR で金額が抽出できず、H27 は ToUnicode 欠落、
    //   R3 は `XPS Conversion Plug-in` でテキスト層が無い）。**いずれも同年度を総括ルートで収録済み**。
    // [年度, ファイル番号, landing ID, 歳入見出し]
    ["R8", "93777", "137804", "会計・款別"],
    ["R7", "83352", "119190", "会計・款別"],
    ["R6", "74375", "100471", "会計・款別"],
    ["R5", "65120", "79415", "うち 固定資産税"],
  ] as const).map(([fy, file, page, heading]) => ({
    id: `shimonoseki-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 下関市当初予算案の概要（会計別前年度対比表・一般会計）`,
    publisher: "下関市",
    url: `https://www.city.shimonoseki.lg.jp/uploaded/attachment/${file}.pdf`,
    landingPage: `https://www.city.shimonoseki.lg.jp/soshiki/14/${page}.html`,
    kind: "pdf" as const,
    ...
    parserOptions: {
      revenuePage: 13,
      expenditurePage: 13,
      revenueHeading: heading,
      expenditureHeading: heading,
      revenueTotalLabel: "一般会計計",
      expenditureTotalLabel: "一般会計計",
      kanNoless: true,
      kanNamePrefixStrip: "一般会計・歳入出",
      revenueHeaderExtra: "一般会計・歳|^うち|費[0-9]",
      expenditureHeaderExtra: "^(?!.*費[0-9])",
      ...(fy === "R8" ? { prevBasis: "補正後" as const } : {}),
    },
  })),

  ...([
    // 松本市（長野県・中核市・団体コード 202029）。⚠ 収録済みの長野県 200000・長野市 202011（`nagano-shi-`）とは別物。
    // 「一般会計予算説明書」の**事項別明細書「１ 総括」**（歳入22〜24款・歳出13〜14款・千円）。
    // ⚠⚠ **「当初予算の概要」PDF は使わない** — 同じ表に円グラフのラベルが重なり、`-layout` が款名に食い込む
    //   （款3 `過去利子割交付金`・款4 `最大市税配当割交付金`・款5 `その他歳入株式等譲渡所得割交付金`）。
    //   **歳入側は Σ 4系統とも差0 のまま**＝目視でしか気づけない。説明書の総括はこの汚染が無い。
    // ⚠ **R4・R3 は歳入・歳出が同じ p.5 に縦積み**（`samePage`）で、合計ラベルが `歳入合計`/`歳出合計` と別語なので
    //   両側とも `合計` を指定する（部分一致で2件当たる）。さらにこの2年度だけ表題 `１ 総 括` が款1 の頭に付き、
    //   歳出には財源内訳ヘッダが付くので `HeaderExtra` で捨てる。
    // ⚠⚠ **R4・R3 の歳入款10 は原典が3行構成で `kanNameContinues` が効かない** — 指定しないと
    //   款10 `国有提供施設等所在市町村` ＋ **款11 `助成交付金地方特例交付金`** になる（**Σ は差0 のまま**）。
    //   **この巡でパーサの折返し待ちを3行構成にも効かせたので、`kanNameContinues` で完全形が取れる**
    //   （R4・R3 とも `国有提供施設等所在市町村助成交付金`）。⚠⚠ **`^助成交付金$` を HeaderExtra に戻さないこと** —
    //   断片を捨てる旧回避策で、戻すと下段が消えて款名が切れる。
    // ⚠⚠ **R6 の歳出には款番号の無い廃止款 `災害復旧費 0 / 77,200` がある**（廃止マーカーも `皆減` も無いので
    //   既定では行ごと落ち**前年度Σ が −77,200 の warning**）。**R6 だけ `kanNoless` を立てる**。
    //   ⚠⚠ **R2 に立ててはいけない** — `うち普通交付税` 等の内訳行を款として拾い Σ +15,254,000 になる。**年度ごとに明示**。
    // ⚠ **R2・H31・H30 は説明書が未公開**なので「主要事務事業説明資料」の概要型表を使う（別 title）。
    //   ⚠ **H30 だけ見出しが `⑵歳出予算(目的別)`**（他は `(款別)`）。
    // ⚠ **R3 は 22688（修正前）と 22690（修正後）の2本があるので 22690 を使う** — 議会修正で
    //   土木費 △47,160 → 予備費 +47,160（総額不変）。**R4 の前年度列は修正後と一致する**。
    // ⚠ **R8 は「修正動議可決後」版に差し替え済み**（landing の更新日 2026-05-27）。再取得時にハッシュが動きうる。
    // 前年度列は**当初**（R8→H30 の8ペア・全款を突合して金額の不一致0。差異は新設/廃止款の 0 行のみ）。
    //   ⚠⚠ **R6 と R2 は市長選の年で骨格予算**だが、**徳島型と違い前年度列は肉付補正後ではなく骨格の当初そのもの**
    //   （R7 の前年度列 101,290,000 は R6 の当初と1円まで一致）。⇒ `prevBasis` は「当初」で正しい。
    //   ⚠ ただし **R7 の +9.0%・R3 の +13.0% は「骨格予算との比較」**なので `prevNote` を置く。次の骨格年は **R10**。
    // ⚠ 資料は OCR ではない（Producer は iTextSharp / SkyPDF / Excel 系）。
    // ライセンス「リンク・著作権・免責事項」（/site/userguide/58387.html・更新 2022-03-29・確認日 2026-08-24）。
    //   ⚠ オープンデータ規約（CC BY 4.0）は「オープンデータサイトにおいて公開するデータ」に自ら範囲を限り、
    //   **一覧に予算・決算・財政は0件**（一覧本文を全文検索して0ヒット）＝及ばない（§9g）。
    //   ⚠ リンク条項は「リンク設定はトップページへ**お願いします**」＝**依頼形**なので `noDeepLink` は立てない
    //   （大和・草加・調布と同群）。⚠ 同じ段落の「リンクであることを明記してください」は**出典表示の要求**であって
    //   深リンクの禁止ではない（画面の出典チップで満たせる）。フレーム条項も無い。
    // [年度, ファイル番号, landing, 歳入p, 歳出p, samePage, kanNoless, 説明書か, 歳出見出し]
    ["R8", "124788", "194629", 5, 6, false, false, true, "（歳出）"],
    ["R7", "105983", "163390", 5, 6, false, false, true, "（歳出）"],
    ["R6", "86733", "132226", 5, 6, false, true, true, "（歳出）"],
    ["R5", "70415", "108211", 5, 6, false, false, true, "（歳出）"],
    ["R4", "53826", "50034", 5, 5, true, false, true, "（歳出）"],
    ["R3", "22690", "6156", 5, 5, true, false, true, "（歳出）"],
    ["R2", "22671", "6144", 36, 37, false, false, false, "⑵歳出予算(款別)"],
    ["H31", "22646", "6121", 42, 43, false, false, false, "⑵歳出予算(款別)"],
    ["H30", "22621", "6112", 44, 45, false, false, false, "⑵歳出予算(目的別)"],
  ] as const).map(([fy, file, page, rev, exp, same, noless, isSetsumei, expHead]) => ({
    id: `matsumoto-yosansho-${fy.toLowerCase()}`,
    title: isSetsumei
      ? `${eraYear(fy)}年度 松本市一般会計予算説明書（歳入歳出予算事項別明細書 １ 総括）`
      : `${eraYear(fy)}年度 松本市当初予算 主要事務事業説明資料（一般会計予算額 歳入予算・歳出予算）`,
    publisher: "松本市",
    url: `https://www.city.matsumoto.nagano.jp/uploaded/attachment/${file}.pdf`,
    landingPage: `https://www.city.matsumoto.nagano.jp/site/matsumoto-zaisei/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "松本市（一般会計・団体コード202029）",
    license:
      "当サイトに掲載されている情報（文章、写真、イラスト、画像等の各ファイル）およびその内容に関する諸権利は、原則として松本市に帰属し、国際条約・法律等によって保護されています。（ただし、一部の画像等の著作権は、原著作者が所有しています。）「私的使用のための複製」や「引用」など著作権法上認められる場合を除き、松本市の許可なく当サイトに掲載されている文書・画像等を無断使用・複製・転載・販売・改変・印刷配布することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: isSetsumei ? "（歳入）" : "⑴歳入予算",
      expenditureHeading: expHead,
      ...(same
        ? {
            samePage: true,
            revenueTotalLabel: "合計",
            expenditureTotalLabel: "合計",
            // この巡でパーサの折返し待ちを3行構成にも効かせたので、`^助成交付金$` を捨てずに
            // `kanNameContinues` で完全形を取る（従来は款10 が `国有提供施設等所在市町村` に切れていた）。
            revenueHeaderExtra: "総括",
            expenditureHeaderExtra: "^本$|一般財源|国県支出金",
          }
        : {}),
      ...(!isSetsumei ? { revenueTotalLabel: "合計", expenditureTotalLabel: "合計" } : {}),
      ...(noless ? { kanNoless: true } : {}),
      ...(same ? { kanNameContinues: { revenue: [10] } } : {}),
      ...(fy === "R7"
        ? { prevNote: "前年度（令和6年度）の当初予算は、市長選挙を控えた骨格予算として編成されている（原典「令和6年度当初予算の概要」p.2）。前年当初比はその骨格予算との比較。" }
        : {}),
      ...(fy === "R3"
        ? { prevNote: "前年度（令和2年度）の当初予算は、市長選挙を控えた骨格予算として編成されている（原典「令和2年度当初予算 主要事務事業説明資料」の印字 p.2＝物理 p.4）。前年当初比はその骨格予算との比較。" }
        : {}),
      ...(fy === "R2"
        ? { prevNote: "本年度（令和2年度）の当初予算は、市長選挙を控えた骨格予算として編成されている（原典の印字 p.2＝物理 p.4）。" }
        : {}),
    },
  })),

  ...([
    // 佐世保市（長崎県・中核市・団体コード 422029）。⚠ 収録済みの長崎県 420000・長崎市 422011（`nagasaki-shi-`）
    //   とは別物。id は `sasebo-`。「佐世保市予算説明資料」（財務部）の
    //   **物理 p.4「一般会計予算（歳入）」/ p.6「歳出（目的別内訳）」**（歳入22〜23款・歳出15款）。
    // ⚠ 印字ノンブルは **R8 のみ物理と一致**（4/6）。R7 以前は 2/4＝**物理 +2**。
    // ⚠⚠ **歳入 p.4 の合計行にはラベルが1文字も無い**（R8〜R4・H25）。この巡で足した `totalNoLabel` で拾う。
    //   ⚠⚠ **金額そのものを `revenueTotalLabel` に書く回避策を採らない** — `"100"` のような短い数字を書くと
    //   `compact.includes(totalLabel)` の読み飛ばしに款行が巻き込まれ、**款1 市税・款7・款9・款23 が丸ごと落ちる**
    //   （偵察が Σ −44,621,701 を実測）。H23 以前は `合 計` があるので `totalNoLabel` は立てない。
    // ⚠⚠ **列は `[当年度, 構成比, 増減率, 増減額, 前年度, 構成比]` で増減額が間に挟まる** —
    //   既定の ints[1] は前年度ではなく**増減額**を掴む。`amountIntIndex: 0` / `prevIntIndex: 2` が必須。
    //   ⚠ 合計行だけ構成比が小数点なしの `100` なので `totalPrevIntIndex: 3`（豊橋 §13-8 と同型の理由）。
    // ⚠⚠ **`CropX` が全年度で必須**（右1/3に「増減の内訳」欄＝事業名＋金額がある）。
    //   ⚠ **右端は年度ごとに完全に別物**（歳入 311〜676pt）。偵察が「内訳が出ない最大 W」と
    //   「前年度合計が入る最小 W」を二分探索して中点を採っている。**年度を外挿しない**。
    // ⚠ **`kanNoCircled` が必須**（丸数字と半角数字が混在。R8 歳入は ①〜⑨・10・⑪⑫・13〜23）。
    // ⚠ `revenueHeading` は `(歳入)` ではなく **`一般会計予算`** — R7 は表題行が款1 に連結して
    //   `一般会計予算市税` になる（**Σ は差0 のまま**＝目視でしか気づけない）。
    // ⚠⚠ **`expenditureHeading` を `歳出` と縮めてはいけない** — 同じ PDF の p.8 が `歳出（性質別内訳）` で
    //   `歳出` を含むため見出しゲートを素通りする（当てると Σ +14,203,634 で落ちるが、守っているのは見出しの長さ）。
    // ⚠ 歳入款9/10 は原典が「款名行 / 金額行 / 下段 `助成交付金`」の3行構成の年度がある。
    //   **この巡でパーサの折返し待ちを3行構成にも効かせたので、`kanNameContinues` で10年度とも完全形になる**。
    //   ⚠⚠ **`助成交付金` を `revenueHeaderExtra` に入れないこと** — 断片を捨てる旧回避策で、
    //   **R6・R5 は同じ款が1行に収まっているため入れると款行ごと消えて Σ −781,200 になる**。
    // ⚠ **R7 だけ歳入に `うち ○ 臨時財政対策債 … 皆減` の内訳行がある**（`皆減` で廃止款の分岐に乗り
    //   前年度Σ +500,000）。⚠ R8・R6・R5・R4 では no-op なので **R8〜R4 にだけ入れてある**（H25 以前は空）。
    // ⚠⚠ **H20 の前年度列は「H19 の6月肉付補正後」**（H19 は市長選 2007年4月の骨格予算・当初 87,993,258 に対し
    //   肉付後 95,073,629）。⚠ **原典が列見出しに `6月補正後予算額` と明記している**ので自動判定も効くが、
    //   **crop を狭くすると列見出しごと切れて効かなくなる**ため `prevBasis` を明示する。
    //   ⚠ 次の市長選は **2027年4月（R9）**。ただし直近の R5（2023年4月選挙）は **R6 の前年度列＝R5 当初と1円まで一致**
    //   したので、**佐世保が毎回この扱いをするとは限らない＝年度を外挿せず毎年確認する**。
    // ⚠ **R3・R2・H31〜H26・H24 はスキャン画像**（抽出がファイル全体で 38〜145 バイト＝ノンブルのみ）、
    //   **H22 は説明資料そのものが無い**。⚠ **H18 以前は発行元の一覧に無い**（未調査）。
    // ⚠ 資料は OCR ではない（Producer は Print To PDF / iTextSharp / Excel / アンテナハウス系）。
    //   ⚠ スキャンの年度は **OCR すらされておらずテキスト層が完全に空**なので誤読の余地が無い（throw する）。
    // 前年度列は**当初**（R8→R5 の4組と H21↔H20 で全款一致。⚠ H20↔H19 だけ9〜10款が不一致＝上記の肉付後）。
    // ライセンス「著作権・免責事項について」（/chosakuken/index.html・更新 2026-03-25・確認日 2026-08-24）。
    //   ⚠ リンク条項は「**原則、リンクフリーですので…ご連絡は必要ありません**」＝**許容の宣言**で
    //   トップページ限定の指定が無い。⇒ `noDeepLink` は立てない（**長崎県とは逆側**）。
    //   ⚠ 唯一の指示形は「注記がある場合はその内容に従ってください」だが、**予算説明資料ページに注記は0件**（実測）。
    //   ⚠ フレーム条項も無い。⚠ オープンデータ（BODIK・119データセット）に**予算は0件**＝及ばない（§9g）。
    // [年度, ファイル名, 歳入crop, 歳出crop, 合計ラベル無しか, HeaderExtra]
    ["R8", "r8_tousyoyosan_setsumei.pdf", 311, 322, true, "臨時財政対策債", 10],
    ["R7", "r7_tousyoyosan_setsumei.pdf", 315, 312, true, "臨時財政対策債", 10],
    ["R6", "r6_tousyo_setsumei.pdf", 447, 372, true, "臨時財政対策債", 10],
    ["R5", "r5_tousho_setsumei_.pdf", 458, 375, true, "臨時財政対策債", 10],
    ["R4", "r4_tousyo_setsumei.pdf", 676, 649, true, "臨時財政対策債", 10],
    ["H25", "h25tousyoyosan_yosansetumei_1.pdf", 645, 558, true, "", 9],
    ["H23", "23yosansetumei_1.pdf", 652, 547, false, "", 9],
    ["H21", "21tousyoyosannsetumei_1.pdf", 394, 362, false, "", 9],
    ["H20", "yosannsetumeisiryou20_1.pdf", 394, 361, false, "", 9],
    ["H19", "zaisei2_1.pdf", 398, 356, false, "", 9],
  ] as const).map(([fy, file, revCrop, expCrop, noLabel, extra, contNo]) => ({
    id: `sasebo-yosansetsumei-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 佐世保市予算説明資料（一般会計予算 歳入・歳出（目的別内訳））`,
    publisher: "佐世保市",
    url: `https://www.city.sasebo.lg.jp/documents/17636/${file}`,
    landingPage: "https://www.city.sasebo.lg.jp/zaimu/zaisei/yosansetsumei.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "佐世保市（一般会計・団体コード422029）",
    license:
      "当サイトに掲載している文字、写真、イラストやデザインといった情報の著作権は、私たち又は原権利者に帰属します。私的使用又は引用等著作権法上認められている行為を除き、無断で転載等を行うことはできません。引用を行う際は、適宜の方法により、必ず出所を明示してください。また、当サイトの内容の全部又は一部について、私たちに無断で改変を行うことはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 6,
      revenueHeading: "一般会計予算",
      expenditureHeading: "歳出（目的別内訳）",
      expenditureTotalLabel: "合計",
      ...(noLabel ? { totalNoLabel: { revenue: true } } : { revenueTotalLabel: "合計" }),
      kanNoCircled: true,
      amountIntIndex: 0,
      prevIntIndex: 2,
      totalAmountIntIndex: 0,
      totalPrevIntIndex: 3,
      revenueCropX: { from: 0, to: revCrop },
      expenditureCropX: { from: 0, to: expCrop },
      revenueHeaderExtra: extra,
      // この巡でパーサの折返し待ちを3行構成にも効かせたので、切れていた款名を完全形で取れる。
      // ⚠ **款番号は年度で違う**（R8〜R4 は款10・H25 以前は款9）。取り違えると
      //   1つ前の款（環境性能割交付金）が下段待ちになり `環境性能割交付金国有提供施設等` になる（実測）。
      kanNameContinues: { revenue: [contNo] },
      ...(fy === "H20"
        ? {
            prevBasis: "補正後" as const,
            prevNote:
              "前年度（平成19年度）列は原典の列見出しが「6月補正後予算額」。平成19年度は市長選（2007年4月）の年で当初は骨格予算（87,993,258千円）、6月の肉付補正後が95,073,629千円。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 上尾市（埼玉県・団体コード 112194）。⚠ 収録済みのさいたま市・川越市・川口市・所沢市・越谷市・草加市とは別団体。
    // 「予算の概要」の**３．一般会計予算状況（１）歳入 /（２）歳出（目的別）**（歳入22〜23款・歳出12款・千円）。
    // ⚠ 物理ページは **R8〜R2 で 5/6 に固定**（印字ノンブルは物理 −2）。
    // ⚠⚠ **`revenueHeading` に節番号 `３．` を含めてはいけない** — パーサは行を**半角化してから**見出しと比較するため、
    //   原典の**全角 `３` が一致せず見出し行が読み飛ばされない**。結果、款1 の款名が **`一般会計予算状況市税`** になる。
    //   ⚠⚠ **Σ は4系統とも差0 のまま**なので款名の目視だけが網（偵察が try-parse で実際に踏んだ）。
    //   ⇒ **数字を含まない `一般会計予算状況` を使う**。歳出側は `（２）歳出（目的別）`（全角の丸数字は半角化の対象外）。
    // ⚠ 合計ラベルは**両側とも `合　　計`**（既定の `歳入合計`/`歳出合計` ではない）。
    // ⚠ 同じ PDF に p.4 会計別予算規模（特別会計・企業会計込み）と p.7 歳出（性質別）が同居するが、
    //   **どちらも款番号が印字されていない**ので当てると「款行が1件も抽出できませんでした」で **throw する**
    //   （偵察が実測）。⚠ **この資料に「Σ 差0 のまま完走する」型は無い** — 款番号の有無が構造的な仕切りになっている。
    // ⚠ 単位は p.5・p.6 とも `(単位：千円)` と明記。⚠ **全年度の PDF 全文に「百万円」は0件**（下関 §13-16 型の
    //   単位違いの同型表は無い）。
    // ⚠⚠ **R3 は必ず「修正後」`76147.pdf` を採る** — 修正前 `76148.pdf` の一般会計は 68,980,000 千円、
    //   修正後は 68,954,925 千円で、**R4 資料の前年度列は修正後と一致する**（偵察が実測）。
    // ⚠ R2 の廃止款は款番号欄が `△`・名称が2行折返し・当年度セルが `－`・伸率が `皆減`。
    //   ⚠ **`dashAsZero` は入れないこと** — `皆減` の語で 0 が確定するので不要なうえ、
    //   入れると増減率欄の `－` が余分な整数 0 になって列がずれる恐れがある（偵察の判断）。
    // ⚠ 2行折返しの款名が4件ある（`株式等譲渡所得割交付金`・`交通安全対策特別交付金`・`地方消費税交付金`・
    //   `ゴルフ場利用税交付金`）が、**いずれも完全形に再結合され次の款へ漏れていない**。
    // ⚠ R7〜R2 は款22「市債」の直下に `うち臨時財政対策債` の小計行があるが、**款番号が無いので二重計上されない**。
    // ⚠ **概要 PDF の表紙にも款別ページにも「上尾市」の文字が無い**（`令和８年度 / 予算の概要` のみ）。
    //   取り違えの検出は p.3 の本文（`第６次上尾市総合計画` 等）で行う。
    // 前年度列は**当初**（R8→R2 の6ペアで全款を突合して不一致0。⚠ p.5〜7 に `補正` も `骨格` も0件）。
    // ⚠ 資料は OCR ではない（Producer は Acrobat Pro / PDF Library / Excel 系）。
    //   ⚠ **H27・H26・H23・H22・H21 の概要はスキャン画像**（抽出が27〜32字＝ノンブルのみ）で、当てると throw する。
    //   ⚠ H31〜H28・H25・H24・H20 は同じ opts で通ることを偵察が実測しているが、**URL を控えていないので今回は見送った**
    //   （「収録できない」ではない）。
    // ライセンス「著作権等」（/site/userguide/copyright.html・更新 2016-02-13・確認日 2026-08-24）。
    //   ⚠ 上尾市は独自カタログを持たず埼玉県ポータル経由で、**上尾市のデータセット19件に予算・決算・財政は0件**
    //   （実検索）＝及ばない（§9g）。⚠ CC の表示も `by-nd` の href も無い。
    //   ⚠ リンク条項は「リンクは原則として自由に設定していただいて**かまいません**」＋
    //   「リンクである旨を**明記してください**」で、**指示の対象は出典の明示であってリンク先の限定ではない**
    //   （下関 §13-16 と同型）。⇒ `noDeepLink` は立てない。フレーム条項も無い。
    // [年度, ファイル番号, landing パス]
    ["R8", "116055", "414165"],
    ["R7", "107329", "387288"],
    ["R6", "99050", "361381"],
    ["R5", "90492", "332362"],
    ["R4", "81723", "301161"],
    ["R3", "76147", "0031210212002"],
    ["R2", "61213", "003120021704"],
  ] as const).map(([fy, file, page]) => ({
    id: `ageo-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 上尾市予算の概要（一般会計予算状況 歳入・歳出（目的別））`,
    publisher: "上尾市",
    url: `https://www.city.ageo.lg.jp/uploaded/attachment/${file}.pdf`,
    landingPage: `https://www.city.ageo.lg.jp/page/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "上尾市（一般会計・団体コード112194）",
    license:
      "上尾市Webサイトから発信する情報（テキスト、画像、PDF、音声そのほかのデータ）の著作権は、上尾市に帰属します。また一部の写真、イラスト画像などの著作権は原著作者が所有しています。本サイト上の文書、画像などの無断使用・転載、二次利用を禁止します。著作権法上の｢私的使用｣や｢引用｣の範囲を越えて、当Webサイトの情報の使用を希望する場合は事前に広報広聴課までお問い合わせください。事前の許可がない限り、転載、変更、発行、配布、掲示などは一切できません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 5,
      expenditurePage: 6,
      revenueHeading: "一般会計予算状況",
      expenditureHeading: "（２）歳出（目的別）",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
    },
  })),
  ...
  ...([
    // 佐賀市（佐賀県・中核市・県庁所在地・団体コード 412015）。⚠ 収録済みの佐賀県 410004 とも
    //   佐世保市 422029（`sasebo-`）とも別物なので id は `saga-shi-`。
    // 「当初予算の概要」の**物理 p.4「一般会計款別分析」**（【歳入】22款 →【歳出】13款の**縦積み**）。
    // ⚠ **単位は百万円**（表ヘッダに `（単位 百万円）` と明記）。⚠ 同じ PDF の p.6「社会保障４経費」は**千円**なので混ぜない。
    // ⚠⚠ **`samePage` は必須** — 落とすと**歳出側が歳入22款のコピーになり、Σ4系統とも差0 のまま完走する**
    //   （偵察が実測）。**款名を見なければ気づけない最危険型**。
    // ⚠⚠ **同じ p.4 に3つ目の表「一般会計性質別分析【歳出】」が縦に続き、その合計行も 119,800 / 113,200 で
    //   款別と完全に一致する**（香川 §11k と同型）。`samePage` は合計行の**先頭2つ**で切るので性質別は
    //   構造的に届かないが、**この一致があるので `samePage` を外したときに Σ が守ってくれない**。
    // ⚠ 款番号が印字されない表なので `kanNoless` が必須（無いと款0件で throw）。合計ラベルは両側とも `合計`。
    // ⚠ 隣接ページを当てた場合は throw する（p.13 特別会計＋企業会計は見出し不一致、p.6 は合計行が1件）。
    // ⚠⚠ **R7・R6 は ToUnicode 欠落の化けで、この巡では収録できなかった** — `decodeGarble` ＋
    //   `decodeGarbleBand: 16044`（0x3EAC・三重 R8 と同じ帯）で**数字・記号は正しく戻る**が、
    //   **`GARBLE_CHAR_MAP` に無い漢字が33字残って throw する**（偵察が版面の1:1対応から真の字を確定済み。
    //   `万人件修処助扶投持析法物理百的積立端維致貸資質防△【】ゴフルＡＢ激`）。
    //   ⚠ 未マップ字の大半は**性質別の表と注記行**にあり款別の帯には無いが、復号はページ全文に掛かるので
    //   **全部足さないと throw する**。⚠ **足す前に健全な R5/R8 の同じ表と突合すること**（`garble-decode.ts` の注意）。
    //   ⚠ **「収録できない」のではなくパーサ側の宿題**なので `unrecordable.ts` には載せない。
    // ⚠ **R4・R3 は収録不可**（`unrecordable.ts`）。⚠ R2 は現行サイトから削除済みで Wayback にのみ存在し、
    //   偵察が実取得してスキャンと確認しているが**私が確かめていないので台帳に載せない**。
    // ⚠ **R7・R6 だけ歳出「農林水産業費」が2行に割れる**（`農 林 水` / 金額行 `産 業 費 …`）ので、
    //   その2年度を通すときは款名を必ず目視すること。
    // 前年度列は**当初**（原典の列見出しが `当初 Ｂ` と基準を明示。加えて R5→R6→R7→R8 の年度間クロスチェックで
    //   全22款＋全13款が一致）。⚠ **佐賀市長選は 2021-10・2025-10 とも10月**なので4月当初が骨格になる年は無い
    //   （次は 2029年10月見込み）。
    // ライセンス「佐賀市公式ホームページの考え方」4.著作権（/shisei/koho-kocho/1/2/2721.html・
    //   更新 2026-06-01・確認日 2026-08-24）。
    //   ⚠ オープンデータカタログ（BODIK・58データセット・**CC BY 4.0**。href も `licenses/by/4.0` で `by-nd` ではない）は
    //   規約自身が「『佐賀市オープンデータ』…の利用に関しての規約」と範囲を限り、
    //   **`予算`・`決算`・`財政` がいずれも0件**（実検索）＝及ばない（§9g）。
    // ⚠⚠ **`noDeepLink` を立てた** — リンク条項が「なお、リンクは市ホームページの**トップページに設定してください**」＝
    //   **指示形**で、しかも**トップページ以外へのリンクを許す文言が無い**（水戸・八尾 §13-13 より強い）。
    //   §13-15 の動詞基準では立てる側。⚠ ただし振替先は年度ページであってトップページではないので、
    //   **条項を完全に満たしてはいない**（発行元へ寄せる方向の妥協）。⚠ フレーム条項は無い。
    // [年度, ファイル名, landing パス]
    ["R8", "p1jkmhfbjheuu1oh6o91jd0s304.pdf", "shisei/gyoseiunei/5/1/6/6810"],
    ["R5", "p1grn0h23g88lr301a5sju01of75.pdf", "shisei/gyoseiunei/5/1/1/3/1462"],
  ] as const).map(([fy, file, page]) => ({
    id: `saga-shi-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 佐賀市当初予算の概要（一般会計款別分析）`,
    publisher: "佐賀市",
    url: `https://www.city.saga.lg.jp/material/files/group/6/${file}`,
    landingPage: `https://www.city.saga.lg.jp/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "佐賀市（一般会計・団体コード412015）",
    license:
      "本サイトで提供する文書、画像等の各ファイルおよびその内容に関する著作権は、原則として佐賀市に帰属します。（ただし、一部の画像等の著作権は、原著作者が所有しています。）「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、文書、画像等の無断使用・転載を禁止します。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
  ...
  ...([
    // 春日部市（埼玉県・団体コード 112143）。⚠⚠ **春日井市（愛知県 232068・`kasugai-`）と別物**。
    //   id は `kasukabe-`。⚠ 収録済みのさいたま市・川越市・川口市・所沢市・越谷市・草加市・上尾市とも別団体。
    // 「当初予算の概要」巻末資料編の**《歳入》/《歳出：目的別》**（歳入21款・歳出13款・千円）。
    // ⚠ **印字ノンブルと物理ページのオフセットが年度で違う**（R8 は 0・R6 は −3）。物理ページで指定する。
    // ⚠⚠ **同じ PDF の《歳出：節別》が Σ4系統とも差0 で完走する**（R8 は p.32・R6 は p.27。
    //   節1〜28 が款として26件取れて合計も目的別と同値）。**`expenditureHeading` を `《歳出：目的別》` と
    //   完全形で書くことだけが網**で、`歳出` と縮めると素通りする（偵察が実測）。
    //   ⚠ **《歳出：性質別》は目的別と同一物理ページにある**ため、見出しを `《歳出：性質別》` と書いても
    //   目的別が返る（見出しゲートは存在チェックだけ）。**見出しはページの取り違えは止めるが同一ページ内の別表は止めない**。
    // ⚠⚠ **`expenditureHeaderExtra` は必須** — 列見出しの `款  別` が款1 に連結して **`款別議会費`** になる
    //   （R8・R6 とも再現。**Σ は差0 のまま**＝款名の目視でしか気づけない）。歳入側は行間の空行で切れるので
    //   no-op だが、版面が動いたときの保険として両側に入れてある。
    // ⚠ 同じ PDF の p.29（会計別予算の概要）は throw、p.33（特別会計・左右2側）は Σ +30,000 で落ちる（＝安全側）。
    // ⚠ 歳出款13 は **`予備費`**（独自款・100,000千円）。⚠ 歳入款8 環境性能割交付金は R8 で**象徴計上の1千円**。
    // ⚠ 単位は両ページとも `(単位：千円、％)` と明記。⚠ 同じ PDF の p.23/p.24（市債・基金の推移）は**百万円**だが
    //   款別表ではない。⚠ 負号は `▲`。ダッシュ・空セルは0件なので `dashAsZero` は不要。
    // ⚠⚠ **R7・R5・R4・R3 は収録不可**（`unrecordable.ts`）— **本文がアウトライン化**していて
    //   抽出文字数が全ページ合計で 48／0／0／0 字（収録時に実測）。⚠ **スキャン画像ではない**
    //   （画像はグラフ等で、文字が図形に変換されている）ので OCR 由来の誤読は無いが、1文字も取れない。
    //   越谷 R7 §13-10 と同型。⚠ **R3 ダイジェスト版はテキストがあるが億円丸め＋「その他」集約**で款別にならない。
    // ⚠ **R2・R1・H30・H29 は現行サイトから削除済み**。R2 は Wayback にあり偵察がアウトライン化と確認しているが
    //   **私が確かめていないので台帳に載せない**。R1 以前は未取得。
    // ⚠ **予算書・事業別概要書も全年度が画像/アウトライン**（抽出が 11〜126字）なので**代替ルートは無い**。
    // 前年度列は**当初**（R7・R5 の概要はテキストが取れないので**偵察が150dpi でレンダリングして目視**し、
    //   R7 の当年度列＝R8 の前年度列が全34項目一致、R5 の当年度列＝R6 の前年度列が全21款＋合計一致を確認）。
    //   ⚠ **市長選は 2021-10-24・2025-10-26 と10月**なので4月当初が骨格になる年度が無い（次は 2029-10）。
    // ⚠ 資料は OCR ではない（R8 の Producer は `Microsoft: Print To PDF`・款別ページに画像0件）。
    // ライセンス「ホームページ基本方針」（/soshikikarasagasu/citysaleskohoka/gyomuannai/5/2/3058.html・
    //   更新 2026-07-08・確認日 2026-08-24）＋「春日部市のオープンデータ」（/shiseijoho/tokeijoho/8619.html・
    //   更新 2026-03-25）。⚠ **市自身が「ホームページ上で公開しているデータ」と「オープンデータ」を明示的に区別**
    //   しているので2文を併記して原文のまま置く。
    //   ⚠ 埼玉県ポータルの春日部市44データセットに**予算0件・決算0件**（`財政` の1件は統計書で中身は決算）＝
    //   PDL1.0 は及ばない（§9g）。⚠ CC の表示も `by-nd` の href も無い。
    //   ⚠ リンク条項は「外部のホームページから市ホームページへのリンクは**原則自由とします**」＝**許容の宣言**で
    //   トップページ限定でも連絡要求でもない。⇒ `noDeepLink` は立てない（佐世保 §13-16 と同じ側）。フレーム条項も無い。
    // [年度, ファイル名, 歳入ページ, 歳出ページ]
    ["R8", "R8toushoyosannnogaiyou0226.pdf", 30, 31],
    ["R6", "R6tousyoyosannnogaiyou.pdf", 25, 26],
  ] as const).map(([fy, file, rev, exp]) => ({
    id: `kasukabe-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 春日部市当初予算の概要（一般会計 歳入・歳出［目的別］）`,
    publisher: "春日部市",
    url: `https://www.city.kasukabe.lg.jp/material/files/group/9/${file}`,
    landingPage: "https://www.city.kasukabe.lg.jp/shiseijoho/yosan_kessan_zaiseijokyo/8211.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "春日部市（一般会計・団体コード112143）",
    license:
      "市ホームページに掲載される全ての記事・画像データなどの著作権は春日部市に帰属します。無断転載・転用はご遠慮ください。ホームページ上で公開しているデータは、行政情報を広くお知らせするためのものであり、掲載されている情報や画像などを無断で転載することは禁止されています。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
  ...
  ...([
    // 宝塚市（兵庫県・団体コード 282146）。⚠ 収録済みの神戸・姫路・尼崎・明石・西宮・加古川とは別団体。
    // **経路を年度で使い分ける**: R8〜R4 は「当初予算説明書」の**歳入歳出予算事項別明細書 １総括**、
    //   H25〜H17 は「予算編成大要」の**一般会計 総括**（見出しも合計ラベルも別）。
    // ⚠⚠ **説明書に特別会計6本の総括が同居し、節ラベルも合計ラベルも一般会計と1文字も違わない**
    //   （R8 は物理 p.317/318 国保・347/348 国保診療施設・367/368 介護・399/400 後期高齢・419/420 財産区・
    //   435/436 市営霊園）。⚠ 偵察が実測: **p.317 を当てると Σ4系統とも差0 で完走**し款名が
    //   `国民健康保険税` `保険給付費` になる。⚠ **国保は歳入合計＝歳出合計なので側の取り違えも Σ では捕まらない**。
    //   ⚠⚠ **見出しは特別会計側にもあるので網にならない — ページの固定だけが網**
    //   （八尾 §13-13・つくば §13-14・徳島/大和 §13-15・調布 §13-16 に続く6例目）。
    // ⚠ **歳入の合計行は次ページにしかない**（p.7 に款23件・p.8 に `歳入合計`）ので `revenuePages` が必須。
    //   単ページ指定は「歳入合計 行が見つかりません」で throw する。
    // ⚠⚠ **`expenditureHeaderExtra` が無いと款1 が `款特定財源一般財源議会費` になる**（財源内訳のヘッダが連結）。
    //   **Σ は差0 のまま**なので款名の目視だけが網。
    // ⚠⚠ **概要「宝塚市の予算」（PPT）は使ってはいけない** — 款名の右に説明文が同一行で入り
    //   `1 市税市民税、固定資産税などの…` になるうえ、款9 の `※令和8年3月末で廃止` 注記が金額を壊して
    //   **款21 繰越金が丸ごと落ちる**（Σ +6 / −97,998）。⚠ **`CropX` では救えない** —
    //   款名が x≈20–101・**説明文が x≈133–433**・金額が x≈500–830 で、説明文が**中間帯**にある。
    // ⚠ 印字ノンブルと物理ページのずれが年度で違う（R8/R7 は +2・R6/R4 は 0・R5 は +1・大要は +1〜+4）。
    // ⚠ **H18 だけ `kanNameContinues` が要る** — 款9 が「款行に名前も金額もあり下段が次行」の型（仙台 §9d と同型）で、
    //   指定しないと `国有提供施設等所在市町村` で切れる（下段は静かに捨てられるので次の款は無傷・**Σ は差0**）。
    //   ⚠ **他年度に付けてはいけない** — H25 等は中央寄せ3行型で、付けると1つ前の款が下段待ちになる。
    // ⚠⚠ **R8 の前年度（R7 当初）は実質的な骨格予算** — 市長交代（2025年4月）前に編成され、原典の
    //   「令和7年度6月補正予算後」資料が「政策的判断を要する経費（新規・拡充等）は2号補正に計上」と明記。
    //   ⚠ **ただし前年度列は肉付後ではなく当初そのもの**（R8 の前年度 95,450,000 ＝ R7 当初。全37款で一致）＝
    //   **松本 §13-16 型であって徳島型ではない**。`prevBasis` は当初のままで `prevNote` を付ける。
    //   ⚠ 同じ構図が R4 にもある（R3 当初 81,510,000。⚠ R3 の原典はスキャンなので**総額でしか裏取りできていない**）。
    //   ⚠ **市は選挙年（H29・R3・R7）にだけ「6月補正予算後」版を別途公表する**。次は **R11**。
    // ⚠ 歳出款6 は **`農林業費`**（`農林水産業費` ではない）。⚠ 款13・14 は `諸支出金`・`予備費`。寄せない。
    // ⚠ 単位は全年度「千円」（`百万円` は説明書・大要とも0件＝下関 §13-16 型の1000倍事故は起きない）。
    // ⚠ 資料は OCR ではない（Producer は Acrobat Pro / iTextSharp / PrimoPDF / Distiller 系）。
    //   ⚠ R3 説明書と H21 大要は**複合機スキャンで OCR すら掛かっていない**（640p で640字・63p で63字）。
    // ⚠ **サイトが 2025-12-01 にリニューアルして全 URL が変わっている**。landingPage には新 URL を使う。
    // ⚠⚠ **ライセンスは草加 §13-15 と同型の論点** — **宝塚市の本体サイトにはサイト全体の著作権ページが存在しない**
    //   （サイトマップ全走査・Wayback CDX とも0件）。唯一の条項は**市議会サイト**のもので、自ら
    //   「宝塚市議会のホームページに掲載されている」と範囲を限っている＝厳密には本 PDF に及ばない。
    //   ⚠ その文言だけを採ると判定語に1つも当たらず `unverified`＝自サーバー配信が開いてしまう。
    //   ⇒ **草加で確立した前例を適用**: **同じ発行元が自ドメインで「許可なく無断で複製や転用をすることはできません」
    //   「営利を目的とした二次利用はお断り」と明示している以上、開ける側へ倒さない**。原文をそのまま置いて要許可にする。
    //   ⚠ **これは規約の射程の判断であって原文の意訳ではない**（§9g）。
    //   ⚠ オープンデータ規約（CC BY 4.0）は「掲載しているページ上にライセンスを表示させており」と範囲を限り、
    //   **予算ページの HTML に CC 表示は0件**・**一覧 xlsx に予算/決算/財政は0件**（実測）＝及ばない（§9g）。
    //   ⚠ CC の href まで確認済み（予算ページには CC のアンカー自体が無く、函館型の BY-ND 偽装は該当しない）。
    //   ⚠ リンク条項は「リンク設定は**原則自由としています**」＝許容の宣言なので `noDeepLink` は立てない。
    //   ⚠ ただし「フレーム内に…表示するようなリンクの設定は**ご遠慮ください**」があるので iframe の導線は作らない。
    // 前年度列は**当初**（R4→R8 の4ペアと H17→H19・H22→H25 の連鎖で全款一致）。
    // ⚠ **R3・R2・H31〜H26・H21・H20 の10年度は収録不可**（`unrecordable.ts` に3群で記録）。
    //   ⚠ **H16 以前は未調査**（発行元の予算ページの一覧は H17 が最古だが、消えたのか元から無いのかは確かめていない）。
    // [年度, url パス, landing ID, 歳入from, 歳入to, 歳出p, 説明書か, H18 か]
    ["R8", "001/062/614/r8_tousyosetumei_2.pdf", "1062614", 7, 8, 9, true, false],
    ["R7", "001/059/274/r7_tousyosetumei.pdf", "1059274", 7, 8, 9, true, false],
    ["R6", "001/055/632/r6_tousyosetumei.pdf", "1055632", 5, 6, 7, true, false],
    ["R5", "001/049/834/yosansetumeir5.pdf", "1049834", 6, 7, 8, true, false],
    ["R4", "001/045/177/yosansetumei.r4.pdf", "1045177", 5, 6, 7, true, false],
    ["H25", "001/012/635/taiyou25.pdf", "1012635", 10, 10, 11, false, false],
    ["H24", "001/012/635/taiyou24.pdf", "1012635", 8, 8, 9, false, false],
    ["H23", "001/012/635/taiyo23.pdf", "1012635", 8, 8, 9, false, false],
    ["H22", "001/012/635/taiyo22.pdf", "1012635", 8, 8, 9, false, false],
    ["H19", "001/012/635/taiyo19.pdf", "1012635", 7, 7, 8, false, false],
    ["H18", "001/012/635/taiyo18.pdf", "1012635", 7, 7, 8, false, true],
    ["H17", "001/012/635/taiyo17.pdf", "1012635", 6, 6, 7, false, false],
  ] as const).map(([fy, path, page, rf, rt, exp, isSetsumei, isH18]) => ({
    id: `takarazuka-yosansho-${fy.toLowerCase()}`,
    title: isSetsumei
      ? `${eraYear(fy)}年度 宝塚市当初予算説明書（歳入歳出予算事項別明細書 総括）`
      : `${eraYear(fy)}年度 宝塚市予算編成大要（一般会計 総括）`,
    publisher: "宝塚市",
    url: `https://www.city.takarazuka.hyogo.jp/_res/projects/default_project/_page_/${path}`,
    landingPage: isSetsumei
      ? `https://www.city.takarazuka.hyogo.jp/1060687/1060727/1000137/1042315/${page}.html`
      : `https://www.city.takarazuka.hyogo.jp/1060687/1060727/1000137/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "宝塚市（一般会計・団体コード282146）",
    license:
      "宝塚市議会のホームページに掲載されている文章や画像等の各ファイル及びその内容に関する権利は、原則として宝塚市に帰属します。「私的利用のための複製」や「引用」など著作権法上認められた場合を除き、許可なく無断で複製や転用をすることはできません。また、営利を目的とした著作物の二次利用はお断りしております。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      ...(rf === rt ? { revenuePage: rf } : { revenuePages: { from: rf, to: rt } }),
      expenditurePage: exp,
      revenueHeading: isSetsumei ? "(歳入)" : "一般会計",
      expenditureHeading: isSetsumei ? "(歳出)" : "（歳出）",
      revenueTotalLabel: isSetsumei ? "歳入合計" : "合計",
      expenditureTotalLabel: isSetsumei ? "歳出合計" : "合計",
      expenditureHeaderExtra: isSetsumei ? "特定財源|一般財源" : "特定財源|一般財源|国県支出金|地方債",
      ...(isH18 ? { kanNameContinues: { revenue: [9] } } : {}),
      ...(fy === "R8"
        ? {
            prevNote:
              "前年度（令和7年度）の当初予算は市長改選前に編成されたもので、政策的判断を要する経費（新規・拡充等）は令和7年度2号補正予算に計上されている（原典「令和7年度6月補正予算後 宝塚市の予算」に明記）。ここでの前年度額は当初額そのもの。",
          }
        : {}),
      ...(fy === "R4"
        ? {
            prevNote:
              "前年度（令和3年度）の当初予算は市長改選前に編成されたもので、政策的な経費は6月補正予算に計上されている。ここでの前年度額は当初額そのもの（令和3年度の原典はスキャン画像のため、総額での確認にとどまる）。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 山形市（山形県・中核市・団体コード 062014）。⚠ 収録不可の山形県 060003 とは別。
    // ⚠⚠ **本命の「当初予算事項別明細書 総括」は使えない** — R8/R5 は**桁が横に約6.9pt 重なって**
    //   `-layout` が金額を3行に割り（`674,` / `475` / …）、R7/R6 は **ToUnicode 欠落で数字が1文字も出ない**
    //   （`grep -o '[0-9]' | wc -l` が 0。⚠ **スキャンではなく born-digital**）。
    //   ⚠ 誤って当てても「款行が1件も抽出できませんでした」で **throw する**（＝静かには通らない）。
    //   ⚠ **`joinWrappedAmounts` では救えない**（断片が2〜3段になり同じ行に別の款の断片が混じる）。
    //   救うには `pdftotext -bbox` で y でまとめ x 順に並べて重なる語を連結する抽出モードが要る（**宿題**）。
    // 代わりに財政課「**財政状況 資料編**」の1枚もの2種を**歳入・歳出で別ファイル**に割り当てる
    //   （`revenueFile` / `expenditureFile`）。⚠ 年度列が12本並び、**右端2列だけが当初予算額・それより左は決算額**
    //   という構造で、その区別は**末尾の ※注記1行にしか書かれていない**。
    // ⚠⚠⚠ **`prevIntIndex` を1つ間違えると前年度が「前々年度の決算額」になり、Σ は4系統とも差0 のまま完走する**
    //   （偵察が実測: `prevIntIndex: 9` で歳入 114,253,059／歳出 111,230,120＝R6 決算を読む）。
    //   **列見出しは年度名だけで当初/決算の区別が無い**。⇒ **年度間クロスチェックだけが検出する**ので
    //   **R8 と R7 を必ず両方入れる**（片方だけだと derive の連鎖ゲートが張れない）。
    //   ⚠ 手がかりとして「歳入の前年度合計と歳出の前年度合計が食い違う」（決算では 114,253,059 ≠ 111,230,120）
    //   が出るので、レビューでは前年度合計を左右で見比べること。
    // ⚠⚠ **`revenueCropX` が無いと歳入の款名が汚れる** — 左端の**縦書きラベル**「自主財源」「依存財源」が
    //   1文字ずつ混入して `自財産収入` `主財寄附金` `源繰入金` `依存環境性能割交付金` になる。**Σ は差0 のまま**。
    //   ⚠ **crop の境界は年度で動く**（R8 のラベル xMax=68.64・款名 xMin=73.20／R7 は 71.04・75.60）。
    //   `from: 70` では R7 が汚れ、`72` で両年度クリーン。**年度を足すたびに款名を全件見ること**。
    // ⚠ **`revenueHeading` を `歳入の推移` と書くと throw する** — タイトルが1トークン `<歳入の推移（一般会計）>` で、
    //   crop が先頭2文字を削るため。`の推移（一般会計）` に逃がしてある（R8 だけ落ちて R7 は通るので紛らわしい）。
    // ⚠ **歳入表に款番号が無く、法定順でもない**（自主財源→依存財源のグループ順）ので `kanNoless` が要る。
    //   ⚠ `revenueHeaderExtra: "小計"` が無いと「自主財源小計」「依存財源小計」を款として拾う。
    // ⚠ 廃止款（歳入`自動車取得税交付金`・歳出`諸支出金`）が `-` で入るので `dashAsZero` が要る。
    // ⚠ 同じフォルダの `-01` は**全会計**版。誤って当てると「当年度に指定した整数トークンがありません」で throw する。
    // ⚠ **R7 版は発行元が削除して 404**（ファイル名は `-r7-` / `-r8-` と年度別なので上書きではない。実測）。
    //   魚拓（Wayback 2025-06-23）から取る。
    //   ⚠ R7 版は Excel 出力 PDF だが列の並びは R8 版と同じで**添字も 11/10 で共通**。
    // ⚠ 資料編には `uni no` のフォントが混在する。R8・R7 では抽出が健全だが、**発行元が出力経路を変えると
    //   明細書と同じ症状になりうる**ので、年度追加時は `pdffonts` と try-parse を両方通すこと。
    // ⚠⚠ **R6 は収録不可**（`unrecordable.ts`）。⚠ **R5 は「収録できない」のではなくパーサ側の宿題**なので載せない。
    // ⚠ **R4 以前は発行元に無い**（2025-02-16 の魚拓でも既に R5 が最古）。「無い」と断定はできないので台帳に載せない。
    // ⚠ **単位は千円**。⚠ 骨格予算のリスクは無い（市長選は9月で当初予算編成期と重ならない）。
    // 前年度列は**当初**（原典の注記が「令和7・8年度は当初予算額」と明示。加えて R8 の前年度列 22款＋13款が
    //   R7 附属書の当年度列と全35款一致）。⚠ 補正後は `令和7年度1月末現計予算額` という**別列**なので取り違えない。
    // ライセンス「このホームページについて」（/about/1003696.html・更新 2023-09-11・確認日 2026-08-24）。
    //   ⚠ オープンデータ規約（**CC BY 4.0**・href も `licenses/by/4.0` で `by-nd` ではない）は
    //   規約自身が「山形市オープンデータ一覧に掲載されているデータの利用に関する規約」と範囲を限り、
    //   全70件に **`予算`・`財政`・`歳出` がいずれも0件**（実検索）＝及ばない（§9g）。
    //   ⚠ リンク条項は「自由にリンクすることができます」「承認を求める必要はありません」＝許可不要なので
    //   `noDeepLink` は立てない（「リンク先が公式ホームページであることを明示してください」は
    //   **リンクの深さの制限ではなく表示の要求**で、画面が発行元名を出している限り満たしている）。
    // [年度, 歳入ファイル, 歳出ファイル, 魚拓プレフィクス（歳入/歳出）]
    ["R8", "siryou-yosan-r8-05.pdf", "siryou-yosan-r8-09.pdf", "", ""],
    ["R7", "siryou-yosan-r7-05.pdf", "siryou-yosan-r7-09.pdf",
      "https://web.archive.org/web/20250623151246id_/", "https://web.archive.org/web/20250623152051id_/"],
  ] as const).map(([fy, revFile, expFile, revArc, expArc]): SourceEntry => {
    const base = "https://www.city.yamagata-yamagata.lg.jp/_res/projects/default_project/_page_/001/003/974/";
    return {
      id: `yamagata-shi-zaisei-siryou-${fy.toLowerCase()}`,
      url: null,
      title: `${eraYear(fy)}年度 山形市当初予算（款別歳入歳出・財政状況 資料編）`,
      publisher: "山形市",
      urls: [`${revArc}${base}${revFile}`, `${expArc}${base}${expFile}`],
      landingPage: "https://www.city.yamagata-yamagata.lg.jp/shiseijoho/zaisei/1007006/1003974.html",
      kind: "pdf" as const,
      fiscalYear: fy,
      scope: "山形市（一般会計・団体コード062014）",
      license:
        "公式ホームページに掲載されている文書や画像、映像、音声等の著作権は、原則として山形市に帰属します（一部の画像等の著作権は、原著作者が所有しています。）。これらの情報は、「私的使用のための複製」や「引用」などの著作権法上認められた場合を除き、無断で転用・引用することはできません。",
      parser: "kofu-yosansho" as const,
      parserOptions: {
        revenueFile: revFile,
        expenditureFile: expFile,
        revenuePage: 1,
        expenditurePage: 1,
        revenueHeading: "の推移（一般会計）",
        expenditureHeading: "歳出経費（目的別）",
        revenueTotalLabel: "合",
        expenditureTotalLabel: "合",
        kanNoless: true,
        dashAsZero: true,
        amountIntIndex: 11,
        prevIntIndex: 10,
        revenueCropX: { from: 72, to: 840 },
        revenueHeaderExtra: "小計",
      },
    };
  }),

  ...([
    // 太田市（群馬県・団体コード 102059）。⚠⚠ **東京都大田区 131113（収録済み・id `ota-yosansho-*`・
    //   スラグ `ota`）とは別団体**。id は `otagunma-`・スラグは `ota-gunma` にした。
    //   ⚠ **片方だけ直すと画面が静かに大田区へ飛ぶ**ので id・BUDGET_SOURCES・MUNI_SLUGS を必ず揃える。
    //   ⚠ 収録済みの群馬県・前橋市・高崎市とも別団体。
    // 財政課「当初予算（案）の概要」の「Ⅲ 予算(案)集計表」→「■ 一般会計（款別）」。
    //   歳入22款・歳出14款が**同一物理ページに縦積み**（`samePage`）。単位は表頭に `(単位：千円・％）`。
    // ⚠⚠ **`textSource: "raw"` が必須** — 表の左端に**縦書きの表側ラベル「歳/入」「歳/出」**があり、
    //   `-layout` だと `歳   9 環境性能割交付金` のように**4行が款行として認識されず脱落する**
    //   （歳入20款・歳出12款になる）。⚠ **`revenueCropX` では直らない**（-x を 40〜70 まで振っても
    //   出力が1文字も変わらないことを偵察が実測）。`-raw` なら縦ラベルが表の**後ろ**に隔離される。
    //   ⚠ この脱落は Σ が −947,269 等と割れるので**静かには壊れない**が、指定を忘れないこと。
    // ⚠⚠ **合計行の構成比が R8〜R2 は整数 `100`・H31/H30 だけ小数 `100.0`**。
    //   整数 `100` は `stripPercents` を通り抜けて前年度合計に化けるので **R8〜R2 は
    //   `totalAmountIntIndex: 0` / `totalPrevIntIndex: 2` が要る**（無指定だと Σ が +99,799,900 ずれる）。
    //   ⚠ **H31/H30 は逆に無指定が正**（指定すると前年度合計が増減額 450,000 に化けて Σ +78,850,000）。
    //   ⚠ **どちらの誤りも Σ が大きく割れる**ので静かには壊れない。
    // ⚠ **合計ラベルは両側とも `合計`**（原文は `歳 入 合 計` と字間空き）。`samePage` は同一ラベルの
    //   2回出現で区画を割るので、`歳入合計`/`歳出合計` と別々に書くと throw する。
    // ⚠ **H30 だけ見出しが `■ 一般会計`**（`（款別）` が付かない）。⚠ **弱い網なので物理ページの固定が本体**。
    // ⚠⚠ **同じ PDF の次ページが「■ 一般会計（性質別）」で、その合計が款別と完全に一致する**
    //   （98,000,000／99,800,000）。⇒ **Σ は網にならない**。当てると throw することは実測されているが、
    //   **網は物理ページの固定と款名の目視**（佐賀 §13-17・香川 §11k と同型）。同ページに特別会計5会計・
    //   企業会計・全会計総合計も並ぶ。
    // ⚠ **印字ノンブルは物理ページより2小さい**（全年度）。registry に書くのは物理ページ。
    // ⚠⚠ **R2 の歳入の廃止款で、掃除側と検出側が見ている文字列の違いが露呈した**（収録レビューで判明）。
    //   原典は `廃 自動車取得税交付金 0 0.0 100,000 ▲ 100,000 皆減` で、**`廃` は款番号の代わりに立つ裸のマーカー**。
    //   ⚠ **検出は raw（空白あり）・掃除は空白を畳んだ款名**に当たるので、`廃(?=\s)` を足しても
    //   掃除側では絶対に当たらず、**款名が `廃自動車取得税交付金` のまま配信されていた**。
    //   ⚠ 掃除側で空白なしの `^廃` を落とすと `廃棄物処理費` を壊すので、
    //   **raw で見分けてから namePart を落とす**二段構えにした（`BARE_ABOLISHED_MARK_RE`）。
    //   ⚠ **Σ は差0・金額も款番号も正しかった**ので、款名の目視でしか気づけない型。
    // 前年度列は**当初**（本文に「補正後」の語0件。加えて R8→H30 の8ペアすべてで前年度列＝前年度資料の
    //   当年度列が全款一致）。⚠ **太田市長選は 2025-04-13 で現職が落選している**が、
    //   **R7 当初は前年度比 +9.8% の通常予算で「骨格」「肉付」の語が0件**・R7 6月補正も +1.3% にとどまり、
    //   **R8 の前年度列も R7 当初と全款一致**＝松本 §13-16 型。骨格の年度は無い。
    // ⚠ **H29 以前は未検証**（直リンクは docs §13-18 に控えた）。⚠ **H30 で見出しが変わっている**ので
    //   **年度を外挿せず1年度ずつ当てること**。
    // ⚠ `pdftotext -bbox` はこの PDF 群で必ずクラッシュする（poppler の std::out_of_range）。
    // ライセンス「著作権・免責事項」（/site/userguide/1018650.html・更新 2023-02-20・確認日 2026-08-24）。
    //   ⚠ オープンデータサイト規約（**CC BY 4.0**・href も `licenses/by/4.0/deed.ja` を実確認で `by-nd` ではない）は
    //   規約自身が範囲を「太田市オープンデータサイトにおいて公開されるウェブサイト」に限り、
    //   一覧13件に **`予算`・`決算`・`歳入`・`歳出` がいずれも0件**（実検索）＝及ばない（§9g）。
    //   ⚠ **サイトにリンク条項が存在しない**（「このサイトについて」配下5ページ＋サイトマップ全走査で0件）
    //   ので `noDeepLink` は立てない（**動詞の形を判定する対象文そのものが無い**）。
    // [年度, landing ページ番号, 添付番号, 款別の物理ページ, 見出し, 合計列指定の要否]
    ["R8", "1056108", "37688", 20, "一般会計（款別）", true],
    ["R7", "1043330", "29715", 18, "一般会計（款別）", true],
    ["R6", "1032124", "21444", 18, "一般会計（款別）", true],
    ["R5", "1014109", "12108", 18, "一般会計（款別）", true],
    ["R4", "2531", "6720", 18, "一般会計（款別）", true],
    ["R3", "2532", "6722", 18, "一般会計（款別）", true],
    ["R2", "4530", "11100", 19, "一般会計（款別）", true],
    ["H31", "2533", "6724", 19, "一般会計（款別）", false],
    ["H30", "2534", "6726", 19, "一般会計", false],
  ] as const).map(([fy, pageId, att, kanPage, heading, totalIdx]) => ({
    id: `otagunma-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 太田市当初予算（案）の概要（予算(案)集計表 一般会計［款別］）`,
    publisher: "太田市",
    url: `https://www.city.ota.gunma.jp/uploaded/attachment/${att}.pdf`,
    landingPage: `https://www.city.ota.gunma.jp/page/${pageId}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "太田市（一般会計・団体コード102059）",
    license:
      "本サイトで使用されている文書・写真・画像等の著作権は、一部の資料を除き、基本的に太田市に帰属します。無断使用・転載を禁止します。必ず太田市の許可を受けてください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: kanPage,
      expenditurePage: kanPage,
      samePage: true,
      textSource: "raw" as const,
      revenueHeading: heading,
      expenditureHeading: heading,
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(totalIdx ? { totalAmountIntIndex: 0, totalPrevIntIndex: 2 } : {}),
    },
  })),

  ...([
    // 厚木市（神奈川県・団体コード 142123）。⚠ 収録済みの神奈川県・横浜・川崎・相模原・横須賀・平塚・
    //   藤沢・茅ヶ崎・大和とは別団体。
    // 「予算の概要」の「１ 歳入 (1) 款別内訳」（歳入22款）/「２ 歳出 (1) 目的別」（歳出12款）。単位は千円。
    // ⚠⚠ **同じ PDF に特別会計7本の款別表が同居し、款番号・列見出し・合計ラベルが一般会計と1文字も違わない**。
    //   ⚠ 偵察が実測: 見出しを素の `款別` にして p.93（国保）へ当てると **歳入7款/歳出7款が Σ4系統とも差0 で完走**する。
    //   ⚠ **国保は歳入合計＝歳出合計なので側の取り違えすら Σ では捕まらない**。
    //   ⇒ **`款別内訳` という見出しと物理ページの固定だけが網**（`款別内訳` は各年度の PDF 中ちょうど
    //   1ページにしか出ないことを全11年度で実測）。宝塚 §13-17 に続く7例目。
    // ⚠ 同じ PDF の次ページ「(2) 財源構造分析」「(2) 性質別」は**合計が款別と完全一致する**が、
    //   款番号が無いので当てると throw する（＝安全側）。
    // ⚠⚠ **印字ノンブルから物理ページを計算してはいけない** — ズレ幅が年度で変わるうえ、
    //   **H30 は歳入と歳出でオフセットが違う**（歳入 +2・歳出 +2 だが H29/H28 は印字より物理が小さい）。
    // ⚠ **款番号は5刻みで連番ではない**（歳入は 18・21・23・24・27・31 と不規則）。§9f で連番チェックを
    //   撤去済みなので通る。⚠ R2 は廃止款 `30 自動車取得税交付金` と新設款 `23 法人事業税交付金` を両方持つ。
    // ⚠⚠ **R5 には「予算の概要（当初予算・補正予算）」という肉付後版の別 PDF が存在する**
    //   （`R05-2budget-summary_atsugicity.pdf`）。**これに同じ opts を当てると前年度列が補正2号の額になり、
    //   Σ は4系統とも差0 のまま完走する**（偵察が実測）。⇒ **R5 は必ず当初版 `R05budget-summary_atsugicity.pdf` を使う**。
    //   ⚠ **年度間クロスチェックだけが検出する**。
    // ⚠⚠ **R5 当初は骨格予算**（市長選 2023-02-12・原典に「『骨格予算』として編成」と明記）。
    //   ⚠ **R6 の原典は前年度欄を `当初予算` `補正２号` `計` の3列に割り**、本文も「前年度予算額は
    //   R5当初＋6月補正＝R5本予算の額としています」と書いている。**パーサは先頭2つの整数を採るので
    //   拾うのは `令和５年度 当初予算` 列**（92,300,000。`計` 97,164,065 ではない）＝**松本 §13-16 型**。
    //   `prevBasis` は当初のままで **R6 だけ `prevNote` を付ける**。⚠ **次の選挙は 2027年2月なので R9 で再発する**。
    // ⚠ **R1 の PDF の表題は「平成31年度一般会計予算」**（発行元のページ名は「令和元年度予算」）。
    // ⚠ **R7 は同一 sha256 の PDF が2つの URL で配信されている**（`…atsugicity.pdf` と `…atsugicity1.pdf`）。
    //   landingPage と同じページに載っている `1` 付きを採った。
    // ⚠ `samePage` は**指定してはいけない**（歳入と歳出が別ページ。縦積みなのは特別会計のページだけ）。
    // ⚠ 年度 URL の規則は `reiwa8_1 / reiwa7_1 / reiwa6 / …` で **R8・R7 だけ `_1` が付く**（規則が破れる）。
    // ⚠ `(1)` は**半角括弧＋半角数字**（上尾 §13-17 の全角節番号の型には該当しない）。
    // ⚠ 資料は OCR ではない（Producer は `JUST PDF 5`。⚠ 下関 §13-16 の「JUST PDF 5 でアウトライン化」は
    //   厚木では起きておらず、全11年度でテキストが健全に取れる）。
    // ⚠ **H27 以前は発行元のナビに無い＝未調査**（「収録できない」ではないので台帳に載せない）。
    // 前年度列は**当初**（R8→H28 の10連鎖・延べ330款すべて一致）。
    // ⚠⚠ **ライセンスは要判断の論点があるが、この収録では安全側（要許可）に倒した** —
    //   本 PDF の配信元はサイトポリシー（/kurashi_tetsuzuki/online/11/10994.html・更新 2025-11-30・
    //   確認日 2026-08-24）が及び「無断で複製、転用等をすることはできません」＝`permission-required`。
    //   ⚠ **ただし厚木のオープンデータポータル（BODIK CKAN 組織 142123）には `予算` 19件・`決算` 18件・
    //   `財政` 10件が実在し**（§9g の「0件だから及ばない」型ではない）、**R5〜H28 の8年度は
    //   「予算の概要」の実体が CC BY 4.0 のリソースとして登録され、市サイト版と sha256 が完全一致する**
    //   （偵察が実測）。⚠ しかも規約4が「他のサイトの規約と異なるときは当サイトの規約が優先する」と書いている。
    //   ⇒ **`open` は開ける側なので発行元に照会せず倒さない**（草加 §13-15・宝塚 §13-17 と同じ判断）。
    //   ⚠ **R8・R7・R6 はポータルのデータセットが `resources: 0` なので、そもそも対象データですらない**。
    //   ⚠ CC の href は `licenses/by/4.0/deed.ja` を実確認（函館型の `by-nd` 偽装ではない）。
    // ⚠ リンク条項は「原則として自由にリンクしていただけます」＋「トップページに対して**お願いします**」＝
    //   **依頼形**なので `noDeepLink` は立てない（下関 §13-16 と同型）。
    //   ⚠ ただし「フレーム内に市ホームページを表示させることはお断りします」があるので iframe の導線は作らない。
    // [年度, ファイル名, landing パス, 歳入ページ, 歳出ページ]
    ["R8", "R08budget-summary_atsugicity.pdf", "reiwa8_1/51021", 9, 13],
    ["R7", "R07budget-summary_atsugicity1.pdf", "reiwa7_1/45942", 9, 13],
    ["R6", "R06_budget-summary_atsugicity.pdf", "reiwa6/40633", 9, 13],
    ["R5", "R05budget-summary_atsugicity.pdf", "reiwa5/35250", 9, 13],
    ["R4", "R04yosannogaiyou.pdf", "reiwa4/28238", 9, 13],
    ["R3", "R03yosannogaiyou.pdf", "reiwa3/19639", 9, 13],
    ["R2", "yosannogaiyou_36948084.pdf", "reiwa2/5858", 8, 12],
    ["R1", "yosannogaiyou.pdf", "reiwa1/4629", 8, 12],
    ["H30", "gaiyou_47837416.pdf", "heisei30/3975", 9, 12],
    ["H29", "H29gaiyou.pdf", "heisei29/3512", 6, 9],
    ["H28", "gaiyou.pdf", "heisei28/3157", 6, 9],
  ] as const).map(([fy, file, page, rev, exp]) => ({
    id: `atsugi-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 厚木市予算の概要（一般会計 款別内訳・目的別）`,
    publisher: "厚木市",
    url: `https://www.city.atsugi.kanagawa.jp/material/files/group/11/${file}`,
    landingPage: `https://www.city.atsugi.kanagawa.jp/soshiki/zaiseika/6/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "厚木市（一般会計・団体コード142123）",
    license:
      "厚木市ホームページに掲載されている情報（文字、写真、イラストなど）は著作権の対象となっています。また、ホームページ全体も編集著作権の対象となっています。これらの著作権は、厚木市および写真撮影者、イラスト作成者などに帰属しており著作権法および国際条約により保護されています。これらの情報について、著作権法上認められた「私的使用のための複製」や「引用」などの場合を除き、無断で複製、転用等をすることはできません。また、本ホームページの内容の全部または一部について無断で改変することもできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "款別内訳",
      expenditureHeading: "(1) 目的別",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(fy === "R6"
        ? {
            prevNote:
              "原典は前年度（令和5年度）の欄を「当初予算」「補正2号」「計」の3列に分け、本文では令和5年度本予算（当初＋6月補正）と比較している。ここでの前年度額は当初予算の列を採っている。令和5年度の当初予算は市長選のため骨格予算として編成されたもの。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 八戸市（青森県・中核市・団体コード 022039）。⚠ 収録済みの青森県 020002・青森市 022012 とは別団体。
    // 「当初予算の概要」の**物理 p.6「一般会計 歳入当初予算」/ 物理 p.8「一般会計【目的別】歳出予算額」**。
    // ⚠ **印字ノンブルと物理ページが一致する**（R8〜R3 の6年度で実測。ズレなし）。単位は `(単位：千円、％）`。
    // ⚠⚠ **見出し語が年度で3系統に割れる** — 歳出が R8 `【目的別】`・R7〜R4 `＜目的別＞`（括弧が違う）・
    //   R3 は**括弧なし** `一般会計目的別歳出予算額`。⚠ 歳入は全年度 `一般会計歳入当初予算` で通る
    //   （R7〜R3 は原典が `令和N年度一般会計歳入当初予算` だが後方一致で当たる）。
    // ⚠ **この資料は「Σ4系統とも差0 で別表を読む」経路が構造的に無い**（偵察が実測）:
    //   ① p.10 の性質別は**合計が目的別と完全一致する**が**款番号を持たない**ので必ず throw する
    //   ② 歳入と歳出が**別ページ**（`samePage` を立てると throw）
    //   ③ **合計ラベルが側で違う**（`歳入合計` / `歳出合計`）ので側の取り違えも必ず露見する
    //   ④ 特別会計の款別総括表が**同一 PDF に無い**
    //   ⇒ **厚木 §13-18・宝塚 §13-17 とは逆に、ここは Σ とラベルが守ってくれる**。
    // ⚠ 列は `[当年度(A), 構成比, 前年度当初(B), 増減額, 増減率, 前年度1月現計(D), …]` で、
    //   小数を除いた整数列が `[当年度, 前年度当初, 増減額, 1月現計]` になるため**既定の 0/1 で正しい**。
    //   ⚠ `prevIntIndex: 3` にすると「前年度1月現計」を読む（＝補正後）。指定しないこと。
    // ⚠ R8 の歳入に**廃止款**の行があり、款名が**原典どおり半角括弧つき** `(環境性能割交付金)` で入る
    //   （R7 では款8 `環境性能割交付金` 83,000 で、金額はクロスチェック一致）。⚠ **括弧は原典なので落とさない**。
    // ⚠ 款9 は `国有提供施設等所在市助成交付金`（`所在市町村` ではない八戸独自の短い表記）。**寄せない**。
    // ⚠⚠ **R3 は現行サイトから削除済みで魚拓からのみ取得**（`R3_tousyo.pdf`・Wayback 20220707173911）。
    //   ⚠ **R2 は現行 404・Wayback の CDX に捕捉0件・WARP は 500 で未確認**なので
    //   **「収録できない」とは書かない**（`unrecordable.ts` に載せない）。R1 以前は未検証。
    // ⚠ **R8 と R5 の PDF は Wayback に捕捉が1つも無い**（偵察が CDX で実測）。SPN 停止が続く限り
    //   この2年度は魚拓が持てない見込み。⚠ **R3 は逆に魚拓しか無い**ので ①git raw が命綱。
    // 前年度列は**当初**（表頭に `令和７年度 当初（Ｂ）` と明記され、`1月現計（Ｄ）` は別列。
    //   加えて R8→R3 の5ペアすべてで前年度列＝前年度資料の当年度列が全款一致）。
    // ⚠ **骨格予算の年度は無い**（市長選は 2021-10-31・2025-10-26 と10月で当初予算の編成期と重ならない。次は2029年秋）。
    // ライセンス「このサイトについて」（/soshikikarasagasu/kohotokeika/kohokocho/site_info/1479.html・
    //   更新 2023-05-02・確認日 2026-08-24）。
    //   ⚠ オープンデータ規約（**CC BY 4.0**・href も `licenses/by/4.0/deed.ja` を実確認）は原文自身が
    //   「当ライセンスは、オープンデータコーナー及び当ライセンスのロゴ画像が表示されたページのデータのみに
    //   適用されます」と範囲を限り、**全20データセットに `予算`/`決算`/`財政`/`歳入`/`歳出` が0件**（実検索）＝及ばない（§9g）。
    // ⚠⚠ **`noDeepLink` を立てた** — リンク条項が「**画像・PDFファイル等の添付データへの直接リンク以外は**
    //   トップページに関わらず自由に行って構いません」＝**PDF 直リンクだけを許諾の対象から外している**。
    //   ⚠ **指示形/依頼形の判定（§13-15）以前に、許諾の範囲そのものが PDF を除いている**ので立てる側
    //   （新宿 §10 と同型）。振替先は各年度の「予算と決算」ページ。
    //   ⚠ 「フレーム分割等」の条項もあるので発行元ページを iframe で開く導線は作らない。
    // [年度, ファイル名, landing のページ番号, 歳出見出し, 魚拓プレフィクス]
    ["R8", "R8_tousho.pdf", "25419", "一般会計【目的別】歳出予算額", ""],
    ["R7", "R7_tousyogaiyou.pdf", "24074", "一般会計＜目的別＞歳出予算額", ""],
    ["R6", "r6_tousho.pdf", "21691", "一般会計＜目的別＞歳出予算額", ""],
    ["R5", "r5tousyo.pdf", "19599", "一般会計＜目的別＞歳出予算額", ""],
    ["R4", "r4_toushixyo.pdf", "18041", "一般会計＜目的別＞歳出予算額", ""],
    ["R3", "R3_tousyo.pdf", "index", "一般会計目的別歳出予算額",
      "https://web.archive.org/web/20220707173911id_/"],
  ] as const).map(([fy, file, page, expHeading, arc]) => ({
    id: `hachinohe-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 八戸市当初予算の概要（一般会計 歳入当初予算・目的別歳出予算額）`,
    publisher: "八戸市",
    url: `${arc}https://www.city.hachinohe.aomori.jp/material/files/group/19/${file}`,
    landingPage: `https://www.city.hachinohe.aomori.jp/gyoseijoho/tokeijoho/zaisei/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "八戸市（一般会計・団体コード022039）",
    license:
      "八戸市ホームページに掲載されている情報（文字、写真、イラストなど）に関する著作権は、原則として八戸市または情報提供者に帰属します。著作権法上認められた場合を除き、当サイトの記事、写真、表、図面、グラフなど、内容の全部又は一部を、八戸市に無断で利用することはできません。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 6,
      expenditurePage: 8,
      revenueHeading: "一般会計歳入当初予算",
      expenditureHeading: expHeading,
    },
  })),

  // 東京都府中市（団体コード 132063）「予算のあらまし」。**R8〜H22 の17年度**が現行サイトに揃う。
  // ⚠⚠ **広島県府中市 342084 とは別団体**。id は `fuchu-tokyo-`・スラグも `fuchu` 単独を使わない
  //   （広島府中は未収録なだけで、将来必ず衝突する）。
  // ⚠ 取り違えの決め手: **歳入款50 が `都支出金`**（広島なら県支出金）・公営企業に競走事業（ボートレース多摩川）。
  // ⚠⚠ **同じ PDF の「歳出予算額（性質別）の状況」の合計が款別と完全一致する**（133,090,000／121,910,000）。
  //   ⚠ 性質別表には款番号が無いので当てると throw するが、**款番号があったら Σ では捕まらなかった**。
  //   ⇒ **ページ固定が網**。⚠ **府中は歳入合計＝歳出合計なので Σ だけでは側の取り違えも検出できない**
  //   （宝塚 §13-17・八尾 §13-13 と同型）。
  // ⚠ **印字ノンブルは基本 −2 だが H29・H27・H26 は印字が無く H22 は −1**。物理ページで指定する。
  // ⚠⚠ **`revenueHeaderExtra` の `一般会計予算のあらまし` は必須** — 節タイトル `３ 一般会計予算のあらまし` が
  //   「款番号3＋款名」と読まれ、**款5 が `一般会計予算のあらまし市税自主` になる**（**Σ は4系統とも差0**）。
  // ⚠⚠ **`kanNameSuffixStrip` も必須** — 款名と金額の間に**財源種別列（自主／依存）**があり、
  //   全22款が `市税自主` `地方譲与税依存` になる（**Σ は4系統とも差0**）。
  //   ⚠ **アンカー `$` を付けない** — H28 の折返し款で `依存` が款名の**中間**に入るため。
  // ⚠⚠ **`revenueHeaderExtra` の `^依存$` はアンカー必須**（無いと全 `依存` 行がヘッダ扱いで消える）。
  // ⚠⚠ **`kanNameContinues: {revenue:[20]}`** — 款20「国有提供施設等所在市町村助成交付金等」が版面で
  //   3行に割れる年度があり、欠くと**款20 が `国有提供施設等所在市町` で切れ、款24 が
  //   `村助成交付金等地方特例交付金` になる**（**Σ は4系統とも差0**）。
  // ⚠ 見出しに丸数字 `①` を含めても通る（`toHalfDigits` は `[０-９]` しか変換しない）が、
  //   上尾 §13-17 の轍を踏まないよう**節番号は入れない**。
  // ⚠ **H30 は巻末の「施策別予算事業一覧」が画像**だが、**款別ページは健全なテキスト**（収録に影響なし）。
  // ⚠ **R5 の歳出ページにだけ単位表記が1文字も無い**（`千円`/`単位` を全文検索して0件）。
  //   歳入ページの `［単位：千円・％］` と年度間クロスチェックで**千円**と確定した。
  // ⚠ **R4 だけ款20 の kanNo が null になる**（原典は 20 を印字しているが、金額行に款番号が無く
  //   廃止款の経路に落ちる）。**款名も金額も正しい**。
  // 前年度列は**当初**（R8→H22 の16ペアを全款突合し、金額の不一致は H25/H24 の1組だけ）。
  // ⚠⚠ **府中市長選は4年ごとの1月下旬**（2012-01-22・2016-01-31・2020-01-26・2024-01-28）で
  //   **当初予算の編成期と重なる**。⚠ **H24 が骨格予算**・**H25 の前年度列は H24 の6月補正後**。
  //   ⚠ **パーサの自動判定は「補正後予算額」という語を探すので、H25 の注記（「６月補正後の予算とその比較を
  //   記載しています」）では当初と誤判定する**。⇒ H25 に `prevBasis: "補正後"` を明示した。
  //   ⚠ **年度間クロスチェックだけが検出手段**（実測で歳入3款・歳出6款が食い違った）。
  //   ⚠ H28・R2・R6 は現職再選で骨格ではない（本文に `骨格` 0件・前年度列の全款一致）。**次は R10**。
  // ライセンス「このサイトの考え方」（/aboutweb/policy.html・更新 2023-04-01・確認日 2026-08-24）。
  //   ⚠⚠ **府中市は例外的に予算書・決算書・行政評価シートをオープンデータ（CC-BY）に載せている**ので、
  //   §9g の「予算は載っていない」型ではない。⚠ **ただしカタログ1,610件を実検索して
  //   「予算のあらまし」は0件**（`あらまし`0・`aramashi`0・`toushoyosan`0）＝**本資料には及ばない**。
  //   ⚠ オープンデータ側の予算書は H28〜R4 の事項別明細書だけで R5〜R8 は載っていない。
  //   ⚠ CC の表示・リンク先を実見（`creativecommons.jp/licenses/`。函館型の `by-nd` 偽装は無い）。
  //   ⚠ リンク条項は「**リンクフリーとなっています**」＝許容の宣言で、続く「リンクであることを
  //   必ず明記してください」は**出典表示の要求であってリンク先の限定ではない**（下関 §13-16・上尾 §13-17 と同型）。
  //   ⇒ `noDeepLink` は立てない。トップページ限定・フレーム禁止の文言も無い。
  // ⚠ **H21 以前は発行元のナビに無い＝未調査**（「収録できない」ではないので台帳に載せない）。
  // ⚠ **R8・R7 の PDF は Wayback に捕捉が0件**（偵察が実測）。SPN 停止中は魚拓が持てない。
  // ⚠ **URL 規則が5か所で破れる** — R5 の landing が CMS の自動 ID・R4 のファイル名が `aramasi`・
  //   H31〜H29 は landing が `H31…` なのにファイルは `31…`・H23 の landing が `H23yosannogaiyou` で
  //   ファイルが `H23yosangaiyou`・H25 のファイルが `20130401`。**年度から組み立てない**。
  //
  // ── 第1型: R8〜H29（見出しが `① 歳入予算額（款別・財源別）の状況`）
  ...([
    ["R8", 13, 16, "8yosannoaramashi", "R8yosannoaramashi"],
    ["R7", 12, 15, "7yosannoaramashi", "R7yosannoaramashi"],
    ["R6", 13, 16, "6yosannoaramashi", "R6yosannoaramashi"],
    ["R5", 13, 16, "_A00200220230216142345928", "R5yosannoaramashi"],
    ["R4", 13, 16, "R04yosannoaramasi", "R04yosannoaramasi"],
    ["R3", 11, 14, "R03yosannoaramashi", "R03yosannoaramashi"],
    ["R2", 11, 14, "R02yosannoaramashi", "R02yosannoaramashi"],
    ["H31", 9, 12, "H31yosannoaramashi", "31yosannoaramashi"],
    ["H30", 12, 15, "H30yosannoaramashi", "30yosannoaramashi"],
    ["H29", 16, 19, "H29yosannoaramashi", "29yosannoaramashi"],
  ] as const).map(([fy, rp, ep, page, file]) => ({
    id: `fuchu-tokyo-aramashi-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 府中市（東京都）予算のあらまし（一般会計 款別歳入・款別歳出）`,
    publisher: "府中市（東京都）",
    url: `https://www.city.fuchu.tokyo.jp/gyosei/zaise/yosan/toushoyosan/${page}.files/${file}.pdf`,
    landingPage: `https://www.city.fuchu.tokyo.jp/gyosei/zaise/yosan/toushoyosan/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "府中市（東京都・一般会計・団体コード132063）",
    license: FUCHU_TOKYO_LICENSE,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "歳入予算額（款別・財源別）の状況",
      expenditureHeading: "歳出予算額（款別）の状況",
      revenueTotalLabel: "歳入総額",
      expenditureTotalLabel: "歳出合計",
      revenueHeaderExtra: "一般会計予算のあらまし|^依存$",
      kanNameSuffixStrip: { revenue: "(?:自主|依存)" },
      kanNameContinues: { revenue: [20] },
    },
  })),
  // ── 第2型: H28〜H26（見出しが `■款別…`・款番号が款名に密着 `05市税`）
  // ⚠ 合計ラベルが H28 だけ `歳入総額`・H27/H26 は `歳入合計`。
  ...([
    ["H28", 14, 18, "歳入総額", "H28yosannoaramashi", "H28yosannoaramashi"],
    ["H27", 13, 16, "歳入合計", "H27yosannoaramashi", "H27yosannoaramashi"],
    ["H26", 13, 16, "歳入合計", "heisei26nendoyosannoaramashi", "heisei26nendoyosannoaramashi"],
  ] as const).map(([fy, rp, ep, revTotal, page, file]) => ({
    id: `fuchu-tokyo-aramashi-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 府中市（東京都）予算のあらまし（一般会計 款別歳入・款別歳出）`,
    publisher: "府中市（東京都）",
    url: `https://www.city.fuchu.tokyo.jp/gyosei/zaise/yosan/toushoyosan/${page}.files/${file}.pdf`,
    ...
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "■款別財源別歳入予算額の状況",
      expenditureHeading: "■款別歳出予算額の状況",
      revenueTotalLabel: revTotal,
      expenditureTotalLabel: "歳出合計",
      revenueHeaderExtra: "一般会計予算のあらまし|^依存$",
      kanNameSuffixStrip: { revenue: "(?:自主|依存)" },
      kanNameContinues: { revenue: [20] },
  ...
  // ── 第3型: H25〜H22（見出しが `【款別歳入予算額】`・右端に「主な増減額等」の自由記述列）
  // ⚠⚠ **`CropX` が必須** — 右端の「主な増減額等」列に `市民税 -33,179 固定資産税 -143,824` のような
  //   **金額入りの自由記述**があり、折返して款名の断片に化ける。境界は `pdftotext -bbox` で年度ごとに実測した
  //   （伸率列の右端と記述列の左端の間）。⚠ **年度で境界が動く**ので年度を足すたびに款名を全件見ること。
  // ⚠ H22 は負号が **ASCII ハイフン**（`△` ではない）。`toAmount` が対応済みなので追加指定は不要。
  // ⚠ この4年度には財源種別列が無いので `kanNameSuffixStrip` は付けない。
  ...([
    ["H25", 29, 33, 410, 407, "yosannnogaiyou", "20130401"],
    ["H24", 30, 36, 410, 405, "H24yosangaiyou", "24yosangaiyou"],
    ["H23", 32, 38, 410, 411, "H23yosannogaiyou", "H23yosangaiyou"],
    ["H22", 31, 37, 421, 412, "yosangaiyo", "H22gaiyou"],
  ] as const).map(([fy, rp, ep, rc, ec, page, file]) => ({
    id: `fuchu-tokyo-aramashi-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 府中市（東京都）予算の概要（一般会計 款別歳入・款別歳出）`,
    publisher: "府中市（東京都）",
    url: `https://www.city.fuchu.tokyo.jp/gyosei/zaise/yosan/toushoyosan/${page}.files/${file}.pdf`,
    landingPage: `https://www.city.fuchu.tokyo.jp/gyosei/zaise/yosan/toushoyosan/${page}.html`,
    kind: "pdf" as const,
    ...
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueCropX: { from: 0, to: rc },
      expenditureCropX: { from: 0, to: ec },
      revenueHeading: "【款別歳入予算額】",
      expenditureHeading: "【款別歳出予算額】",
      revenueTotalLabel: "歳入合計",
      expenditureTotalLabel: "歳出合計",
      kanNameContinues: { revenue: [20] },
      ...(fy === "H25"
        ? {
            prevBasis: "補正後" as const,
            prevNote:
              "平成24年度の当初予算は市長選挙のため骨格予算として編成されたので、原典は6月補正後の予算と比較している。ここでの前年度額も6月補正後のもの。",
          }
        : {}),
      ...(fy === "H24"
        ? {
            prevNote:
              "この年度の当初予算は市長選挙のため骨格予算として編成されたもので、政策的な経費は6月補正予算に計上されている。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 流山市（千葉県・団体コード 122203）。⚠ 収録済みの千葉県・千葉市・市川・船橋・松戸・柏・市原とは別団体。
    // 「予算に関する説明書」の**歳入歳出予算事項別明細書「１ 総括」**（歳入21款・歳出14款・千円）。
    // ⚠ **歳入の合計行は次ページにある**（款21 と `歳入合計` だけが送られる）ので `revenuePages` が必須。
    //   単ページ指定は「歳入合計 行が見つかりません」で throw する。
    // ⚠⚠ **この資料は罠が二重にある**（偵察が実測。**守ってくれるのはページ番号と款名の目視だけ**）:
    //   ① **特別会計4本の総括が同一 PDF・同一見出し・同一合計ラベルで同居する**（R8 は介護 p.537-539・
    //      国保 p.611-613 ほか）。当てると **Σ4系統とも差0 で完走**する。
    //      ⚠ **国保は歳入合計＝歳出合計なので側の取り違えすら Σ では捕まらない**。宝塚 §13-17・厚木 §13-18 に続く8例目。
    //   ② **別冊「予算附属資料」の「一般会計節別対前年度当初予算比較表」の合計が款別と1円まで同じ**
    //      （89,692,000／85,512,000）。⚠ **Σ4系統とも差0 で完走し、款名が `報酬` `給料` `職員手当等` になるだけ**。
    //      ⇒ **款名の目視だけが網**（春日部 §13-17 の節別表と同型だが、こちらは別ファイル）。
    // ⚠⚠ **`HeaderExtra` は両側とも必須** — 外すと **Σ は4系統とも差0 のまま**款1 だけが
    //   `歳入歳出予算事項別明細書総括市税`（歳入）・`一般財源国県支出金地方債その他議会費`（歳出）に壊れる。
    // ⚠ **物理ページが2世代ある**（H20/H21/H23/H24 は 5-6/7・それ以外は 7-8/9）。
    //   ⚠⚠ **H22 だけ H21/H23 と違って 7-8/9**。**年度を外挿しないこと**（下表は全年度を実測した値）。
    // ⚠ 見出しの空白は年度で揺れるので、空白除去後の `（歳入）`／`（歳出）` を指定する。
    // ⚠ 款構成は年度で変わる（H20〜R1 は歳入20款で `7 自動車取得税交付金` があり `環境性能割交付金`・
    //   `法人事業税交付金` が無い。R3〜R8 は21款）。**寄せない**。歳出は19年度とも14款で不変。
    // ⚠ **R1 の表紙は「平成３１年度」**（発行時の元号）。⚠ **R3 は pdftotext が表紙・目次で Syntax Error を出す**が
    //   総括ページは無傷。
    // ⚠⚠ **R2 は収録していない** — 廃止款 `△ 自動車取得税交付金` の**当年度セルが空欄**で、しかも
    //   **この資料には「皆減」の語彙欄そのものが無い**。パーサは `皆減` を見て当年度=0 を決める設計なので
    //   前年度額を当年度として読み、**Σ が +26,500 / +88,000 割れて error になる**（＝静かには壊れない）。
    //   ⚠ `dashAsZero` はダッシュ用・`prevBlankAsZero` は前年度側かつ款番号指定なので**どちらも効かない**
    //   （廃止款は款番号を持たない）。⇒ **「収録できない」のではなくパーサ側の宿題**なので `unrecordable.ts` に載せない。
    // ⚠ **H19 以前は発行元に無い**（インデックスの最古が H20）。Wayback/WARP は未調査。
    // 前年度列は**当初**（列見出しが `本年度予算額 / 前年度予算額 / 比較 / 構成比％`。
    //   加えて H20→R8 の18ペアを全款突合して不一致0）。
    // ⚠ **骨格予算の年度は無い** — 市長選は統一地方選の4月（直近 2023-04-23）で当初予算の編成期と重ならず、
    //   H20 以降ずっと現職の連続当選。**次は 2027年4月**。
    // ライセンス「著作権・免責事項・リンク」（/site/copyright.html・更新 2021-04-23・確認日 2026-08-24）。
    //   ⚠ **オープンデータのカタログには `予算` 9件・`決算` 33件・`財政` 63件が実在する**（＝厚木 §13-18 型を疑う場面）が、
    //   **9件はすべて Excel「一般会計＜事業財源別予算＞」で、URL に `.pdf` を含むエントリは0件**（全413件を実検索）。
    //   ⇒ **この説明書 PDF はカタログに登録されていない＝CC BY は及ばない**（従来の §9g 型）。
    //   ⚠ **各予算ページの「オープンデータ」節にも CC BY バッジが付く**が、注記が「**本セクションで公開している
    //   データ**」と範囲を限っており、その節に入っているのは Excel 1本だけで**3つの PDF は節の外**。
    //   ⚠ CC の href は `licenses/by/4.0/deed.ja` を実確認（`by-nd` 偽装ではない）。
    //   ⚠ リンク条項は「他サイトからのリンクについては、**原則として自由とします**」＋「URL については
    //   基本的にはトップページを**指定します**」＝**市が自らの運用を述べる書き方**で、リンクする側への
    //   指示形でも依頼形でもない。⚠ **八戸 §13-18 型（許諾の範囲が PDF 直リンクを除く）にも当たらない**
    //   （「画像・PDF ファイル等への直接リンク」を除く文言が全文に無い）。⇒ `noDeepLink` は立てない。
    // [年度, PDF パス, landing パス, 歳入from, 歳入to, 歳出p]
    ["R8", "001/051/523/r8toushoyosansetumeisho.pdf", "1051522/1051523", 7, 8, 9],
    ["R7", "001/047/710/r7toushoyosansetumeisho.pdf", "1047709/1047710", 7, 8, 9],
    ["R6", "001/043/454/setumeisyo.pdf", "1043453/1043454", 7, 8, 9],
    ["R5", "001/038/916/yosannsetumeisyo.pdf", "1038913/1038916", 7, 8, 9],
    ["R4", "001/032/384/r4_setumesyo.pdf", "1032383/1032384", 7, 8, 9],
    ["R3", "001/028/236/r3yosansetsumeisyo.pdf", "1028235/1028236", 7, 8, 9],
    ["R1", "001/019/612/31_yosansetsumei.pdf", "1019611/1019612", 7, 8, 9],
    ["H30", "001/016/319/30_setumeisyo_teisei.pdf", "1016318/1016319", 7, 8, 9],
    ["H29", "001/008/340/h29yosansetumeisyo.pdf", "1008339/023034", 7, 8, 9],
    ["H28", "001/008/343/h28yosan_setsumei.pdf", "1008342/023034", 7, 8, 9],
    ["H27", "001/008/346/27yosann.pdf", "1008345/1008346", 7, 8, 9],
    ["H26", "001/008/349/h26setumei.pdf", "1008348/1008349", 7, 8, 9],
    ["H25", "001/008/352/h25setumei.pdf", "1008351/1008352", 7, 8, 9],
    ["H24", "001/008/355/h24setumei_all.pdf", "1008354/1008355", 5, 6, 7],
    ["H23", "001/008/358/h23setumei_all.pdf", "1008357/1008358", 5, 6, 7],
    ["H22", "001/008/362/h22setumei_all.pdf", "1008361/1008362", 7, 8, 9],
    ["H21", "001/008/366/h21setumei-all.pdf", "1008365/1008366", 5, 6, 7],
    ["H20", "001/008/369/h20setumei_all.pdf", "1008368/1008369", 5, 6, 7],
  ] as const).map(([fy, pdf, page, rf, rt, exp]) => ({
    id: `nagareyama-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 流山市予算に関する説明書（歳入歳出予算事項別明細書 総括）`,
    publisher: "流山市",
    url: `https://www.city.nagareyama.chiba.jp/_res/projects/default_project/_page_/${pdf}`,
    landingPage: `https://www.city.nagareyama.chiba.jp/information/1008311/1008338/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "流山市（一般会計・団体コード122203）",
    license:
    ...
    parserOptions: {
      ...
      revenueHeading: "（歳入）",
      expenditureHeading: "（歳出）",
      revenueHeaderExtra: "事項別明細書|^1総括$",
      expenditureHeaderExtra: "^一般財源$|^国県支出金地方債その他$",
    },
  })),

  ...([
    // 伊勢崎市（群馬県・団体コード 102041）。⚠ 収録済みの群馬県 100005・前橋 102016・高崎 102024・
    //   太田 102059 とは別団体。
    // 「当初予算の概要」の「１．歳入予算の概要」（歳入21款）/「２．歳出予算の概要（目的別）」（歳出13款）。単位は千円。
    // ⚠ 同じ PDF の次ページ「３．歳出予算の概要（性質別）」は**合計が目的別と完全一致する**が、
    //   **性質別の行は款番号を持たない**ので当てると throw する（＝安全側・偵察が実測）。
    //   前橋 §13-10・明石 §13-11 の「静かに `人件費` が款1 になる」経路はこの資料には無い。
    // ⚠ 歳入と歳出は**別ページ**（`samePage` を立てると throw）。⚠ 側を取り違えると見出しチェックで throw する。
    //   ⚠ ただし**合計ラベルは R8〜R2 が両側とも `合計`** なので、守っているのは見出しだけ。
    // ⚠ 特別会計の款別総括表は**同一 PDF に無い**（特会は主要事業一覧のみ）。会計区分別予算表は款番号が無く throw する。
    // ⚠⚠ **太田 §13-18 の縦書き表側ラベルは伊勢崎には存在しない**（`-layout` で款の脱落なし）。
    //   同じ群馬でも様式は別物なので **`textSource: "raw"` を横展開しない**。
    // ⚠ **物理ページが年度で動く**（R8〜R5=5/6・R4〜H31=6/7・H29〜H25=5/6・H24/H23=8/10）。印字から計算しない。
    // ⚠ **合計ラベルが年度で3系統**（R8〜R2=`合計`/`合計`・H31〜H24=`歳入合計`/`合計`・H23=`会計合計`/`合計`）。
    // ⚠ **H24・H23 は見出し語も別世代**（`歳入の概要`／`２．一般会計当初予算`）。
    //   ⚠ **H23 は歳出ページに目的別と性質別が同居し合計行が2本**（どちらも 67,700,000）。目的別が先なので
    //   既定の1本目採用で正しいが、**`samePage` を立てると throw する**。
    // ⚠ R3・R2 の歳入に**廃止款が款番号 `-` の裸行**で入る（ゴルフ場利用税交付金・自動車取得税交付金）。
    // ⚠ 歳出は**13款で固定**（`予備費` を含む。前橋・越谷と同じ独自款）。**寄せない**。
    //   ⚠ 「全16年度」は**発行元にある年度数**（H30・H28 を含む）であって収録年度数（14）ではない。
    // ⚠⚠ **H30・H28 は ToUnicode 化けで現状 throw する**（`decodeGarble` を付けても
    //   `GARBLE_CHAR_MAP` に無い字が数文字残る）。⚠ **偵察が手元で復号し、H30 歳入の全21款が
    //   H31 の前年度列・H29 の当年度列と1円まで一致することを実測している**＝**中身は読める**。
    //   ⇒ **「収録できない」のではなくパーサ側の宿題**なので `unrecordable.ts` に載せない。
    //   直リンクは `h30toushoyosangaiyou320.pdf` / `h28tousyoyosan_gaiyou_pdf.pdf`（同じ group/18 配下）。
    // ⚠ **H22 以前は発行元のナビに無い＝未調査**。⚠ **R8 の「当初予算（案）概要」`R8yosanangaiyou.pdf` は現在 404**
    //   （現行ページがリンクしているのは確定版 `yosangaiyouR8.pdf` のみ）。
    // 前年度列は**当初**（H23・H24 は列見出しが `平成２３年度当初` と原典が明示。加えて連続する全ペアで
    //   前年度列＝前年度資料の当年度列が全款一致。全16年度で `骨格` の出現0件）。
    // ⚠ **市長選は 2025-01-19 で当初予算の編成期と重なる**が、R7 は骨格ではない
    //   （原典が「合併以降、過去最大の予算規模」と書き、議決後の広報の額も概要と一致）。
    // ⚠ **魚拓が薄い** — R8 のファイルもランディングも Wayback に捕捉0件。R7 は捕捉があるが
    //   **サイズが現物と違う（差替え版）**ので sha256 は一致しない見込み。
    // ライセンス「著作権・リンク・免責事項」（/gyoseijoho/koho_kocho/konosaitonitsuite/14818.html・
    //   更新 2018-02-01・確認日 2026-08-24）。
    //   ⚠ オープンデータ規約（**CC BY 4.0**・href も `licenses/by/4.0/deed.ja` を実確認）は規約自身が
    //   範囲を「伊勢崎市オープンデータサイトにおいて公開されるウェブサイト」に限り、
    //   **カタログは11件しかなく `予算`・`決算`・`財政`・`歳入`・`歳出` がいずれも0件**（実検索）。
    //   ⚠ **`group/18`（予算 PDF の置き場）を指すエントリも0件**＝厚木 §13-18 型の二重登録でもない。
    //   ⚠ リンク条項は「リンクは**ご自由に設定して頂いて構いません**」＋「**トップページ以外のページに
    //   リンクしても構いません**」＝**ディープリンクを明示的に許諾**しているので `noDeepLink` は立てない。
    //   ⚠ 「フレーム内に本サイトを表示させる…はご遠慮ください」があるので iframe の導線は作らない。
    //   ⚠⚠ **人の判断が要る論点**: 原文の第2段落が「**出所を明示することにより、引用・転載・複製することが
    //   できます**」と実質的な許諾を与えており、太田・草加より開いている。⚠ **だが `open` は開ける側なので
    //   倒さず要許可に置いた**（草加 §13-15・宝塚 §13-17・厚木 §13-18 と同じ判断）。
    // [年度, ファイル名, landing ページ番号, 歳入p, 歳出p, 歳入合計ラベル]
    ["R8", "yosangaiyouR8", "15824", 5, 6, "合計"],
    ["R7", "R7_tousyoyosann_gaiyou", "15824", 5, 6, "合計"],
    ["R6", "R6_tosyoyosan_gaiyo", "15824", 5, 6, "合計"],
    ["R5", "R5_yosan_gaiyou", "15824", 5, 6, "合計"],
    ["R4", "R4_yosan_gaiyou", "15824", 6, 7, "合計"],
    ["R3", "R3_yosan_gaiyou", "15824", 6, 7, "合計"],
    ["R2", "h02tousyoyosannnogaiyou", "15824", 6, 7, "合計"],
    ["H31", "h31toushoyosangaiyou319", "15824", 6, 7, "歳入合計"],
    ["H29", "H29yosangaiyou", "15825", 5, 6, "歳入合計"],
    ["H27", "h27toushoyosan_gaiyou", "15825", 5, 6, "歳入合計"],
    ["H26", "H26toshoyosan_gaiyo", "15825", 5, 6, "歳入合計"],
    ["H25", "H25toshoyosan_gaiyo", "15825", 5, 6, "歳入合計"],
    ["H24", "H24toshoyosan_gaiyo", "15825", 8, 10, "歳入合計"],
    ["H23", "H23toshoyosan_gaiyo", "15825", 8, 10, "会計合計"],
  ] as const).map(([fy, file, pageId, revPage, expPage, revTotal]) => ({
    id: `isesaki-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 伊勢崎市当初予算の概要（一般会計 歳入予算の概要・歳出予算の概要［目的別］）`,
    publisher: "伊勢崎市",
    url: `https://www.city.isesaki.lg.jp/material/files/group/18/${file}.pdf`,
    landingPage: `https://www.city.isesaki.lg.jp/gyoseijoho/gyozaisei/zaisei_yosan_kessan/yosan/${pageId}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "伊勢崎市（一般会計・団体コード102041）",
    license:
      "伊勢崎市ウェブサイトに掲載されている、写真・イラスト・音声・動画および記事は、著作権の対象となっています。また、伊勢崎市ウェブサイト全体も編集著作物として著作権の対象となっており、ともに日本国著作権法および国際条約により保護されています。原則、著作権は伊勢崎市に帰属しており、一部の画像等の著作権は、原著作者が所有していますので、無断での使用や転載を禁じます。本ウェブサイトの内容の全部または一部については、私的使用または引用等著作権法上認められた行為として、適宜の方法により出所を明示することにより、引用・転載・複製することができます。ただし、「無断転載を禁じます」などの注記があるものについては、それに従ってください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: revPage,
      expenditurePage: expPage,
      revenueHeading:
        fy === "H24" ? "歳入の概要" : fy === "H23" ? "２．一般会計当初予算" : "１．歳入予算の概要",
      expenditureHeading:
        fy === "H24" ? "歳出の概要（目的別）" : fy === "H23" ? "（目的別）" : "２．歳出予算の概要（目的別）",
      revenueTotalLabel: revTotal,
      expenditureTotalLabel: "合計",
    },
  })),
  ...
  ...([
    // 呉市（広島県・中核市・団体コード 342025）。⚠ 収録済みの広島県・広島市・福山市とは別団体。
    //   ⚠ **広島県府中市 342084 は未収録**（東京都府中市 132063 とは別）なので混同しない。
    // 財政課「当初予算基礎資料」の「（１）歳入 款別内訳」（23款・R2 は24款）/「（２）歳出 目的別内訳」（14款）。千円。
    // ⚠⚠ **同じ PDF の「（３）歳出 性質別内訳」の合計が目的別と完全に一致する**（115,430,000／111,690,000）。
    //   ⚠ `expenditureHeading` を素の `歳出` に縮めると **Σ4系統とも差0 のまま `人件費` `扶助費` を収録して完走する**
    //   （R8 で実測）。⚠⚠ **しかも年度で挙動が変わる**（R5 は同じ誤適用で Σ が −6,453,839 割れる）
    //   ＝**Σ に期待できない**。**見出しの完全形とページ固定だけが網**。
    // ⚠ 見出しに節番号 `（１）` を**入れない**（上尾 §13-17 の型：比較の前に数字が半角化されて一致しなくなる）。
    // ⚠⚠ **`kanIndentMax: 1` が必須**（R2 で実測）— 歳入の款内訳行 `⑵ 減債基金繰入金 0 … 350,000 … 皆減` が
    //   **皆減マーカー経由で款として拾われ、前年度Σが +350,000 ずれる**。⚠ **前年度Σの不一致は validate では
    //   warning なので derive まで流れる**型。R8〜R4 では no-op（差分0 を実測）。
    // ⚠ 合計ラベルは両側とも `合計`（既定の `歳入合計` では throw する）。
    // ⚠ p.3（R5 以前は p.1）の「予算総括表」は会計別一覧で款番号が無く throw する（＝安全側）。
    // ⚠ 物理ページ＝印字＋2（R8〜R6 は 5/6・R5/R4/R2 は 4/5）。
    // ⚠⚠ **R2 は発行元から削除済みで魚拓からのみ取得**（file・landing とも Wayback）。
    //   ⚠ **R3 は収録不可**（`unrecordable.ts`）で年度の鎖が切れるため、R2 の前年度列は
    //   **同年の「当初予算のポイント」が `令和元年度（当初）` と明記し 100,294 百万円が一致する**ことを根拠にした。
    // ⚠ **市長選は4年ごとの11月**（2025-11-09）で当初予算の編成期と重ならない＝骨格予算の年度は無い。
    // 前年度列は**当初**（R8→R4 の4リンクで歳入23款・歳出14款が全款一致）。
    // ⚠ **R8・R7・R6 の PDF に Wayback の捕捉が無い**（landing はある）。SPN 停止中は魚拓が持てない。
    // ライセンス「リンク・著作権・免責事項」（/site/userguide/link-policy.html・更新 2017-08-28・確認日 2026-08-24）。
    //   ⚠ オープンデータは「データプラットフォームくれ」（外部・CC BY 4.0・href は
    //   `licenses/by/4.0/legalcode.ja` を実確認）へ移行済みだが、**カタログ175データセットを
    //   `予算`/`決算`/`財政`/`歳入`/`歳出`/`市税` で全文検索してすべて0件**＝及ばない（§9g）。
    //   ⚠ **旧 LinkData は令和8年3月31日で公開終了**と発行元が明記。
    //   ⚠ リンク条項は「リンクは、**原則として自由です**。ご希望のページにリンクを設定していただいて
    //   構いません」＝**許容の宣言**なので `noDeepLink` は立てない。⚠ 「フレーム内リンクなどはご遠慮ください」
    //   があるので iframe の導線は作らない。
    // [年度, ファイル番号 or 魚拓 URL, landing, 歳入p, 歳出p]
    ["R8", "https://www.city.kure.lg.jp/uploaded/attachment/109287.pdf",
      "https://www.city.kure.lg.jp/soshiki/89/soshiki89tousho08.html", 5, 6],
    ["R7", "https://www.city.kure.lg.jp/uploaded/attachment/100886.pdf",
      "https://www.city.kure.lg.jp/soshiki/89/soshiki89tousho07.html", 5, 6],
    ["R6", "https://www.city.kure.lg.jp/uploaded/attachment/89293.pdf",
      "https://www.city.kure.lg.jp/soshiki/89/soshiki89tousho06.html", 5, 6],
    ["R5", "https://www.city.kure.lg.jp/uploaded/attachment/80628.pdf",
      "https://www.city.kure.lg.jp/soshiki/89/tousho05.html", 4, 5],
    ["R4", "https://www.city.kure.lg.jp/uploaded/attachment/69221.pdf",
      "https://www.city.kure.lg.jp/soshiki/89/shisei40-yosan-tousyo.html", 4, 5],
    ["R2", "https://web.archive.org/web/20220324184939id_/https://www.city.kure.lg.jp/uploaded/attachment/46933.pdf",
      "https://web.archive.org/web/20250116083313/https://www.city.kure.lg.jp/soshiki/89/shisei20-yosan-tousyo.html", 4, 5],
  ] as const).map(([fy, url, landingPage, rev, exp]) => ({
    id: `kure-yosan-kiso-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 呉市当初予算基礎資料（一般会計 歳入 款別内訳・歳出 目的別内訳）`,
    publisher: "呉市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "呉市（一般会計・団体コード342025）",
    license:
  ...
  ...([
    // 西東京市（東京都・団体コード 132292）。⚠ 収録済みの東京都・特別区23区・八王子・調布・町田・
    //   府中市（東京 132063）とは別団体。
    // 「予算の概要」の「２ 一般会計予算 （１）歳入 / （２）歳出」。**物理ページは全年度 歳入 p.4 / 歳出 p.5 で固定**
    //   （印字ノンブルは 2/3 ＝ +2 ずれ）。単位は `(単位：千円・％)`。
    // ⚠⚠ **同一 PDF に「合計が歳出合計と完全一致する別表」が2つある**（`４ 節別対前年度比較表` p.11 と
    //   `５（１）性質別経費対前年度比較表` p.16。いずれも 91,578,000／88,694,083）。
    //   ⚠ **どちらも合計ラベルが `合 計` の2文字で歳入/歳出の語を持たない**ので、既定ラベルのままなら必ず throw する。
    //   ⇒ **`expenditureTotalLabel` を `合計` に緩めてはいけない**（緩めると節別・性質別に当たる）。
    //   ⚠ **合計ラベル `歳入合計`/`歳出合計` が全35ページ中この2ページにしか無い**ことが守りの中心。
    // ⚠⚠ **同じ年度ページに `予算要求の概要`（12月公表・要求額）がある**。⚠ **前年度列は正しい前年度当初なので
    //   年度間クロスチェックでは捕まらない**（当年度合計だけが違う）。⇒ **ファイル名で見分ける**。
    // ⚠ 特別会計の款別総括表は**この PDF に無い**（特会は総額表のみ）＝取り違え経路が無い。
    // ⚠ **ファイル名に規則が無い**（`R8_yosannnogaiyou`＝n×3・`R6_yosannogaiyou`＝n×2・`26yosannogaiyo`＝末尾 u なし
    //   ・H24 はディレクトリ名とファイル名が違う）。**年度から組み立てない**。
    // ⚠⚠ **R2 以前は現行サイトが 404 で WARP からのみ取得**。⚠ **H31 の Wayback 版は 1 MiB ちょうどの打ち切り**
    //   （§9b と同型）なので **WARP 版を使う**。
    // ⚠ R2 歳入の廃止款 `自動車取得税交付金` は款番号なし・当年度 `-` だが**マーカーは付いていない**。
    // 前年度列は**当初**（原典 p.2 の注記「令和７年度以前の数値については、当初予算額です。」が H30 以降の全年度に有り。
    //   加えて R8→H24 の14ペアで全款一致）。⚠ **H29 以前にはこの注記が無い**のでクロスチェックだけが根拠。
    // ⚠ 市長選は 2021-01-31・2025-02-02 と**当初予算の編成期に重なる**が、
    //   **R8〜H30 の全 PDF に「骨格」「肉付」「補正後」の語が0件**（実測）＝骨格の年度は見当たらない。
    // ライセンス「免責事項・著作権など」（/aboutweb/menseki.html・更新 2026-07-24・確認日 2026-08-24）。
    //   ⚠ オープンデータは**東京都オープンデータカタログ**（組織 t132292・CC BY）に載るが、
    //   **全37データセットに `予算`・`決算`・`財政`・`歳入`・`歳出` が0件**、さらに
    //   **`url:city.nishitokyo.lg.jp/siseizyoho/zaisei` の resource 検索も0件**＝**同一ファイルの登録も無い**
    //   （厚木 §13-18 型ではない）。⚠ 市サイト側に CC の href は0件。
    // ⚠⚠ **`noDeepLink` を立てた** — リンク条項が「リンク先の URL は…**ホームページ“…/”に設定してください**」＝
    //   **指示形かつリンク先をトップページに限定**し、さらに**事後の連絡まで求めている**
    //   （徳島・茅ヶ崎・富士 §13-15 と同型）。⚠ フレーム条項は無い。
    // [年度, PDF URL, landing]
    ["R8", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-8/R8yosannogaiyou.files/R8_yosannnogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-8/R8yosannogaiyou.html"],
    ["R7", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-7/R7yosannogaiyou.files/R7_yosannnogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-7/R7yosannogaiyou.html"],
    ["R6", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-6/R6yosannogaiyou.files/R6_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-6/R6yosannogaiyou.html"],
    ["R5", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-5/R5yosannogaiyou.files/R5_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-5/R5yosannogaiyou.html"],
    ["R4", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-4/R4yosannogaiyou.files/R4_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-4/R4yosannogaiyou.html"],
    ["R3", "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-3/R3yosannogaiyou.files/R3_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-3/R3yosannogaiyou.html"],
    ["R2", "https://warp.ndl.go.jp/20210207/20210207182403/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/reiwa-2/r2yosannogaiyou.files/R2yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["R1", "https://warp.ndl.go.jp/20190808/20190808230302/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H31yosan/31yosannogaiyou.files/31yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H30", "https://warp.ndl.go.jp/20190209/20190209154502/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H30yosan/30yosannogaiyou.files/30_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H29", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H29yosan/29yosannogaiyou.files/29_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H28", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H28yosan/28yosannogaiyou.files/28_yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H27", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H27yosan/27yosannogaiyou.files/27yosannogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H26", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H26yosan/26yosannogaiyo.files/26yosannogaiyo.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H25", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H25yosan/25yosannnogaiyou.files/25yosannnogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
    ["H24", "https://warp.ndl.go.jp/20180213/20180213130902/https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/H24yosan/24yosannogaiyou.files/24yosannnogaiyou.pdf",
      "https://www.city.nishitokyo.lg.jp/siseizyoho/zaisei/yosan/kako.html"],
  ] as const).map(([fy, url, landing]) => ({
    id: `nishitokyo-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 西東京市予算の概要（一般会計 款別歳入歳出）`,
    publisher: "西東京市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "西東京市（一般会計・団体コード132292）",
    license:
      "西東京市Web上の情報は、原則として西東京市が著作権を有しています。法律で認められた場合を除き、無断で転用・引用することを禁じます。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
  ...
  ...([
    // 八千代市（千葉県・団体コード 122211）。⚠ 収録済みの千葉県・千葉市・市川・船橋・松戸・柏・市原とは別団体。
    //   ⚠ 大和市（神奈川 142131・収録済み）とも別。
    // 「予算の概要」の「① 歳入」/「② 歳出（目的別）」。単位は `（単位：千円，％）`（全角コンマを使う資料）。
    // ⚠ **印字ノンブルと物理ページが +2〜+4 ずれる**（年度ごとに違う）。物理ページで指定する。
    // ⚠⚠ **歳入表の左端に縦書きの財源種別ラベル `自 主 財 源` / `依 存 財 源` がある**。指定しないと
    //   **款番号の前に1文字入って lead が当たらず、その行の款が丸ごと落ちる**（R8 で 24→16款）。
    //   ⚠ ただし **Σ が −7,581,381 で割れるので静かには壊れない**。⇒ `revenueCropX` が必須。
    //   ⚠⚠ **crop 値は年度ごとに違う**（版面座標が違う）。⚠ **R3・R2・H27・H26 は `pdftotext -bbox` が
    //   poppler ごと落ちて座標を測れない**ので、偵察が Σ差0 になる値を総当たりで確定した
    //   （**R3 だけ 128 が必要** — `自` のグリフ矩形が右へはみ出す。§9j「crop は重なる語を残す＝右端で境界を引く」型）。
    //   ⇒ **年度を足すたびに款名を全件見ること**。
    // ⚠⚠⚠ **合計ラベルが歳入・歳出とも `合  計` で同一**なので、**側を取り違えても Σ4系統とも差0 で完走する**
    //   （偵察が実測）。⇒ **款名の目視が唯一の検出手段**。**歳入側に `市税`・歳出側に `議会費` が来ていることを必ず見る**。
    // ⚠⚠ **R6 は同じ様式の別 PDF が2本ある** — `28809.pdf`（**一般会計補正予算（第1号）を含む**）は
    //   **補正1号が財源振替で合計が動かないため、当てると4系統とも Σ差0 で完走する**（違うのは3款だけ）。
    //   ⚠ **表紙2行目の `（一般会計補正予算（第１号）を含む）` が唯一の見分け**（款別ページには表記が無い）。
    //   ⇒ 正は **`28810.pdf`**。R7 の前年度列（繰入金 2,013,133／諸収入 1,896,208／県支出金 5,943,892）と
    //   突合して確定した。⚠ `28791.pdf` は `28810.pdf` と md5 一致の重複なので registry に入れない。
    // ⚠ 同一 PDF の「③ 歳出（性質別）」は合計が目的別と完全一致するが款番号を持たず throw する（＝安全側）。
    //   ⚠ **特別会計の款別総括表はこの PDF に無い**（特会は事業ベースの記述表）＝§13-17 型の経路は無い。
    // ⚠ **`小 計`（自主財源計・依存財源計）が2行入るが `合計` を含まない**ので款行にも合計行にもならない。
    // ⚠⚠ **validate が全18年度で「款番号が昇順ではありません」の warning を出すが、これは実態どおり** —
    //   **原典が款番号順ではなく「自主財源→依存財源」の順に並べている**（`1 市税 / 13 分担金及び負担金 /
    //   14 使用料及び手数料 / 17 財産収入 …` の後に `2 地方譲与税` 以降が続く）。⚠ **収録時に款番号と款名の
    //   対応を全年度で目視して確かめた**。⚠ この warning は `caveats.gen.ts` 経由で画面の「データの注意」に出る。
    // ⚠⚠ **款番号が R7→R8 で振り直されている**（`分担金及び負担金` 14→13・`市債` 23→22・`環境性能割交付金` 9→24）。
    //   ⇒ **年度間クロスチェックは款名で行う**（款番号で突合すると全滅する）。
    // ⚠ 款名の折返し（`国有提供施設等所在市町村助成交付金`）はパーサ既定の経路で正しく組める（`kanNameContinues` 不要）。
    // 前年度列は**当初**（R8→R2 と H28→H21 の2本の鎖で全款一致。⚠ **H29 が収録不可なので H30↔H28 で鎖が切れる**）。
    // ⚠⚠ **市長選は4年ごとの5月**（2025-05-18）なので当初予算の編成期とは重ならないが、
    //   **選挙のある年度の当初予算は骨格予算**（R7・R3・H29 の原典に明記）。
    //   ⚠ **肉付け後の別 PDF は R7・R3 のどちらにも無い**（年度ページを実確認）。
    //   ⇒ `prevBasis` は当初のままで、**R8・R4 に `prevNote`** を付けた（前年度が骨格だったことを断る）。次は R11。
    // ⚠ **H29・H20・H19 は収録不可**（`unrecordable.ts`）。⚠ **H17 以前は旧ドメインに PDF があるが未検証**。
    // ⚠ 主な事業・事業報告はどちらも**既存の様式に当てはまらない**ので別 PR に回す。
    // ライセンス「サイトポリシー」（/site/userguide/23802.html・更新 2023-06-16・確認日 2026-08-24）。
    //   ⚠ **オープンデータ規約が自ら「オープンデータ以外のデータは本市ホームページのサイトポリシーに準ずる」と
    //   書いている**（規約 PDF 3⑵イ）＝§9g の型そのもの。カタログ全20件に `予算`・`決算`・`財政`・`歳入` は0件。
    //   ⚠ **カタログ HTML に `creativecommons` の href が0件**なので **license 欄に CC を書いてはいけない**。
    //   ⚠ リンク条項は「リンクは、**原則として自由です**」が先に立ち、「**なるべく**トップページに設定してください」と
    //   緩めており、その理由も「予告なく変更することがあります」＝リンク切れの注意であって許諾の制限ではない。
    //   ⚠ 八戸 §13-18 型（PDF 直リンクを許諾範囲から除く）にも当たらない。⇒ `noDeepLink` は立てない
    //   （山形・春日部・宝塚 §13-17 と同じ「許容の宣言が先に立つ」側）。⚠ フレーム条項も無い。
    // [年度, ファイル番号, landing ページID, 歳入p, 歳出p, revenueCropX.from]
    ["R8", "45788", "75423", 7, 8, 99],
    ["R7", "36528", "60533", 8, 9, 108],
    ["R6", "28810", "45406", 8, 9, 109],
    ["R5", "20440", "11935", 8, 9, 110],
    ["R4", "10180", "2595", 7, 8, 118],
    ["R3", "10166", "2585", 8, 9, 128],
    ["R2", "36603", "2569", 7, 8, 110],
    ["H31", "10109", "2561", 8, 9, 98],
    ["H30", "10092", "2549", 8, 9, 98],
    ["H28", "10004", "2522", 8, 9, 99],
    ["H27", "10244", "2619", 7, 8, 100],
    ["H26", "9968", "2511", 7, 8, 100],
    ["H25", "9946", "2499", 7, 8, 98],
    ["H24", "9874", "2441", 9, 10, 99],
    ["H23", "9875", "2444", 9, 10, 99],
    ["H22", "9876", "2447", 9, 10, 107],
    ["H21", "9877", "2450", 9, 10, 107],
    ["H18", "9879", "2453", 6, 7, 106],
  ] as const).map(([fy, file, page, rev, exp, cropFrom]) => ({
    id: `yachiyo-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 八千代市予算の概要（一般会計 歳入・歳出［目的別］）`,
    publisher: "八千代市",
    url: `https://www.city.yachiyo.lg.jp/uploaded/attachment/${file}.pdf`,
    landingPage: `https://www.city.yachiyo.lg.jp/site/yachiyo-zaisei/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "八千代市（一般会計・団体コード122211）",
    license:
      "八千代市公式ホームページ（以下、「本ホームページ」という）全体及び掲載情報の著作権は、原則として八千代市（以下、「本市」という）に帰属します（一部の画像等の著作権は、原著作者が所有しています。）。「私的使用のための複製」や「引用」など、著作権法上認められた場合を除き、本市の許可なく本ホームページに掲載している文書・画像等を無断で複製・転用することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      ...
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      revenueCropX: { from: cropFrom, to: 845 },
      ...(fy === "R8" || fy === "R4"
        ? {
            prevNote:
              fy === "R8"
                ? "前年度（令和7年度）の当初予算は、令和7年5月の市長選挙を控えた骨格予算として編成されたもの（令和7年度予算の概要に明記）。ここでの前年度額はその骨格予算の当初額。"
                : "前年度（令和3年度）の当初予算は、令和3年5月の市長選挙を控えた骨格予算として編成されたもの（令和3年度予算の概要に明記）。ここでの前年度額はその骨格予算の当初額。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 伊丹市（兵庫県・団体コード 282073）。⚠ 収録済みの兵庫県・神戸・姫路・尼崎・明石・西宮・加古川・宝塚 282146 とは別団体。
    // 「当初予算（案）及び前年度2月補正予算（案）」概要 PDF の**「一般会計総括表」**。列見出しが
    //   `本年度 当初予算額（A）` / `前年度 当初予算額（B）` と**当初を明記**。単位は千円。
    // ⚠⚠ **様式が3世代ある**（偵察が9年度すべてに try-parse を当てて実測）:
    //   R8〜R6: 歳入・歳出が**別ページ**。見出し `一般会計総括表（歳入）`/`（歳出）`。**合計ラベルは両側とも `合計`**
    //           ⚠ 八千代 §13-19 型だが、**見出しが側ごとに違うのでページを入れ替えると throw する**（実測）。
    //   R5:     別ページ。見出しは両側とも `一般会計総括表`。合計ラベルは `歳入合計`/`歳出合計`（既定）。
    //   R4〜H30: **同一ページに縦積み**（`samePage`）。合計ラベルは原典 `歳入合計`/`歳出合計` だが、
    //           samePage は同一 totalLabel の2出現で区画を切るので**部分一致の `合計` を両側に指定する**。
    //           ⚠ samePage を外すと歳出側が歳入22款を巻き込んで36款・Σ +83,400,000 で割れる（＝ゲートが捕まえる）。
    // ⚠ 同一 PDF の「各会計総括表」（特会・企業込み）と「性質別歳出予算の状況」（**合計が款別と同額**）は
    //   どちらも款番号が無いので当てると throw する（＝安全側・実測）。
    // ⚠⚠ **R3 以前は同じ PDF の末尾に「前年度2月補正」の「一般会計総括表」が同名見出しで載る**（R2 は p.26）。
    //   当てると**当年度 Σ は差0（現計予算額 78,017,459）で、前年度 Σ だけ +42,596,570 割れる**。
    //   ⚠ **前年度 Σ の不一致は validate では warning なので derive まで流れる**。⇒ ページ番号を年度ごとに実ファイルで固定する
    //   （当初の表は R2〜H30 とも p.15）。年度間クロスチェック（80,000,000 ≠ 78,017,459）で捕まる。
    // ⚠ 廃止款 `（自動車取得税交付金）`（全角括弧・番号なし・R8/R3 に前年度1千円）はパーサ既定の経路で `—` 款として拾える。
    // ⚠ 款9 `国有提供施設等所在市町村助成交付金` の折返し（R5〜R3 は上段折返し・R4/R3 は金額が3行目）も既定経路で完全形に組める。
    // ⚠ **款番号が年度で振り直される**（R7→R8 で `環境性能割交付金` 9→8）。⇒ **年度間クロスチェックは款名で行う**。
    // ⚠ 歳出款6 は **`農業費`**（`農林水産業費` ではない）。**寄せない**。
    // ⚠ 年度ディレクトリの規則が破れる（**R4 が `reiwa3nendoyosan_1`**・R1/H31 が `news`）。年度から組み立てない。
    // ⚠ R3 のファイル名に `(` `)` を含む。
    // ⚠ 「予算に関する説明書（総括）」も款別＋前年度比較を持つが、**R7・R6 は罫線文字で描画されて throw**、
    //   R8 は半角括弧の廃止款を落として前年度 Σ が −1・**物理 p.5 が p.4 の完全重複**なので採らない。
    // 前年度列は**当初**（R8→H30 の8組・全款一致を偵察が実測）。
    // ⚠⚠ **骨格予算**: 市長選は4年ごとの4月（2021-04-18・2025-04-13）。**R3 当初は原典 p.2 に
    //   「継続事業を中心とした骨格的な予算編成」と明記**（6月補正で肉付け・予備費1.5億）。⇒ **R4 に `prevNote`**。
    //   ⚠ **R7 当初（2025-04 選挙の年）は「骨格」の語が0件で「過去最高を更新する927億円」**＝骨格ではない。
    //   ⚠ **H30 の前年度（H29・2017-04 選挙）は H29 資料未取得のため未確認**（prevNote は保留）。次は R11。
    // ⚠ **H29 以前は Wayback に年度ディレクトリが残るが未調査**（「収録できない」ではないので台帳に載せない）。
    // ⚠ 各年度ページに概要 PDF は1本だけ（八千代 §13-19 型の「補正を含む別版」は無い）。
    // ライセンス「伊丹市役所について」（/SOSIKI/SOGOSEISAKU/KOHO/1392875351617.html・更新 2023-04-01・確認日 2026-09-02）。
    //   ⚠ フッターには無く**検索で発見したページ**。
    //   ⚠ オープンデータ規約は「**G空間情報センター 伊丹市ページにて公開している情報**」に自ら範囲を限り（CC BY 4.0・
    //   href も `licenses/by/4.0/deed.ja` を実確認）、**カタログ62件に `予算`・`決算`・`財政`・`歳入`・`歳出`・`総括` が0件、
    //   `city.itami.lg.jp/material/files/group/12|53` を指す resource も0件**（実検索）＝及ばない（§9g）。
    //   ⚠ リンク条項は「リンクは**原則フリーです**」＝許容の宣言なので `noDeepLink` は立てない。フレーム条項も無い。
    // [年度, ファイル名, landing パス, 歳入p, 歳出p]
    ["R8", "R8_yosan.pdf", "reiwa8nendo/47177", 14, 15],
    ["R7", "R07press0101.pdf", "reiwa7nendoyosan/42737", 12, 13],
    ["R6", "r6-gaiyou.pdf", "reiwa6nendoyosan/38125", 11, 12],
    ["R5", "reiwa5nenntousyoyosanngaiyou2.pdf", "reiwa5nendoyosan/32763", 11, 12],
    ["R4", "reiwa4tousyo.pdf", "reiwa3nendoyosan_1/27416", 11, 11],
    ["R3", "reiwa3nenndo(2021nenndo)toushoyosanoyobireiwa2nenndo(2020nenndo)2gatuhoseiyosan.pdf", "reiwa3nendoyosan/21174", 11, 11],
    ["R2", "reiwaninenndotousyoyosannannhoka.pdf", "reiwaninendoyosann/23534", 15, 15],
    ["H31", "H31_press1_2.pdf", "news/1549951419871", 15, 15],
    ["H30", "H30press.pdf", "H30yosan/1518744136392", 15, 15],
  ] as const).map(([fy, file, page, rev, exp]) => ({
    id: `itami-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 伊丹市当初予算（案）の概要（一般会計総括表 歳入・歳出）`,
    publisher: "伊丹市",
    url: `https://www.city.itami.lg.jp/material/files/group/12/${file}`,
    landingPage: `https://www.city.itami.lg.jp/SOSIKI/ZAISEIKIBAN/ZAISEI/ITAMI_ZAISEI/YOSAN/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "伊丹市（一般会計・団体コード282073）",
    license:
      "伊丹市ホームページに掲載している情報（文字・画像・写真など）に関する諸権利は、原則として伊丹市に帰属します。無断転載は禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      ...(fy === "R8" || fy === "R7" || fy === "R6"
        ? {
            revenueHeading: "一般会計総括表（歳入）",
            expenditureHeading: "一般会計総括表（歳出）",
            revenueTotalLabel: "合計",
            expenditureTotalLabel: "合計",
          }
        : fy === "R5"
          ? { revenueHeading: "一般会計総括表", expenditureHeading: "一般会計総括表" }
          : {
              revenueHeading: "一般会計総括表",
              expenditureHeading: "一般会計総括表",
              samePage: true,
              revenueTotalLabel: "合計",
              expenditureTotalLabel: "合計",
            }),
      ...(fy === "R4"
        ? {
            prevNote:
              "前年度（令和3年度）の当初予算は、令和3年4月の市長選挙を控え「継続事業を中心とした骨格的な予算編成」とされたもの（令和3年度当初予算（案）の概要に明記）。ここでの前年度額はその骨格予算の当初額。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 小平市（東京都・団体コード 132110）。⚠ 収録済みの東京都・特別区・八王子・調布・町田・府中（132063）・
    //   西東京（132292）とは別団体。
    // 一般会計予算書（分冊1）の「歳入歳出予算総括表 (1) 歳入予算款別比較 / (2) 歳出予算款別比較」（歳入23款・歳出13款・千円）。
    // ⚠⚠ **表が罫線文字 `┃┣┏┗━` で組まれている**（さいたま R6・草加 R5 以前と同型の宿題）。行頭が `┃` で款番号の
    //   lead が当たらず、既存パーサは「款行が1件も抽出できませんでした」で throw していた（＝大声で落ちる型）。
    //   ⇒ **`stripBoxDrawing: true`**（この収録で新設・opt-in）で罫線素片を空白化してから読む。
    //   ⚠ **空白化であって削除ではない** — 罫線は列区切りを兼ねる字なので空白にする。⚠ **収録レビューで実測したところ、
    //   削除しても款行のトークン列は変わらない**（`交付金┃    849,000` のように罫線の右に空白がある）。違いが出るのは
    //   **列見出し行だけ**（`本年度予算額┃前年度予算額` が連結する）。空白化の選択は無害で妥当だが、根拠は列見出しの側。
    // ⚠ 合計ラベルは原文 `歳 入 合 計`/`歳 出 合 計`（**側で違う**）＝既定のまま。側の取り違えは throw する。
    // ⚠ 同じ分冊に `(3) 歳出予算款別財源内訳`（本年度のみ・前年度列なし）が続く。**特別会計・下水道の予算書（別 PDF）にも
    //   同じ見出し `(1) 歳入予算款別比較` がある**ので、ファイルを取り違えないこと。
    // ⚠⚠ **R7・R5 は総括表の2ページだけ文字が図形化**（pdftotext がノンブルのみ・画像0枚＝東大阪 R6〜R4 型）で収録不可。
    //   ⚠ H30・H29 は予算書も概要も未掲載（「わかりやすい予算」のみで款別表なし）。
    // ⚠ **物理ページ＝印字＋18 前後で年度ごとに違う**（R8 p.19/20・R6 p.19/20・R4 p.18/19・R3 p.17/18・R2 p.17/18・R1 p.18/19）。
    // ⚠ ページ ID・ファイル名に規則が無い（`att_00000NN` の番号が年度で変わる。R4 以前は1つ目のディレクトリがゼロ詰めなし）。
    // ⚠ 代替の「予算（案）の概要」は歳出13款は通るが**歳入が集約表**（`交付金※1`＝款3+4+5+8）なので採らない。
    // ⚠ 歳出款6 は **`農業費`**（伊丹と同じ。`農林水産業費` に寄せない）。
    // ⚠ **同じ年度ページに「予算編成過程の公表」（要求額）と XLSX 5本（CC BY）がある**。PDF はどちらでもない。
    // 前年度列は**当初**（偵察が R8→R7（画像は目視）→R6→R5（画像は目視）→R4→R3→R2→R1 を款名で突合して一致）。
    // ⚠⚠ **骨格予算**: 市長選は 2021-04-04・2025-04-06（4年ごと4月）。**R3 は骨格予算**（R3 概要に明記）。
    //   ⚠ R4 概要の推移表は「令和３年度当初予算額は４号補正後（いわゆる肉付け予算）」と注記するが、
    //   **R4 予算書総括表の前年度列は 68,670,000 ＝ R3 骨格の当初**（肉付け後 70,326 百万ではない）＝松本型。
    //   ⇒ R4 に `prevNote`。⚠ R7 は再選で「骨格」の語0件。次は R11。
    // ライセンス「サイトポリシー」（/kurashi/026/026916.html・更新 2026-05-08・確認日 2026-09-02）。
    //   ⚠ オープンデータ（CC BY 4.0・href は `licenses/by/4.0/deed.ja`）は**年度ページ別節の XLSX 5本だけ**が対象で、
    //   規約ページ自身が「当ライセンスは、上記対象データのみに適用されます」と範囲を限る。東京都カタログ（組織 t132110・
    //   21件）に予算/決算/財政 0件・`124053` 配下の resource 0件（実検索）＝PDF には及ばない（§9g）。
    //   ⚠⚠ **`noDeepLink` を立てた** — ⚠ リンク条項は**サイトポリシーではなく別ページ「リンクの設定」
    //   （/kurashi/026/026247.html・更新 2026-05-08）**にある（サイトポリシーは参照するだけ）。
    //   その条項が「**トップページ以外の各ページへのリンクを希望される場合は、
    //   当該ページを所管するそれぞれの担当課までお問合せください**」＝**ディープリンクに限って事前の問合せを求める指示形**
    //   （寝屋川 §13-18 と同じ書き方）。⚠ 「フレーム内に…表示させるリンク設定は行わないでください」もある。
    // [年度, ファイル URL, landing, 歳入p, 歳出p]
    ["R8", "https://www.city.kodaira.tokyo.jp/kurashi/files/124053/124053/att_0000010.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/124/124053.html", 19, 20],
    ["R6", "https://www.city.kodaira.tokyo.jp/kurashi/files/108990/108990/att_0000009.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/108/108990.html", 19, 20],
    ["R4", "https://www.city.kodaira.tokyo.jp/kurashi/files/93834/093834/att_0000015.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/093/093834.html", 18, 19],
    ["R3", "https://www.city.kodaira.tokyo.jp/kurashi/files/85735/085735/att_0000029.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/085/085735.html", 17, 18],
    ["R2", "https://www.city.kodaira.tokyo.jp/kurashi/files/78209/078209/att_0000010.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/078/078209.html", 17, 18],
    ["R1", "https://www.city.kodaira.tokyo.jp/kurashi/files/71986/071986/att_0000010.pdf",
      "https://www.city.kodaira.tokyo.jp/kurashi/071/071986.html", 18, 19],
  ] as const).map(([fy, url, landing, rev, exp]) => ({
    id: `kodaira-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 小平市一般会計予算書（歳入歳出予算総括表 款別比較）`,
    publisher: "小平市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "小平市（一般会計・団体コード132110）",
    license:
      "東京都小平市ホームページから発信するコンテンツ（テキスト、画像、PDF、そのほかのデータ）の著作権は市に帰属します。また、一部の画像などの著作権は原著作者が所有しています。本サイト上の文書・画像などの無断使用・転載、二次利用を禁止します。掲載されている情報は、個人的かつ非営利的な使用目的だけのために利用する場合に限り、複製、使用、ダウンロードすることができます。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      ...
      ...(fy === "R4"
        ? {
            prevNote:
              "前年度（令和3年度）の当初予算は、令和3年4月の市長選挙を控えた骨格予算として編成されたもの（令和3年度予算（案）の概要に明記）。ここでの前年度額はその骨格予算の当初額で、肉付け後の額ではない。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 熊谷市（埼玉県・団体コード 112020）。⚠ さいたま・川越・川口・所沢・春日部・上尾・草加・越谷とは別団体。
    // **経路が2つ**: R8〜R2 は「予算分析表」（`１ 歳入歳出予算款別比較表`・物理 p.3/p.4）、H31〜H23 は分析表が無く
    //   「予算に関する説明書」（一括版）の「１ 総括」（物理 p.5/p.6・**H27 だけ p.6/p.7**＝p.5 に扉が1枚入る）。
    // ⚠ 分析表の合計ラベルは**両側とも `計`**（`歳入合計` ではない）。説明書は `歳入合計`/`歳出合計`。
    // ⚠⚠ **分析表 p.11「人口、世帯当たりの歳入歳出予算額」にも `（歳 入）` `（歳 出）` と `計` があり、
    //   p.12 を歳出に当てると Σ4系統とも差0 のまま前年度列が「1人当たり円」になる**（偵察が実測）。
    //   歳入側は前年度 Σ が −2（四捨五入の端数）だが **validate では warning**。⇒ ページを動かさないこと。
    // ⚠⚠ **市長提出議案ページに同名の「予算分析表（補正第N号）」が年に数本あり、当初と同じ指定で Σ4系統とも差0 で
    //   完走する**（R7 補正7号は前年度列＝前回までの累計 84,122,951）。表紙3行目と列見出しでしか見分けられない。
    //   ⇒ **URL を年度ページ（yosangaiyo/）から取り、議案ページ（/about/gian/）から取らない**。
    //   年度間クロスチェック（R8 の前年度列 79,700,000）でだけ捕まる型。
    // ⚠ 説明書ルートは **`HeaderExtra` が両側とも必須** — 無いと款1 が `総括歳入歳出予算事項別明細書市税`／
    //   `国県支出金地方債その他議会費` になる（**Σ は差0 のまま**）。
    // ⚠ R2〜R8 の説明書にも同じ総括があるが総括のページが年度で動く（R8 は p.7/8）ので R2 以降は分析表を使う。
    //   ⚠ R2 の説明書総括は廃止款 `△ 自動車取得税交付金 - 130,000` を 130,000/130,000 と読んで Σ +130,000 で落ちる（分析表なら通る）。
    // ⚠ ファイル名は年度ごとに不規則。R2 だけ 2.5MB。
    // ⚠ 歳出は12款（`災害復旧費`・`諸支出金` が無い）。**寄せない**。
    // 前年度列は**当初**（分析表の列見出しが `前年度（当初）予算額` と明記。R8→R2 の6組・各34款が全件一致）。
    // ⚠ 骨格予算の年度は無い（市長選は10月で編成期と重ならず、H23〜R8 の全年度で「骨格」0件）。
    // ⚠ 収録できない年度は無い（H23〜R8 の16年度すべて実測で通った）。H22 以前は発行元の一覧に無い。
    // ライセンス「熊谷市ホームページの考え方」（/aboutweb/policy.html・更新 2015-11-21・確認日 2026-09-02）。
    //   ⚠ 市のオープンデータ規約（CC BY 4.0）は `/about/opendata/ 以下のディレクトリ` に自ら範囲を限り、予算は0件。
    //   ⚠⚠ **埼玉県ポータル（PDL1.0）に【熊谷市】当初予算（datasets/1460）が実在する**が、中身は **CSV**
    //   （細節レベル・R6/R7）とメタデータ XLSX で、**分析表 PDF そのものは登録されていない**（`.pdf` 参照0件・
    //   `city.kumagaya` 参照0件）＝本 PDF には及ばない（§9g。流山 §13-19 と同じ「カタログに予算が実在するが同一ファイルは無い」型）。
    //   ⚠⚠ **`noDeepLink` を立てた** — 「リンクされる際は、**必ずトップページへお願い致します。トップページ以外への
    //   リンクはご遠慮ください**」＝トップページ限定の指示形（西東京 §13-19・佐賀 §13-17 と同型）。
    // [年度, 経路, 年度ページ, ファイルパス, 歳入p, 歳出p]
    ["R8", "bunseki", "R8yosangaiyo.html", "R8yosangaiyo.files/R8bunsekihyo.pdf", 3, 4],
    ["R7", "bunseki", "0001.html", "0001.files/R7bunsekihyou.pdf", 3, 4],
    ["R6", "bunseki", "ta04025560.html", "ta04025560.files/R6bunsekihyou.pdf", 3, 4],
    ["R5", "bunseki", "R5toushoyosangaiyou.html", "R5toushoyosangaiyou.files/R5bunsekihyou.pdf", 3, 4],
    ["R4", "bunseki", "R4yosan.html", "R4yosan.files/R4-bunsekihyou.pdf", 3, 4],
    ["R3", "bunseki", "R3gaiyou.html", "R3gaiyou.files/R3bunseki.pdf", 3, 4],
    ["R2", "bunseki", "r2-gaiyou.html", "r2-gaiyou.files/bunsekihyou.pdf", 3, 4],
    ["H31", "setsumei", "h31-no2.html", "h31-no2.files/ikkatsu31.pdf", 5, 6],
    ["H30", "setsumei", "h30-no2.html", "h30-no2.files/ikkatsu30.pdf", 5, 6],
    ["H29", "setsumei", "h29-no2.html", "h29-no2.files/ikkatsu29.pdf", 5, 6],
    ["H28", "setsumei", "h28-no2.html", "h28-no2.files/ikkatsu.pdf", 5, 6],
    ["H27", "setsumei", "h27-no2.html", "h27-no2.files/ikkatsu.pdf", 6, 7],
    ["H26", "setsumei", "h26-no2.html", "h26-no2.files/ikkatsu.pdf", 5, 6],
    ["H25", "setsumei", "h25-no2.html", "h25-no2.files/ikkatsu.pdf", 5, 6],
    ["H24", "setsumei", "h24-no2.html", "h24-no2.files/No.2-ikkatsu.pdf", 5, 6],
    ["H23", "setsumei", "23_yosansyo2.html", "23_yosansyo2.files/No.2.pdf", 5, 6],
  ] as const).map(([fy, route, page, file, rev, exp]) => {
    const isB = route === "bunseki";
    const dir = isB ? "yosangaiyo" : "yosansyo";
    return {
      id: `kumagaya-yosan-${isB ? "bunseki" : "setsumeisho"}-${fy.toLowerCase()}`,
      title: isB
        ? `${eraYear(fy)}年度 熊谷市一般会計歳入歳出予算分析表（歳入歳出予算款別比較表）`
        : `${eraYear(fy)}年度 熊谷市一般会計予算に関する説明書（総括 歳入歳出予算事項別明細書）`,
      publisher: "熊谷市",
      url: `https://www.city.kumagaya.lg.jp/about/zaisei/yosantuite/${dir}/${file}`,
      landingPage: `https://www.city.kumagaya.lg.jp/about/zaisei/yosantuite/${dir}/${page}`,
      kind: "pdf" as const,
      fiscalYear: fy,
      scope: "熊谷市（一般会計・団体コード112020）",
      license:
        "本市ホームページに掲載されている文書や、画像等の情報及び内容に関しては著作権の対象になっています。著作権は本市およびウェブ作成者、画像等提供者、撮影者等に帰属しており、日本国著作権法等により保護されています。なお、本市ホームページに掲載されている文書・画像等の無断使用・転載を禁止します。ただし、熊谷市ホームページ内の各ページに特段の定めがある場合には、その取り扱いが優先されます。",
      noDeepLink: true,
      parser: "kofu-yosansho" as const,
      parserOptions: {
        revenuePage: rev,
        ...
        expenditureHeading: "（歳 出）",
        ...(isB
          ? { revenueTotalLabel: "計", expenditureTotalLabel: "計" }
          : {
              revenueTotalLabel: "歳入合計",
              expenditureTotalLabel: "歳出合計",
              revenueHeaderExtra: "総括|歳入歳出予算事項別明細書",
              expenditureHeaderExtra: "国県支出金地方債その他",
            }),
      },
    };
  }),
  ...([
    // 松江市（島根県・中核市・団体コード 322016）。⚠ 収録済みの島根県 320005 とは別団体。**島根県内で初の市町村**。
    // **経路が2つ**: R8〜R4 は「当初予算の概要」内の `２ 一般会計歳入予算`/`３ 一般会計歳出予算`、
    //   R3〜H24 は別 PDF「会計別・一般会計予算」（歳入 p.2〜3・歳出 p.3〜4）。単位は千円。
    // ⚠⚠ **概要 PDF は表の右に円グラフがあり、そのラベルが款名に混入する**（`依存歳入地方交付税`・
    //   `災害復旧費総務費`・`商工費衛生費教育費` …）。**Σ は4系統とも差0 のまま**＝`CropX` で表の右端（率列 ≈482pt）で切る。
    // ⚠ 列見出し `科目 / 構成 構成 / 比率 比率` が款1 の頭に付く（Σ差0）ので `HeaderExtra` も必須。
    // ⚠⚠ **同じ PDF の「性質別経費分析表」の合計が款別と完全一致する**（119,391,000／112,990,000）。
    //   見出しを `性質別` にして当てると **R8・R5・R3 とも Σ4系統とも差0 で `人件費 物件費 …` の12款を収録して完走する**
    //   （偵察が実測）。守るのは見出しの完全形 `一般会計歳出予算` だけ（性質別ページの見出しはこれを含まない）。
    // ⚠⚠ **合計ラベルは歳入・歳出とも `合 計`（同一）**。見出しを `一般会計` に緩めると**側を入れ替えても Σ4系統とも差0 で完走**する。
    //   ⇒ 見出しは完全形で書く（八千代 §13-19 と同型）。
    // ⚠ 歳入の内数行（`うち電源立地地域対策交付金等`・`うち財政調整基金`）は款番号が無く自動で捨てられる。
    // ⚠ R3 以前の会計別 PDF は**款名の下段折返しの位置が年度で揺れる**ので `kanNameContinues` の款番号が年度ごとに違う
    //   （R3 `[5,8,10,13]`・R2 `[5,8,10]`・R1 `[10]`・H30/H26/H25/H24 `[9]`・**H29〜H27 は不要**）。
    //   欠くと `株式等譲渡所得割`／`交付金法人事業税交付金` のように切れて次の款を汚す（**Σ は差0 のまま**）。
    // ⚠ **H29〜H24 の款名 `ｺﾞﾙﾌ場利用税交付金` は原典が半角カナ**。**原典どおり残す**（寄せない）。
    // ⚠ **H27・H26 の会計別 PDF は版面に「松江」の文字が0件**（島根県 §11l と同型）。**ファイル URL（発行元ドメイン配下）と
    //   合計額の年度間一致**で同定した。
    // ⚠⚠ **landing の年度ページは偵察の申し送りの13本が 404 だった**（収録レビューで判明・2023-02-01 のサイトリニューアルで
    //   ID が変わっている）。発行元の索引 `gyozaisei/1/3/index.html` から**当該 PDF へのリンクを実際に含むページ**を引き直した
    //   （R5 だけ旧パスがまだ 200 で PDF リンクもあるのでそのまま）。⚠ **`noDeepLink` の振替先が landing なので、
    //   ここが 404 だと画面のエビデンス導線が 404 に飛ぶ**。年度を足すときは landing を必ず自分で開くこと。
    // ⚠⚠ **H24 の前年度列は「平成23年度当初（合算）」**（松江市＋東出雲町・2011-08-01 合併）。同表に「（松江市のみ）」列も
    //   あるがパーサは合算列を取る（H25 の前年度列と一致するのは合算側）。⇒ `prevNote` で断る。
    //   `HeaderExtra` に `合算|松江市のみ` も足す。
    // ⚠ R8 概要は 10.6MB（他年度は 4〜10MB）。**「補正を含む」版の概要は無い**（年度ページの PDF を全部列挙して確認）。
    // 前年度列は**当初**（列見出しが `令和7年度当初 予算額`。R8→H24 の14リンク・全款一致）。
    // ⚠⚠ **骨格予算**: 市長選は4年ごとの4月（2025-04-20）。**R3・H29・H25 は原典が「骨格予算」と明記**。
    //   ⚠ **R7 は選挙年度なのに「骨格」の語が0件**（実測）。肉付け後の別 PDF は無い。
    //   ⇒ **R4・H30・H26 に `prevNote`**（前年度が骨格）。R8 には付けない。次は R11。
    // ⚠ **H23 以前は年度一覧に無い＝未調査**。
    // ライセンス「著作権・免責事項・リンク等」（/soshikikarasagasu/matsueshiyakusho/3631.html・更新 2025-04-21・確認日 2026-09-02）。
    //   ⚠ 島根県 data eye（CC BY 4.0）の松江市組織は9件で予算0件・市自身のデータセット一覧 CSV にも0件＝及ばない（§9g）。
    //   ⚠⚠ **松江市統計情報データベース（CC BY 2.1 JP）に「一般会計歳入歳出予算・決算額」XLS が載り、
    //   数値が概要 PDF と一致する**（偵察が実測）。**同一ファイルではないが同一データがオープンで出ている**。
    //   ⚠ 規約は「当サイトの『松江市統計情報データベース』のページに掲載された著作物」に限るので**概要 PDF には及ばない**。
    //   ⇒ 本 PDF は要許可のまま。**統計DB は代替ソースとして別途検討**（新パーサが要る・R7/R8 はまだ載っていない）。
    //   ⚠⚠ **`noDeepLink` を立てた** — 「**リンクはトップページとしてください**」＝指示形＋トップページ限定＋事後連絡の要求
    //   （西東京 §13-19 と同型）。
    // [年度, ファイル名, landing パス, 歳入p, 歳出p, 概要か, kanNameContinues]
    ["R8", "R8toushoyosangaiyou_.pdf", "16_2/24268", 11, 13, true, null],
    ["R7", "R7toushoyosangaiyou.pdf", "16_1/21128", 10, 12, true, null],
    ["R6", "01_R6toushoyosangaiyou.pdf", "16/17006", 10, 12, true, null],
    ["R5", "01_R5toushoyosangaiyou.pdf", "OLD:gyoseijoho/gyozaisei/3/3/16/10188", 10, 12, true, null],
    ["R4", "01_R4toushoyosangaiyou.pdf", "14/1198", 10, 12, true, null],
    ["R3", "02_R3kaikeibetu_ippankaikeiyosan.pdf", "13/1197", 2, 3, false, [5, 8, 10, 13]],
    ["R2", "02_R2kaikeibetu_ippankaikeiyosan.pdf", "12/3592", 2, 3, false, [5, 8, 10]],
    ["R1", "02_H31kaikeibetu_ippankaikeiyosan2.pdf", "11/3783", 2, 3, false, [10]],
    ["H30", "H30kaikeibetsu2.pdf", "10/1885", 2, 3, false, [9]],
    ["H29", "55910014.pdf", "9/3027", 3, 4, false, null],
    ["H28", "H28kaikeibetu.pdf", "8/1091", 3, 4, false, null],
    ["H27", "H27_tosyo_02.pdf", "7/976", 3, 4, false, null],
    ["H26", "H26_tosyo_02.pdf", "6/971", 3, 4, false, [9]],
    ["H25", "H25_tosyo_02.pdf", "5/1194", 3, 4, false, [9]],
    ["H24", "H24_tosho_02.pdf", "4/3003", 3, 4, false, [9]],
  ] as const).map(([fy, file, page, rev, exp, isGaiyou, cont]) => ({
    id: `matsue-yosangaiyou-${fy.toLowerCase()}`,
    title: isGaiyou
      ? `${eraYear(fy)}年度 松江市当初予算の概要（一般会計歳入予算・一般会計歳出予算）`
      : `${eraYear(fy)}年度 松江市会計別・一般会計予算（歳入・歳出）`,
    publisher: "松江市",
    url: `https://www.city.matsue.lg.jp/material/files/group/10/${file}`,
    landingPage: page.startsWith("OLD:")
      ? `https://www.city.matsue.lg.jp/${page.slice(4)}.html`
      : `https://www.city.matsue.lg.jp/soshikikarasagasu/zaiseibu_zaiseika/gyozaisei/1/3/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "松江市（一般会計・団体コード322016）",
    license:
      "「松江市ウェブサイト」に掲載されている文章、写真、イラスト、画像等の著作権は、松江市またはコンテンツ提供者が所有しています。これらの情報は、「私的使用のための複製」や「引用」などの著作権法上認められた場合を除き、無断で使用・転載等することはできません。利用の許諾については、各ページに記載されている課等へお問い合わせください。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "一般会計歳入予算",
      expenditureHeading: "一般会計歳出予算",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      // ⚠ 列見出し `科目 / 構成 構成 / 比率 比率` は**概要 PDF だけでなく会計別 PDF（R3〜H24）にもある**
      //   （収録時に判明。H25/H24 は3列なので `構成構成構成比率比率比率`）。⇒ 全年度に付ける。
      revenueHeaderExtra: fy === "H24" ? "科目|構成構成|比率比率|合算|松江市のみ" : "科目|構成構成|比率比率",
      expenditureHeaderExtra: fy === "H24" ? "科目|構成構成|比率比率|合算|松江市のみ" : "科目|構成構成|比率比率",
      ...(isGaiyou ? { revenueCropX: { from: 0, to: 490 }, expenditureCropX: { from: 0, to: 490 } } : {}),
      ...(cont ? { kanNameContinues: { revenue: [...cont] } } : {}),
      ...(fy === "R4" || fy === "H30" || fy === "H26"
        ? {
            prevNote: `前年度（${fy === "R4" ? "令和3" : fy === "H30" ? "平成29" : "平成25"}年度）の当初予算は、4月の市長選挙を控えた骨格予算として編成されたもの（当該年度の当初予算資料に明記）。ここでの前年度額はその骨格予算の当初額。`,
          }
        : {}),
      ...(fy === "H24"
        ? {
            prevNote:
              "前年度（平成23年度）の額は、平成23年8月に合併した旧東出雲町の分を合算したもの（原典の「平成23年度当初（合算）」列）。原典には「松江市のみ」の列もあるが、ここでは合算の列を採っている。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 鈴鹿市（三重県・団体コード 242071）。⚠ 収録済みの三重県・津市 242012（全年度収録不可）・四日市市 242021 とは別団体。
    // 本命は**予算書の事項別明細書「１．総括」**（歳入23款・歳出14款・千円）。R8〜H31 の8年度が市議会「議案等一覧」に残る
    //   （財政課の年度ページは R6 以前が現行サイトから消失）。⚠ **歳入の合計行が次ページ**なので `revenuePages`。
    // ⚠ 合計ラベルは `歳入合計`/`歳出合計`（**側で違う**）＝既定。側の入替えは throw する（偵察が実測）。
    // ⚠⚠ **同じ PDF に特別会計4本の同名「１．総括」が同居する**（R8 は国保 p.225/226・土地取得 p.265・介護 p.281・
    //   後期高齢 p.307）。国保に当てると **throw せず Σ4系統とも差0 で完走**（歳入合計＝歳出合計 17,091,570）。
    //   ⇒ **物理ページの固定と款名の目視だけが網**（宝塚 §13-17・流山 §13-19 に続く9例目）。
    // ⚠ 物理ページは年度で動く（R8〜R6 p.24-25/26・R5〜R3 p.20-21/22・R2/H31 p.21-22/23）。
    // ⚠⚠ **R2 は `dashAsZero` が必須** — 廃止款 `△. 自動車取得税交付金 - 60,000 △60,000` を、無いと
    //   `60,000 / -60,000` と読んで Σ +60,000／−120,000 で割れる（＝大声で落ちる）。
    // ⚠ R3〜R8 の `自動車取得税交付金` は象徴計上の **1千円**（廃止ではない）。R3 は pdftotext が stderr に
    //   Syntax Error を出すが抽出は正常。
    // ⚠ **H30 だけ経路が違う** — 予算書が議案等一覧に無く、WARP の「当初予算資料」p.6/7 の構成比表（款番号なし）で読む。
    //   ⚠ 本文行（`地方交付税については…`・`一方`・`民生費が…`）が款として混ざるので `HeaderExtra` が必須。
    //   ⚠ **`民生費が` を落とさないと `— 民生費が 0 0` が款に混ざり、Σ は差0 のまま**（静かに通る型）。
    //   ⚠ WARP の実体 URL は `warp.ndl.go.jp/<収集日>/<ts>/<元URL>`。`/web/<ts>/` はラッパー HTML なので使えない。
    // ⚠ 当初予算資料（R8/R7）の款/項同居表も通る（`kanIndentMax: 0`）が、予算書が議決対象なのでそちらを本命にした。
    //   ⚠ **R8 資料は2月の「予算(案)」版**（R7 は2月版と3月版で款別が完全一致した前例あり）。予算書なら影響なし。
    // 前年度列は**当初**（R8→H30 の8組×2側で全款不一致0）。
    // ⚠⚠ **骨格予算**: 市長選は統一地方選の4月（2023-04-23・次は 2027-04）。**R5 当初は骨格**（R5 資料 p.3 に明記・
    //   R6 資料も「肉付けを行った第３号補正後」と別記）。**H31 当初も骨格**（H31 資料 p.3・R2 資料 p.5 に明記）。
    //   ⚠ **予算書の前年度列は骨格の当初そのもの**（R6 の前年度 69,210,000＝R5 当初。肉付後 72,787,925 ではない）＝松本型。
    //   ⇒ **R6・R2 に `prevNote`**。
    // ⚠ **H29 以前は未調査**（WARP の index1.html に「平成30年度予算」まで。それ以前の有無は見ていない）。
    // ライセンス「著作権とリンク」（/about/1008771.html・更新 2026-03-26・確認日 2026-09-02）。
    //   ⚠⚠ **区分は人の判断が要る論点** — 「著作権の原則」＋「引用の原則」だけを置くと判定語に当たらず `unverified`
    //   （＝自サーバー配信が開く）。同ページの「データの複製」の一文（`複製物を改変しての使用は、原則禁止`）を含めると
    //   `permission-required` に落ちる。⚠ その一文は画像データ限定なので PDF には及ばないが、**開ける側へ倒さない**前例
    //   （草加 §13-15・宝塚 §13-17）に従い、同一ページの原文をそのまま置いて要許可にした（意訳ではない）。
    //   ⚠ オープンデータ規約は「**csv形式及びxls形式で公開している情報**」に限り「csv形式・xls形式以外のもの」を自ら除外＝
    //   PDF には及ばない（§9g）。カタログ24件に `予算`・`決算` 0件・`.pdf` 0件（実検索）。
    //   ⚠ リンク条項は「自由に URL を…掲載する**ことができます**」＝許容の宣言。`noDeepLink` は立てない。
    // [年度, 経路, PDF URL, landing, 歳入from, 歳入to, 歳出p]
    ["R8", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/016/327/3.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1016223/1016326/1016327.html", 24, 25, 26],
    ["R7", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/014/749/yosan.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1014585/1014748/1014749.html", 24, 25, 26],
    ["R6", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/448/528_005.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012087/1012446/1012448.html", 24, 25, 26],
    ["R5", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/496/482_006.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012088/1012489/1012496.html", 20, 21, 22],
    ["R4", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/534/416_006.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012502/1012506/1012534.html", 20, 21, 22],
    ["R3", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/535/363_006.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012503/1012515/1012535.html", 20, 21, 22],
    ["R2", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/607/304_006.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012504/1012522/1012607.html", 21, 22, 23],
    ["H31", "yosansho", "https://www.city.suzuka.lg.jp/_res/projects/default_project/_page_/001/012/538/266_006.pdf",
      "https://www.city.suzuka.lg.jp/gikai/1011649/1012505/1012529/1012538.html", 21, 22, 23],
    ["H30", "shiryou", "https://warp.ndl.go.jp/20200214/20200213203750/https://www.city.suzuka.lg.jp/kouhou/gyosei/plan/zaisei/data/pdfs/yosan_2018.pdf",
      "https://warp.ndl.go.jp/20200214/20200213203750/https://www.city.suzuka.lg.jp/gyosei/plan/zaisei/index1.html", 6, 6, 7],
  ] as const).map(([fy, route, url, landing, rf, rt, exp]) => ({
    id: `suzuka-${route === "yosansho" ? "yosansho" : "yosan-shiryou"}-${fy.toLowerCase()}`,
    title: route === "yosansho"
      ? `${eraYear(fy)}年度 鈴鹿市予算書（歳入歳出予算事項別明細書 総括）`
      : `${eraYear(fy)}年度 鈴鹿市当初予算資料（一般会計 歳入・歳出 構成比）`,
    publisher: "鈴鹿市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "鈴鹿市（一般会計・団体コード242071）",
    license:
      "このサイト（○○○.city.suzuka.lg.jp）から発信するコンテンツの著作権は、鈴鹿市に帰属します。このサイトから発信するコンテンツについて、次項の例外を除いて引用を承認します。ただし、引用する場合は、出所を明示する等の一般的な引用のルールに従ってください。アイコン・写真等の画像データの複製(デジタルデータとしての単純な複製）による使用または複製物を改変しての使用は、原則禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      ...(route === "yosansho"
        ? {
            ...
            revenueHeading: "総括",
            expenditureHeading: "歳出",
            ...(fy === "R2" ? { dashAsZero: true } : {}),
          }
        : {
            revenuePage: rf,
            expenditurePage: exp,
            revenueHeading: "構成比",
            expenditureHeading: "構成比",
            kanNoless: true,
            revenueHeaderExtra: "^地方交付税については",
            expenditureHeaderExtra: "^(一方|民生費が)",
          }),
      ...(fy === "R6"
        ? {
            prevNote:
              "前年度（令和5年度）の当初予算は、令和5年4月の市長選挙を控えた骨格的予算として編成されたもの（令和5年度当初予算資料に明記）。ここでの前年度額はその骨格予算の当初額で、肉付けを行った第3号補正後の額ではない。",
          }
        : {}),
      ...(fy === "R2"
        ? {
            prevNote:
              "前年度（令和元年度）の当初予算は、平成31年4月の市長選挙を控えた骨格的予算として編成されたもの（令和元年度・令和2年度の当初予算資料に明記）。ここでの前年度額はその骨格予算の当初額。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 三鷹市（東京都・団体コード 132047）。⚠ 収録済みの東京都・特別区・八王子・調布 132080・町田・府中（東京 132063）・
    //   西東京 132292・小平 132110 とは別団体。
    // 「施政方針・予算概要(1)」の「１ 歳入の款別内訳」（歳入20〜22款）/「１ 歳出の款別内訳」（歳出12〜13款）。千円。
    // ⚠ **歳入は必ず2ページ跨ぎ**で合計行は2ページ目にしか無い（`revenuePages`）。単ページだと throw する。
    // ⚠ 印字ノンブルと物理ページのズレが年度で違う（R8 は +4・他は +6）。物理ページで指定する。
    // ⚠⚠ **合計ラベルは歳入・歳出とも `合 計`（同一）**。見出しを `款別内訳` に緩めると**側を入れ替えても静かに通る**
    //   （偵察が実測）。⇒ 見出しは `歳入の款別内訳`/`歳出の款別内訳` の完全形で書く（側で違うので throw する）。
    // ⚠⚠ **同じ PDF の「款別財源内訳」（p.51）が最も危険** — 当年度 Σ 差0、前年度 Σ も国庫支出金列と一致して差0 で完走し、
    //   款1 が `財源内訳財源比国庫支出金都支出金市債その他一般財源特定一般議会費` に壊れる。**款名の目視だけが網**。
    // ⚠ 列見出しの断片 `款 増 △減`（歳入）/`増 △減`（歳出）が款1 に付く（`款増△減市税`・`増△減議会費`。**Σ は差0 のまま**）。
    //   ⇒ `HeaderExtra: "^款?増△減$"`（両側）。
    // ⚠ 節別 p.55 は Σ −100,000 で落ちる・性質別は款番号が無く throw する（＝安全側）。
    // ⚠ 歳出款6 は **`農林費`**。**R1 以前は `諸支出金` が款12 に入る13款**（H26〜R1 の6年度・R2 で廃止款）。**寄せない**。
    // ⚠ R2 歳出に廃止款 `(諸支出金)`（当年度0・前年度 24,232）が入り、既定の経路で拾える（`dashAsZero` 不要）。
    // ⚠ 款番号は R1→R2 で振り直し（法人事業税交付金の挿入）。**年度間クロスチェックは款名で行う**。
    // ⚠⚠ **R8 の URL `037/037785/attach_37785_1.pdf` は「今年度の予算概要」用で毎年上書きされる**（R7〜R2 は
    //   `024/024595/attach_24595_*` に退避済み）。⚠ 要許可で出典リンクは発行元へ直リンクなので、**R9 が公開されたら
    //   R8 の url を 024595 側の退避 URL に差し替える**（さもないと「令和8年度」の出典が R9 を開く**か 404 になる** —
    //   添付番号は回る: R3 は `_1/_2`・R6 は `_4/_5`・R8 は `_1/_2`）。
    //   ⚠ 台帳の R8 file エントリ（2021-03 捕捉・不一致）は同 URL の**5年前の別文書**で R8 の魚拓ではない。
    // ⚠ 予算書「１ 総括」（R2〜R8）は数値が概要と同一の別冊。**登録は概要側に統一**。
    //   同じ年度ページに要求額版・補正含み版は無い（(1)(2) の2本のみ・偵察が実測）。
    // 前年度列は**当初**（H27→R8 の12組・全款一致）。
    // ⚠⚠ **骨格予算**: 市長選は4年ごとの4月（2015・2019・2023、次は 2027-04＝R9）。**H27 当初は骨格**
    //   （H27 概要 p.11 に「『骨格予算』の考え方を基本に据えて編成」と明記）。**H28 の前年度列 69,401,456 は H27 の骨格当初**
    //   （補正後 69,401,456+2,499,954 ではない）＝松本型。⇒ **H28 に `prevNote`**。⚠ R1・R5 は「骨格」0件。
    //   ⚠ **H27 概要は p.36-41 に6月補正の総括表を同梱**する（当てていない）。
    // ⚠ **H25 以前は発行元のナビに無い＝未調査**。
    // ライセンス「三鷹市ホームページの考えかた」（/c_service/002/002215.html・更新 2025-09-12・確認日 2026-09-02）。
    //   ⚠ 著作権条項が「**オープンデータとして公開されているものを除き**」と自ら範囲を切るが、**概要 PDF はオープンデータに
    //   載っていない**（市のオープンデータ一覧【予算・決算】は予算情報 CSV・決算情報 CSV・固定資産情報 CSV・市税の概要 PDF の4件。
    //   東京都カタログでも同じ4件）＝§9g。⚠ 予算情報 CSV（R8〜H28・政府標準利用規約準拠・CC BY）は**CSV のみ**に及ぶ。
    //   ⚠ **CSV の規約文を PDF の license 欄に書くと open に落ちる**（書かない）。
    //   ⚠ リンク条項は「**原則としてフリー**となっており、リンクのご連絡も不要です」＝許容の宣言。`noDeepLink` は立てない。
    //   ⚠ 「フレーム内に…表示させるリンク設定は行わないでください」があるので iframe の導線は作らない。
    // [年度, landing パス, 添付ファイル名, 歳入from, 歳入to, 歳出p]
    ["R8", "037/037785", "037/attached/attach_37785_1.pdf", 38, 39, 48],
    ["R7", "024/024595", "024/attached/attach_24595_9.pdf", 42, 43, 52],
    ["R6", "024/024595", "024/attached/attach_24595_11.pdf", 42, 43, 51],
    ["R5", "024/024595", "024/attached/attach_24595_1.pdf", 42, 43, 51],
    ["R4", "024/024595", "024/attached/attach_24595_3.pdf", 48, 49, 57],
    ["R3", "024/024595", "024/attached/attach_24595_5.pdf", 46, 47, 55],
    ["R2", "024/024595", "024/attached/attach_24595_7.pdf", 36, 37, 45],
    ["R1", "044/044181", "044/attached/attach_44181_11.pdf", 50, 51, 59],
    ["H30", "044/044181", "044/attached/attach_44181_9.pdf", 46, 47, 55],
    ["H29", "044/044181", "044/attached/attach_44181_1.pdf", 50, 51, 59],
    ["H28", "044/044181", "044/attached/attach_44181_3.pdf", 46, 47, 55],
    ["H27", "044/044181", "044/attached/attach_44181_5.pdf", 48, 49, 54],
    ["H26", "044/044181", "044/attached/attach_44181_7.pdf", 46, 47, 54],
  ] as const).map(([fy, page, file, rf, rt, exp]) => ({
    id: `mitaka-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 三鷹市施政方針・予算概要(1)（一般会計 歳入・歳出の款別内訳）`,
    publisher: "三鷹市",
    url: `https://www.city.mitaka.lg.jp/c_service/${file}`,
    landingPage: `https://www.city.mitaka.lg.jp/c_service/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "三鷹市（一般会計・団体コード132047）",
    license:
      "三鷹市ホームページから発信するコンテンツ（テキスト、画像、PDF、そのほかのデータ）は、オープンデータとして公開されているものを除き、著作権は三鷹市に帰属します。また、一部の画像などの著作権は原著作者が所有しています。本サイト上の文書・画像などの無断使用・転載、二次利用を禁止します。掲載されている情報は、個人的かつ非営利的な使用目的のために利用する場合に限り、複製、使用、ダウンロードすることができます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePages: { from: rf, to: rt },
      expenditurePage: exp,
      revenueHeading: "歳入の款別内訳",
      expenditureHeading: "歳出の款別内訳",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      revenueHeaderExtra: "^款?増△減$",
      expenditureHeaderExtra: "^款?増△減$",
      ...(fy === "H28"
        ? {
            prevNote:
              "前年度（平成27年度）の当初予算は、平成27年4月の市長選挙を控え「骨格予算」の考え方で編成されたもの（平成27年度施政方針・予算概要に明記）。ここでの前年度額はその骨格予算の当初額で、6月補正後の額ではない。",
          }
        : {}),
    },
  })),
  ...
  ...([
    // 安城市（愛知県・団体コード 232122）。⚠ 収録済みの愛知県・名古屋・豊橋・岡崎・一宮・春日井 232068・豊川・豊田とは別団体。
    // 年度ページの**1ページ PDF ペア**「一般会計款別歳入」「一般会計目的別歳出」（3年度列・千円）を
    //   `revenueFile`/`expenditureFile` の分冊形式で読む。⚠ **同一 PDF 内に別表が存在しない**ので「隣の表を読む」事故が
    //   構造的に起きない（同じ数字が予算書 p.4/5・説明書 p.9/10 にもあり、3経路とも一致を偵察が実測）。
    // ⚠⚠ **合計行だけ構成比が整数 `100`**（豊橋 §13-8 と同型）→ `totalAmountIntIndex: 0` / `totalPrevIntIndex: 2` が必須。
    //   外すと**前年度合計が 100 と読まれ Σ +83,359,900 だが、当年度は差0 のままなので validate では warning 止まりで
    //   derive まで流れる**（偵察が実測）。
    // ⚠ 合計ラベルは**両側とも `合計`**（`歳入合計` ではない）。⚠ 見出し語（`一般会計款別歳入`/`一般会計目的別歳出`）が
    //   側で違うので、ファイルを取り違えると throw する（実測）。
    // ⚠ 款番号は **5刻みの独自採番**（26・36 が挿入）。昇順なので validate の warning は出ない。
    // ⚠ **1ページ PDF には自治体名が無い**（タイトルのみ）。取り違え検出は予算書（表紙「安城市予算書」）p.4/5 の全款が
    //   同値であること・報道機関配付資料の総額 85,780 百万円と一致することで代替した。
    // ⚠ **R3 以前は年度ページ・PDF とも 404**、Wayback CDX 0件、WARP も 404 ＝**未回収**（「無い」とは断定しない）。
    // 前年度列は**当初**（列見出しの年度列 `[当年度, 前年度, 前々年度]`。R8→R4 の4リンク・合計行込み148行が全件一致。
    //   各 PDF 自身の第3列（前々年度）も同じ値で補正後の混入は無い）。
    // ⚠ 骨格予算: 市長選は **2023-02-05**（新人初当選・任期 2023-02-15〜2027-02-14）。R5 当初は交代直後の編成だが
    //   全 PDF と年度ページに「骨格」「肉付け」が0件・総額は前年比 100.3% ＝骨格ではない。⚠ **次回 2027-02 → R9 が骨格になりうる**。
    // ⚠ R7 予算書は表紙〜p.2 にテキスト層が無い（画像）が、この収録では予算書を使わない。
    // ⚠⚠ **ライセンスは判定語に当たらず `unverified` に落ちる**（四日市・盛岡型）。原文「転載、引用等を希望する場合は、
    //   安城市へ問い合せてください」は実質的な要許可だが、**意訳しない**規約に従い原文のまま置く。
    //   ⚠ **`unverified` は自サーバー配信が開く側**なので、人の判断が要る論点として docs §13-21 に残す。
    //   確認ページ: /about/index.html（更新 2026-02-27・確認日 2026-09-03）。
    //   ⚠ オープンデータ規約（CC BY 4.0・href は `licenses/by/4.0/deed.ja` を実確認）はカタログサイト
    //   （`fc.i-imageworks.jp/anjo-opendetalibrary`）掲載データに自ら範囲を限る。⚠ **カタログ本体は SPA で curl では
    //   中身が取れず、予算データの登録有無・同一ファイルの登載は未確認**。実測できない限り CC BY を流用しない（§9g）。
    //   ⚠ リンク条項は「安城市への連絡なしに**自由に設定できます**」＝許容の宣言。`noDeepLink` は立てない。
    ["R8"], ["R7"], ["R6"], ["R5"], ["R4"],
  ] as const).map(([fy]) => {
    const n = fy.toLowerCase();
    return {
      id: `anjo-kanbetsu-yosan-${n}`,
      title: `${eraYear(fy)}年度 安城市一般会計款別歳入・目的別歳出`,
      publisher: "安城市",
      url: null,
      urls: [
        `https://www.city.anjo.aichi.jp/shisei/zaisei/yosan/documents/documents/${n}kanbetsusainyu.pdf`,
        `https://www.city.anjo.aichi.jp/shisei/zaisei/yosan/documents/documents/${n}mokutekibetsusaisyutsu.pdf`,
      ],
      landingPage: `https://www.city.anjo.aichi.jp/shisei/zaisei/yosan/documents/${n}.html`,
      kind: "pdf" as const,
      fiscalYear: fy,
      scope: "安城市（一般会計・団体コード232122）",
      license:
        "安城市ホームページに掲載している文章、画像等の著作権は、安城市又は原著作者に帰属します。転載、引用等を希望する場合は、安城市へ問い合せてください。",
      parser: "kofu-yosansho" as const,
      parserOptions: {
        revenueFile: `${n}kanbetsusainyu.pdf`,
        expenditureFile: `${n}mokutekibetsusaisyutsu.pdf`,
        revenuePage: 1,
        expenditurePage: 1,
        revenueHeading: "一般会計款別歳入",
        expenditureHeading: "一般会計目的別歳出",
        revenueTotalLabel: "合計",
        expenditureTotalLabel: "合計",
        totalAmountIntIndex: 0,
        totalPrevIntIndex: 2,
  ...
  ...([
    // 日野市（東京都・団体コード 132128）。⚠ 収録済みの八王子・調布・町田・府中（132063）・西東京・小平・三鷹 132047 とは別団体。
    // 予算書の分冊 PDF「歳入歳出予算事項別明細書（総括）」（歳入20〜22款・歳出13款・千円）。**R8〜H16 の23年度が同一様式**。
    // ⚠⚠ **R8 だけ p.1 に歳入（左）と歳出（右）が横並び**（A3・rot 90）。`CropX` で分割（歳入 0〜600pt・歳出 630〜1191pt）。
    //   p.2 は同内容の重複。⚠ `samePage` は throw する。R7〜H16 は p.1=歳入・p.2=歳出。
    // ⚠ 合計ラベルは側で違う（`歳入合計`/`歳出合計`・既定）。見出しは歳入=`総括`・歳出=`歳出`。側の入替えは throw する。
    // ⚠ **R2 と H18 は廃止款が款番号なし・マーカーなしで末尾に載る**（R2 `自動車取得税交付金 0 84,000`・
    //   H18 `特別地方消費税交付金 0 1`）。無指定だと落ちて**前年度 Σ が −84,000／−1 の warning 止まり**（derive まで流れる型）。
    //   `kanNoless: true` で拾う。⚠ 他年度に付けない。
    // ⚠⚠ **同じ年度ページの「14.令和6年度から令和8年度当初予算比較」に同じ指定を当てると、Σ4系統とも差0 のまま
    //   R6/R7 を当年度/前年度として完走する**（款名も `構構対前構対前市税` に壊れる）。ファイルを取り違えないこと。
    // ⚠ **総括 PDF 単体に「日野」の文字は無い**（分冊ゆえ表紙なし・全年度）。landing のページ名と「当初予算概要」の
    //   〔表２〕〔表４〕との全款一致、および年度間クロスチェックで同定した。
    // ⚠ **H28 以前の年度ページは「一般会計予算・特別会計予算」の合同ページ**で、特別会計の同名「総括」PDF が3〜4本並ぶ。
    //   一般会計は各ページの最初の「総括」（合計額と前年度クロスチェックで同定）。
    // ⚠ 歳出款6 は **`農業費`**（小平・伊丹と同じ）。H17/H16 は歳入に `特別地方消費税交付金` を含む21款。**寄せない**。
    // ⚠ ページ ID・ファイル名に規則なし（`6.soukatu.pdf`・日付名・`soukatsu-ippan.pdf` など）。年度から組み立てない。
    // 前年度列は**当初**（R8→H16 の22組・全款一致）。
    // ⚠ 市長選は4年ごとの4月（2025-04-13・2021-04-18・2017-04-16・2013-04-14）だが、R7・R3 の概要と補正1号に
    //   「骨格」「肉付」は0件（R7 補正1号は +126,422 千円の最終日補正）。R8 の前年度列 78,074,000 = R7 当初。骨格の年度なし。
    // ライセンス「ホームページの考え方」（/about/1005477.html・更新 2025-02-06・確認日 2026-09-03）。
    //   ⚠ 原文が「（テキスト、画像、**PDF**、その他データ）」と自ら PDF を含めている。
    //   ⚠ オープンデータ: 「地図情報に関するオープンデータ」の shp/CSV に CC BY 4.0 が付くほか、**H30〜R2 の予算ページ自体に
    //   CC BY 4.0 の「オープンデータ」節（条文・第1〜3表の PDF）がある**（収録レビューが実確認）。⚠ **総括 PDF はこの節に
    //   含まれない**ので要許可のまま。東京都カタログの日野市（t132128・30件）に予算/決算/財政 0件（実検索）。
    //   ⚠ リンク条項は「原則自由です」＋明記の依頼＋フレーム禁止のみで、ディープリンクを断る文言は無い。`noDeepLink` は立てない。
    // [年度, ファイル URL, landing ページ番号, 歳入p, 歳出p]
    ["R8", "001/030/505/r8ippann_soukatu.pdf", "1030505", 1, 1],
    ["R7", "001/028/423/6.soukatu.pdf", "1028423", 1, 2],
    ["R6", "001/025/883/6.soukatu.pdf", "1025883", 1, 2],
    ["R5", "001/023/301/6.soukatu.pdf", "1023301", 1, 2],
    ["R4", "001/019/187/6.soukatu.pdf", "1019187", 1, 2],
    ["R3", "001/015/489/6.soukatu.pdf", "1015489", 1, 2],
    ["R2", "001/013/546/r2ippan06soukatsu.pdf", "1013546", 1, 2],
    ["R1", "001/011/333/06soukatu.pdf", "1011333", 1, 2],
    ["H30", "001/007/588/soukatu.pdf", "1007588", 1, 2],
    ["H29", "001/005/069/20170222-163212.pdf", "1005069", 1, 2],
    ["H28", "001/005/072/20160330-090509.pdf", "1005072", 1, 2],
    ["H27", "001/005/075/20150409-134546.pdf", "1005075", 1, 2],
    ["H26", "001/005/078/20140304-142033.pdf", "1005078", 1, 2],
    ["H25", "001/005/081/20130221-184129.pdf", "1005081", 1, 2],
    ["H24", "001/005/084/20120223-132417.pdf", "1005084", 1, 2],
    ["H23", "001/005/087/20110216-083549.pdf", "1005087", 1, 2],
    ["H22", "001/005/090/20110215-113344.pdf", "1005090", 1, 2],
    ["H21", "001/005/093/20090326-161406.pdf", "1005093", 1, 2],
    ["H20", "001/005/094/soukatsu-ippan.pdf", "1005094", 1, 2],
    ["H19", "001/005/095/20090105-164846.pdf", "1005095", 1, 2],
    ["H18", "001/005/096/ippan_soukatsu18.pdf", "1005096", 1, 2],
    ["H17", "001/005/097/ippankaikeisoukatu.pdf", "1005097", 1, 2],
    ["H16", "001/005/099/ippannjikousoukatu.pdf", "1005099", 1, 2],
  ] as const).map(([fy, path, page, rev, exp]) => ({
    id: `hino-yosansho-soukatsu-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 日野市一般会計予算書 歳入歳出予算事項別明細書（総括）`,
    publisher: "日野市",
    url: `https://www.city.hino.lg.jp/_res/projects/default_project/_page_/${path}`,
    landingPage: `https://www.city.hino.lg.jp/shisei/gyozaisei/yosan/yosan/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "日野市（一般会計・団体コード132128）",
    license:
      "日野市ホームページから発信するコンテンツ（テキスト、画像、PDF、その他データ）の著作権は日野市に帰属します。「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。もし、これらの文書等について転用等を希望される場合は、その旨ご連絡下さい。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "総括",
      expenditureHeading: "歳出",
      ...(fy === "R8" ? { revenueCropX: { from: 0, to: 600 }, expenditureCropX: { from: 630, to: 1191 } } : {}),
      ...(fy === "R2" || fy === "H18" ? { kanNoless: true } : {}),
    },
  })),

  ...([
    // 東広島市（広島県・団体コード 342122）。⚠ 呉市 342025・広島県府中市 342084・広島市と取り違えない。
    // 「予算に関する説明書」の歳入歳出予算事項別明細書「１ 総括」（歳入22〜23款・歳出14款・千円）。
    //   R8〜R5 は現行サイト、**R4〜R2 は年度ページも PDF も現行 404 で Wayback の `id_` 実体 URL から取得**。
    // ⚠ 歳入は R8 で `環境性能割交付金`（0 / 116,843）が**款番号なし・マーカーなし・「皆減」なしの廃止行**になる
    //   （R2 の `自動車取得税交付金` 0 / 136,000 も同型）→ `kanNoless` で拾う。⚠ 無いと**前年度 Σ が −116,843 割れるが
    //   validate では warning 止まり**（derive まで流れる型）。R7〜R3 に付けても副作用なし（偵察が実測）。
    // ⚠⚠ 歳出は財源内訳の列見出し `一般財源` `国県支出金 地方債 その他` が款1 の頭に付く（**Σ は差0 のまま**）→ `HeaderExtra` 必須。
    //   ⚠ 列見出しは字間が空くので `\s*` で結ぶ。
    // ⚠⚠ **同じ PDF に特別会計7本の同名「１ 総括」**（R8: p.161/162・167/168・174/175・186/187・202/203・225/226・239/240）。
    //   p.161/162（墓園）に当てると **throw せず歳入2款・歳出1款で Σ4系統とも差0、しかも歳入合計＝歳出合計 9,302** で完走する
    //   （10例目）。ページは年度ごとに実ファイルで固定する（**R3・R2 は目次が1枚多く p.8/p.9**）。
    // ⚠ 見出しは表の直前行 `(歳   入)`/`(歳   出)`（半角括弧＋全角空白）。側で違うので入替えは throw する。
    // ⚠ 予算書の第1表は前年度列が無い。「予算の概要（全編）」の款別内訳（H31〜H22）は別様式で、R4 で当てると
    //   内訳行（`（普通交付税）`）を拾い Σ +15,327,900 で割れる（大声で落ちる）＝**H31 以前をこの経路で採るならパーサ側の宿題**。
    // ⚠ **H31 の説明書は Wayback に無く、WARP はこの環境から 500/403 で未確認**＝未回収（「無い」とは断定しない）。
    // 前年度列は**当初**（R8→R2 の6組・37款すべて一致。R2 の前年度合計 82,280,000 は H31 概要と一致）。
    // ⚠ 市長選は H30-02-04・R4-01-30・R8-02-01（4年ごとの1〜2月）。**R8・R4 は選挙年度だが「骨格」0件**
    //   （R4 概要は「過去最大」と明記）。⚠ **H30 当初は骨格**（H31 概要の前年度列に「（骨格のみ）」が5件）
    //   → H30 を収録するときは H31 に prevNote。
    // ライセンス「著作権・免責事項」（/soshiki/somu/1_1/4/2/4441.html・更新 2024-04-01・確認日 2026-09-02）。
    //   ⚠ 原文が「本サイトに掲載されている**各ファイル**」と PDF に直接及ぶ。
    //   ⚠ オープンデータは外部の広島広域都市圏ポータル（組織 id 83・70件）で `予算`・`決算` 0件・`財政` 10件は
    //   すべて統計書＝説明書 PDF も同一データも未登録。規約は「本サイト上のコンテンツ」に限り CC の href も無し（§9g）。
    //   ⚠ リンク条項は「**原則として自由にリンクしていただけます**」＋「トップページに対してリンクを**おすすめします**」＝
    //   許容の宣言が先・推奨形（八千代 §13-19 型）。`noDeepLink` は立てない。
    // [年度, URL, landing, 歳入p, 歳出p]
    ["R8", "https://www.city.higashihiroshima.lg.jp/material/files/group/11/R8_setsumeisyo.pdf",
      "https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/44737.html", 7, 8],
    ["R7", "https://www.city.higashihiroshima.lg.jp/material/files/group/11/r7toushosetumeisho.pdf",
      "https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/41645.html", 7, 8],
    ["R6", "https://www.city.higashihiroshima.lg.jp/material/files/group/11/6tousyosetumeisyo.pdf",
      "https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/39037.html", 7, 8],
    ["R5", "https://www.city.higashihiroshima.lg.jp/material/files/group/11/reiwagonenndoyosannnikannsurusetumeisixyo.pdf",
      "https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/35258.html", 7, 8],
    ["R4", "https://web.archive.org/web/20250914062713id_/https://www.city.higashihiroshima.lg.jp/material/files/group/11/R4setsumeisyo.pdf",
      "https://web.archive.org/web/20250815120742/https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/31525.html", 7, 8],
    ["R3", "https://web.archive.org/web/20220709013745id_/https://www.city.higashihiroshima.lg.jp/material/files/group/11/yosansetumeisyo_R3.pdf",
      "https://web.archive.org/web/20220709013519/https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/26155.html", 8, 9],
    ["R2", "https://web.archive.org/web/20220710004158id_/https://www.city.higashihiroshima.lg.jp/material/files/group/11/R2setsumeisho.pdf",
      "https://web.archive.org/web/20201029015708/https://www.city.higashihiroshima.lg.jp/shisei/zaisei/6/22917.html", 8, 9],
  ] as const).map(([fy, url, landing, rev, exp]) => ({
    id: `higashihiroshima-setsumeisho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 東広島市予算に関する説明書（歳入歳出予算事項別明細書 総括）`,
    publisher: "東広島市",
    url,
    landingPage: landing,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "東広島市（一般会計・団体コード342122）",
    license:
      "本サイトに掲載されている各ファイル及びその内容に関する諸権利は、原則として東広島市に帰属します。私的利用の範囲や、引用など著作権法上認められる場合を除き、当サイト上の情報を、許可なく使用・複製・転載・販売・改変・印刷配布する行為等は禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      ...
      expenditureHeading: "(歳   出)",
      kanNoless: true,
      expenditureHeaderExtra: "一\\s*般\\s*財\\s*源|国\\s*県\\s*支\\s*出\\s*金|財\\s*源\\s*内\\s*訳|特\\s*定\\s*財\\s*源",
    },
  })),

  ...([
    // 岸和田市（大阪府・団体コード 272027）。⚠ 収録済みの大阪府・大阪市・堺・豊中・吹田・高槻・枚方・八尾・和泉・東大阪とは別団体。
    //   ⚠ 茨木 272116・寝屋川 272159 は全年度収録不可と判定済み。⚠ 旧ドメイン `city.kishiwada.osaka.jp` は DNS 解決不能（現行 `.lg.jp`）。
    // 一般会計予算書「予算に関する説明書 １ 総括」（歳入21〜22款・歳出14款・千円）。見出し `（歳入）`/`（歳出）`・合計ラベルは既定（側で違う）。
    // ⚠⚠ **R2・H30〜H26 は歳出総括が A3 で財源内訳表と同一ページに横並び** → `expenditureCropX` が必須。
    //   無いと **Σ4系統とも差0 のまま款1 が `一般財源国庫支出金府支出金地方債その他議会費` になる**（静かに通る型・偵察が実測）。
    // ⚠⚠ **R2 の歳入は廃止款の印が全角アスタリスク `＊`**（`＊ 自動車取得税交付金 0 85,000 △85,000`・「皆減」の語なし）。
    //   `ABOLISHED_MARK_RE` に無く行ごと落ちて**前年度 Σ −85,000（validate では warning 止まり）**だった。
    //   この収録で `＊`/`*` を文字クラスに足した（既存全ソースを再 parse して回帰0を確認）。
    // ⚠⚠ **各予算書ページには当初＋各回補正の予算書が全部並ぶ**（R7 は6組）。当初は「当初予算」見出し下の一番下（ID 最小）。
    //   ⚠ URL 規則が破れる（R3〜R8 `-yosansyo`・**R2/H31 は「予算」と「予算書」のスラグが入れ替わっている**・
    //   H30 `13-h30hoseiyosansho`・H29〜H26 はハイフン無し・H27/H26 の PDF は `/uploaded/life/…_misc.pdf`）。
    // ⚠ **R4・R3・H31 は収録不可**（`unrecordable.ts`。R4=数字がテキスト層に無い／R3=ベクター外形化／H31=歳出の総括ページが説明書に無い）。
    // ⚠ 歳入款16 は **`府支出金`**。H30〜H26 は21款（法人事業税・環境性能割が無く自動車取得税がある）。**寄せない**。
    // 前年度列は**当初**（読めた全ペアで全款一致。R2→H31 は raw から突合 22/23＝R2 新設の法人事業税交付金のみ差）。
    // ⚠ 市長選は 2025-04-06・2022-01-30・2018-02-04。R7 当初は市長選後の 5月提出・臨時会可決で「骨格」「肉付」0件。
    //   **大阪府「市町村財政データベース」R8 当初予算状況 xlsx で岸和田の区分欄は空欄**（豊中・貝塚・泉南は「骨格」）＝骨格ではない。
    //   ⚠ **R6 は当初予算が否決され 4〜7月を暫定予算で運用**、本予算は 2024-04-25 提出・臨時会可決。R2 は「修正可決後の予算書」。
    //   ⚠ **R4 が読めるようになった場合は前年度列の基準を必ず突合する**（R4 概要 HTML は「令和3年度第1回定例会で可決された補正予算を含み計算」と明記＝R3 は当初と同時に補正が組まれている）。
    // ⚠⚠ **ライセンスは発行元サイトに利用規約・著作権条項が存在しない**（「リンク・著作権・免責事項」ページの本文は免責事項のみ・
    //   著作権/複製/転載/許可/無断の語が0件・リンク条項も無し）。フッターの表示 `Copyright Kishiwada City All rights reserved.` を
    //   原文のまま置く。⇒ **判定語に当たらず `unverified`**（自サーバー配信が開く側）。**人の判断が要る論点として docs §13-21 に残す**。
    //   確認: /site/userguide/menseki.html（更新 2025-01-06・確認日 2026-09-03）。
    //   ⚠ オープンデータは BODIK（odcs.bodik.jp/272027・102件・CC BY 4.0）だが「当サイトの内容」に限り、`予算`・`決算`・`財政` が0件（実検索）＝及ばない。
    // [年度, PDF パス, landing スラグ, 歳入p, 歳出p, crop]
    ["R8", "attachment/160864.pdf", "13-r8-yosansyo", 23, 24, false],
    ["R7", "attachment/152185.pdf", "13-r7-yosansyo", 21, 22, false],
    ["R6", "attachment/140025.pdf", "13-r6-yosansyo", 21, 22, false],
    ["R5", "attachment/127888.pdf", "13-r5-yosansyo", 21, 22, false],
    ["R2", "attachment/85110.pdf", "13-r2-yosan", 4, 5, true],
    ["H30", "attachment/59447.pdf", "13-h30hoseiyosansho", 4, 5, true],
    ["H29", "attachment/48177.pdf", "13-h29yosansho", 3, 4, true],
    ["H28", "attachment/36894.pdf", "13-h28yosansho", 3, 4, true],
    ["H27", "life/77313_92637_misc.pdf", "13-h27yosansho", 3, 4, true],
    ["H26", "life/59832_59059_misc.pdf", "13-h26yosansho", 3, 4, true],
  ] as const).map(([fy, path, slug, rev, exp, crop]) => ({
    id: `kishiwada-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 岸和田市一般会計予算書（予算に関する説明書 総括）`,
    publisher: "岸和田市",
    url: `https://www.city.kishiwada.lg.jp/uploaded/${path}`,
    landingPage: `https://www.city.kishiwada.lg.jp/page/${slug}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "岸和田市（一般会計・団体コード272027）",
    license: "Copyright Kishiwada City All rights reserved.",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rev,
      expenditurePage: exp,
      revenueHeading: "（歳入）",
      expenditureHeading: "（歳出）",
      ...(crop ? { expenditureCropX: { from: 0, to: 560 } } : {}),
    },
  })),

  ...([
    // 小田原市（神奈川県・団体コード 142069）。「令和N年度 一般会計・特別会計・企業会計 予算書」（単一 PDF・約580p）の
    //   冒頭「一般会計予算額構成比一覧表」＝物理 p.6 歳入（印字 2）/ p.7 歳出（印字 3）。R8〜R5 の4年とも同ページ（自分で実測）。
    //   列は `[当年度, 構成比, 前年度, 構成比, 伸率]`（増減額列なし・小数を除いた整数列が `[当年度, 前年度]` に当たる）。
    // ⚠⚠ 同じ PDF に**同じ数字・同じ見出し「（歳入）」「（歳出）」の別表**がある: p.71-73 事項別明細書「１ 総括」（本年度/前年度/比較
    //   ＋歳出は財源内訳8列）と、p.231/253/273/305/321/345/373/393/417 の**特別会計の総括**（同じ見出し）。ページを動かすと
    //   別会計を Σ 差0 で読む（同名総括の 11 例目）。p.5 各会計集計表・p.8-9 経費別内訳・p.12-15 第1表にも歳入合計/歳出合計がある。
    // ⚠⚠ **R7 のウェブ掲載は発行元ページの掲載名が「令和7年度当初予算書（原案）」**（PDF の表紙自体には「（原案）」の字は無い・
    //   レビューが描画して実測）＝議会修正（修正案その1「分野別市民会議開催事業費」削除・その2「主権者教育推進事業」削除）前の額。R8 の前年度列は修正後（総務費 9,935,666 / 教育費 8,076,452 / 予備費 32,514・総額は不変）で、R7 の当年度列
    //   （9,937,138 / 8,077,494 / 30,000）と 1,472 / 1,042 / 2,514 千円ずれる。**derive の年度間クロスチェーンは合計だけを見る**ので
    //   止まらない。R7 の title に「（原案）」を残し、R8 に prevNote を付けて画面に出す。修正案 PDF の別表は画像（17p・653字）で
    //   機械読取不可＝差額は R8 との突合から導いた値。
    // ⚠ R4〜H26 の予算書はスキャン画像（先頭40p 文字0・フォント0・全年度）→ unrecordable。H25 以前は予算書の掲載なし（概要のみ）。
    // ⚠ R8 の PDF はファイル名日付 2026-03-31（2月時点の「（案）」から**同じ unit 716196 で差し替え**られている可能性）。
    //   三鷹 R8 と同型の懸念＝魚拓の sha256 は必ず年度と突合する。
    // 骨格予算: 概要 H21〜R8 の全文 grep で「骨格」は H21 概要の H20 についての注記のみ（範囲外）。R2・R6（市長選の年）も骨格ではない。
    // ライセンス: 「著作権・免責事項・プライバシーポリシー・リンクポリシーについて」（/tyosaku/tyosakuetc.html・確認日 2026-09-03）。
    //   「複製・転用」が判定語に当たり permission-required。オープンデータ利用規約（CC BY 4.0）は「オープンデータサイトに掲載されている
    //   データ」に範囲を限り、同サイトの掲載は CSV 4件（財政資料0件）→ 予算書には及ばない。
    //   リンク条項「可能な限りトップページの URL に対してリンクをお願いします」＋「事後でも結構ですので…お知らせください」＋フレーム禁止
    //   ＝§11h 第5群（事後連絡の依頼）＋第3群（フレーム禁止）の複合で、第2群（トップページ以外は要相談）ではない → `noDeepLink` は立てない。
    //   「フレーム内に市ホームページのページを表示させるリンク設定は行わないでください」は要許可なので発行元ディープリンクで済む。
    // [年度, landing パス, ファイルパス]
    ["R8", "p40853", "716196/1-20260331165651_b69cb7e431fe39.pdf"],
    ["R7", "p39351", "674706/1-20250207103915_b67a56443c26ff.pdf"],
    ["R6", "p37649", "617101/1-20240207131432_b65c303a88005b.pdf"],
    ["R5", "p35711", "568572/1-20230207111353.pdf"],
  ] as const).map(([fy, page, file]) => ({
    id: `odawara-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 小田原市予算書${fy === "R7" ? "（原案）" : ""}（一般会計予算額構成比一覧表・歳入歳出の款別＋前年度当初比較）`,
    publisher: "小田原市",
    url: `https://www.city.odawara.kanagawa.jp/global-image/units/${file}`,
    landingPage: `https://www.city.odawara.kanagawa.jp/field/municipality/finance/details/budget/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "小田原市（一般会計・団体コード142069）",
    license:
      "小田原市ホームページ（以下、「当ホームページ」といいます。）で提供するすべてのコンテンツ（情報・文章・画像など）について、権利者の許可なく複製・転用・販売といった二次利用をすることを固く禁じます。当ホームページに掲載した著作物（文章・画像・音声など）、プログラムに係る著作権は、特別の断りがない限り小田原市が保有します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 6,
      expenditurePage: 7,
      ...
      ...(fy === "R8"
        ? {
            prevNote:
              "前年度（令和7年度）の額は市議会の修正後の当初予算額。ウェブに掲載されている令和7年度予算書は「（原案）」で、修正前の額（総務費・教育費・予備費で計2,514千円の振替。総額は同じ）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 立川市（東京都・団体コード 132021）。「令和N年度立川市一般会計予算及び事項別明細書」（単一 PDF・約430p）の
    //   「予算に関する説明書 1. 総括」＝歳入23款・歳出12款・千円・列 `[本年度, 前年度, 比較]`（歳出は右に財源内訳4列が続くが
    //   先頭2整数で読める）。物理ページは年度ごとに動く（下表・自分で実測）。印字ノンブルは物理 −1（R8）。
    // ⚠ 見出しは**半角括弧の `(歳入)` `(歳出)`**（原典 `(歳     入)`）。全角にすると throw。款番号は `1.` のピリオド付き（松戸型）。
    // ⚠ 同 PDF の p.7〜11「第1表 歳入歳出予算」は款・項・金額のみで**前年度列が無い**（合計は総括と同額）。見出し `(歳入)` が無いので
    //   取り違えは throw する。特別会計は別 PDF（`rN_tokkai.pdf`）＝同名総括の同居なし。
    // ⚠⚠ **R6・H31・H30 は ToUnicode 欠落**（#159 の豊島・荒川と同じ固定ガーブル）→ `decodeGarble`。`GARBLE_CHAR_MAP` に
    //   14字（有提供在町村助／法人／内定他防／※）を足した（`pipeline/lib/garble-decode.ts`・真の字は R7/R8 の同款名と突合）。
    //   復号後の見出しは括弧が残らないので `歳入`/`歳出`。⚠ **H28 の予算書はスキャン画像**（先頭40p 数字0・フォント3＝ノンブルのみ）
    //   → 同年の「予算案の概要」の款別表で代替（下の別エントリ）。
    // ⚠ URL は年度ごとに綴りが揺れる（`rN_ippan` / `…yosansho` / **H31・H30 は `yosansyo`**）→ 機械生成せず個別に書く。
    // 前年度列は**当初**（年度間クロスチェックを歳入・歳出の全款で突合し不一致0・自分で実測）。象徴計上 `自動車取得税交付金 1 1` は原典。
    // 骨格予算: 概要 H28〜R8 と骨子 R8 に「骨格」0件。市長選は 2019-09・2023-09 で当初予算期と重ならない。
    // ライセンス: 「著作権」（/shisei/koho/1005487/1005541.html・更新 2024-04-18・確認日 2026-09-03）。「無断」で permission-required。
    //   オープンデータ（CC BY）は「本ページで公開しているデータ」に自ら範囲を限り、財政系の登載は CSV/XLSX の統計表のみ
    //   （予算書・概要 PDF は非登載）→ 予算書には及ばない（§9g）。
    // ⚠⚠ リンク方針（/shisei/koho/1005487/1005542.html・更新 2024-04-22）: 「リンク設定はトップページ(https://www.city.tachikawa.lg.jp)へ
    //   お願いします。他のページへのリンクは、予告なくアドレスが変更になる可能性がありますのでご遠慮ください」＝**トップページ以外を
    //   断る**（§11h 第2群・沖縄型）→ `noDeepLink` を立てる。「開ける側へ倒さない」。⚠ landingPage も「他のページ」ではあるが、
    //   振替先の選択肢は landing か発行元ファイルかしか無い（現行の仕組み）。
    // [年度, ファイル名, 歳入 物理p, 歳出 物理p, 復号]
    ["R8", "r8_ippan.pdf", 17, 18, false],
    ["R7", "r7_ippan.pdf", 17, 18, false],
    ["R6", "r6ippankaikeiyosansho.pdf", 18, 19, true],
    ["R5", "r5ippankaikeiyosansho.pdf", 22, 23, false],
    ["R4", "r4ippankaikeiyosansho.pdf", 18, 19, false],
    ["R3", "r3ippankaikeiyosansho.pdf", 20, 21, false],
    ["R2", "r2ippankaikeiyosansho.pdf", 20, 21, false],
    ["H31", "h31ippankaikeiyosansyo.pdf", 20, 21, true],
    ["H30", "h30ippankaikeiyosansyo.pdf", 20, 21, true],
    ["H29", "h29ippankaikeiyosansho.pdf", 18, 19, false],
  ] as const).map(([fy, file, rp, ep, garble]) => ({
    id: `tachikawa-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 立川市一般会計予算及び事項別明細書（予算に関する説明書 総括・歳入歳出の款別＋前年度当初比較）`,
    publisher: "立川市",
    url: `https://www.city.tachikawa.lg.jp/_res/projects/default_project/_page_/001/007/125/${file}`,
    landingPage: "https://www.city.tachikawa.lg.jp/shisei/yosan/1007100/1007125.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "立川市（一般会計・団体コード132021）",
    noDeepLink: true,
    license:
      "「立川市ホームページ」(以下、当サイトといいます)に掲載されている情報(文章、写真、イラスト、画像等の各ファイル)およびその内容に関する諸権利は、原則として立川市に帰属し、国際条約・法律等によって保護されています(一部の画像等の著作権は、原著作者が所有しています)。また、当サイトに掲載された社名・商品名などの名称は、一般に関係各社の商標・登録商標です。「私的使用のための複製」や「引用」など著作権法上認められる場合を除き、立川市の許可なく当サイトに掲載されている文書・画像等を無断使用・複製・転載・販売・改変・印刷配布することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...(garble
        ? { revenueHeading: "歳入", expenditureHeading: "歳出", decodeGarble: true }
        : { revenueHeading: "(歳入)", expenditureHeading: "(歳出)" }),
    },
  } satisfies SourceEntry)),

  {
    // 立川市 H28 — 予算書がスキャン画像なので「予算案の概要及び主要施策の概要」の「３ 歳入歳出の状況（一般会計）(1)歳入／(2)歳出 ①目的別」
    //   で代替（款別・前年度当初・増減額・千円・款番号は全角密着 `１市税`）。物理 p.10 歳入 / p.13 歳出。
    // ⚠⚠ **歳出ページは同じ頁に「②性質別」が縦積みで合計が同額**（自分で実測）→ 見出し `目的別` を必ず指定する。
    // ⚠ 合計ラベルは両側とも `合計`。H29 予算書の前年度列と全21款＋12款が一致（クロスチェック）。
    // ⚠ H31・R2 の概要は当該ページが画像＝概要は他年度の代替にならない（予算書側が健全なので不要）。
    id: "tachikawa-yosan-gaiyou-h28",
    title: "平成28年度 立川市予算案の概要及び主要施策の概要（歳入歳出の状況・款別＋前年度当初比較）",
    publisher: "立川市",
    url: "https://www.city.tachikawa.lg.jp/_res/projects/default_project/_page_/001/007/123/h28yosannannnogaiyou.pdf",
    landingPage: "https://www.city.tachikawa.lg.jp/shisei/yosan/1007100/1007123.html",
    kind: "pdf",
    fiscalYear: "H28",
    scope: "立川市（一般会計・団体コード132021）",
    noDeepLink: true,
  ...
  ...([
    // 鎌倉市（神奈川県・団体コード 142042）。「鎌倉市一般会計予算に関する説明書」冒頭「歳入歳出予算事項別明細書 １ 総括」
    //   ＝物理 p.5 歳入 / p.6 歳出（印字ノンブル＝物理）。列 `[本年度, 前年度, 比較]`・千円。R8・R7 とも同ページ（自分で実測）。
    // ⚠ **款番号は5刻み**（5,10,15,16,17,…,90）で連番でない（原典どおり）。**「観光費」を独立の款に持つ**（歳出14款）。
    // ⚠ 特別会計は別 PDF（`toushoyosan-tokubetsu-*.pdf`）＝同一ファイル内に同名総括は無い。R8 全199ページで総額 86,024,400 が
    //   出るのは p.5/p.6 のみ（自分で実測）。p.7 は財源内訳（款名なし）。
    // ⚠⚠ **R6・H31・H30 は画像のみ**（p.5-6 の文字数6＝ノンブル）、**R5〜R2 は OCR 層**で「歳」が `'`・`.`・`威` に化けて
    //   合計行が見つからず throw、数字も `l,8i9,223`（正 1,819,223）の型で壊れる → 7年度を unrecordable に記録。
    // ⚠⚠ **使ってはいけない代替**: 同ページの小 PDF `yosan-mokuteki-r08.pdf`（目的別・1p）と HTML 表は列が
    //   `[当初予算額, 構成比, 対前年度比較 金額, 伸び率]` で**前年度額の列が無い** → kofu パーサは増減額を前年度として保存しつつ
    //   **Σ が両側で成立する「静かに通るが中身が違う」型**（推定・当てていない）。**オープンデータ CSV（R6〜H16・CC BY）**は
    //   節レベルで `査定現在額` の Σ は当初総額に一致するが **`前年当初額` は R5 以外で Σ 不一致**（R6 歳出で 456 行が 0）
    //   ＝前年度列を印字した一次資料ではないので今回は使わない（要判断として §13-23 に残す）。
    // ⚠ URL 規則は年度で3回破れる（`-r08`/`-r07` → `06`〜`02` → `31` → `30toushoyosan-ippan`）＝機械生成しない。
    // ⚠ 年度ページに「議決を得るまでは予算（案）」の注記があるが PDF 表紙に「案」は無い（3/16 更新後を取得）。
    // 骨格予算: R8・R7 の説明書と概要で `骨格|肉付` 0件（市長選は 2025 年秋・当初予算期と重ならない）。
    // ライセンス: 「著作権・免責事項」（/kouhou/copyright.html・確認日 2026-09-09）。「無断」で permission-required。
    //   オープンデータ（CC BY 4.0）は「上記対象データのみに適用」と自ら範囲を限り、対象一覧に説明書 PDF は無い（実検索）。
    //   リンク条件は「トップページへのリンクは自由」のみで制限なし → `noDeepLink` は立てない。
    ["R8", "toushoyosan-ippan-r08.pdf"],
    ["R7", "toushoyosan-ippan-r07.pdf"],
  ] as const).map(([fy, file]) => ({
    id: `kamakura-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 鎌倉市一般会計予算に関する説明書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "鎌倉市",
    url: `https://www.city.kamakura.kanagawa.jp/zaisei/documents/${file}`,
    landingPage: "https://www.city.kamakura.kanagawa.jp/zaisei/yosan-setsumei.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "鎌倉市（一般会計・団体コード142042）",
    license:
      "当サイトに掲載されている文字、写真、イラストやデザインといった情報の著作権は、原則として鎌倉市または原権利者に帰属します。私的使用または引用等著作権法上認められている行為を除き、無断で転載等を行うことはできません。引用を行う際は、適宜の方法により、必ず出所を明示してください。また、当サイトの内容の全部または一部について、鎌倉市に無断で改変を行うことはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 5,
      expenditurePage: 6,
  ...
  ...([
    // 習志野市（千葉県・団体コード 122165）。「予算に関する説明書」冒頭「１ 総括 歳入歳出予算事項別明細書」＝歳入21款・歳出13款・
    //   列 `[本年度, 前年度, 比較増減]`・千円。**R8〜H27 の12年が現行サイトに同型**（発行元の年度一覧を全ページ開いて実確認）。
    // ⚠⚠ **`HeaderExtra` を付けないと Σ 差0 のまま款名が壊れる**（最も危険な型・自分で実測）: 歳入款1 が
    //   `総括歳入歳出予算事項別明細書市税`、歳出款1 が `国県支出金地方債その他議会費`、款12 が `国県支出金地方債その他諸支出金`
    //   （歳出 p.9 に財源内訳ヘッダが再掲される）。`expenditureHeaderExtra` は歳出側限定なので歳入の国庫支出金/県支出金 款には触らない。
    //   R4 以前の財源内訳見出しは `国庫支出金 県支出金 地方債 その他` だが `地方債` で同じ行ごと落ちるので同じ指定で効く。
    // ⚠ ページ位置は4群で違う: R8〜R6 = p.7 / 8-9、R5 = p.5 / 6-7、R4・R3 = p.4 / 5、R2〜H27 = p.5 / 6（印字との差 0〜+6）。
    //   R8〜R5 は歳出が2物理ページ（款12・13と合計行が次ページ）→ `expenditurePages`。R2〜H27 の歳出は1物理ページに印字2ページの 2-up。
    // ⚠⚠ **単一 PDF に国保・介護・後期の3特別会計を同梱**し、各総括が**同じ見出し `（歳入）`/`（歳出）`・同じ合計ラベル**
    //   （R8: p.317/318・345/346・383/384。H27 は4会計）＝ページを誤ると特別会計を Σ 差0 で読む（同名総括 12例目）。
    // ⚠ R2 歳入に廃止款 `△自動車取得税交付金 … 皆減`（款番号なし）→ kanNo null で出る。前年度 62,000 は R1 の当年度と一致（自分で実測）。
    // ⚠ URL は年度ごとに**完全に不規則**（`69431095.pdf` 等）→ 機械生成せず個別に書く。年度更新は年度ページから拾い直す。
    // 前年度列は**当初**（11リンク × 全款で不一致0・自分で実測。新設款の前年度0のみ）。骨格予算: 12年の冒頭10p・概要・年度ページで 0件。
    // ライセンス: 「習志野市ウェブサイト（ホームページ）のご利用にあたって」（/aboutweb/website_chui.html・更新 2022-09-29・
    //   確認日 2026-09-09）。「無断」で permission-required。オープンデータ（CC BY 2.1 JP）は「本ページに掲載しているデータ」に
    //   自ら範囲を限り、47ファイル中に予算・財政は0件（実検索）→ 説明書には及ばない。リンク条項は無い → `noDeepLink` は立てない。
    // [年度, landing, ファイル, 歳入p, 歳出p(from), 歳出p(to|null)]
    ["R8", "27688.html", "R8yosansetumeisyo.pdf", 7, 8, 9],
    ["R7", "24727.html", "R7yosannnikannsurusetumeisyo1.pdf", 7, 8, 9],
    ["R6", "22147.html", "R6setumeisyo.pdf", 7, 8, 9],
    ["R5", "18901.html", "R5yosannikansurusetumeisyo.pdf", 5, 6, 7],
    ["R4", "R4yosannogaiyou.html", "R4yosannikansurusetumeisyo.pdf", 4, 5, null],
    ["R3", "reiwa3nendotousyoyosannnogaiyou.html", "3setumeisyo.pdf", 4, 5, null],
    ["R2", "reiwa2nenndoyosasnnnogaiyou.html", "69431095.pdf", 5, 6, null],
    ["R1", "narashino_city201899.html", "31setumeisyo.pdf", 5, 6, null],
    ["H30", "narashino_city_12.html", "30yosannnikannsurusetumeisyo.pdf", 5, 6, null],
    ["H29", "H29yosannogaiyou.html", "H29setsumeisyo.pdf", 5, 6, null],
    ["H28", "160220151102165208111.html", "28setumeisyo.pdf", 5, 6, null],
    ["H27", "160220141104173855689.html", "27setumeisho.pdf", 5, 6, null],
  ] as const).map(([fy, page, file, rp, ef, et]) => ({
    id: `narashino-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 習志野市予算に関する説明書（一般会計 総括・歳入歳出予算事項別明細書 款別＋前年度当初比較）`,
    publisher: "習志野市",
    url: `https://www.city.narashino.lg.jp/material/files/group/17/${file}`,
    landingPage: `https://www.city.narashino.lg.jp/soshiki/zaisei/gyomu/zaiseikaikei/yosangaiyo/${page}`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "習志野市（一般会計・団体コード122165）",
    license:
      "当ウェブサイト全体および掲載情報（資料、画像、文章）などは、著作権法および国際条約による著作権保護の対象となっています。個人的な使用または引用等の著作権法上認められた行為をのぞいて、無断で転載等を行うことはできません。 また、一部を引用、転載する場合においても出所を明示してください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      ...(et === null ? { expenditurePage: ef } : { expenditurePages: { from: ef, to: et } }),
      revenueHeading: "（歳入）",
      expenditureHeading: "（歳出）",
      revenueHeaderExtra: "事項別明細書",
      expenditureHeaderExtra: "国県支出金|地方債|その他",
    },
  } satisfies SourceEntry)),

  ...([
    // 浦安市（千葉県・団体コード 122271）。「令和N年度当初予算書」（単一 PDF・415〜482p）の「歳入歳出予算事項別明細書 １ 総括」
    //   ＝歳入21款・歳出11〜12款・千円・列 `[本年度, 前年度, 比較]`（歳出は右に財源内訳5列が続くが先頭2整数で読める）。
    //   物理ページは年度で動く（R8 25/26・R7〜R4 21/22・R3 19/20・自分で実測）。印字ノンブルは R8・R7 で物理と一致するが **R3 は物理 p.20 が印字「－ 16 －」**（レビューが実測）＝印字で探さない。
    // ⚠⚠ **`HeaderExtra` を付けないと Σ 差0 のまま款名が壊れる**（自分で実測）: 見出し行より前の「歳入歳出予算事項別明細書」
    //   「１ 総括 （△印は減）」が款5 に混ざり `歳入歳出予算事項別明細書総括（△印は減）市税`、歳出は財源内訳の列見出し
    //   `国庫支出金県支出金地方債その他` が `議会費` の頭に付く。
    // ⚠ 款48 災害復旧費の本年度欄が `-`（R7: `- 100 △100`）→ **`dashAsZero` 必須**（無いと prev を 100/100 と読み前年度 Σ −100）。
    // ⚠ 款番号は5刻み＋27・29 の挿入（原典どおり）。ページ中に隠しテキスト `当初<23>` が出るが金額を持たず無害（実測）。
    // ⚠⚠ 同 PDF に**同じ見出し「（歳 入）」「（歳 出）」の特別会計の総括**が6本（R8: p.223/224 国保・261/262 墓地公園・293/294 介護保険・
    //   327/328 介護サービス・355/356 後期高齢者・391 下水道）＝ページを動かすと別会計を Σ 差0 で読む（同名総括 13例目）。
    // ⚠⚠ **骨格予算: R7・R3（・H29）**。R8 概要 p.6「令和７年度当初予算は骨格予算であったため、６月補正後予算額と比較」、
    //   R4 概要も同文。**予算書の総括は骨格年の翌年も当初（骨格）額と比較している**ので prevBasis は原典どおり「当初」で、
    //   R8 前年度 83,000,000 = R7 当年度（骨格）。R8・R4 に prevNote で「前年度は骨格予算の当初額」を明示する。
    //   ⚠ **概要 PDF の款別表（R8 p.10/14）を使うと前年度が6月補正後**で、原典の語が「６月補正後予算」なので自動判定は
    //   「補正後予算額」に当たらず**静かに「当初」と判定される**（偵察が実測）＝概要は使わない。
    // ⚠ R2 以前の予算書はスキャン（自分で実測: R2/R1/H30/H29/H28 の先頭40p で金額0）。代替の概要系列は年度で可否が割れる:
    //   H30・H27〜H20・H18〜H16 は概要/一般会計予算 PDF に款別表がある（前年度は当初または6月補正後）が、横展開優先で今回は見送り。
    //   **R2・H22 の概要は皆減行 `－ － 62,500 … 皆減` の構成比欄の `－` も dashAsZero が 0 にして整数列がずれる**（既存 options で
    //   救えない・上越 R2 の「皆増が前年度側の列」と同族＝パーサ手当て待ち）。R1・H29・H28（歳入頁が画像）・H19 は概要も画像 → unrecordable。
    // ライセンス: 「著作権法等」（/site/1005105.html・更新 令和5年7月10日・確認日 2026-09-09）。「無断」「複製・転用」で permission-required。
    //   オープンデータ利用規約（CC BY 4.0）は「地図データ」「コンテンツデータ」ページに範囲を限り「すべての情報に該当するものではありません」
    //   と明記。コンテンツデータの財政系は決算書 PDF（R1〜R5）のみで予算書は非登載（§9g）。リンクポリシーの独立ページは無い → `noDeepLink` は立てない。
    // [年度, 年度ページID, ファイル名, 歳入p, 歳出p]
    ["R8", "1046970", "r8.yosannsyo.pdf", 25, 26],
    ["R7", "1043323", "yosansyo.pdf", 21, 22],
    ["R6", "1040763", "r6yosansyo2.pdf", 21, 22],
    ["R5", "1037629", "r5toushoyosan.pdf", 21, 22],
    ["R4", "1034029", "r4yosansho.pdf", 21, 22],
    ["R3", "1030294", "r0301tosyoyoansyo.pdf", 19, 20],
  ] as const).map(([fy, pid, file, rp, ep]) => ({
    id: `urayasu-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 浦安市当初予算書（歳入歳出予算事項別明細書 総括・歳入歳出の款別＋前年度当初比較）`,
    publisher: "浦安市",
    url: `https://www.city.urayasu.lg.jp/_res/projects/default_project/_page_/001/${pid.slice(1, 4)}/${pid.slice(4)}/${file}`,
    landingPage: `https://www.city.urayasu.lg.jp/shisei/zaisei/yosan/${pid}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "浦安市（一般会計・団体コード122271）",
    license:
      "当サイトに掲載されている文章・写真・イラストなどの情報は、著作権の対象となっています。また、「浦安市公式ホームページ」全体も編集著作物として著作権の対象となっており、ともに著作権法により保護されています。浦安市または第三者が著作権を有しており、「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      expenditureHeading: "（歳   出）",
      dashAsZero: true,
      revenueHeaderExtra: "歳入歳出予算事項別明細書|^１総括|△印は減",
      expenditureHeaderExtra: "△印は減|^国庫支出金県支出金地方債その他$",
      ...(fy === "R8"
        ? { prevNote: "前年度（令和7年度）の当初予算は骨格予算（発行元の令和7年度予算ページに「令和7年度当初予算は…『骨格予算』として編成しました」と明記。令和8年度当初予算案の概要も「令和７年度当初予算は骨格予算であったため、６月補正後予算額と比較」と注記）。ここでの前年度額はその骨格予算の当初額で、6月補正後の額ではない。" }
        : fy === "R4"
          ? { prevNote: "前年度（令和3年度）の当初予算は骨格予算（発行元の令和3年度予算ページに「令和3年度当初予算は、市長選挙を控えていることから…『骨格予算』を編成しました」と明記。令和4年度当初予算案の概要にも「令和３年度当初予算は骨格予算であったため」と注記）。ここでの前年度額はその骨格予算の当初額で、6月補正後の額ではない。" }
          : {}),
    },
  } satisfies SourceEntry)),

  ...([
    // 西尾市（愛知県・団体コード 232131）。⚠ 収録済みの愛知県・名古屋・豊橋・岡崎・一宮・春日井・豊川・豊田・安城とは別団体。
    // 「当初予算の概要」（8p・全年度同一様式・JUST PDF）p.4「一般会計予算款別調（歳入）」/ p.5「（歳出）」。千円。
    //   列 `[当年度, 構成比, 前年度, 構成比, 増減, 増減率]`（小数を除いた整数列が `[当年度, 前年度]`）。R8〜H29 の10年で同一（自分で実測）。
    // ⚠ 合計ラベルは両側とも `合計`（`歳入合計` ではない）。見出しは `（ 歳 入 ）`（空白入り・compact 照合で当たる）。
    // ⚠ 同 PDF の p.6「財源別調」・p.7「経費別調（性質別）」は合計が同値（77,650,000）だが、見出し（歳入）/（歳出）が無いので
    //   ページを取り違えると **throw する**（自分で当てて確認）。p.3「会計別予算」は特別会計・企業会計を含む別表。
    // ⚠⚠ **現行サイトには R8 しか無い**（年度ページ `1003218.html` は毎年上書き・過年度 URL は全 404）。R7〜R3 は WARP
    //   （waid/11193・四半期捕捉）、R2〜H29 は旧 CMS（`index.cfm`）URL の WARP 捕捉。**ファイル名は年度ごとに不規則**＝機械生成しない。
    //   landing も同じ捕捉 ts の WARP URL（10年ぶん 200 を実測・R7 のページに `r7yosangaiyou.pdf` のリンクが実在）。
    //   ⚠ **R8 の URL も来春上書きされる見込み**（三鷹 R8 型）→ 収録直後に archive し、R9 収録時に WARP 側へ差し替える。
    // ⚠ 歳入の款数は R2〜R8=23・H31=22（法人事業税交付金なし）・H30/H29=21（環境性能割交付金もなし）。歳出は全年度14。
    //   象徴計上の1円行（自動車取得税交付金 R2〜R8・災害復旧費 R5 以前・諸支出金 全年度）は原典。
    // 前年度列は**当初**（R8→H29 の9リンク・総額と款別が全件一致・偵察と自分で実測）。骨格予算: R3〜R8 の概要本文で「骨格|肉付|暫定」0件。
    // ⚠⚠ **ライセンスは判定語に当たらず `unverified` に落ちる**（岸和田型）。現行サイト（2021-04 リニューアル後）に著作権・利用規約・
    //   リンクのページが無く（サイトマップ・利用ガイドを実検索）、フッター「© NISHIO City, All Rights Reserved.」のみ。
    //   ⚠ 旧サイト（WARP 2020-05 捕捉・更新 2016-03-16）の「このサイトについて」は「無断でデータの使用、改変等はお断りいたします」
    //   「トップページ以外に直接リンクする場合は、リンク切れになる場合がありますのでご承知ください」。現行に無い規約を license 欄に
    //   書かない（§9g の逆向き）が、**発行元の直近の方針は「無断お断り」**＝安城・岸和田と同じ**人の判断が要る論点**として §13-23 に残す。
    //   オープンデータ（CC BY 4.0）は「クリエイティブ・コモンズ・ライセンスが付与されているデータ」に適用と規約に明記、一覧178ファイルに
    //   予算・財政は0件 → 及ばない。
    // [年度, ファイル URL, landing URL]
    ["R8", "https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r8yosangaiyou.pdf",
           "https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R7", "https://warp.ndl.go.jp/20250809/20250808190652/https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r7yosangaiyou.pdf",
           "https://warp.ndl.go.jp/20250809/20250808190652/https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R6", "https://warp.ndl.go.jp/20240809/20240808061225/https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r6gaiyou.pdf",
           "https://warp.ndl.go.jp/20240809/20240808061225/https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R5", "https://warp.ndl.go.jp/20230512/20230511024620/https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r5gaiyou.pdf",
           "https://warp.ndl.go.jp/20230512/20230511024620/https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R4", "https://warp.ndl.go.jp/20220810/20220809123428/https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r4toushoyosannogaiyou.pdf",
           "https://warp.ndl.go.jp/20220810/20220809123428/https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R3", "https://warp.ndl.go.jp/20210815/20210813112634/https://www.city.nishio.aichi.jp/_res/projects/default_project/_page_/001/003/218/r3yosangaiyou.pdf",
           "https://warp.ndl.go.jp/20210815/20210813112634/https://www.city.nishio.aichi.jp/shisei/yosan/1001510/1003218.html"],
    ["R2", "https://warp.ndl.go.jp/20200517/20200515102643/http://www.city.nishio.aichi.jp/_resources/content/2549/20200319-103050.pdf",
           "https://warp.ndl.go.jp/20200517/20200515102643/http://www.city.nishio.aichi.jp/index.cfm/10,2549,107,500,html"],
    ["H31", "https://warp.ndl.go.jp/20190513/20190511095020/http://www.city.nishio.aichi.jp/index.cfm/10,2549,c,html/2549/20190319-140458.pdf",
            "https://warp.ndl.go.jp/20190513/20190511095020/http://www.city.nishio.aichi.jp/index.cfm/10,2549,107,500,html"],
    ["H30", "https://warp.ndl.go.jp/20180524/20180522030632/http://www.city.nishio.aichi.jp/index.cfm/10,2549,c,html/2549/20180328-093708.pdf",
            "https://warp.ndl.go.jp/20180524/20180522030632/http://www.city.nishio.aichi.jp/index.cfm/10,2549,107,500,html"],
    ["H29", "https://warp.ndl.go.jp/20170516/20170515002437/http://www.city.nishio.aichi.jp/index.cfm/10,2549,c,html/2549/20170331-092555.pdf",
            "https://warp.ndl.go.jp/20170516/20170515002437/http://www.city.nishio.aichi.jp/index.cfm/10,2549,107,500,html"],
  ] as const).map(([fy, url, landingPage]) => ({
    id: `nishio-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 西尾市 当初予算の概要（一般会計予算款別調・歳入歳出の款別＋前年度当初比較）`,
    publisher: "西尾市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "西尾市（一般会計・団体コード232131）",
    license: "© NISHIO City, All Rights Reserved.",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 5,
  ...
  ...([
    // 出雲市（島根県・団体コード 322032）。「一般会計・特別会計予算の概要」（Pdf_as・テキスト層あり）の「(２)一般会計歳入予算額」
    //   「(３)一般会計歳出予算額(目的別)」。千円。列 `[当年度予算額, 構成比, 左の一般財源, 構成比, 前年度予算額, 増減額, 増減率]`
    //   → **`amountIntIndex:0 / prevIntIndex:2`（合計行も同添字）**。合計ラベルは両側 `合計`。
    // ⚠⚠ **収録は「既存パーサで Σ4系統差0 の年度」だけ**（R8・H30〜H26・H24・H23・H18 の9年・自分で実測）。残りはパーサ手当て待ち（§13-23）:
    //   R6〜H31 は款行の**数字の字間スペース**（`2 1 ,4 2 0 ,0 0 0` / `396, 681`）で崩壊（R6 は Σ=71 vs 7）。**R7・H25 は皆減の内数行**
    //   （`うち臨時財政対策債 0 … 160,000 皆減`／`（公債費のうち繰上償還分） 0 … 1,000,000 皆減`）が廃止款として拾われ**前年度 Σ だけ
    //   割れる＝warning 止まりで derive まで流れる型**。H22〜H20 は「左の一般財源」が空セルの款で整数2個になり throw。
    // ⚠⚠ 同 PDF の「(４)一般会計歳出予算額(性質別)」の合計が款別と同値。H30 で性質別ページ（p.24）を歳出に当てると**当年度 Σ 差0 で
    //   人件費…13款を完走し前年度 Σ だけ割れる（warning）**（偵察が実測）。ページ番号は年度ごとに実ファイルで固定する。
    // ⚠⚠ **年度ページの contents id は毎年上書き**（1741751998868 は 2025-07 時点で R7、現在は R8。`R7tousyo_gaiyou.pdf` は発行元 404）
    //   → **R8 も来春消える見込み。収録直後に archive**（三鷹 R8 型）。過年度の contents id は年度ごとに別で現存。
    // ⚠ H24 以前は歳入表の右端に「備考」列（`自主財源`／`臨財債を除く増減`）があり、款22 市債の行末の日本語で行が捨てられて
    //   Σ −9,047,100 → `revenueCropX {to:468}` で切る（備考は x≥470pt・偵察が実測・自分で差0 を確認）。
    // ⚠⚠ **H26 の前年度列は「平成25年度（6月補正後）」**（H25 当初が市長選の骨格予算・原典に「※平成25年度当初は市長選挙を控えた
    //   骨格予算編成であったため6月補正後と比較している」）→ `prevBasis:"補正後"` を明示＋prevNote。自動判定語「補正後予算額」に当たらない。
    //   同型の H22（H21 骨格）は空セルで未収録。R7・R3（市長選年度）は「骨格|肉付」0件。
    // ⚠ 款名は原典どおりで寄せない: H26 以前 `寄付金`（H27〜 `寄附金`。H26 p.16 も `寄 付 金`・レビューが実測）、H30 以前 `交通安全特別交付金`（R8 は `交通安全対策特別交付金`。未収録の R7・H31 も偵察時の PDF で同じ表記を実測＝切替は H31〜）。
    // ライセンス: 「著作権・免責」（/www/contents/1294125067220/・登録 2023-02-28・確認日 2026-09-09）。「無断」で permission-required。
    //   島根県オープンデータカタログ（CC BY 4.0）の出雲市組織は11件で予算・決算・財政0件＝及ばない。リンク方針の記述は無し → `noDeepLink` は立てない。
    // [年度, ファイル名, contents id, 歳入p, 歳出p, CropX, 前年度=6月補正後]
    ["R8", "R8tousyo_gaiyou.pdf", "1741751998868", 31, 33, false, false],
    ["H30", "h30_yosangaiyou.pdf", "1518498554541", 20, 22, false, false],
    ["H29", "H29yosangaiyou.pdf", "1487045213637", 18, 20, false, false],
    ["H28", "H28yosan_H2703hosei.pdf", "1455524807404", 16, 18, false, false],
    ["H27", "yosangaiyou27.pdf", "1424323444539", 16, 18, false, false],
    ["H26", "h26tousyo.pdf", "1392361585471", 16, 18, false, true],
    ["H24", "yosanngaiyou24.pdf", "1329116269635", 16, 19, true, false],
    ["H23", "23gaiyou.pdf", "1297650693835", 17, 20, true, false],
    ["H18", "H18gaiyou.pdf", "1141374852515", 16, 18, false, false],
  ] as const).map(([fy, file, cid, rp, ep, crop, hoseigo]) => ({
    id: `izumo-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 出雲市一般会計・特別会計予算の概要（一般会計歳入予算額・一般会計歳出予算額(目的別)・款別＋前年度比較）`,
    publisher: "出雲市",
    url: `https://www.city.izumo.shimane.jp/www/contents/${cid}/files/${file}`,
    landingPage: `https://www.city.izumo.shimane.jp/www/contents/${cid}/index.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "出雲市（一般会計・団体コード322032）",
    license:
      "出雲市ホームページに掲載されている文章、写真、イラスト、画像等の著作権は、出雲市またはコンテンツ提供者の方にあります。これらの情報は、「私的使用のための複製」や「引用」などの著作権法上認められた場合を除き、無断で転用・引用することはできません。利用許諾については各課等へお問い合わせください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "一般会計歳入予算額",
      expenditureHeading: "一般会計歳出予算額",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      amountIntIndex: 0,
      prevIntIndex: 2,
      totalAmountIntIndex: 0,
      totalPrevIntIndex: 2,
      ...(crop ? { revenueCropX: { from: 0, to: 468 } } : {}),
      ...(hoseigo
        ? {
            prevBasis: "補正後" as const,
            prevNote:
              "前年度（平成25年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「平成25年度（6月補正後）」で、注記に「平成25年度当初は市長選挙を控えた骨格予算編成であったため6月補正後と比較している」とある）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 佐倉市（千葉県・団体コード 122122）。「当初予算（案）説明資料（主な事業の一覧）」の「一般会計歳入款別集計表」「歳出款別集計表」
    //   ＝歳入21〜23款・歳出12款（H20・H19 は13款）・千円。**R8〜H19 の19年度が現行サイトに同型**（H22 だけ別様式「款別増減内訳」）。
    //   ページは年度群で違う（R8〜R5・R2〜H29 = p.3/4、R4・R3・H28〜H23・H21 = p.4/5、H20・H19 = p.4/6）。全年度テキスト層あり。
    // ⚠ 合計ラベルは歳入 `合　計`（「歳入」なし・H29 と H24 以前は `歳入合計`）、歳出 `歳出合計`。`revenueTotalLabel:"歳入合計"` を
    //   R8 に指定すると throw（偵察が実測）。見出しは `歳入款別集計表`/`歳出款別集計表`（p.2「会計別総括表」に同じ総額があるが款別でないので外れる）。
    // ⚠⚠ **骨格予算の翌年度は前年度列が「６月補正後」**（R6 = R5 骨格・R2 = H31 骨格・H28 = H27 骨格・H20 = H19 骨格。H24 = H23 骨格は未収録）。
    //   列見出しは `令和５年度６月補正後`・`令和元年度６月補正後`・`平成27年度6月補正後`・`19年度(6月補正後）予算額` で、**自動判定語
    //   「補正後予算額」に当たらず静かに「当初」になる**（偵察が実測）→ `prevBasis:"補正後"` を明示＋prevNote。年度間クロスチェックで
    //   この4年だけ前年資料の当年度と一致しない（R6 前年度 54,171,148 ≠ R5 51,830,000 など・自分で実測）。
    // ⚠ **H24 は歳入合計行が回転ノンブル `-33/1-` で2行に割れて throw**（パーサ手当て待ち・未収録）。H22 は別様式（2p・注記列混在）で未収録。
    // ⚠ H20 歳出に `５１ 諸支出金 0 / 236,680`（予備費の後・原典の印字どおり。H19 では款12 諸支出金・款13 予備費）＝廃止款の番号が 51。
    //   `1…12,51` は昇順なので validate の「款番号が昇順でない」warning は出ない（レビューが実測）。
    // ⚠ H31 以前は増減額列に負号が文字層に無い（先頭2整数しか使わないので Σ には無関係）。H24 以前は全角数字（半角化して通る）。
    // ⚠ 予算書本体（`section/toshoyosan/03_yosansyo/R08.pdf`・620p）は総括の本文ページが画像＝代替にならない。
    // ⚠ URL は年度ごとに不規則（`R6_0226_02.pdf`・`setumei_06072307.pdf`・`29syuyo.pdf` …）＝機械生成しない。R8 PDF は Wayback 捕捉0件→即 archive。
    // ライセンス: サイトポリシー「著作権」（/about_site.html・更新 2022-06-01・確認日 2026-09-09）。「無断」で permission-required。
    //   「リンクについて」（/about_link.html）は「原則としてリンクフリー」＋「リンクである旨を明記」のみ＝ `noDeepLink` は立てない。
    //   オープンデータ規約（政府標準利用規約 2.0 準拠）は「佐倉市オープンデータとして公開しているコンテンツ」に限定、行財政カテゴリの
    //   19 CSV に説明資料は無い（実検索）→ 及ばない（§9g）。
    // [年度, 年度ページID, ファイル, 歳入p, 歳出p, 歳入合計ラベル, 前年度=6月補正後]
    ["R8", "20914", "R08tousyoyosansetumei.pdf", 3, 4, "合計", false],
    ["R7", "19645", "R07tousyoyosansetumei02.pdf", 3, 4, "合計", false],
    ["R6", "18109", "R6_0226_02.pdf", 3, 4, "合計", true],
    ["R5", "16440", "R05_0220_02.pdf", 3, 4, "合計", false],
    ["R4", "14340", "R4_jigyouichirann.pdf", 4, 5, "合計", false],
    ["R3", "4398", "setumei_06072307.pdf", 4, 5, "合計", false],
    ["R2", "4187", "setumei.pdf", 3, 4, "合計", true],
    ["H31", "5566", "02_setumei_93409991.pdf", 3, 4, "合計", false],
    ["H30", "3954", "02_setumei.pdf", 3, 4, "合計", false],
    ["H29", "3870", "29syuyo.pdf", 3, 4, "歳入合計", false],
    ["H28", "3781", "H28syuyou.pdf", 4, 5, "合計", true],
    ["H27", "5831", "H27syuyou.pdf", 4, 5, "合計", false],
    ["H26", "4878", "H26setumei.pdf", 4, 5, "合計", false],
    ["H25", "3566", "H25setumei.pdf", 4, 5, "合計", false],
    ["H23", "3226", "h23main.pdf", 4, 5, "歳入合計", false],
    ["H21", "3232", "21_yosan_setsumei.pdf", 4, 5, "歳入合計", false],
    ["H20", "3235", "20_yosanan_setumei.pdf", 4, 6, "歳入合計", true],
    ["H19", "2619", "19_yosanangaiyo.pdf", 4, 6, "歳入合計", false],
  ] as const).map(([fy, pid, file, rp, ep, rl, hoseigo]) => ({
    id: `sakura-yosan-setsumei-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 佐倉市当初予算（案）説明資料（一般会計歳入・歳出款別集計表・款別＋前年度比較）`,
    publisher: "佐倉市",
    url: `https://www.city.sakura.lg.jp/material/files/group/9/${file}`,
    landingPage: `https://www.city.sakura.lg.jp/soshiki/zaiseika/76/${pid}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "佐倉市（一般会計・団体コード122122）",
    license:
      "本サイトが提供する文書、写真、イラスト等の情報は、告示・訓令・通達その他これらに類するもの（及びそれらの翻訳物及び編集物含む）を除き、佐倉市ならびに第三者が保有する著作権により保護されています。私的な目的で使用する場合や、著作権法上認められている場合を除き、無断で転載、引用、複製することは禁じられています。ただし、個々の情報に別途定めがある場合はこの取り扱いに優先するものとします。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "歳入款別集計表",
      expenditureHeading: "歳出款別集計表",
      revenueTotalLabel: rl,
      expenditureTotalLabel: "歳出合計",
      ...(hoseigo
        ? {
            prevBasis: "補正後" as const,
            prevNote:
              fy === "R6"
                ? "前年度（令和5年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「令和５年度６月補正後」、注記に「「前年度予算額」は、令和5年度6月補正後の予算額です」）。"
                : fy === "R2"
                  ? "前年度（令和元年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「令和元年度６月補正後」、注記に「前年度予算額は、令和元年度6月補正後（肉付け後）の額」）。"
                  : fy === "H28"
                    ? "前年度（平成27年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「平成27年度6月補正後」、注記に「前年度予算額は、平成２７年度６月補正後（肉付け後）の額」）。"
                    : "前年度（平成19年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「19年度(6月補正後）予算額」、注記に「19年度当初予算が市長及び市議会議員選挙を直前にした骨格的編成であっため、政策的経費を肉付けした6月補正後時点の額と比較しています」）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 苫小牧市（北海道・団体コード 012131）。「一般会計予算書」（単一 PDF・208〜257p）の「予算に関する説明書 １ 総括」
    //   ＝歳入22〜24款・歳出14款・千円・列 `[本年度, 構成比, 前年度, 構成比, 比較, 伸率]`（歳出は右に財源内訳4列。先頭2整数で読める）。
    //   **物理ページは12年度とも p.13/p.14**（自分で実測）。印字ノンブルは年度で違う（16/17・13/14・「- 15 -」）＝印字で探さない。
    // ⚠ `revenueHeaderExtra:"総括"` 必須: 見出し「（歳 入）」の前行「１．総括」が款1 に連結して `総括市税` になる（Σ 差0 のまま・偵察が実測）。
    // ⚠⚠ **R2 は印字ノンブルが2重に重なって款行の行頭に混入**（p.13 `- -1515- -  11 地方特例交付金`・p.14 `- 18 -  8 土木費`）→ 款11・款8 が
    //   落ちて Σ 割れ（歳入 −173,000 / 歳出 −6,398,688＝Σ が捕まえる）。`CropX {from:40,to:900}` で左余白を切ると差0（自分で実測）。
    // ⚠ 独自の款名（原典どおり・寄せない）: 歳出 `環境衛生費`・`農水産業費`・`職員費`（款13・人件費を独立款に持つ）・`諸支出金`、歳入 `道支出金`。
    // ⚠ 廃止款: R8 `○ 環境性能割交付金 … 皆減`（前年度 79,000 = R7 当年度）、R2 `〇 自動車取得税交付金 … 皆減`（58,000 = H31）→ kanNo null。
    // ⚠⚠ **H26 の歳出「前年度」列は H25 予算書の当年度列と5款で一致しない**（総務費 3,526,118 vs 2,044,299・民生費・商工費・土木費・
    //   教育費 2,398,959 vs 4,370,039。総額 72,721,000 は同じ・歳入22款は全件一致・原典 p.12〜14 に注記なし・自分で実測）。
    //   derive の年度間クロスチェーンは合計だけを見るので止まらない。**推定を書かず事実だけを prevNote に置く**。
    // ⚠ H28・H27 は全ページ画像（p.13-14 文字0・フォント0）→ unrecordable。H27・H26・H25 は現行 landing から消えているが
    //   PDF は発行元で現存（2026-09-09 に 200）。H26・H25 の landing は Wayback 2015-06-09 の捕捉。⚠ H26・H25 の PDF は Wayback 捕捉なし→即 archive。
    // ⚠ H25 の URL は `[1]` を含む（`yosan-ippan-h25%5B1%5D.pdf`）。
    // 特別会計は別 PDF＝同名総括の同居なし。第1表（p.6/p.9）は総額が同値だが見出し「（歳入）」が無く throw する（偵察が実測）。
    // 骨格予算: 予算書・主要事業概要・予算編成ページ（R8〜H27）で「骨格」0件（無いとは断定しない）。
    // ⚠ 取得時に素の UA で一時 403（WAF）。fetch.ts の UA では 200。
    // ライセンス: 「リンク・著作権について」（/copyright.html・確認日 2026-09-09）。「無断」で permission-required。オープンデータ（CC BY 4.0）は
    //   「本市オープンデータのみ適用」と自ら範囲を限り、一覧13件に予算0件＝及ばない（§9g）。
    // ⚠⚠ リンク条項「トップページのURL以外は随時変更する場合がありますので、**リンクはトップページに張ってください**」＋「事後でも結構ですので、
    //   リンクを張った旨をお知らせください」＝**指示形**（東大阪「原則としてトップページにお願いします」と同型）→ `noDeepLink` を立てる。
    // [年度, ファイル, landing（現行 | wayback）, R2 の CropX]
    ["R8", "20260310161626.pdf", "current", false],
    ["R7", "20250304115126.pdf", "current", false],
    ["R6", "yosan-ippan-r6.pdf", "current", false],
    ["R5", "yosan-ippan-r5.pdf", "current", false],
    ["R4", "yosan-ippan-r4.pdf", "current", false],
    ["R3", "20210310133124.pdf", "current", false],
    ["R2", "20200310085807.pdf", "current", true],
    ["H31", "yosan-ippan-h31.pdf", "current", false],
    ["H30", "yosan-ippan-h30.pdf", "current", false],
    ["H29", "yosan-ippan-h29.pdf", "current", false],
    ["H26", "yosan-ippan-h26.pdf", "wayback", false],
    ["H25", "yosan-ippan-h25%5B1%5D.pdf", "wayback", false],
  ] as const).map(([fy, file, page, crop]) => ({
    id: `tomakomai-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 苫小牧市一般会計予算書（予算に関する説明書 総括・歳入歳出の款別＋前年度当初比較）`,
    publisher: "苫小牧市",
    url: `https://www.city.tomakomai.hokkaido.jp/files/00004100/00004109/${file}`,
    landingPage:
      page === "wayback"
        ? "https://web.archive.org/web/20150609144740/http://www.city.tomakomai.hokkaido.jp/shisei/zaisei/zaiseikenzenka/yosannitsuite.html"
        : "https://www.city.tomakomai.hokkaido.jp/shisei/zaisei/zaiseikenzenka/yosannitsuite.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "苫小牧市（一般会計・団体コード012131）",
    noDeepLink: true,
    license:
      "掲載している記事や写真の無断使用はお断りします。 苫小牧市公式ホームページに掲載されている個々の情報（文字、写真、イラスト等）に関する諸権利は、著作権の対象であり、法律によって保護されています。これらの情報について、「私的使用のための複製」や「引用」など著作権法上で認められた場合を除き、苫小牧市の許可なく複製、転用等をすることは法律で禁止されています。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 13,
      expenditurePage: 14,
      ...
      expenditureTotalLabel: "合計",
      revenueHeaderExtra: "総括",
      ...(crop ? { revenueCropX: { from: 40, to: 900 }, expenditureCropX: { from: 40, to: 900 } } : {}),
      ...(fy === "H26"
        ? {
            prevNote:
              "前年度（平成25年度）の歳出の額は、平成25年度予算書の当年度列と総務費・民生費・商工費・土木費・教育費の5款で一致しない（総額 72,721,000 千円は同じ。歳入22款は一致）。原典に注記は無く、理由は不明。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 小山市（栃木県・団体コード 092088）。「小山市予算書〔付 予算に関する説明書〕」の「歳入歳出予算事項別明細書 １．総括」
    //   ＝歳入21〜22款・歳出14款・千円・列 `[本年度, 前年度, 比較]`（歳出は右に財源内訳列。先頭2整数で読める）。R8〜H30 の9年（H29 以前は予算書の掲載なし）。
    // ⚠ 物理ページは年度で動く（R8/R7 16/17・R6 17/18・R5〜R3・H31・H30 15/16・R2 16/17）。印字ノンブルとのズレも年度で違う（0/+3/+2）＝印字で探さない。
    // ⚠⚠ **R4 以前は数字が ToUnicode 欠落の MS-Mincho**（`\x14`=1 の制御文字）→ `decodeGarble`（既定帯 −0x1D で復号・既存マップに追加なし）。
    // ⚠⚠ **R2 は廃止款の印が款名の末尾に付く `自動車取得税交付金(廃款) - 110,000 △110,000`** → 行頭アンカーの検出に当たらず前年度 Σ −110,000
    //   （warning 止まり）。`TAIL_ABOLISHED_MARK_RE` を検出と掃除に足した（2026-09-09・既存全ソースの再 parse で差分0 を確認）。
    // ⚠ 歳出ページは各款の下に財源内訳の `国 273,078` `県 417,466` 行が挟まるが款名・金額を汚染しない（自分で実測）。
    // ⚠ 同 PDF に**特別会計の「１．総括」が8〜12本**（同じ見出し「歳入」「歳出」・R8: p.194/226/266/284/296/311/323/335）＝ページを誤ると別会計を読む。
    //   R2 は会計別分冊（一般会計 PDF に特別会計なし）。予算参考書 p.6 の款別一覧表は ☆印の8款が落ちて Σ −47,132,843（偵察が実測）＝使わない。
    // 前年度列は**当初**（8ペア・全款一致・自分で実測）。`寄付金` は原典の表記。骨格予算: 予算書9年＋概要13年で「骨格|肉付け」0件。
    // ライセンス: 「リンク・著作権・免責事項」（/guide/link-cyo-menseki.html・確認日 2026-09-09）。「無断」で permission-required。
    //   オープンデータ規約（CC BY 4.0）は「小山市オープンデータカタログ以下のディレクトリ」に自ら範囲を限り、行財政分類は1件で予算0件（§9g）。
    // ⚠⚠ リンク条項「リンク設定はトップページへお願いします。他のページへのリンクは…ご遠慮ください」＝立川と同文（§11h 第2群）→ `noDeepLink` を立てる。
    // [年度, 年度ページ, ファイル, 歳入p, 歳出p, 復号]
    ["R8", "page010570.html", "1773881593_doc_359_0.pdf", 16, 17, false],
    ["R7", "page008634.html", "1741850597_doc_359_0.pdf", 16, 17, false],
    ["R6", "page006222.html", "1708489368_doc_45_0.pdf", 17, 18, false],
    ["R5", "page001839.html", "1691132677_doc_13_0.pdf", 15, 16, false],
    ["R4", "page001848.html", "1691133667_doc_13_0.pdf", 15, 16, true],
    ["R3", "page002988.html", "1693393138_doc_13_0.pdf", 15, 16, true],
    ["R2", "page002993.html", "1693393639_doc_13_0.pdf", 16, 17, true],
    ["H31", "page003046.html", "1693458469_doc_14_0.pdf", 15, 16, true],
    ["H30", "page003043.html", "1693456893_doc_14_0.pdf", 15, 16, true],
  ] as const).map(([fy, page, file, rp, ep, garble]) => ({
    id: `oyama-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 小山市予算書〔付 予算に関する説明書〕（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "小山市",
    url: `https://www.city.oyama.tochigi.jp/data/doc/${file}`,
    landingPage: `https://www.city.oyama.tochigi.jp/shisei/gyouzaisei/zaisei/yosan/${page}`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "小山市（一般会計・団体コード092088）",
    noDeepLink: true,
    license:
      "「小山市ホームページ」（以下、当サイトといいます）に掲載されている情報（文章、写真、イラスト、画像等の各ファイル）およびその内容に関する諸権利は、原則として小山市に帰属し、国際条約・法律等によって保護されています（一部の画像等の著作権は、原著作者が所有しています）。「私的使用のための複製」や「引用」など著作権法上認められる場合を除き、当サイトに掲載されている文書・画像等を無断使用・複製・転載・販売・改変・印刷配布することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      // R2 の廃止款は当年度セルが `-`（整数2個になり列がずれる）→ dashAsZero で 0 にして [0, 110,000, △110,000] と読む
      ...(fy === "R2" ? { dashAsZero: true } : {}),
    },
  } satisfies SourceEntry)),

  ...([
    // 磐田市（静岡県・団体コード 222119）。資料の選び方が年度で3通り（自分で実測・14年度とも Σ4系統差0）:
    //   (a) **R8・R7 は「当初予算説明資料」の「２. 一般会計の概要（１）目的別予算の状況（款別）」**（p.16/17）。予算書は R8 が画像
    //       （391p 中 354p が画像・総括 p.15-16 は文字6）、R7 はテキスト層があるが1〜2字ずつ縦に砕けて読めない。
    //       ⚠⚠ 同ページ右側に円グラフの凡例が重なり、`CropX` 無しだと款名に混入して **Σ 差0 のまま `地方消費繰入金利子割交付金` になる**
    //       （表の右端 x≈522pt・凡例の左端 x≈530pt → 526/528 で切る）。見出し行「２. 一般会計の概要」が款1 に連結するので
    //       `revenueHeading` にその見出し自体を指定して読み飛ばす。⚠ 説明資料は2月の審議用（p.3「計数その他訂正を要する場合も
    //       あります」）だが、R6〜R2・H31 で説明資料の款別＝予算書総括と全款一致（偵察が実測）。
    //   (b) **R6〜R2・H31・H30 は一般会計予算書「歳入歳出予算事項別明細書 １総括」**（p.15/16・列 `[本年度, 前年度, 比較]`）。
    //       R3・R2・H31・H30 は現行 404 → WARP（Wayback は 1MiB 打ち切りで壊れている・§9b）。
    //   (c) **H29〜H26 は「事項別明細書総括」だけの3頁 PDF**（p.1/2・WARP）。
    // ⚠⚠ **R2 は当初予算が議会で修正可決**され、予算書（原案）の当年度列と R3 の前年度列（修正後）が国庫支出金・繰入金・諸収入・
    //   教育費・予備費の5款でずれる（総額 66,970,000 は同じ・差は WARP の「令和2年度一般会計当初予算修正内容」の増減と一致・偵察が実測）。
    //   小田原 R7 と同型 → R2 の title に「（原案）」、R3 に prevNote。derive のクロスチェーンは合計だけ見るので止まらない。
    // ⚠ 予算書 R6 以前は一般会計と特別会計が分冊＝同名総括なし。第1表（p.6-9）は前年度列が無く見出し違いで届かない。
    //   説明資料の性質別・節別（同総額）は見出し違い。廃止款 R2 `— 自動車取得税交付金 0 / 220,000`（kanNo null・H31 当年度と一致）。
    // 前年度列は**当初**（13リンク・全款一致・R3→R2 だけ上記の修正差）。骨格予算: 説明資料 R8〜H29・総括 H29〜H26 で「骨格|肉付」0件。
    // ⚠ Wayback の PDF は信用しない（R3・H31 が 1,048,576B ちょうどで打ち切り）。WARP 実体 URL は viewer shell の iframe src から取った。
    // ライセンス: 「著作権・リンク」（/about/1004497.html・更新 2025-03-31・確認日 2026-09-09）。「無断使用・転載」で permission-required。
    //   オープンデータ（CC BY 2.1 日本）は「公開しているデータは…」と自ら範囲を限り、一覧に予算・決算は0件＝及ばない（§9g）。
    //   リンクは「原則自由」＋「できる限りトップページへのリンクをお薦めします」＝推奨（第3群寄り）→ `noDeepLink` は立てない。
    // [年度, 種別, ファイル URL, landing URL, 歳入p, 歳出p]
    ["R8", "setsumei", "https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/015/750/R8setsumeisiryo.pdf", "https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1015750.html", 16, 17],
    ["R7", "setsumei", "https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/014/066/r7_setumei.pdf", "https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1014066.html", 16, 17],
    ["R6", "yosansho", "https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/013/099/24-00.pdf", "https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1013099.html", 15, 16],
    ["R5", "yosansho", "https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/012/039/23-10.pdf", "https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1012039.html", 15, 16],
    ["R4", "yosansho", "https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/010/507/22-10.pdf", "https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1010507.html", 15, 16],
    ["R3", "yosansho", "https://warp.ndl.go.jp/20211113/20211113011215/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/009/172/10.pdf", "https://warp.ndl.go.jp/20211113/20211113011215/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1009172.html", 15, 16],
    ["R2", "yosansho", "https://warp.ndl.go.jp/20201114/20201113080318/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/007/821/R200ippa.pdf", "https://warp.ndl.go.jp/20201114/20201113080318/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1007821.html", 15, 16],
    ["H31", "yosansho", "https://warp.ndl.go.jp/20191113/20191112101814/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/006/469/3101.pdf", "https://warp.ndl.go.jp/20191113/20191112101814/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1006469.html", 15, 16],
    ["H30", "yosansho", "https://warp.ndl.go.jp/20181113/20181112225202/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/002/614/02_001.pdf", "https://warp.ndl.go.jp/20181113/20181112225202/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1002614.html", 15, 16],
    ["H29", "soukatsu", "https://warp.ndl.go.jp/20181113/20181112225254/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/002/615/03_001.pdf", "https://warp.ndl.go.jp/20181113/20181112225254/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1002615.html", 1, 2],
    ["H28", "soukatsu", "https://warp.ndl.go.jp/20181113/20181112225411/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/002/616/03_001.pdf", "https://warp.ndl.go.jp/20181113/20181112225411/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1002616.html", 1, 2],
    ["H27", "soukatsu", "https://warp.ndl.go.jp/20181113/20181112225509/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/002/617/03_001.pdf", "https://warp.ndl.go.jp/20181113/20181112225509/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1002617.html", 1, 2],
    ["H26", "soukatsu", "https://warp.ndl.go.jp/20181113/20181112225554/https://www.city.iwata.shizuoka.jp/_res/projects/default_project/_page_/001/002/618/03_001.pdf", "https://warp.ndl.go.jp/20181113/20181112225554/https://www.city.iwata.shizuoka.jp/shiseijouhou/gyouzaisei/yosan/1002618.html", 1, 2],
  ] as const).map(([fy, kind, url, landingPage, rp, ep]) => ({
    id: `iwata-${kind === "setsumei" ? "yosan-setsumei" : kind === "soukatsu" ? "soukatsu" : "yosansho"}-${fy.toLowerCase()}`,
    title:
      kind === "setsumei"
        ? `${eraYear(fy)}年度 磐田市当初予算説明資料（一般会計・目的別予算の状況（款別）・款別＋前年度当初比較）`
        : kind === "soukatsu"
          ? `${eraYear(fy)}年度 磐田市一般会計予算 歳入歳出予算事項別明細書 総括（款別＋前年度当初比較）`
          : `${eraYear(fy)}年度 磐田市一般会計予算書（附 予算に関する説明書）${fy === "R2" ? "（原案）" : ""}（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "磐田市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "磐田市（一般会計・団体コード222119）",
    license:
      "磐田市ホームページに掲載しているコンテンツ（テキスト、画像、PDF、その他のデータ）の著作権は磐田市に帰属します。また、一部の画像などの著作権は原著作者が所有しているものもあります。磐田市ホームページ上の文書・画像などの無断使用・転載、二次利用を原則禁止します。ただし掲載されている情報は、個人的かつ非営利的な使用目的だけのために利用する場合に限り、複製、使用、ダウンロードすることができます。著作権法上の「私的使用」や「引用」の範囲を越えて、本ページのコンテンツの使用を希望する場合は事前に広報広聴・シティプロモーション課までお問い合わせください。事前の許可がない限り、転載、変更、発行、配布、掲示などは一切できません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...(kind === "setsumei"
        ? {
            revenueHeading: "一般会計の概要",
            expenditureHeading: "②歳出",
            revenueCropX: { from: 0, to: 526 },
            expenditureCropX: { from: 0, to: 528 },
          }
        : { revenueHeading: "(歳入)", expenditureHeading: "(歳出)" }),
      ...(fy === "R3"
        ? {
            prevNote:
              "前年度（令和2年度）の額は市議会で修正可決された後の当初予算額。ウェブに残る令和2年度予算書（WARP）は原案で、国庫支出金・繰入金・諸収入・教育費・予備費の5款が修正前の額（総額 66,970,000 千円は同じ）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 新座市（埼玉県・団体コード 112305）。「新座市一般会計予算書」冒頭の「歳入歳出予算事項別明細書 １ 総括」。千円。
    //   **歳入は見開き2ページ型**（款名＋本年度が namePage、前年度＋増減が amountPage）→ `revenueSpread`。歳出は1ページ。
    //   R8〜R5（現行）＋ H29〜H25（WARP・url/landing とも同じ捕捉 ts）の9年が同型（自分で実測・Σ4系統差0）。
    // ⚠ 見出しの括弧が **R8〜R5 は半角 `(歳 入)`、H29〜H25 は全角 `（歳 入）`**（H29 に半角を当てると throw・偵察が実測）。
    // ⚠ 特別会計は別 PDF＝同名総括なし。第1表（p.6-12）と総括の財源内訳ページ（p.19）は総額が同じだが見出し違いで throw。
    //   spread を1ページずらすと「見開きの行数が合いません（款名 23 行 / 金額 13 行）」で throw（偵察が実測）。
    // ⚠⚠ **R4・R3・H31（令和元年度）・H30 は予算書も概要もスキャン**（自分で実測: R4 予算書 先頭40p 文字0・画像176／概要 16字、
    //   R3 335字（ノンブル）／0字、H31 1,910字・金額0／0字、H30 338字／0字）→ unrecordable。**R2 は予算書だけスキャン**（1,922字・金額0）で
    //   「予算概要」の款別表がテキスト → 別エントリ（下）。⚠ 平成31年度の年度ページは `yosan-31.html`（`h31` ではない）。
    // ⚠ R3 以前の年度ページは現行 404。R6・R5 の表紙・目次はフォント化け（BIZ-UDGothic）だが総括は正常。印字ノンブルは物理 +3〜+4。
    // 前年度列は**当初**（R8→R5・H29→H25 の8リンク・全款一致・自分で実測。R5→R4・R2→H31 は年度ページ本文の総額で補完・偵察）。
    //   骨格予算: 概要・予算書 R8〜R5 で「骨格|肉付け」0件。
    // ⚠⚠ **ライセンスは判定語に当たらず `unverified` に落ちる**（判断待ち4市目・安城/岸和田/西尾と並べる）。原文は「新座市の許可・承諾を
    //   得ないままほかの媒体へ転載することはできません」「必ず転載の許可を得てください」＝**規約が有って許可を求めている**ので、
    //   `unverified`＝自サーバー配信が開く側は発行元の意図と逆。意訳せず原文のまま置き、§13-24 に論点として残す。
    //   サイトポリシー（/site/userguide/sitepolicy.html・更新 2026-02-26・確認日 2026-09-09）。埼玉県ポータル（PDL1.0）の新座市10件に予算0件。
    //   リンクは「原則として、自由に設定していただいて構いません」＋フレーム禁止 → `noDeepLink` は立てない（unverified では no-op でもある）。
    // [年度, ファイル URL, landing URL, 款名p, 金額p, 歳出p, 括弧]
    ["R8", "https://www.city.niiza.lg.jp/uploaded/attachment/70073.pdf", "https://www.city.niiza.lg.jp/soshiki/8/yosan-r8.html", 16, 17, 18, "half"],
    ["R7", "https://www.city.niiza.lg.jp/uploaded/attachment/64351.pdf", "https://www.city.niiza.lg.jp/soshiki/8/yosan-r7.html", 17, 18, 19, "half"],
    ["R6", "https://www.city.niiza.lg.jp/uploaded/attachment/58880.pdf", "https://www.city.niiza.lg.jp/soshiki/8/yosan-r6.html", 18, 19, 20, "half"],
    ["R5", "https://www.city.niiza.lg.jp/uploaded/attachment/54112.pdf", "https://www.city.niiza.lg.jp/soshiki/8/yosan-r5.html", 15, 16, 17, "half"],
    ["H29", "https://warp.ndl.go.jp/20180418/20180416030713/https://www.city.niiza.lg.jp/uploaded/attachment/23296.pdf", "https://warp.ndl.go.jp/20180418/20180416030713/https://www.city.niiza.lg.jp/soshiki/8/yosan-h29.html", 16, 17, 18, "full"],
    ["H28", "https://warp.ndl.go.jp/20180418/20180416030714/https://www.city.niiza.lg.jp/uploaded/attachment/19727.pdf", "https://warp.ndl.go.jp/20180418/20180416030714/https://www.city.niiza.lg.jp/soshiki/8/yosan-h28.html", 16, 17, 18, "full"],
    ["H27", "https://warp.ndl.go.jp/20180418/20180416030715/https://www.city.niiza.lg.jp/uploaded/attachment/15369.pdf", "https://warp.ndl.go.jp/20180418/20180416030715/https://www.city.niiza.lg.jp/soshiki/8/yosan-h27.html", 14, 15, 16, "full"],
    ["H26", "https://warp.ndl.go.jp/20180418/20180416030717/https://www.city.niiza.lg.jp/uploaded/attachment/12387.pdf", "https://warp.ndl.go.jp/20180418/20180416030717/https://www.city.niiza.lg.jp/soshiki/8/yosan-h26.html", 14, 15, 16, "full"],
    ["H25", "https://warp.ndl.go.jp/20170715/20170714011553/https://www.city.niiza.lg.jp/uploaded/attachment/9238.pdf", "https://warp.ndl.go.jp/20170715/20170714011553/https://www.city.niiza.lg.jp/soshiki/8/yosan-h25.html", 14, 15, 16, "full"],
  ] as const).map(([fy, url, landingPage, np, ap, ep, paren]) => ({
    id: `niiza-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 新座市一般会計予算書（歳入歳出予算事項別明細書 総括・歳入歳出の款別＋前年度当初比較）`,
    publisher: "新座市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "新座市（一般会計・団体コード112305）",
    license:
      "新座市ホームページに掲載している記事、画像、データ類の著作権は、新座市又はその情報提供者に帰属します。また、その全てについて、「私的利用のための複製」や「引用」など著作権法上認められた場合を除き、新座市の許可・承諾を得ないままほかの媒体へ転載することはできません。新座市ホームページに掲載している情報や画像等の転載を希望する場合は、必ず転載の許可を得てください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenueSpread: { namePage: np, amountPage: ap },
      expenditurePage: ep,
      revenueHeading: paren === "half" ? "(歳入)" : "（歳入）",
      expenditureHeading: paren === "half" ? "(歳出)" : "（歳出）",
    },
  } satisfies SourceEntry)),

  {
    // 新座市 R2 — 予算書（35520.pdf）はスキャン（自分で実測: 先頭40p 1,922字・金額0）なので「令和2年度予算概要」（テキストあり）の
    //   p.22「歳入予算内訳」/ p.23「歳出予算内訳（款別）」で代替。款番号なし → `kanNoless`（全款 kanNo null）。
    // ⚠ ページ冒頭「Ⅴ 一般会計予算概要 / １ 歳入・歳出予算内訳」が款1「市税」の頭に連結して **Σ 差0 のまま通る**（偵察が実測）→ HeaderExtra 必須。
    // ⚠ p.23 は款別表の下に「歳出予算内訳（性質別）」（合計同値）が縦積み。最初の「合計」で止まるので混ざらない（偵察が実測）。
    // ⚠ 歳入に廃止款「自動車取得税交付金 0 / 30,000」（23款目・kanNo null）。発行元 404 → WARP（Wayback にも捕捉あり）。
    id: "niiza-yosangaiyou-r2",
    title: "令和2年度 新座市予算概要（一般会計予算概要 歳入・歳出予算内訳・款別＋前年度当初比較）",
    publisher: "新座市",
    url: "https://warp.ndl.go.jp/20220711/20220709010451/https://www.city.niiza.lg.jp/uploaded/attachment/35518.pdf",
    landingPage: "https://warp.ndl.go.jp/20220711/20220708191140/https://www.city.niiza.lg.jp/soshiki/8/yosan-r2.html",
    kind: "pdf",
    fiscalYear: "R2",
    scope: "新座市（一般会計・団体コード112305）",
    license:
    ...
    parserOptions: {
      ...
      expenditurePage: 23,
      revenueHeading: "歳入予算内訳",
      expenditureHeading: "歳出予算内訳（款別）",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      kanNoless: true,
      revenueHeaderExtra: "一般会計予算概要|歳入・歳出予算内訳",
    },
  },

  ...([
    // 日立市（茨城県・団体コード 082023）。「○年度 日立市予算の概要」（市長公室）の「１ 総括 (3) 一般会計 歳入予算 / (4) 歳出予算（目的別）」。
    //   千円・列 `[本年度, 構成比, 前年度, 増減額, 伸率]`（小数を除いた整数列が `[当年度, 前年度, 増減]`）。**R8〜H24 の15年が現行サイトに全部ある**
    //   （自分で15年の款名を全件目視・Σ4系統差0）。R8〜H27 は1冊、**H26〜H24 は歳入・歳出が別 PDF**（各2p・p.1 が表）→ `revenueFile`/`expenditureFile`。
    // ⚠ 概要は R5〜R8 が2分冊で、**款別のある「予算の概要」（37〜38p）が 01 か 02 かは年で入れ替わる**（R8=01・R7=02・R6=01・R5=無番）。
    // ⚠ ページは H29 が p.6/8、H27 が p.5/8（歳入内訳が2ページ）、他は p.5/7。見出し「日立市一般会計歳入予算」は p.6（内訳）にも出るが
    //   合計行が無いので throw。p.8 の性質別は総額が同じだが見出し「（性質別）」で throw（偵察が実測）。特別会計の款別総括は同 PDF に無い。
    // ⚠ 廃止款は `▲` 接頭（R2 自動車取得税交付金・R4 災害復旧費）→ kanNo null。R5 の歳出は災害復旧費なし（12款）。
    // 前年度列は**当初**（14ペア・全款一致・偵察が実測。自分の目視でも総額の鎖が H24 63,043,000 → R8 77,370,000 まで連続）。
    //   骨格予算: 概要15年で「骨格|肉付|6月補正後」0件。スキャン年度なし。
    // ⚠⚠ ライセンス（判断待ち・open に読める根拠あり）: 市オープンデータカタログのデータセット「予算・決算（担当課：財政課）」
    //   （CC BY・リリース 2026-04-23）の資源「予算（HTML）」が**この資料の年度ページの親（予算インデックス）**を指す（神奈川 R8 と同型の根拠）。
    //   ただし PDF はインデックス → 年度ページ → PDF の**2階層下**で、神奈川（1階層）より一段弱い。「開ける側へ倒さない」に従い、
    //   サイトポリシー（/about/1010691.html・更新 令和6年2月13日・確認日 2026-09-09）の原文を置く（「無断」で permission-required）。
    //   カタログの登載を「PDF に及ぶ」と読むなら open に置き換える（§13-25 に論点として残す）。
    //   リンクは「特記事項がない限り原則として自由」＋フレーム禁止 → `noDeepLink` は立てない。
    // [年度, ディレクトリ, ファイル, 歳入p, 歳出p, 年度ページ]
    ["R8", "001/019/108", "r8yosannnogaiyou01.pdf", 5, 7, "1019108"],
    ["R7", "001/015/884", "r7yosannnogaiyou02.pdf", 5, 7, "1015884"],
    ["R6", "001/011/818", "r6yosannnogaiyou01.pdf", 5, 7, "1011818"],
    ["R5", "001/003/332", "r5yosannogaiyou.pdf", 5, 7, "1003332"],
    ["R4", "001/003/334", "r4yosannogaiyou.pdf", 5, 7, "1003334"],
    ["R3", "001/003/336", "r3yosannogaiyou.pdf", 5, 7, "1003336"],
    ["R2", "001/003/338", "r2yosannogaiyou.pdf", 5, 7, "1003338"],
    ["R1", "001/003/340", "toushoyosangaiyou.pdf", 5, 7, "1003340"],
    ["H30", "001/003/342", "30yosannnogaiyou.pdf", 5, 7, "1003342"],
    ["H29", "001/003/344", "29yosannnogaiyou.pdf", 6, 8, "1003344"],
    ["H28", "001/003/346", "pdf3.pdf", 5, 7, "1003346"],
    ["H27", "001/003/348", "pdf3.pdf", 5, 8, "1003348"],
  ] as const).map(([fy, dir, file, rp, ep, page]) => ({
    id: `hitachi-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 日立市予算の概要（一般会計 歳入予算・歳出予算（目的別）・款別＋前年度当初比較）`,
    publisher: "日立市",
    url: `https://www.city.hitachi.lg.jp/_res/projects/default_project/_page_/${dir}/${file}`,
    landingPage: `https://www.city.hitachi.lg.jp/shisei/seisaku_zaisei/1003330/1003331/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "日立市（一般会計・団体コード082023）",
    license:
      "日立市ホームページに掲載している文章、写真、イラストなどの情報に関する著作権は、原則として日立市に帰属します。ただし、一部の画像等の著作権は、原著作者が所有しています。「私的使用のための複製」や「引用」など、著作権法上認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "日立市一般会計歳入予算",
      expenditureHeading: "日立市一般会計歳出予算（目的別）",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 日立市 H26〜H24 — 歳入・歳出が別 PDF（各2p・p.1 が款別表）。同じ見出し・合計ラベル。
    // [年度, ディレクトリ, 歳入ファイル, 歳出ファイル, 年度ページ]
    ["H26", "001/003/350", "h26sainyu.pdf", "h26saisyutu.pdf", "1003350"],
    ["H25", "001/003/351", "04288_20130509_0005.pdf", "04288_20130509_0006.pdf", "1003351"],
    ["H24", "001/003/352", "04645_20120503_0015.pdf", "04645_20120503_0016.pdf", "1003352"],
  ] as const).map(([fy, dir, rf, ef, page]) => ({
    id: `hitachi-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 日立市の予算（一般会計 歳入予算・歳出予算（目的別）・款別＋前年度当初比較）`,
    publisher: "日立市",
    url: null,
    urls: [`https://www.city.hitachi.lg.jp/_res/projects/default_project/_page_/${dir}/${rf}`, `https://www.city.hitachi.lg.jp/_res/projects/default_project/_page_/${dir}/${ef}`],
    landingPage: `https://www.city.hitachi.lg.jp/shisei/seisaku_zaisei/1003330/1003331/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "日立市（一般会計・団体コード082023）",
  ...
  ...([
    // 弘前市（青森県・団体コード 022021）。「弘前市各会計予算書・予算説明書」（単一 PDF・一般＋特別3本＋企業2本の合冊）の
    //   「歳入歳出予算事項別明細書 １ 総括」＝歳入23款・歳出13款・千円・列 `[本年度, 構成比, 前年度, 構成比, 比較, 伸率]`（歳出は右に財源内訳4列）。
    //   **R8〜R4 の5年**（予算書の掲載は R4 から。R3 以前は年度ページに予算書なし・Wayback は偵察時に停止・WARP 未確認＝「無い」とは断定しない）。
    // ⚠ 物理ページは年度で動く（R8 21/22・R7 19/20・R6 22/23・R5 21/22・R4 22/23。印字は 15/16 か 16/17）＝印字で探さない。
    // ⚠⚠ 同 PDF に**特別会計の同名「１ 総括」が3本**（R8: 国保 p.216/217・後期高齢 p.249/250・介護 p.272/273）＝ページを動かすと別会計を Σ 差0 で読む。
    //   第1表（p.8〜14）は前年度列なし。企業会計は「予算実施計画」で様式が違う。
    // ⚠ 骨格予算: **R8**（概要 p.6「本年４月に実施される市長選を踏まえ「骨格予算」として編成」）・R4・H30・H26（偵察が原典で確認）。
    //   **予算書の前年度列は常に前年当初**（R5 前年度 78,820,000 = R4 当初。肉付け後 80,062,157 ではない。4リンク36款一致・自分で実測）。
    //   ⚠⚠ **前年度側が骨格なら prevNote に書く**（§11k・秋田 R8 型）→ R5 に prevNote（R4 の5月補正予算の概要に「令和４年度当初予算は…骨格予算」
    //   「骨格予算に政策的経費を肉付け」と明記・レビューが原文確認）。R8 自身が骨格である点は当年度側なので書かない。
    //   ⚠ **R9 収録時に前年度列が R8 当初か肉付け後かをクロスチェックで確かめ、当初なら R9 に prevNote**。
    // ⚠ 「予算の概要」（R8〜H23）の款別表は自主財源/依存財源の群行と繰入金・公債費の内訳行が混ざり、kanNoless では Σ が合わない
    //   （歳入 +76,480,000）＝行名の除外オプションが要るパーサ手当て待ち（上越・出雲と同族）。H29・H28 の概要はテキスト層が化ける（Meiryo・ToUnicode 無し）。
    // ⚠ 年度ページの URL は規則が破れる（R6 `2024-0208-0959-20.html`・R5 `2023-0209-1548-20.html`・R7 だけ `yosansho`）。
    // ライセンス: 「著作権・リンク・免責事項」（/copyright/index.html・確認日 2026-09-09）。「無断」「複製・転用」で permission-required。
    //   「オープンデータひろさき」（CC BY 4.0）は「本サイトで公開しているデータ」に自ら範囲を限り、予算・決算・財政は0件＝及ばない。
    //   リンクは「フリー」＋「トップページ…とすることをおすすめします」＝推奨（磐田型）→ `noDeepLink` は立てない。
    // [年度, ファイル, 年度ページ, 歳入p, 歳出p]
    ["R8", "R8yosansyo.pdf", "yosan_r8.html", 21, 22],
    ["R7", "R7yosansho2.pdf", "yosan_r7_yosansho.html", 19, 20],
    ["R6", "R6yosannsho.pdf", "yosan_r6_yosansyo.html", 22, 23],
    ["R5", "R5yosansyo.pdf", "yosan_r5_yosansyo.html", 21, 22],
    ["R4", "R4yosansyo.pdf", "yosan_r4_yosansyo.html", 22, 23],
  ] as const).map(([fy, file, page, rp, ep]) => ({
    id: `hirosaki-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 弘前市各会計予算書・予算説明書（歳入歳出予算事項別明細書 総括・歳入歳出の款別＋前年度当初比較）`,
    publisher: "弘前市",
    url: `https://www.city.hirosaki.aomori.jp/jouhou/keikaku/yosan/${file}`,
    landingPage: `https://www.city.hirosaki.aomori.jp/jouhou/keikaku/yosan/${page}`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "弘前市（一般会計・団体コード022021）",
    license:
      "弘前市ホームページに掲載している文章、写真、画像、動画、その他全ての情報は著作権の対象となっています。また、「弘前市ホームページ」全体も編集著作物として著作権の対象となっており、共に著作権法により保護されています。これらの情報については、弘前市または第三者が著作権を有しており、「私的利用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      ...(fy === "R5"
        ? {
            prevNote:
              "前年度（令和4年度）の当初予算は骨格予算（「令和４年５月補正予算案の概要（一般会計補正予算第１号）」に「令和４年度当初予算は、義務的経費、経常的経費及び総合計画に位置付けられている事業を中心とした骨格予算」「骨格予算に政策的経費を肉付け」と明記）。ここでの前年度額はその骨格予算の当初額（78,820,000千円）で、肉付け後の額（80,062,157千円）ではない。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 帯広市（北海道・団体コード 012076）。資料が年度で4通り（自分で9年の款名を全件目視・Σ4系統差0）:
    //   (a) **R8・R7・R6・R3・H30 は一般会計 事項別明細書の冒頭「歳入歳出予算事項別明細書」総括**（p.3/4・H30 だけ p.2/3）。千円・列 `[本年度, 前年度, 比較]`。
    //       ⚠ `revenueHeading` に「（歳入）」を指定するとタイトル行が款5 に連結して `歳入歳出予算事項別明細書市税`（Σ 差0 のまま）→
    //       見出しをタイトル行自体「歳入歳出予算事項別明細書」にして読み飛ばす。R7・R6・R3 は ToUnicode 欠落 → `decodeGarble`（既存マップ）。
    //   (b) **R5・R4 は「予算説明資料」の総括表**（p.4/5・事項別明細書の総括ページが画像＝p.3-4 文字44・画像1,941）。
    //   (c) **R2・H31 は「予算編成概要」の総括表**（p.6/7）。R2 の事項別明細書は皆増/皆減にラベルが無く落ちる（当年度空欄を 0 と読む指定が無い＝パーサ手当て待ち）。
    //       H31 の事項別明細書は画像。
    // ⚠⚠ **骨格予算は H30・R4・R8**（「予算編成について」に「改選期にあたることから…骨格予算を編成」）。**翌年度の R5・H31 は説明資料/概要の
    //   前年度列が6月補正後**（列見出し「令和４年度６月補正後」「平成30年度6月補正後」。R5 前年度 86,307,544 ≠ R4 当初 84,207,000・H31 82,744,507 ≠
    //   H30 81,896,000・自分で実測）→ `prevBasis:"補正後"` 明示＋prevNote（**自動判定は「補正後予算額」が連結しないので静かに「当初」になる**）。
    //   R9 も同型になる見込み。R8 自身は骨格予算（自身の年なので prevNote なし）。
    // ⚠ 独自の款名 `道支出金`（原典）。廃止/新設は R2 の `法人事業税交付金 152,923/0`・`自動車取得税交付金 0/75,042`・H31 `環境性能割交付金 44,766/0`。
    //   歳出は H31 以前 15款（災害復旧費あり）。象徴計上の1千円行（寄附金・繰越金）は原典。
    // ⚠ 予算説明資料 p.14-15「関係職員給与費配分表」は款・項別で Σ款=総額（前年度列なし）＝ページを誤ると… 見出し違いで throw する見込み。
    //   特別会計は別冊＝同名総括なし。年度ページは CMS の ID・ファイル名も年度で揺れる＝機械生成しない。R8/R7 PDF は Wayback 未捕捉→即 archive。
    // ライセンス: 「著作権・免責事項・リンク」（/shisei/kouhou/homepage/1004146.html・更新 2020-12-14・確認日 2026-09-09）。
    //   「転載することを禁止」で permission-required。オープンデータ（CC BY 4.0）は「本ページのデータ」に限定・公開3件に予算0件＝及ばない。
    //   リンクは「トップページ…原則自由」＋「詳細ページのアドレスは…変更される場合があります」（断りではない）→ `noDeepLink` は立てない。
    // [年度, 種別, 年度ページID, ファイルパス, 歳入p, 歳出p, 復号, 前年度=6月補正後]
    ["R8", "jikoubetsu", "1023191", "023/191/5_r08_ippannkaikeizikoubetu.pdf", 3, 4, false, false],
    ["R7", "jikoubetsu", "1019420", "019/420/5_r07_ippannkaikeizikoubetu.pdf", 3, 4, true, false],
    ["R6", "jikoubetsu", "1016762", "016/762/r6_ippankaikei.pdf", 3, 4, true, false],
    ["R5", "setsumei", "1013664", "013/664/r5_yosansetsumeisiryou.pdf", 4, 5, false, true],
    ["R4", "setsumei", "1011136", "011/136/r4_yosansetsumeisiryou.pdf", 4, 5, false, false],
    ["R3", "jikoubetsu", "1008617", "008/617/r3ippannkaikei_zikoubetu.pdf", 3, 4, true, false],
    ["R2", "gaiyou", "1003553", "003/553/03_r2henseigaiyou.pdf", 6, 7, false, false],
    ["H31", "gaiyou", "1003555", "003/555/h31_gaiyou.pdf", 6, 7, false, true],
    ["H30", "jikoubetsu", "1003557", "003/557/h30yosansho_02ippan_jikoubetsu.pdf", 2, 3, false, false],
  ] as const).map(([fy, kind, pid, path, rp, ep, garble, hoseigo]) => ({
    id: `obihiro-${kind === "jikoubetsu" ? "yosansho" : kind === "setsumei" ? "yosan-setsumei" : "yosan-gaiyou"}-${fy.toLowerCase()}`,
    title:
      kind === "jikoubetsu"
        ? `${eraYear(fy)}年度 帯広市一般会計 事項別明細書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`
        : kind === "setsumei"
          ? `${eraYear(fy)}年度 帯広市予算説明資料（一般会計歳入歳出総括表・款別＋前年度比較）`
          : `${eraYear(fy)}年度 帯広市予算編成概要（一般会計歳入歳出総括表・款別＋前年度比較）`,
    publisher: "帯広市",
    url: `https://www.city.obihiro.hokkaido.jp/_res/projects/default_project/_page_/001/${path}`,
    landingPage: `https://www.city.obihiro.hokkaido.jp/shisei/yosan/yosan/${pid}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "帯広市（一般会計・団体コード012076）",
    license:
      "本ホームページに掲載している著作物（文章・画像・映像・音声などで、プログラムデータを含む）にかかる著作権は、帯広市もしくは、帯広市に情報提供していただいた提供元に帰属します。本ホームページで掲載している情報・文章・画像などのデータは、権利者の許可なく使用・転載することを禁止します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...(kind === "jikoubetsu"
        ? { revenueHeading: "歳入歳出予算事項別明細書", expenditureHeading: "（歳出）" }
        : { revenueHeading: "(歳入)", expenditureHeading: "(歳出)", revenueTotalLabel: "合計", expenditureTotalLabel: "合計" }),
      ...(garble ? { decodeGarble: true } : {}),
      ...(hoseigo
        ? {
            prevBasis: "補正後" as const,
            prevNote:
              fy === "R5"
                ? "前年度（令和4年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「令和４年度６月補正後」。令和4年度当初予算は改選期の骨格予算）。"
                : "前年度（平成30年度）の額は当初予算ではなく6月補正後の額（原典の列見出しが「平成30年度6月補正後」。平成30年度当初予算は改選期の骨格予算）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 都城市（宮崎県・団体コード 452025）。「当初予算説明資料（No.2）」の「一般会計歳入予算総括表 ≪款別≫」（p.4）／「歳出予算総括表 ≪款別≫」（p.8）。
    //   千円・**款番号なし**（`kanNoless`）・**列順 `[前年度当初, 構成比, 当年度当初, 構成比, 増減額, 増減率]`＝前年先行**（`prevColumnFirst`・全6年の列見出しで確認）。
    //   ⚠⚠ **テキスト層があるのは R6・R4・R3・H30・H26・H25 の6年だけ**（R8・R7・R5・R2・H31・H29・H28・H27 はスキャン・文字0・自分で実測）＝飛び飛び。
    //   予算書（議会ページ・R8〜R5）は歳入総括にテキストがあるが**歳出総括がパス描画でテキストなし**（R8 p.58-59 文字8＝ノンブルのみ）→ 最新2年は収録不可。
    // ⚠⚠ **`CropX` 必須（年度ごとに実測値）**: 右側に円グラフのラベルが並び、切らないと **Σ 差0 のまま `商工費総務費`・`衛生費民生費` に連結**（偵察が実測）。
    //   境界は増減率列の右端とラベル左端の間。⚠ H26・H25 の歳出は余裕 5pt（版面が動くとノンブルが金額列に混入しうる）。
    // ⚠⚠ **歳入に小計行「自主財源 ※1」「依存財源 ※2」**が款と同じ形で挟まる → `revenueHeaderExtra` で落とす（無いと当年度 Σ が約2倍で落ちる）。
    //   表題「…総括表」が款1 に連結する年度があるので `総括表` も入れる。見出しは crop 内に必ず残る列ラベル（`歳入科目`/`款名`）。
    // ⚠ 同 PDF の p.12「歳出予算総括表 ≪性質別≫」は合計が歳出と同値（見出し違い）。p.5-7/9-11「主な増減内訳」は再掲。
    // 前年度列は**当初**（R6→R5 予算書・R4→R3・H26→H25 など総額・款で一致。骨格予算なし＝市長選は11〜12月）。折返し「国有提供施設等所在市町村助成交付金」は結合。
    // ⚠ URL は R4・R3 だけ `/uploaded/life/…_misc.pdf` 形式。R6 説明資料は Wayback 捕捉0件→即 archive。
    // ライセンス: 「リンク・著作権・免責事項」（/site/guide/8868.html・更新 2020-01-02・確認日 2026-09-09）。「無断」で permission-required。
    //   BODIK（CC BY 4.0）は「当サイトの内容」に限定・376データセットに説明資料・予算書は無い（統計書 XLSX のみ）＝及ばない。
    //   リンクは「原則自由」＋「事前に…連絡ください」＋フレーム禁止＝事前連絡の依頼（第5群）→ `noDeepLink` は立てない。
    // [年度, ファイルパス, 年度ページID, 歳入 CropX to, 歳出 CropX to]
    ["R6", "attachment/31143.pdf", "61724", 445, 398],
    ["R4", "life/44032_67939_misc.pdf", "44032", 445, 403],
    ["R3", "life/44014_67906_misc.pdf", "32911", 445, 403],
    ["H30", "attachment/6558.pdf", "4568", 462, 426],
    ["H26", "attachment/6306.pdf", "4467", 462, 415],
    ["H25", "attachment/6232.pdf", "4445", 490, 441],
  ] as const).map(([fy, path, pid, rc, ec]) => ({
    id: `miyakonojo-yosan-setsumei-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 都城市当初予算説明資料（一般会計歳入・歳出予算総括表≪款別≫・款別＋前年度当初比較）`,
    publisher: "都城市",
    url: `https://www.city.miyakonojo.miyazaki.jp/uploaded/${path}`,
    landingPage: `https://www.city.miyakonojo.miyazaki.jp/soshiki/19/${pid}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "都城市（一般会計・団体コード452025）",
    license:
      "都城市ホームページ上に掲載している個々の情報(文章、写真、イラストなど)は、著作権保護の対象となっています。また、都城市ホームページ全体も編集著作物として著作権保護の対象となっています。「私的使用のための複製」や「引用」など、著作権法上認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 8,
      revenueHeading: "歳入科目",
      expenditureHeading: "款名",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      kanNoless: true,
      prevColumnFirst: true,
      revenueHeaderExtra: "自主財源|依存財源|総括表",
      revenueCropX: { from: 0, to: rc },
      expenditureCropX: { from: 0, to: ec },
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 高岡市（富山県・団体コード 162027）。予算書（一般会計）「歳入歳出予算事項別明細書 １ 総括」＝**歳入 p.37-38（合計行だけ p.38）／歳出 p.39**
    //   （7年度とも同じ・印字 25-27＝物理 +12）。千円・列 `[本年度, 前年度, 比較]`。R8〜R2 の7年（自分で全件目視・Σ4系統差0・6リンク不一致0）。
    // ⚠⚠ **数字が ToUnicode 欠落の MS-Mincho**（制御文字）→ `decodeGarble`（既定帯 −0x1D・既存マップで復号）。
    // ⚠⚠ **歳出表の右側「本年度予算額の財源内訳」の列見出し2行が款1 に連結**して `一般財源国・県支出金地方債その他議会費`（Σ 差0 のまま）→
    //   `expenditureHeaderExtra: "一般財源|国・県支出金"`。
    // ⚠⚠ **廃止款が款番号・印・「皆減」のいずれも無し**で並ぶ（R8 `環境性能割交付金 0 91,000 △91,000`・R2 `自動車取得税交付金 0 84,000 △84,000`）
    //   → `kanNoless` で kanNo null として拾う（無いと前年度 Σ −91,000 が warning 止まりで derive まで流れる）。
    // ⚠ 同 PDF の p.14-17「歳入歳出予算現計表」（款別・当年度のみ・数字は PUA の別帯）と p.23-29 第1表は前年度列なし。特別会計は別 PDF。
    // ⚠ **R1 は会計別予算の皆増行 `９ 環境性能割交付金 － － 30,000 … 皆増`** が `prevColumnFirst` と両立せず当年度 Σ −30,000（`dashAsZero` は
    //   [0,0,30,000] と読んで当年度 0・`prevBlankAsZero` は併用禁止で throw）＝**パーサ手当て待ち・未収録**（浦安 R2/H22・出雲 R2 と同族）。
    // ⚠ H30〜H28 は現行 404 → **WARP の「会計別予算（案）」4頁 PDF**（前年先行 `prevColumnFirst`・**H30 だけ歳入 p.3／歳出 p.2 と逆**）。
    //   Wayback は偵察時に IA 停止で未確認。H27・H26 は WARP に年度ページのみ確認（PDF 未取得）。
    // 年度ディレクトリは連番でない（R2=1・R3=2・R4=3・R5=4・R6=4_1・R7=4_2・R8=4_3）・PDF 名も揺れる＝機械生成しない。骨格予算 0件。
    // ライセンス: サイトポリシー（/konositenitsuite/2512.html・更新 2024-03-25・確認日 2026-09-09）。「無断」で permission-required。
    //   オープンデータカタログ（CC BY）は「本カタログページに掲載しているデータ」に限定。財務欄に R7 会計別予算 PDF はあるが予算書・概要は無い＝及ばない。
    //   リンクは「原則自由」＋注意書き＋フレーム禁止 → `noDeepLink` は立てない。
    // [年度, 種別, ファイル URL, landing URL, 歳入p(from), 歳入p(to|null), 歳出p]
    ["R8", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r8yosansho_ippan.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/4_3/13605.html", 37, 38, 39],
    ["R7", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r7yosansho_ippan.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/4_2/12151.html", 37, 38, 39],
    ["R6", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/6yosansho_ippan.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/4_1/9239.html", 37, 38, 39],
    ["R5", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r5ippankaikei.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/4/2836.html", 37, 38, 39],
    ["R4", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r4ippann.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/3/2753.html", 37, 38, 39],
    ["R3", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r3ippan.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/2/2752.html", 37, 38, 39],
    ["R2", "yosansho", "https://www.city.takaoka.toyama.jp/material/files/group/10/r2ippan.pdf", "https://www.city.takaoka.toyama.jp/soshiki/zaiseika/2/1/2/3/1/2381.html", 37, 38, 39],
    ["H30", "kaikeibetsu", "https://warp.ndl.go.jp/20190505/20190505063922/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/h30yosan/documents/kaikeibetsu.pdf", "https://warp.ndl.go.jp/20190505/20190505063922/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/h30yosan/h30yosan1.html", 3, null, 2],
    ["H29", "kaikeibetsu", "https://warp.ndl.go.jp/20180515/20180513145242/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/h29yosan/documents/_h29kaikeibetsu.pdf", "https://warp.ndl.go.jp/20180515/20180513145242/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/h29yosan/h29yosan1.html", 2, null, 3],
    ["H28", "kaikeibetsu", "https://warp.ndl.go.jp/20170511/20170510012940/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/documents/h28kaikeibetsu.pdf", "https://warp.ndl.go.jp/20170511/20170510012940/https://www.city.takaoka.toyama.jp/zaisei/shise/yosan/yosan/h28yosan1.html", 2, null, 3],
  ] as const).map(([fy, kind, url, landingPage, rf, rt, ep]) => ({
    id: `takaoka-${kind === "yosansho" ? "yosansho" : "kaikeibetsu-yosan"}-${fy.toLowerCase()}`,
    title:
      kind === "yosansho"
        ? `${eraYear(fy)}年度 高岡市予算書（一般会計）（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`
        : `${eraYear(fy)}年度 高岡市当初予算（案） 会計別予算（一般会計 款別＋前年度当初比較）`,
    publisher: "高岡市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "高岡市（一般会計・団体コード162027）",
    license:
      "高岡市ホームページに掲載されている情報・文章・画像等の著作権は、高岡市にあります。ただし、これらの提供者の方にある場合もあります。これらの著作物は、著作権法上認められた場合を除き、無断で転用・引用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      ...(rt === null ? { revenuePage: rf } : { revenuePages: { from: rf, to: rt } }),
      expenditurePage: ep,
      ...(kind === "yosansho"
        ? {
            revenueHeading: "歳   入",
        ...
        : { revenueHeading: "（歳入）", expenditureHeading: "（歳出）", prevColumnFirst: true }),
    },
  } satisfies SourceEntry)),

  ...([
    // 松阪市（三重県・団体コード 242047）。「○年度 当初予算説明資料」（総務部財務課）の「一般会計款別構成表（歳入）／（歳出）」。
    //   千円・列 `[当年度, 構成比, 前年度, 構成比, 比較増減, 増減率]`（整数列は `[当年度, 前年度, 増減]` の既定順）。物理ページ＝印字ページ。
    //   **R8〜R2・H30・H28〜H24 の13年が同型**（自分で13年の款名を全件目視・Σ4系統差0・隣接年度の款単位クロスチェック一致）。
    // ⚠ 歳入款数: R3〜R8 = 23（法人事業税交付金は R3 新設・R3 の前年度は 0）／R2 = 22（環境性能割あり・法人事業税なし）／
    //   H30〜H24 = 22（8 が自動車取得税交付金）。歳出は全年 13款。歳入款10「国有提供施設等所在市町村助成交付金」は3行折返し（パーサが結合）。
    // ⚠ 前年度列は**当初**（列見出しに「当初」の語は無い。R8↔R7 … R3↔R2・H28↔H27 … H25↔H24 の隣接ペアで款単位に全一致。
    //   H30 の前年度 63,646,220 = H29 予算書総括の本年度、H29 の前年度 62,233,574 = H28 当年度＝総額の鎖が連続）。
    // ⚠ H27 は年度ページの表記が「平成27年度予算説明資料（修正議決後）」（表紙は「当初予算説明資料」）→ title に添える。H28 の前年度列と全款一致。
    // ⚠ 収録不可: H31(R1)・H29 = スキャン画像（600dpi CCITT・pdftotext 667/707字）／H23 = 自主財源印 `*` が款番号の前に付き8款が
    //   廃止款扱いで落ちる（Σ −23,406,854 で大声で落ちる型）／H22 = 前年度列が「平成21年度 肉付け後」（当初比較でない）→ unrecordable。
    // ライセンス: サイトポリシー「著作権（引用・免責）、リンクなどについて」（/site/kouhou/52.html・更新 2023-02-01・確認日 2026-09-10）。
    //   「無断」「禁止」の語なし・CC/公共データ利用規約の語もなし → unverified（実測）。市のオープンデータ（PDL1.0）は BODIK の8データセット
    //   （施設系）に限られ予算・決算は0件 → 本資料には及ばない（§9g）。リンクは「各ページへのリンクは、特に制限はありません」→ `noDeepLink` 不要。
    // 年度ページ URL の規則: 令和は2桁ゼロ埋め `r08-yosan.html`、H30 `30-yosan.html`、H28 以前は数字 ID。
    // [年度, attachment ID, 歳入p, 歳出p, 年度ページ, title 補足]
    ["R8", "102649", 6, 7, "r08-yosan", ""],
    ["R7", "93017", 6, 7, "r07-yosan", ""],
    ["R6", "84088", 6, 7, "r06-yosan", ""],
    ["R5", "73684", 6, 7, "r05-yosan", ""],
    ["R4", "62753", 6, 7, "r04-yosan", ""],
    ["R3", "51951", 5, 6, "r03-yosan", ""],
    ["R2", "42125", 5, 6, "r02-yosan", ""],
    ["H30", "32881", 5, 6, "30-yosan", ""],
    ["H28", "6288", 6, 7, "33", ""],
    ["H27", "6249", 6, 7, "30", "（修正議決後）"],
    ["H26", "6195", 6, 7, "26", ""],
    ["H25", "5993", 4, 5, "7", ""],
    ["H24", "5956", 4, 5, "6", ""],
  ] as const).map(([fy, att, rp, ep, page, sub]) => ({
    id: `matsusaka-yosan-setsumei-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 松阪市当初予算説明資料${sub}（一般会計款別構成表 歳入・歳出・前年度当初比較）`,
    publisher: "松阪市",
    url: `https://www.city.matsusaka.mie.jp/uploaded/attachment/${att}.pdf`,
    landingPage: `https://www.city.matsusaka.mie.jp/site/zaisei/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "松阪市（一般会計・団体コード242047）",
    license:
    ...
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "一般会計款別構成表（歳入）",
      expenditureHeading: "一般会計款別構成表（歳出）",
    },
  } satisfies SourceEntry)),

  ...([
    // ひたちなか市（茨城県・団体コード 082210。⚠ 日立市 082023 と別）。「○年度予算の概要」（総務部財政課）の
    //   「３ 一般会計歳入予算款別構成 (1) 歳入予算総括」（p.5）／「４ 一般会計歳出予算款別構成 (1) 歳出予算目的別総括」（p.8）。
    //   千円・列 `[前年度(A), 構成比, 当年度(B), 構成比, 増減額, 増減率]`＝**前年度が左**（`prevColumnFirst`）。
    //   R8〜R2 の7年が同じ物理ページ・同じ opts（自分で7年の款名を全件目視・Σ4系統差0・R8→R2 の6リンクを款単位で突合し不一致0）。
    // ⚠ 見出しは節見出し「歳入予算総括」でなく**表題行「一般会計歳入予算款別構成」**にする。R2 は表題行と節見出しの間に空行が無く、
    //   節見出しを指定すると表題が款1 に連結して `一般会計歳入予算款別構成市税`（Σ 差0 のまま通る型・偵察が実測）。
    // ⚠ 歳出の款番号 13（諸支出金）は欠番（11 災害復旧費・12 公債費・14 予備費）＝原典。R2 歳入6 法人事業税交付金は皆増行（前年度 `―`）で
    //   前年度 0 に読める。同 PDF p.10 の性質別総括は表題「…性質別構成」なので見出しでは当たらない。
    // ⚠ 年度ページの予算書（事項別明細書 総括）は**歳出の前年度列が組替後**（R8 総務費 7,353,804 ≠ R7 当初 7,366,863）で、
    //   見出し `総括`・財源内訳の列見出しが款1 に連結する（Σ 差0 のまま）ため採らない。
    // 前年度列は**当初**。骨格予算: 概要7年で「骨格」0件。
    // ⚠ 収録不可: H31 = 概要のテキスト層が化ける（款名が「杝杝」に潰れる）／H30・H29 = 概要・予算書ともスキャン → unrecordable。
    //   H28〜H18 の年度ページは現行 404（Wayback 2022-07 捕捉のみ・H28/H27 の概要はスキャン・H26 はテキスト層あり）→ 孤立年度なので収録しない（横展開優先）。
    // ライセンス: 「公式ウェブサイトについて」（/about/1001515.html・更新 2022-01-30・確認日 2026-09-10）の著作権条項。「無断」「禁止」の語なし → unverified（実測）。
    //   オープンデータ利用規約（CC BY 2.1 JP）は「当サイト（ひたちなか市オープンデータ）」に自ら範囲を限り、カタログに予算は0件 → 及ばない（§9g）。
    //   リンクは「原則として自由ですが、トップページへリンクさせるようにお願いします」＝依頼形 → `noDeepLink` は立てない。
    // 年度ページ URL は連番でない。
    // [年度, ディレクトリ, ファイル, 年度ページ]
    ["R8", "001/016/646", "r8yosangaiyou.pdf", "1016646"],
    ["R7", "001/015/193", "r7yosangaiyou.pdf", "1015193"],
    ["R6", "001/013/804", "r6yosangaiyou.pdf", "1013804"],
    ["R5", "001/011/692", "r5yosangaiyou.pdf", "1011692"],
    ["R4", "001/009/337", "r4yosannogaiyou.pdf", "1009337"],
    ["R3", "001/004/682", "yosangaiyou.pdf", "1004682"],
    ["R2", "001/004/681", "r2yosanngaiyou.pdf", "1004681"],
  ] as const).map(([fy, dir, file, page]) => ({
    id: `hitachinaka-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 ひたちなか市予算の概要（一般会計 歳入予算総括・歳出予算目的別総括・款別＋前年度当初比較）`,
    publisher: "ひたちなか市",
    url: `https://www.city.hitachinaka.lg.jp/_res/projects/default_project/_page_/${dir}/${file}`,
    landingPage: `https://www.city.hitachinaka.lg.jp/shisei/zaisei/1004671/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "ひたちなか市（一般会計・団体コード082210）",
    license:
      "「ひたちなか市公式ウェブサイト」に掲載されている情報（文書、写真、イラストなど）は原則としてひたちなか市に帰属します。また、一部の画像等の著作権は原著作者が所有しています。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 5,
      expenditurePage: 8,
      revenueHeading: "一般会計歳入予算款別構成",
      expenditureHeading: "一般会計歳出予算款別構成",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      prevColumnFirst: true,
    },
  ...
  ...([
    // 秦野市（神奈川県・団体コード 142115）。「予算のあらまし」（財政課）の「【一般会計当初予算（歳入）の前年度比較】」（物理 p.7）／
    //   「【一般会計当初予算（目的別歳出）の前年度比較】」（物理 p.9）。歳入22款・歳出11款・千円・列 `[当年度, 前年度, 増減, 伸率, 構成比×2]`。
    //   印字ノンブルは年で動く（-4-/-6- と -5-/-7-）が**物理 p.7/p.9 は R2〜R8 で不変**（自分で6年の款名を全件目視・Σ4系統差0・
    //   R8→R2 の6リンクを款単位で突合し不一致0）。歳出表の下に性質別の図が同一ページに続くが合計行で止まる。
    // ⚠ **R5 のあらましは款別表が JPEG 画像**（p.7 テキスト68字・画像2枚・自分で実測）→ R5 だけ「予算資料」で代替（次のブロック）。
    // ⚠ R3・R2 は現行「予算の概要」ページ（2026-03-26 更新で R4 以降のみ）から消失 → **WARP**（秦野市コレクション waid/10957）から取得。
    //   R3 は 2026-02 捕捉の現行サイト構成、R2 は 2023-05 捕捉の旧サイト（/www/contents/1001000003696/）。landingPage も同じ捕捉の年度ページ（200 を実測）。
    // ⚠ 廃止款: R2 歳入「自動車取得税交付金 0 / 89,000」（款番号なし・「皆減」）→ kanNo null で拾える。
    // 前年度列は**当初**（R2 前年度 市税 23,100,000 = H31 あらまし p.9 の市税・偵察が実測）。
    // ⚠ H31〜H22 のあらましは WARP 2023-05 捕捉の旧ページに残るが、H31 は【】見出しが無くページも違う別様式（未 try-parse）＝横展開優先で見送り・記録しない。
    // ライセンス: 「リンク方針・著作権」（/shisei/koho/9/9176.html・更新 2026-01-30・確認日 2026-09-10）。「無断で転用・引用することを禁じます」＝permission-required。
    //   オープンデータライブラリ（CC BY 4.0）は「このページで公開しているデータのみに適用」と自ら範囲を限り、行政・財政区分に予算・決算は0件 → 及ばない（§9g）。
    // ⚠⚠ リンク条項「リンクを貼る際にはトップページ（…）に貼ってください」＝**指示形**（§11h 第2群・立川・小山と同型。理由がリンク切れの注意でも文は指示形）
    //   ＋「ご一報いただけますよう」（第5群・依頼）→ `noDeepLink` を立てる。
    // [年度, ファイル URL, landing URL]
    ["R8", "https://www.city.hadano.kanagawa.jp/material/files/group/4/yosannoaramashi.pdf", "https://www.city.hadano.kanagawa.jp/soshiki/1/1003/4/2380.html"],
    ["R7", "https://www.city.hadano.kanagawa.jp/material/files/group/4/yosannnoaramasi.pdf", "https://www.city.hadano.kanagawa.jp/soshiki/1/1003/4/2380.html"],
    ["R6", "https://www.city.hadano.kanagawa.jp/material/files/group/4/r6.pdf", "https://www.city.hadano.kanagawa.jp/soshiki/1/1003/4/2380.html"],
    ["R4", "https://www.city.hadano.kanagawa.jp/material/files/group/4/r4aramashi.pdf", "https://www.city.hadano.kanagawa.jp/soshiki/1/1003/4/2380.html"],
    ["R3", "https://warp.ndl.go.jp/20260207/20260206162041/https://www.city.hadano.kanagawa.jp/material/files/group/4/R03yosannoaramashi.pdf", "https://warp.ndl.go.jp/20260207/20260206162041/https://www.city.hadano.kanagawa.jp/soshiki/1/1003/4/2380.html"],
    ["R2", "https://warp.ndl.go.jp/20230511/20230509133115/https://www.city.hadano.kanagawa.jp/www/contents/1001000003696/simple/R02yosanaramashi.pdf", "https://warp.ndl.go.jp/20230511/20230509133115/https://www.city.hadano.kanagawa.jp/www/contents/1001000003696/index.html"],
  ] as const).map(([fy, url, landingPage]) => ({
    id: `hadano-yosan-aramashi-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 秦野市予算のあらまし（一般会計当初予算 歳入／目的別歳出の前年度比較・款別）`,
    publisher: "秦野市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "秦野市（一般会計・団体コード142115）",
    license:
      "秦野市役所ホームページの情報は、原則として秦野市が著作権を有しています。法律で認められた場合を除き、無断で転用・引用することを禁じます。",
    noDeepLink: true,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 7,
      expenditurePage: 9,
      revenueHeading: "一般会計当初予算（歳入）の前年度比較",
      expenditureHeading: "一般会計当初予算（目的別歳出）の前年度比較",
    },
  } satisfies SourceEntry)),

  {
    // 秦野市 R5 — あらましの款別表が画像なので「令和５年度 予算資料」（財政課）の「２ 一般会計 (1) 歳入 p.4 / (2) 歳出 p.5」で代替。
    //   同じ22款/11款・千円・前年度=当初（R6 あらましの前年度列と款単位で全一致・自分で実測）。現行 404 → WARP 2023-05 捕捉から取得。
    // ⚠ 予算資料には款の下に「うち 普通交付税」「うち 財政調整基金繰入金」「うち 臨時財政対策債」の内数行が入る年がある。R5 の p.4 は「うち」行なし。
    //   **R7 の予算資料は「うち 臨時財政対策債 0 / 300,000 皆減」が廃止款分岐に入り前年度 Σ +300,000**（偵察が実測・ゲートが捕まえる型）、
    //   R8 は同行が 0/0/－ で静かに通る → 予算資料を他年度に広げるなら「うち」行の除外が要る。R5 だけなら不要。
    id: "hadano-yosan-siryou-r5",
    title: "令和5年度 秦野市予算資料（一般会計 歳入／歳出・款別＋前年度当初比較）",
    publisher: "秦野市",
    url: "https://warp.ndl.go.jp/20230511/20230509133115/https://www.city.hadano.kanagawa.jp/www/contents/1001000003696/simple/R05_yosansiryou.pdf",
    landingPage: "https://warp.ndl.go.jp/20230511/20230509133115/https://www.city.hadano.kanagawa.jp/www/contents/1001000003696/index.html",
    kind: "pdf" as const,
    fiscalYear: "R5",
    scope: "秦野市（一般会計・団体コード142115）",
    license:
  ...
  ...([
    // 宇部市（山口県・団体コード 352021）。「○年度当初予算（案）の概要」（総合政策部財政課）の「１ 歳入の状況（一般会計）」（p.4・22款）／
    //   「２ 歳出の状況（一般会計）」（p.9・13款）。千円・款番号なし（`kanNoless`）・列 `[当年度, 構成比, 前年度, 構成比, 増減額, 増減率]`。
    //   前年度列は「令和７年度 当初予算額」と明記＝**当初**（R8→R7→…→R1→H30 の鎖が総額で連続・R4→R3 は款単位で全一致・偵察が実測。
    //   自分でも R8↔R6 …の隣接可能ペアを款単位で突合）。
    // ⚠ 表の上に本文段落（【市税】…約１億３９００万円…）があり、kanNoless だと款に化けて Σ +47 等になる → `HeaderExtra: "【|億|万円"` で落とす。
    //   歳出は公債費の下に元金/利子の内訳行（字下げ）→ `kanIndentMax: 3`。歳入の主な一般財源に ※ 接尾辞 → `kanNameSuffixStrip`。
    // ⚠ 現行サイトは R8・R7・R6 の3年だけ（R5 以前はページも PDF も 404）→ R4・R3 は **WARP 2024-03-05 収集**から取得
    //   （年度ページの ts は年度ごとに違う。landingPage も同じ捕捉の年度ページ・CC ブロック付きを実測）。
    // ⚠ 収録不可: R7・R5 = ToUnicode 欠落で全面文字化け（R7 は自分で実測: 全フォント uni=no・p.4 が CJK 化け）。R2 = 歳出ページだけ化け。
    //   H30 も化け・H29 は混在（平成なので未追及）。→ R7・R5・R2 を unrecordable に記録。
    //   R1 = 文字層は正常だが本文行「…下水道事業会計負担金が」が『計』を含み整数4個で合計行の pre-scan に選ばれて款0件＝パーサ都合なので記録しない（§13-27）。
    // ライセンス: **open（CC BY 4.0）**。年度ページに「オープンデータ 令和8年度当初予算 この作品はクリエイティブ・コモンズ 表示 4.0 国際 ライセンスの下に
    //   提供されています」のブロックがあり、**概要 PDF がその直下に列挙されている**（＝資料そのものに付されている・愛媛 §11i 型・自分で R8/R6 の現行ページと
    //   R4/R3 の WARP 捕捉ページで実測）。山口県オープンデータカタログ「【宇部市】当初予算の概要」も cc-by-40-intl。
    //   サイトポリシーの著作権条項（事前の許諾）は CC ブロックが個別に上書きする形。リンク条項「原則としてトップページにリンクしてください」は
    //   open なので振替の対象外（noDeepLink は要許可の資料にだけ効く）。
    // [年度, ファイル URL, landing URL]
    ["R8", "https://www.city.ube.yamaguchi.jp/_res/projects/default_project/_page_/001/028/599/gaiyou_r8.pdf", "https://www.city.ube.yamaguchi.jp/shisei/zaisei/yosan/1028599.html"],
    ["R6", "https://www.city.ube.yamaguchi.jp/_res/projects/default_project/_page_/001/021/463/gaiyou_r6_0213.pdf", "https://www.city.ube.yamaguchi.jp/shisei/zaisei/yosan/1021463.html"],
    ["R4", "https://warp.ndl.go.jp/20240305/20240304041250/https://www.city.ube.yamaguchi.jp/_res/projects/default_project/_page_/001/014/906/gaiyou_ikkatu_r4.pdf", "https://warp.ndl.go.jp/20240305/20240304041250/https://www.city.ube.yamaguchi.jp/shisei/zaisei/yosan/1014906.html"],
    ["R3", "https://warp.ndl.go.jp/20240305/20240304041251/https://www.city.ube.yamaguchi.jp/_res/projects/default_project/_page_/001/011/172/gaiyou_ikkatu_r3.pdf", "https://warp.ndl.go.jp/20240305/20240304041251/https://www.city.ube.yamaguchi.jp/shisei/zaisei/yosan/1011172.html"],
  ] as const).map(([fy, url, landingPage]) => ({
    id: `ube-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 宇部市当初予算（案）の概要（一般会計 歳入・歳出の状況・款別＋前年度当初比較）`,
    publisher: "宇部市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "宇部市（一般会計・団体コード352021）",
    // 年度ページのブロック見出しは年度ごと（「オープンデータ 令和6年度当初予算」…・R6/R4/R3 も自分で実測）なので原文のまま年度を差し替える
    license: `オープンデータ ${eraYear(fy)}年度当初予算 この作品はクリエイティブ・コモンズ 表示 4.0 国際 ライセンスの下に提供されています。データのご利用に際して 本セクションで公開しているデータは、クリエイティブ・コモンズ・ライセンスのもとで提供しております。対象データのご利用に際しては、表示されている各ライセンスの利用許諾条項に則ってご利用ください。`,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 9,
  ...
  ...([
    // 大垣市（岐阜県・団体コード 212024）(a) 「予算明細書（総括〜一般会計［歳入］）」の「歳入歳出予算事項別明細書 １ 総括」。
    //   歳入 p.7-8（2ページ・合計行は p.8）／歳出 p.9。千円・列 `[本年度, 前年度, 比較]`（歳出は右に財源内訳4列・`-` セルあり → dashAsZero）。
    //   R8〜R2 の7年が同じページ・同じ opts（自分で7年の款名を全件目視・Σ4系統差0・R8→R2 の6リンクを款単位で突合し不一致0）。
    // ⚠ R5〜R2 は現行サイトで 404（索引ページに R8〜R6 のみ）→ **Wayback の id_ URL** から取得（自分で 200・サイズを実測）。
    //   ファイル名は R4 以降 `R{n}yosanmeisai1.pdf`、R3 以前 `R{n}yosanmeisai_1.pdf`。landingPage は現行の索引ページ（当該年度はもう載っていない）。
    // ⚠ 廃止款は `○.` 接頭（R8 `○. 災害復旧費 - 77,000`・R2 `自動車取得税交付金 - 88,000`）→ kanNo null で拾う。R7 は災害復旧費が款13（14款）。
    // ⚠ **R3 は骨格予算**（概要 3tousyo.pdf p.1「本年４月に市長選挙を控えていることから…骨格予算として編成した」・自分で実測）。
    //   当年度側が骨格の R3 には書かず、**前年度側が骨格になる R4 に prevNote**（§11k）。R4 の前年度 58,280,000 = R3 当初（自分で実測）。
    // ⚠ H31 の明細書は WARP にしか無くスキャン（偵察が実測）。H30 以前の明細書はウェブ未掲載 → H31〜H23 は「当初予算の概要」で収録（次のブロック）。
    // ライセンス: 「大垣市ホームページについて」（/site_policy/0000000013.html・更新 2026-06-29・確認日 2026-09-10）。「無断引用及び転載は禁止」で permission-required。
    //   岐阜県オープンデータカタログの大垣市20件に予算・決算は0件 → 及ばない（§9g）。
    //   リンクは「原則として自由ですが…ご連絡ください」＝依頼形・事後連絡・トップページ限定なし（§11h 第5群）→ `noDeepLink` は立てない。
    // [年度, url]
    ["R8", "https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R8yosanmeisai1.pdf"],
    ["R7", "https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R7yosanmeisai1.pdf"],
    ["R6", "https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R6yosanmeisai1.pdf"],
    ["R5", "https://web.archive.org/web/20230601133657id_/https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R5yosanmeisai1.pdf"],
    ["R4", "https://web.archive.org/web/20220707131230id_/https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R4yosanmeisai1.pdf"],
    ["R3", "https://web.archive.org/web/20220708214433id_/https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R3yosanmeisai_1.pdf"],
    ["R2", "https://web.archive.org/web/20200930224020id_/https://www.city.ogaki.lg.jp/cmsfiles/contents/0000044/44983/R2yosanmeisai_1.pdf"],
  ] as const).map(([fy, url]) => ({
    id: `ogaki-yosanmeisai-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 大垣市予算明細書（総括〜一般会計［歳入］）（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "大垣市",
    url,
    landingPage: "https://www.city.ogaki.lg.jp/0000044983.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "大垣市（一般会計・団体コード212024）",
    license:
    ...
    parserOptions: {
      ...
      ...(fy === "R4"
        ? {
            prevNote:
              "前年度（令和3年度）の当初予算は骨格予算（「令和３年度大垣市当初予算の概要」p.1 に「本年４月に市長選挙を控えていることから、市民生活と密接な関わりを持つものや緊急性の高いものを優先して、事業を絞り込み、骨格予算として編成した」と明記）。ここでの前年度額はその骨格予算の当初額（58,280,000千円）。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 大垣市 (b) 「大垣市当初予算の概要」の「２．一般会計歳入予算」（p.2）／「５．一般会計歳出（目的別）予算」（p.6）。H31〜H23 の9年のうち8年。
    //   千円・合計行「合計」・列見出しに「平成N年度当初／平成N-1年度当初」と明記。**H29 以降 `[当年度, 構成比, 前年度比, 前年度, 構成比]`、
    //   H28 以前 `[前年度, 構成比, 当年度, 構成比, 前年度比]`＝列順が逆（`prevColumnFirst`）**。⚠⚠ prevColumnFirst 無しでも Σ4系統差0 で静かに通る
    //   （当年度と前年度が入れ替わる・最も危険な型）→ 列見出しの実物と年度間チェーンで確定（偵察が実測・自分でも隣接ペアを款単位で突合）。
    //   内訳行「うち普通交付税」「うち財政調整基金繰入金」「うち競輪事業収入」「うち臨時財政対策債」は款番号が無く拾われない。
    // ⚠ H25 は歳出 p.6 の皆増行 `11 災害復旧費 - - 222,000 0.4 皆増` の `- -` が prevColumnFirst と両立せず当年度 Σ −222,000
    //   ＝高岡 R1・浦安 R2 と同族のパーサ手当て待ち（記録しない）。概要 PDF は表紙の「の」等が化けるが款名には影響なし。
    // [年度, ファイル, prevColumnFirst]
    ["H31", "31tousyo.pdf", false],
    ["H30", "30tousyo.pdf", false],
    ["H29", "29tousyo.pdf", false],
    ["H28", "28tousyo.pdf", true],
    ["H27", "27tousyo.pdf", true],
    ["H26", "26tousyo.pdf", true],
    ["H24", "24tousyo.pdf", true],
    ["H23", "23tousyo.pdf", true],
  ] as const).map(([fy, file, pcf]) => ({
    id: `ogaki-yosangaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 大垣市当初予算の概要（一般会計歳入予算・歳出（目的別）予算の款別＋前年度当初比較）`,
    publisher: "大垣市",
    url: `https://www.city.ogaki.lg.jp/cmsfiles/contents/0000002/2490/${file}`,
    landingPage: "https://www.city.ogaki.lg.jp/0000002490.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "大垣市（一般会計・団体コード212024）",
    license:
    ...
    parserOptions: {
      ...
      expenditurePage: 6,
      revenueHeading: "一般会計歳入予算",
      expenditureHeading: "一般会計歳出（目的別）予算",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      dashAsZero: true,
      ...(pcf ? { prevColumnFirst: true } : {}),
    },
  } satisfies SourceEntry)),

  ...([
    // 釧路市（北海道・団体コード 012068）。「○年度 釧路市予算（案）の概要」（財政課）の
    //   「一般会計歳入予算総括表」（p.3）／「一般会計歳出予算内訳表」（p.5）。千円・列 `[当年度, 前年度, 増減, 伸び率, 構成比×2]`。
    //   **H30〜R8 の9年が同じ物理ページ・同じ opts**（自分で9年の款名を全件目視・Σ4系統差0・8リンクを款単位で突合し不一致0）。
    //   列見出しに「当初予算額」と明記。骨格予算 0件・スキャン年度なし・消失年度なし。
    // ⚠ 歳出は16款（港湾費・職員費・諸支出金を含む釧路固有の構成）。廃止款は `▲` 接頭（R8 環境性能割交付金・R2 自動車取得税交付金）→ kanNo null。
    // ⚠ **R1 年度の資料は表紙が「２０１９（平成３１）年度」**（自分で実測）＝資料の呼称に従い `H31` で登録する（年度ページの表記は「令和元年度」）。
    // ⚠ ファイル名の規則は3世代で変わる（R7/R8 は `r?yosanangaiyou.pdf`、R5/R6 は `<pageid>_N.pdf`、R4 以前は `000NNNNNN.pdf`）＝機械生成しない。
    //   年度ページ ID も連番でない。
    // ライセンス: 「リンク・著作権・免責事項」（/about/1007335.html・更新 2022-08-29・確認日 2026-09-10）。「無断」で permission-required。
    //   オープンデータ利用規約（CC BY 4.0）は「釧路市オープンデータサイトに掲載されているデータ」に自ら範囲を限り、カタログ83件に予算は0件 → 及ばない（§9g）。
    //   リンクは「原則、自由です。事前の連絡は必要ありませんが…お知らせいただければ」＝依頼形・事後連絡（§11h 第5群）→ `noDeepLink` は立てない。
    // [年度, ファイル URL, landing URL]
    ["R8", "r8yosanangaiyou.pdf", "001/018/509", "1018509"],
    ["R7", "r7yosanangaiyou.pdf", "001/016/038", "1016038"],
    ["R6", "1013888_06.pdf", "001/013/888", "1013888"],
    ["R5", "1011046_4.pdf", "001/011/046", "1011046"],
    ["R4", "000186459.pdf", "001/007/152", "1007152"],
    ["R3", "000166290.pdf", "001/007/153", "1007153"],
    ["R2", "000147217.pdf", "001/007/154", "1007154"],
    ["H31", "000132324.pdf", "001/007/155", "1007155"],
    ["H30", "000141150.pdf", "001/007/156", "1007156"],
  ] as const).map(([fy, file, dir, page]) => ({
    id: `kushiro-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 釧路市予算（案）の概要（一般会計歳入予算総括表・歳出予算内訳表・款別＋前年度当初比較）`,
    publisher: "釧路市",
    url: `https://www.city.kushiro.lg.jp/_res/projects/default_project/_page_/${dir}/${file}`,
    landingPage: `https://www.city.kushiro.lg.jp/shisei/zaisei/1007149/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "釧路市（一般会計・団体コード012068）",
    license:
      "当ホームページに掲載されている記事・写真等の「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断での転載・転用を禁じます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 3,
      expenditurePage: 5,
      revenueHeading: "一般会計歳入予算総括表",
      expenditureHeading: "一般会計歳出予算内訳表",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 栃木市（栃木県・団体コード 092037。⚠ 栃木県 090000 と別）。「○年度 栃木市（当初）予算のあらまし」（財政課）の
    //   「歳入一覧」（p.4）／「歳出目的別一覧(※11)」（p.7）。千円・列 `[当年度, 構成比, 前年度, 構成比, 増減額, 増減率]`。
    //   **H30〜R8 の9年**（自分で9年の款名を全件目視・Σ4系統差0・8リンクを款単位で突合し不一致0＝総額の鎖が H30 から連続）。
    // ⚠⚠ **歳入の款名末尾に脚注番号 `(※1)`〜`(※6)` が付く**（9年度すべて）。パーサに「款名末尾の `(※n)` を金額化の前に落とす」
    //   処理を足して収録した（2026-09-10・§13-28）。落とさないと `1` が第1金額に読まれてその款だけ列が1つずれる。
    // ⚠⚠ **R4 の歳出は円グラフの中心ラベル `8.7%  671 億 9 千万円` が同一行に組まれる** → 行頭の `8` が款番号・`671`/`9` が金額の
    //   偽の款になっていた。同じ改修で「款番号の直後が小数の百分率なら款行でない」を足して解消（§13-28）。
    // ⚠ ページは年で動く（R5 は歳入 p.5／歳出 p.8・H31/H30 は歳出 p.6）＝外挿しない。R5 だけ 17p。
    // ⚠ 見出しは `歳入`（ページ冒頭のタイトル行）にする。`歳入一覧` にするとタイトル行が款1 に連結して `歳入市税(※`（偵察が実測）。
    // ⚠ H31・H30 は原典の表題が「栃木市予算のあらまし」（「当初」なし）。
    // ⚠ 予算書本編（R3〜R8）の事項別明細書 総括は罫線文字（┃│）の表で款行が1件も取れない（偵察が実測）＝あらましを採る。
    //   主要事務事業説明書の款別一覧表は款名に財源区分の印（`市税一自`）が連結するので採らない。
    // 前年度列は**当初**。骨格予算: あらまし9年・予算書 R8・主要事務事業説明書 R8 で「骨格」「肉付」0件。スキャン年度なし。
    // ライセンス: 「リンク・著作権・免責事項」（/site/userguide/12507.html・更新 2019-01-24・確認日 2026-09-10）。「無断」で permission-required。
    //   オープンデータ（CC BY 4.0）は「このページに掲載するデータ」に自ら範囲を限り、登載17件に予算は0件（財政指標のみ）→ 及ばない（§9g）。
    //   リンクは「自由です」「トップページに限らず、サイト内のどのページに対しても」＝§11h のどの群にも当たらない → `noDeepLink` は立てない。
    // [年度, attachment 番号, 歳入p, 歳出p]
    ["R8", "68661", 4, 7],
    ["R7", "61216", 4, 7],
    ["R6", "54891", 4, 7],
    // ⚠ R5 の歳入5「株式等譲渡所得割交付金」の前年度は原典が `146, 000` と桁区切りの直後に空白を入れており、
    //   `146` と `000` の2トークンに割れて前年度 Σ −145,854（warning で derive まで流れる型）→ amountTypos で1か所だけ直す。
    ["R5", "54840", 5, 8],
    ["R4", "42040", 4, 7],
    ["R3", "33830", 4, 7],
    ["R2", "24589", 4, 7],
    ...
    id: `tochigi-shi-yosan-aramashi-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 栃木市${fy === "H31" || fy === "H30" ? "" : "当初"}予算のあらまし（一般会計 歳入一覧・歳出目的別一覧・款別＋前年度当初比較）`,
    publisher: "栃木市",
    url: `https://www.city.tochigi.lg.jp/uploaded/attachment/${att}.pdf`,
    landingPage: "https://www.city.tochigi.lg.jp/soshiki/14/1853.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "栃木市（一般会計・団体コード092037）",
    license:
      "栃木市ホームページ上に掲載されている文書や画像等のコンテンツの無断使用・転載・引用を禁じます。栃木市公式サイト上における文書・画像等コンテンツの著作権は、栃木市に帰属します。一部の画像等の著作権は、撮影者や画像提供者などの原著作者が所有します。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "歳入",
      expenditureHeading: "歳出目的別一覧",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(fy === "R5" ? { amountTypos: { revenue: { "146, 000": "146,000" } } } : {}),
    },
  } satisfies SourceEntry)),

  ...([
    // 刈谷市（愛知県・団体コード 232106）。**年度で資料の型が3つに変わる**（自分で9年の款名を全件目視・Σ4系統差0・
    //   8リンクを款単位で突合し不一致0）:
    //   型A R8・R7 =「当初予算 主要事業の概要」の「歳入歳出予算事項別明細書（一般会計）」（歳入 p.7／歳出 p.9）
    //   型B R6〜R4 =「各会計予算説明書」の「１．総括」（歳入 p.7／歳出 p.8・見出しが半角括弧「(歳 入)」）
    //   型C R3〜H30 = 旧サイトの**歳入・歳出が別ファイル**（各1p）→ `revenueFile`/`expenditureFile`。発行元から消えて **WARP** のみ。
    // ⚠⚠ 型A/C で見出しを「（歳入）」にすると**直前のタイトル行が款1 に連結**して `歳入歳出予算事項別明細書（一般会計）市税` になる
    //   （Σ 差0 のまま静かに通る型・偵察が実測）→ **タイトル行そのものを見出しに指定**する。
    // ⚠ 型B の R8・R7 の説明書は総括ページのテキストが取れない（R8 はアウトライン化・R7 はスキャン）ので型A を採る。
    //   主要事業の概要 R6〜R4 の款別ページは逆にスキャン。**年度ごとに「読める方の資料」が入れ替わる**。
    // ⚠ 予算書（第1表）は全年度テキストがあるが**前年度列が無い**ので使わない。
    // 前年度列は**当初**。骨格予算: 概要 R8〜R4・説明書 R6〜R4・あらまし R7〜H29 で「骨格」「肉付」0件。
    // ライセンス: 「このサイトの考え方」（/about/1001754.html・更新 2025-04-16・確認日 2026-09-10）に**著作権・転載の条項が無い**
    //   → フッター表記のみを置いて **unverified**（松阪・ひたちなか・野田と同じ判断待ちの群）。
    //   オープンデータ規約（CC BY 4.0）は「刈谷市オープンデータサイトに掲載されているデータ」に自ら範囲を限り、
    //   カタログの行財政区分は警察署・交番の CSV だけで予算0件 → 及ばない（§9g）。
    //   リンクは「原則としてフリーです」＋相互リンク不可のみ → `noDeepLink` は立てない。
    // [年度, 型, 歳入 URL, 歳出 URL（型C のみ）, landing URL, 歳入p, 歳出p]
    ["R8", "a", "https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/017/657/r8_syuyozigyo2.pdf", null, "https://www.city.kariya.lg.jp/shisei/zaisei/1017657/index.html", 7, 9],
    ["R7", "a", "https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/017/657/r7syuyouzigyou.pdf", null, "https://www.city.kariya.lg.jp/shisei/zaisei/1017657/index.html", 7, 9],
    ["R6", "b", "https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/017/657/20240325165135/r6yosannsetumeisyo.pdf", null, "https://www.city.kariya.lg.jp/shisei/zaisei/1017657/index.html", 7, 8],
    ["R5", "b", "https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/017/657/20240325165135/r5yosannsetumeisyo.pdf", null, "https://www.city.kariya.lg.jp/shisei/zaisei/1017657/index.html", 7, 8],
    ["R4", "b", "https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/017/657/20240325165135/r4yosannsetumeisyo.pdf", null, "https://www.city.kariya.lg.jp/shisei/zaisei/1017657/index.html", 7, 8],
    ["R3", "c", "https://warp.ndl.go.jp/20210514/20210513232832/https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/004/552/jikoubetumeisaisyoh3.pdf", "https://warp.ndl.go.jp/20210514/20210513232832/https://www.city.kariya.lg.jp/_res/projects/default_project/_page_/001/004/552/jikoubetumeisaisyosaisyutuh3.pdf", "https://warp.ndl.go.jp/20210514/20210513232832/https://www.city.kariya.lg.jp/shisei/zaisei/1004552.html", 1, 1],
    ["R2", "c", "https://warp.ndl.go.jp/20200817/20200815192025/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyoh2.pdf", "https://warp.ndl.go.jp/20200817/20200815192025/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyosaisyutuh2.pdf", "https://warp.ndl.go.jp/20200817/20200815192025/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.html", 1, 1],
    ["H31", "c", "https://warp.ndl.go.jp/20190814/20190812132449/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyoh31.pdf", "https://warp.ndl.go.jp/20190814/20190812132449/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyosaisyutuh31.pdf", "https://warp.ndl.go.jp/20190814/20190812132449/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.html", 1, 1],
    ["H30", "c", "https://warp.ndl.go.jp/20180813/20180811132858/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyoh30.pdf", "https://warp.ndl.go.jp/20180813/20180811132858/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.files/jikoubetumeisaisyosaisyutuh30.pdf", "https://warp.ndl.go.jp/20180813/20180811132858/https://www.city.kariya.lg.jp/shisei/zaiseishisai/kariyanozaisei.html", 1, 1],
  ] as const).map(([fy, kind, url, expUrl, landingPage, rp, ep]) => ({
    id: `kariya-${kind === "a" ? "syuyojigyo" : kind === "b" ? "yosan-setsumeisho" : "jikobetsu-meisaisho"}-${fy.toLowerCase()}`,
    title:
      kind === "a"
        ? `${eraYear(fy)}年度 刈谷市当初予算 主要事業の概要（歳入歳出予算事項別明細書 一般会計・款別＋前年度当初比較）`
        : kind === "b"
          ? `${eraYear(fy)}年度 刈谷市各会計予算説明書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`
          : `${eraYear(fy)}年度 刈谷市当初予算 歳入歳出予算事項別明細書（一般会計 歳入・歳出・款別＋前年度当初比較）`,
    publisher: "刈谷市",
    ...(expUrl == null ? { url } : { url: null, urls: [url, expUrl] }),
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "刈谷市（一般会計・団体コード232106）",
    license:
      "Copyright © 2021 Kariya City,Aichi,Japan. All Rights Reserved.",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...(kind === "b"
        ? { revenueHeading: "(歳入)", expenditureHeading: "(歳出)" }
        : { revenueHeading: "歳入歳出予算事項別明細書（一般会計）", expenditureHeading: "（歳出）" }),
      ...(expUrl == null
        ? {}
        : { revenueFile: url.slice(url.lastIndexOf("/") + 1), expenditureFile: expUrl.slice(expUrl.lastIndexOf("/") + 1) }),
    },
  } satisfies SourceEntry)),

  ...([
    // 野田市（千葉県・団体コード 122084）。「○年度 予算に関する説明書（一般会計）」の「歳入歳出予算事項別明細書 １．総括」。
    //   **歳入は2ページにまたがる**（款1〜17／款18〜23＋歳入合計）→ `revenuePages`。歳出は1ページ（財源内訳4列つき）。
    //   千円・列 `[本年度予算額, 前年度予算額, 比較, 構成比]`。R8〜H29 の9年（自分で9年の款名を全件目視・Σ4系統差0・
    //   8リンクを款単位で突合し不一致0）。⚠ 物理ページは年で動く（R8〜R3 が 3-4/5、H31 が 4-5/6、H30 が 5-6/7、H29 が 3-4/5）。
    // ⚠⚠ **R8 は ToUnicode 欠落**（352フォント中328が uni=no）→ `decodeGarble`。**7字がマップ外**だったので
    //   `garble-decode.ts` に足した（項・明・細・書・ゴ・ル・フ＝表題「歳入歳出予算事項別明細書」と款8「ゴルフ場利用税交付金」。
    //   真の字は同じ表の R7 と突き合わせて自分で確定・§13-28）。**川西 H30〜H28 も同じ7字**で止まっていた。
    // ⚠ H29 は数字が制御文字の帯に化ける（漢字は正常）→ 同じく `decodeGarble`。
    // ⚠ 新設款の前年度セルが完全な空欄になる年がある（R3 款9・H31 款9 環境性能割交付金）→ `prevBlankAsZero`。
    // ⚠ **R2 は歳入款9「自動車取得税交付金」が当年度セル空欄・記号も「皆減」の語も無い廃止款**で、
    //   `[88,500, △88,500]` を [当年度, 前年度] と読んで当年度 Σ +88,500（大声で落ちる型）＝パーサ手当て待ち・記録しない
    //   （岸和田 R2・出雲/浦安 R2 と同族）。H28・H27 は年度ページ本文の HTML 表（款別・前年度当初列つき）で新様式。
    // 前年度列は**当初**。骨格予算: 年度ページ本文と概要に「骨格」「肉付」0件。
    // ライセンス: 「ホームページについて」（/about/homepage.html）はアクセシビリティと運営要領へのリンクだけで
    //   **著作権・転載の条項が無い**（偵察が全文を読んで0件）→ フッター表記のみを置いて **unverified**（確認日 2026-09-10）。
    //   オープンデータ（CC BY 4.0）は「本ページに掲載しているデータ」に自ら範囲を限り、登載5件に予算0件 → 及ばない（§9g）。
    //   リンク基準はリンク元サイトの内容要件だけで、トップページ指定・連絡の条項なし → `noDeepLink` は立てない。
    // [年度, 年度ページID, ファイルパス, 歳入from, 歳入to, 歳出p, 復号, 前年度空欄の款]
    ["R8", "1050187", "001/050/187/R8setumeishoippan.pdf", 3, 4, 5, true, null],
    ["R7", "1045502", "001/045/502/1037158-2/R7setumeisho_ippan.pdf", 3, 4, 5, false, null],
    ["R6", "1040853", "001/040/853/1037158-2/R6setsumeisho_ippan.pdf", 3, 4, 5, false, null],
    ["R5", "1037158", "001/037/158/1037158-2/R5setsumeisho_ippan.pdf", 3, 4, 5, false, null],
    ["R4", "1037098", "001/037/098/R4setumeisho_ippan.pdf", 3, 4, 5, false, null],
    ["R3", "1037100", "001/037/100/reiwa3yosansetsumeiippan.pdf", 3, 4, 5, false, 9],
    ["H31", "1037102", "001/037/102/08_H31_se_ippan.pdf", 4, 5, 6, false, 9],
    ["H30", "1037103", "001/037/103/08_H30_se_ippann.pdf", 5, 6, 7, false, null],
    ["H29", "1037108", "001/037/108/01.pdf", 3, 4, 5, true, null],
  ] as const).map(([fy, pid, path, rf, rt, ep, garble, blankKan]) => ({
    id: `noda-yosan-setsumeisho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 野田市予算に関する説明書（一般会計）（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "野田市",
    url: `https://www.city.noda.chiba.jp/_res/projects/default_project/_page_/${path}`,
    landingPage: `https://www.city.noda.chiba.jp/shisei/shisei/zaisei/1037088/${pid}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "野田市（一般会計・団体コード122084）",
    license: "Copyright © City Noda, All Rights Reserved.",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePages: { from: rf, to: rt },
      expenditurePage: ep,
      revenueHeading: "総括（歳入）",
      expenditureHeading: "（歳出）",
      ...(garble ? { decodeGarble: true } : {}),
      ...(blankKan == null ? {} : { prevBlankAsZero: { revenue: [blankKan] } }),
    },
  } satisfies SourceEntry)),

  ...([
    // 川西市（兵庫県・団体コード 282171）。「○年度 一般会計・特別会計予算説明書」（財政課）冒頭の
    //   「歳入歳出予算事項別明細書 １ 総括」（歳入23款・歳出13款・千円・列 `[本年度予算額, 前年度予算額, 比較]`・歳出は財源内訳4列）。
    //   R7・R6・R5・H31・H30〜H28 の7年（自分で7年の款名を全件目視・Σ4系統差0）。物理ページは年で動く（印字は物理−2）。
    // ⚠ H30・H29・H28 は ToUnicode 欠落 → `decodeGarble`。**野田 R8 と同じ7字**（項・明・細・書・ゴ・ル・フ）がマップ外だったので
    //   `garble-decode.ts` に足して開いた（§13-28）。⚠ H31 は新設款「環境性能割交付金」の前年度が空欄 → `prevBlankAsZero`。
    // ⚠⚠ **前年度列は「当初」だが毎年 組替後**（自分で款単位に突合）— 総額は毎年一致し歳入も全款一致するが、
    //   歳出の2〜5款が前年度資料の当年度と食い違う（R8 概要 p.28 は「ふるさとづくり寄附金事業を総務費から商工費に移管したことに伴い、
    //   令和７年度の予算の概要と289,669千円のずれがあります」と注記するが、R7・R6 の説明書・概要に注記は無い）。
    //   → **推定を書かず事実だけ prevNote に置く**（高岡 H26・§13-25 と同じ扱い）。
    // ⚠ 歳出の総括ページは同じ表が2ページ連続で複製される年がある（R8 p.12/p.13）→ 前の方を指定。
    // ⚠ 収録できなかった年度（偵察が実測）: R4 = 総括の歳入ページにテキスト層なし／R3・R2 = 総括ページがノンブル以外テキスト0・画像0
    //   （文字がアウトライン化）→ unrecordable。R8 は歳入「環境性能割交付金」が款番号なし・当年度セル空・印も「皆減」の語も無い廃止款で
    //   行ごと落ちる（前年度Σ −66,899 の warning）＝パーサ手当て待ち・記録しない。
    // 前年度列の基準は説明書 p.3「各会計別前年度比較表」の「前年度 当初予算額」と一致（偵察が実測）。骨格予算 0件（市長選は10月）。
    // ライセンス: サイトポリシー（/policy/site_policy.html・更新 令和8年8月7日・確認日 2026-09-10）。「複製・転用」で permission-required。
    //   オープンデータ（CC BY 2.1 JP）は「本ページに掲載しているデータ」に自ら範囲を限り、行財政18件に予算0件 → 及ばない（§9g）。
    //   リンクは「原則として自由にリンクしていただけます」＝トップページ指定も連絡もなし → `noDeepLink` は立てない。
    // [年度, ディレクトリ, ファイル, 年度ページ, 歳入p, 歳出p, 復号, 前年度空欄の款]
    ["R7", "001/021/134", "R7yosannsetumeisho0214.pdf", "1021134", 10, 12, false, null],
    ["R6", "001/019/058", "R6_yosansetumei_2_060321.pdf", "1019058", 11, 12, false, null],
    ["R5", "001/016/966", "R5yosansetumei2.pdf", "1016966", 9, 11, false, null],
    ["H31", "001/008/286", "H31yosansetsumeisyo.pdf", "1008286", 11, 12, false, 9],
    ["H30", "001/008/287", "H30yosannsetumeisyo.pdf", "1008287", 11, 12, true, null],
    ["H29", "001/008/292", "H29yosansetumeisyo.pdf", "1008292", 11, 12, true, null],
    ["H28", "001/008/311", "H28yosansetsumeisho.pdf", "1008311", 8, 9, true, null],
  ] as const).map(([fy, dir, file, page, rp, ep, garble, blankKan]) => ({
    id: `kawanishi-yosan-setsumeisho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 川西市一般会計・特別会計予算説明書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "川西市",
    url: `https://www.city.kawanishi.hyogo.jp/_res/projects/default_project/_page_/${dir}/${file}`,
    landingPage: `https://www.city.kawanishi.hyogo.jp/shiseijoho/1008858/zaisei/1008284/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "川西市（一般会計・団体コード282171）",
    license:
      "本ホームページで掲載している情報（文章・画像などの全てのコンテンツデータ）の著作権は、原則として川西市に帰属します（一部の画像等は原著作者が所有しています）。著作権法上認められている場合を除き、権利者の許可なく複製・転用・販売・改変といった二次利用をすることを固く禁じます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      ...(garble ? { decodeGarble: true } : {}),
      ...(blankKan == null ? {} : { prevBlankAsZero: { revenue: [blankKan] } }),
      ...(KAWANISHI_PREV_NOTES[fy] ? { prevNote: KAWANISHI_PREV_NOTES[fy] } : {}),
    },
  } satisfies SourceEntry)),

  ...([
    // 上田市（長野県・団体コード 202037。⚠ 長野県 200000・長野市 202011 とは別）。「当初予算の概要」（財政課）の
    //   p.3「１ 歳入」／p.4「２ 歳出（目的別）」。千円・列 `[本年度, 構成比, 前年度, 構成比, 比較増減額, 増減率]`。
    //   R8〜R2 の7年が同じ物理ページ・同じ opts（自分で7年の款名を全件目視・Σ4系統差0・6リンクを款単位で突合し不一致0）。
    // ⚠⚠ **cropX が必須** — 表の右に円グラフと凡例（`そ の 他 9.3%` / `※各種交付金 …`）が重なり、無指定だと凡例語が款名に食い込む。
    //   **Σ ゲートは半分しか守らない**: 歳入は款が丸ごと消えて Σ が割れる（止まる）が、**歳出は Σ 差0 のまま
    //   款3 が `教民生費`・款6 が `土木農林水産業費` になる**（静かに壊れる型・偵察が R8/R6/R5/R4 で実測）。
    //   実測の境界は歳入 545 / 歳出 548（A4 横 841.92pt）。**新年度を足すときは必ず款名を目視し直す**（座標指定なので版面が変われば破れる）。
    // ⚠ 見出し `１ 歳 入`／`２ 歳 出 （目的別）` と表ヘッダの `歳 入`／`歳 出` が同じページに2回出るが、
    //   heading は「見つけた行を読み飛ばす」実装なので `歳入`/`歳出` の指定で両方落ちる。
    // ⚠ 歳出ページの下の `≪参考≫ 当初予算額の推移`（H18〜R8・億円）は合計行より後ろなので款に入らない。
    // ⚠⚠ **R8・R4 は骨格予算**（原典 p.4 の脚注「※H22、H26、H30、R4、R8は骨格予算編成」）。
    //   当年度側が骨格の R8・R4 には prevNote を書かない（§11k・長崎 R8 型）。R8 の6月補正（肉付け）は +1,025,271千円。
    // ⚠⚠ **R4 が骨格なので R5 に `prevNote` が要る**（前年度側が骨格になる年度）。
    //   第30巡のレビューで指摘されるまで落としていた＝弘前 §13-25・朝霞 §13-30 と同じ取り違えの3例目。
    //   ⚠ 上田の資料には肉付け後の額が載らない（R5 の概要を `肉付` で全文検索して0件）ので、注記は
    //   「前年度額は骨格予算の当初額である」という事実だけにとどめる（推定の金額は書かない）。
    // ⚠ R2 歳入は廃止款「自動車取得税交付金」が款番号 `-`・当年度セル `-`＋「皆減」で出る（款6 法人事業税交付金は前年度が `-`＋「皆増」）。
    //   既存の皆増減処理で 0 として拾える（`dashAsZero` は不要）。R6〜R2 の歳出は13款（災害復旧費が入る）。
    // ⚠ 収録できなかった年度（自分で実測・`unrecordable.ts` に記録した）: **H27** = 概要（`attachment/1981.pdf`・89p）の
    //   テキスト層に日本語が1文字も無い（非 ASCII は記号9種だけ・本文がアウトライン化）。**H20** = 年度ページの
    //   「平成20年度予算」の節に「当初予算の概要」のリンクが無い（H22・H21・H19・H18 にはある）。
    // ⚠ 年度ページは全年度で共有（年度ごとの別ページが無い）。直リンクは CMS の attachment ID で連番規則なし＝機械生成しない。
    // ライセンス: 「リンク・著作権・免責事項」（/soshiki/uedapr/132245.html・更新 2026-06-23・確認日 2026-09-10）。「無断で、転載、複製」で permission-required。
    //   オープンデータ規約（CC BY 4.0）は「オープンデータサイトに掲載されているデータ」に自ら範囲を限り、CSV 106件に予算0件 → 及ばない（§9g）。
    //   リンクは「原則リンクフリー」「どのページでも自由」「申し出は特に必要ありません」＝依頼形 → `noDeepLink` は立てない。
    // [年度, attachment ID]
    ["R8", "79509"],
    ["R7", "67291"],
    ["R6", "56914"],
    ...
    id: `ueda-yosan-gaiyou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 上田市当初予算の概要（一般会計 歳入・歳出（目的別）・款別＋前年度当初比較）`,
    publisher: "上田市",
    url: `https://www.city.ueda.nagano.jp/uploaded/attachment/${aid}.pdf`,
    landingPage: "https://www.city.ueda.nagano.jp/soshiki/zaisei/1256.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "上田市（一般会計・団体コード202037）",
    license:
      "上田市ホームページの掲載情報は、日本国の著作権法及び国際条約による著作権保護の対象となっています。当ホームページの内容及びこれらの配置・編集などについて私的使用又は引用等著作権法上、認められた行為を除き、著作権は上田市又はその作成者に帰属しておりますので、これらを無断で、転載、複製、出版、放送、上映等を行うことはできません。また、引用を行う場合は、適宜の方法により、必ず出所を明示してください。なお、上田市ホームページの内容の全部または一部について、上田市に無断で改変を行うことはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: 3,
      expenditurePage: 4,
      revenueHeading: "歳入",
      expenditureHeading: "歳出",
      revenueCropX: { from: 0, to: 545 },
      expenditureCropX: { from: 0, to: 548 },
      ...(fy === "R5"
        ? {
            prevNote:
              "前年度（令和4年度）の当初予算は骨格予算（原典 p.4 の脚注に「※H22、H26、H30、R4は骨格予算編成」と明記）。" +
              "ここでの前年度額はその骨格予算の当初額（68,413,000千円）で、肉付け後の額ではない。" +
              "肉付け後の額は上田市の当初予算の概要には載っていない。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 東村山市（東京都・団体コード 132136）。「一般会計予算書」の「歳入歳出予算事項別明細書 １ 総括」。千円・列 `[当年度, 前年度, 比較]`。
    //   R8〜R4 の5年（R3 は予算書がスキャンなので次のブロックで「予算概要」から採る）。
    // ⚠ **年度で面付けが2種類**（同じ内容・同じ発行元）: R8・R7 は A4 2-up（1物理ページに印刷2枚）で歳入・歳出が各1ページに収まる。
    //   R6〜R4 は 1-up で**歳入だけ表が横に割れる**（p.14=款名＋本年度／p.15=前年度＋比較）→ `revenueSpread`。歳出は p.16 に揃う。
    // ⚠⚠ **HeaderExtra を側ごとに分ける**（無いと Σ 4系統差0 のまま款名が壊れる型・偵察が実測）:
    //   歳入は表題「歳入歳出予算事項別明細書」が款1 に前置 → `revenueHeaderExtra: "事項別明細書"`。
    //   歳出は財源内訳ヘッダ「一般財源」「国都支出金 地方債 その他」が款1 に前置 → `expenditureHeaderExtra: "一般財源|国都支出金"`。
    //   ⚠ 歳入側に「一般財源」を足すと款15/22 を壊すので**側で分ける**。
    // ⚠ 歳出は**款13 が存在しない**（12 諸支出金 → 14 予備費・全年度）。款6 は原典が **`農林業費`**（農林水産業費ではない）＝寄せない。
    // 前年度列は**当初**（基準の明記は無く、隣接5ペアの総額・款単位の一致で確定）。骨格予算 0件。
    // ⚠ URL は年度から外挿できない（2023年頃のサイト移行で `<年度ページのスラグ>_<元ファイル名>` に改名された年度がある）。
    // ⚠ R2 の予算書（`yosan_r2.files/1_2yosan_jikobetsu.pdf`・580,394B）は**スキャン画像**（自分で実測:
    //   pdftotext の可読文字0・埋め込みフォント0）。R2 以前は発行元の年度一覧から消えており Wayback にしか無い。
    //   ⚠ **代替の「予算概要」が使えるかは未確認**（2026-09-10 に確かめようとしたが Internet Archive が
    //   Temporarily Offline で CDX も replay も引けなかった）→ **理由が特定できていないので unrecordable には記録しない**
    //   （「理由が特定できていない年度は載せない」の規約どおり）。H31・H30 も同様に未確認のまま。
    // ライセンス: 「著作権・免責事項」（/shisei/keikaku/joho/aboutweb/mensekijikou.html・確認日 2026-09-10）。「無断使用・転載・引用は禁じます」で permission-required。
    //   オープンデータ利用規約（CC BY 2.1 日本）は規約自身が「本規約は東村山市公式ホームページ掲載のすべての情報に該当するものではありません」と範囲を限り、
    //   東京都カタログの東村山市45件に予算0件 → 及ばない（§9g）。リンクは「原則自由」＋フレーム禁止＋事後連絡の依頼 → `noDeepLink` は立てない。
    // [年度, ファイル名, 年度ページ, 歳入の指定, 歳出ページ]
    ["R8", "8yosan_an.pdf", "zaisei20251007", 8, 9],
    ["R7", "1_7yosan_jikobetsu.pdf", "zaisei20241003", 8, 9],
    ["R6", "1_6yosan_jikobetsu.pdf", "zaisei20231003", 0, 16],
    ["R5", "yosan_r5_1_5yosan_jikobetsu.pdf", "yosan_r5", 0, 16],
    ["R4", "yosan_r4_1_4yosan_jikobetsu.pdf", "yosan_r4", 0, 16],
  ] as const).map(([fy, file, page, rp, ep]) => ({
    id: `higashimurayama-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 東村山市一般会計予算書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "東村山市",
    url: `https://www.city.higashimurayama.tokyo.jp/shisei/gyozaisei/gyozaisei/yosan/files/${file}`,
    landingPage: `https://www.city.higashimurayama.tokyo.jp/shisei/gyozaisei/gyozaisei/yosan/${page}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "東村山市（一般会計・団体コード132136）",
    license:
      "東村山市ホームページ上の文書や画像等の各ファイル、及びその内容に関する諸権利は、原則として東村山市に帰属します。また、一部の画像などの著作権は原著作権者が所有しています。著作権法上認められた「私的使用のための複製」や「引用」などの場合を除き、東村山市の許可なく本ホームページ上の文書・画像等の各ファイルの無断使用・転載・引用は禁じます。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      ...(rp === 0 ? { revenueSpread: { namePage: 14, amountPage: 15 } } : { revenuePage: rp }),
      expenditurePage: ep,
      revenueHeading: "総括",
      revenueHeaderExtra: "事項別明細書",
      expenditureHeading: "歳出",
      expenditureHeaderExtra: "一般財源|国都支出金",
    },
  } satisfies SourceEntry)),

  {
    // 東村山市 R3 — 予算書（1_3yosan_jikobetsu.pdf）は 600dpi のスキャン画像（pdftotext 86バイト・偵察が実測）なので、
    //   同じ年度ページの「予算概要」から款別を採る（Ⅲ-1 歳入・款別内訳 p.19／Ⅳ-2 歳出・款別予算額 p.25・合計ラベルは「合計」）。
    // ⚠ 歳入表は款1 の下に市民税・固定資産税等の項行が挟まるが款名も Σ も汚さない（偵察が実測）。
    id: "higashimurayama-yosan-gaiyou-r3",
    title: "令和3年度 東村山市予算概要（一般会計 歳入・款別内訳／歳出・款別予算額・前年度当初比較）",
    publisher: "東村山市",
    url: "https://www.city.higashimurayama.tokyo.jp/shisei/gyozaisei/gyozaisei/yosan/files/zaisei20201005_3gaiyou.pdf",
    landingPage: "https://www.city.higashimurayama.tokyo.jp/shisei/gyozaisei/gyozaisei/yosan/zaisei20201005.html",
    kind: "pdf" as const,
    fiscalYear: "R3",
    scope: "東村山市（一般会計・団体コード132136）",
    license:
  ...
  ...([
    // 久喜市（埼玉県・団体コード 112321）。「○年度 久喜市一般会計予算に関する説明書」の「歳入歳出予算事項別明細書 １総括」。
    //   千円・列 `[当年度, 前年度, 増減]`。歳入 20〜22款（年で新設・廃止が動く）／歳出 14款。
    //   R8〜H25 の14年（自分で14年の款名を全件目視・Σ4系統差0・13リンクを款単位で突合し不一致0＝新設/廃止款の 0 だけ）。
    // ⚠ 見出しは**半角括弧**の `(歳入)`／`(歳出)`（原典は字間空きだがパーサが空白を畳む）。全角で書くと当たらない。
    // ⚠ 物理ページは3世代で違う（R8〜R2 = 3/4、H31 = 1/2〈表紙なし〉、H29〜H25 = 2/3）。印字ノンブルは年でバラバラ＝外挿しない。
    // ⚠⚠ **R4〜H25 は発行元から消滅**（旧パスは 404）。現行サイトの「令和4年度以前の予算」ページ自身が WARP を指しているので
    //   その捕捉（収集日 20240111・ts 20240109232246）を url・landingPage に使う。WARP 起点は魚拓の台帳対象外。
    // ⚠⚠ **R2 の廃止款は `廃自動車取得税交付金`**（`廃` の直後に空白が無いのでパーサの廃止款マーカーに当たらず款名に `廃` が残る。
    //   Σ は差0 のまま通るので目視でしか気づけない型）→ R2 だけ `kanNamePrefixStrip: "廃"`。
    // ⚠ **H30 の明細書は使えない**（テキスト層はあるが ToUnicode 欠落で化けの帯が ASCII 印字域の任意置換＝`decodeGarble` が素通りして
    //   金額が黙って壊れる型・偵察が実測）→ 同年の「予算の概要」で代替（次のブロック）。
    // 前年度列は**当初**（明細書の列見出しは「前年度予算額」としか書かないが、H30 概要は両列とも「当初予算額」と明記。13リンクの一致でも確定）。
    //   骨格予算: 概要13本で「骨格」0件。
    // ライセンス: サイトポリシー（/about/1006508.html・更新 2025-03-10・確認日 2026-09-10）。「無断で複製・転用」で permission-required。
    //   埼玉県オープンデータポータル（PDL1.0）は「当サイトで公開している情報」に自ら範囲を限り、久喜市の登載38件に予算0件 → 及ばない（§9g）。
    //   リンクは「自由に設定いただけます」「ご連絡いただく必要はありません」→ `noDeepLink` は立てない。
    // [年度, ファイル URL, landing URL, 歳入p, 歳出p]
    ["R8", "https://www.city.kuki.lg.jp/_res/projects/default_project/_page_/001/012/000/r8_jikoubetumeisaisyo.pdf", "https://www.city.kuki.lg.jp/shisei/zaisei/zaisei_yosan/1012000.html", 3, 4],
    ["R7", "https://www.city.kuki.lg.jp/_res/projects/default_project/_page_/001/009/855/r7_jikoubetumeisaisyo.pdf", "https://www.city.kuki.lg.jp/shisei/zaisei/zaisei_yosan/r7nendoyosan.html", 3, 4],
    ["R6", "https://www.city.kuki.lg.jp/_res/projects/default_project/_page_/001/005/568/1005568_003.pdf", "https://www.city.kuki.lg.jp/shisei/zaisei/zaisei_yosan/1005568.html", 3, 4],
    ["R5", "https://www.city.kuki.lg.jp/_res/projects/default_project/_page_/001/005/571/1005571_003.pdf", "https://www.city.kuki.lg.jp/shisei/zaisei/zaisei_yosan/1005571.html", 3, 4],
    ["R4", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/R4nenndoyosann.files/R4jikioubetumeisaisyo.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/R4nenndoyosann.html", 3, 4],
    ["R3", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/R3nenndoyosann.files/R3jikoubetumeisaisyo.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/R3nenndoyosann.html", 3, 4],
    ["R2", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/02tosho.files/R2jikobetsumeisaisho.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/02tosho.html", 3, 4],
    ["H31", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/01tousho.files/31jikoubetu.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/01tousho.html", 1, 2],
    ["H29", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/a1000200102901.files/2902jikoubetsu.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/a1000200102901.html", 2, 3],
    ["H28", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/28tousho.files/02jikoubetsumeisai.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/28tousho.html", 2, 3],
    ["H27", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h27yosan.files/02jikoubetsu.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h27yosan.html", 2, 3],
    ["H26", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h26yosan.files/02jikoubetsu.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h26yosan.html", 2, 3],
    ["H25", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h25yosan.files/02jikoubetsu.pdf", "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/h25yosan.html", 2, 3],
  ] as const).map(([fy, url, landingPage, rp, ep]) => ({
    id: `kuki-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 久喜市一般会計予算に関する説明書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "久喜市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "久喜市（一般会計・団体コード112321）",
    license:
      "久喜市ホームページに掲載されている文書や画像などの各情報に関する諸権利は、原則として久喜市に帰属します。また、一部の画像等の著作権は、原著作者が所有します。法令などに認められた場合を除き、無断で複製・転用することはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "(歳入)",
      expenditureHeading: "(歳出)",
      ...(fy === "R2" ? { kanNamePrefixStrip: "廃" } : {}),
    },
  } satisfies SourceEntry)),

  {
    // 久喜市 H30 — 明細書が ASCII 域の化け（上記）なので「平成30年度 久喜市予算の概要」の
    //   「（1）款別歳入」p.6／「（2）款別歳出」p.7 で代替。⚠ 列見出しが両年度とも「当初予算額」と明記されており prevBasis の直接の根拠。
    //   ⚠ 歳出ページの下に「（3）性質別歳出」が続くが款番号が無いので拾われない（偵察が実測・14款ちょうど）。
    id: "kuki-yosangaiyou-h30",
    title: "平成30年度 久喜市予算の概要（一般会計当初予算案 款別歳入・款別歳出＋前年度当初比較）",
    publisher: "久喜市",
    url: "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/11111.files/30gaiyou.pdf",
    landingPage: "https://warp.ndl.go.jp/20240111/20240109232246/http://www.city.kuki.lg.jp/shisei/zaisei_yosan_kessan/yosan/zaisei_yosan/11111.html",
    kind: "pdf" as const,
    fiscalYear: "H30",
    scope: "久喜市（一般会計・団体コード112321）",
    license:
    ...
    parserOptions: {
      ...
      expenditurePage: 7,
      revenueHeading: "款別歳入",
      expenditureHeading: "款別歳出",
    },
  },

  ...([
    // 小牧市（愛知県・団体コード 232190）。「当初予算の概要」冊子内の「一般会計当初予算款別表」
    //   （１ 歳入 / ２ 歳出）。列は [当年度当初予算額, 構成比, 前年度当初予算額, 構成比, 比較, 伸率] で、
    //   小数を除いた整数の先頭2つが [当年度, 前年度]＝既定の並び（prevColumnFirst 不要）。単位は千円。
    // ⚠ 見出しは `歳 入`／`歳 出`（全角スペース入り）。節ラベル `１ 歳    入` を見出しに指定すると
    //   その行が款行として読まれ款1 が `歳入市税`／`歳出議会費` になる。**Σ は4系統とも差0 のまま**通る。
    // ⚠ 款「国有提供施設等所在市町村助成交付金」が3行に割れるので `kanNameContinues` が要る。
    //   **款番号は年度で動く**（H29/H30=9・H31=10・R2〜R8=11）。ずれると款名が
    //   `国有提供施設等所在市町村`／`助成交付金地方特例交付金` に壊れ、これも Σ 差0 のまま通る。
    // ⚠ R8 だけ歳入・歳出が同一ページの左右2側（CropX で切る）。他年度は別ページ。
    // ⚠ 印字ノンブルと物理ページは年度ごとに +42〜+66 ずれる（冊子の抜粋のため）。物理ページで指定する。
    // ⚠ 同じ冊子の「一般会計予算額の推移等」PDF は1款が3行に割れる別様式で、既存パーサでは throw する。採らない。
    // ⚠ 「予算に関する説明書（総括）」は H30〜R7 が600dpi スキャン画像で使えない（R8 のみ born-digital）。
    //   款別表があるので実害なし。unrecordable には入れない（年度でなく資料の問題）。
    // 前年度列は**当初**（原典の列見出しが両列とも「当初予算額」・年度間クロスチェックが H29→R8 の全鎖で成立）。
    // 骨格予算: H30〜R8 の概要9本を `骨格|肉付` で全文検索して0件。
    // ⚠ H29 は発行元から消えており WARP（waid/11198）から採る。旧サイトのファイル名は無意味な連番。
    // ライセンス: 「サイトポリシー」（/admin/soshiki/shicokoshitsu/kouhou/media/1/2542.html・更新 2021-05-06・確認日 2026-09-10）。
    //   ⚠ 「無断」を使わず「著作権者の許可なく…利用することは禁止」と書くため `licenseClassOf` は `unverified` を返す
    //   （文意は permission-required 側。判定は人に委ねる＝未確認のまま置く）。
    //   オープンデータライブラリの CC BY 4.0 は「本ページで掲載しているデータ」に限る書き方で予算 PDF に及ばない（§9g）。
    // [年度, ファイル名, 年度ページID, 歳入p, 歳出p, kanNameContinues の款番号]
    ["R8", "R8toushoyosangaiyou3.pdf", "49378", 11, 11, 11],
    ["R7", "R702yosannogaiyou.pdf", "47332", 13, 14, 11],
    ["R6", "06-02toushoyosannogaiyou.pdf", "44045", 13, 14, 11],
    ["R5", "05-02toushoyosannogaiyou.pdf", "39195", 14, 15, 11],
    ["R4", "04-02tousyoyosannogaiyou.pdf", "35515", 14, 15, 11],
    ["R3", "03-02tousyoyosannogaiyou.pdf", "31451", 14, 15, 11],
    ["R2", "02-02tousyoyosannogaiyou.pdf", "26818", 14, 15, 11],
    ["H31", "H31gaiyo.pdf", "22650", 14, 15, 10],
    ["H30", "H30gaiyo.pdf", "17502", 12, 13, 9],
  ] as const).map(([fy, file, pageId, rp, ep, kn]) => ({
    id: `komaki-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 小牧市当初予算の概要（一般会計当初予算款別表・款別＋前年度当初比較）`,
    publisher: "小牧市",
    url: `https://www.city.komaki.aichi.jp/material/files/group/8/${file}`,
    landingPage: `https://www.city.komaki.aichi.jp/admin/shisei/shisei/zaisei/1/3/${pageId}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "小牧市（一般会計・団体コード232190）",
    license: KOMAKI_LICENSE,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      kanNameContinues: { revenue: [kn] },
      ...(fy === "R8"
        ? { revenueCropX: { from: 0, to: 420 }, expenditureCropX: { from: 430, to: 842 } }
        : {}),
    },
  } satisfies SourceEntry)),

  {
    // 小牧市 H29 — 発行元から消えているので WARP（waid/11198・2018-05 の捕捉）から。
    //   旧サイトのファイル名は無意味な連番（`11495675.pdf`）で年度から推測できない。
    id: "komaki-yosansho-h29",
    title: "平成29年度 小牧市当初予算の概要（一般会計当初予算款別表・款別＋前年度当初比較）",
    publisher: "小牧市",
    url: "https://warp.ndl.go.jp/20180524/20180523093048/http://www.city.komaki.aichi.jp/material/files/group/8/11495675.pdf",
    landingPage:
      "https://warp.ndl.go.jp/20180522/20180522123202/http://www.city.komaki.aichi.jp/admin/shisei/shisei/zaisei/1/3/12738.html",
    kind: "pdf" as const,
    fiscalYear: "H29",
    scope: "小牧市（一般会計・団体コード232190）",
    license: KOMAKI_LICENSE,
  ...
  ...([
    // 狭山市（埼玉県・団体コード 112151）。「○年度 狭山市一般会計予算に関する説明書」冒頭の
    //   「歳入歳出予算比較表」（歳入 p.3／歳出 p.4）が R8〜H26 の13年度で同型。単位は千円。
    //   列は [本年度予算額, 構成比, 前年度予算額, 構成比, 比較増減]（prevColumnFirst 不要）。
    // ⚠⚠ 同じ冊子の「歳入歳出予算事項別明細書 １ 総括」（p.11／p.12）を採ってはいけない —
    //   歳出側は財源内訳のヘッダが款1 に連結して `一般財源国県支出金地方債その他議会費` になり、
    //   **Σ は4系統とも差0 のまま通る**（validate も汚染ゲートも捕まえない）。比較表 p.3/p.4 を採る。
    // ⚠ 見出しは `歳入`／`歳出`。`歳入歳出予算比較表` にすると歳出ページ（表題行が無い）で throw する。
    // 前年度列は**当初**（隣接12ペアを款単位で突合し不一致0。差は新設・廃止款のみ＝
    //   環境性能割交付金 H31 新設・法人事業税交付金 R2 新設・自動車取得税交付金 R2 廃止・諸支出金 R4 廃止）。
    // 骨格予算: R8〜H30 の資料を `骨格|肉付` で grep して0件（H29 以前は未確認）。
    // ⚠ R2〜H26 は発行元から削除済み → WARP（waid/10792）。Wayback にも同一バイト数の写しがあるが
    //   **H26 だけ Wayback 側が 1 MiB 境界で打ち切られる**（§9b）ので WARP を正とする。
    // ⚠ H26 だけ資料の切り方が違い、比較表は予算書ファイル `0126yosan.pdf`（23p）の p.14/p.15。
    // ⚠ H31 は年度ページが「令和元年度（平成31年度）」だが**資料の表紙は「平成３１年度」** → H31 とする。
    // ⚠ R8 のランディングは `/yosan/3.html`（年度が変わると kako へ移動して URL が変わる）。
    // ライセンス: 「著作権・免責事項」（/about/menseki.html・更新 2020-03-21・確認日 2026-09-10）＝permission-required。
    //   狭山市のオープンデータは埼玉県ポータル（PDL1.0）に登載だが規約が範囲を当サイトに限り、
    //   登載20件に予算は無いので及ばない（§9g。`公共データ利用規約` を license に書くと open へ落ちる）。
    //   リンクについての条項はサイト内に存在しない → `noDeepLink` は立てない。
    // [年度, ファイル URL, ランディング URL, 歳入p, 歳出p]
    ["R8", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/3.files/01_R8hyoushimokujihikakuhyouetc.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/3.html", 3, 4],
    ["R7", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r7yosansyo.files/01_R7hyoushimokujihikakuhyouetc.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r7yosansyo.html", 3, 4],
    ["R6", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r6yosansho.files/01_R6hyoushimokujihikakuhyouetc.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r6yosansho.html", 3, 4],
    ["R5", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r5yosansyo.files/01_R5hyoushimokuji.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r5yosansyo.html", 3, 4],
    ["R4", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r4yosansyo.files/01_hyoushimokujisoukatsuhyoutou.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r4yosansyo.html", 3, 4],
    ["R3", "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r3yosansyo.files/R3_hyoushi.mokuji.hikakuhyou.pdf",
      "https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r3yosansyo.html", 3, 4],
    ["R2", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r2yosansyo.files/soukatutou.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/r2yosansyo.html", 3, 4],
    ["H31", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/31yosansyo.files/31ippan-hikaku.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/31yosansyo.html", 3, 4],
    ["H30", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/h30yosan.files/30ippan-hikaku.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/h30yosan.html", 3, 4],
    ["H29", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/29sayamashiyosan.files/2900ippan-hikakuhyo.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/29sayamashiyosan.html", 3, 4],
    ["H28", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/yosansho28.files/2803hyousimokujihikakuhyou.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/yosansho28.html", 3, 4],
    ["H27", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/27yosansho.files/27yosan03.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/27yosansho.html", 3, 4],
    ["H26", "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/yosan.files/0126yosan.pdf",
      "https://warp.ndl.go.jp/20220709/20220709002004/https://www.city.sayama.saitama.jp/shisei/zaisei/yosan/yosansho_kako/yosan.html", 14, 15],
  ] as const).map(([fy, url, landingPage, rp, ep]) => ({
    id: `sayama-yosan-setsumeisho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 狭山市一般会計予算に関する説明書（歳入歳出予算比較表・款別＋前年度当初比較）`,
    publisher: "狭山市",
    url,
    landingPage,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "狭山市（一般会計・団体コード112151）",
    license:
      "当サイトの著作権は原則として狭山市が所有しています。掲載している文章・写真・イラストなどの各々の情報及び各ページは、著作権法の保護の対象となる著作物であり、当サイトの全体についても編集著作物として著作権の対象となっています。私的使用のための複製や引用など、著作権法上認められた場合を除き、無断で複製・転用をすることはできません。著作権法上認められた範囲を超える引用・転用・転載などを希望する場合には、各ページ下部に記載された問合せ先に直接ご相談ください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
  ...
  ...([
    // 朝霞市（埼玉県・団体コード 112275）。「○年度 朝霞市一般会計予算及び予算説明書」冒頭の
    //   「歳入歳出予算事項別明細書 １ 総括表」。千円・列 `[本年度予算額, 前年度予算額, 増減, 対前年割合, 対予算総額割合]`。
    // ⚠⚠ **`revenueHeaderExtra: "総括表|割合"` が無いと歳入の款1 が
    //   `総括表割合対する割合市税` になる**（ページ冒頭の節見出しと多段ヘッダが連結する）。
    //   **Σ は4系統とも差0 のまま通る**ので目視でしか気づけない。⚠ 歳出側はヘッダの間に空行があるので
    //   指定すると逆に危ない ＝ **側で分ける**（東村山と同じ思想）。
    // ⚠ 物理ページは年度群ごとに動く（R8〜R4 が 18/19・R3 と R2 が 20/21・H31〜H26 が 15/16）。
    //   印字ノンブルとのズレも年度群で違う（+5 / +3 / 0〜-1）＝目次の数字を信じず実ページを開く。
    // ⚠ H31 は歳入款9「環境性能割交付金」の前年度セルが完全な空欄（`皆増` の語も `－` も無い）→ `prevBlankAsZero`。
    // ⚠⚠ **R2 は収録していない** — 歳入の廃止款「自動車取得税交付金」が行頭 `△` の印つきで、
    //   **当年度セルが完全な空欄**（`0` でも `-` でも `皆減` でもない）。ints が `[前年度, 増減]` の2個になり
    //   列が左に1つずれる。Σ が +40,000 / −80,000 で割れる（＝大声で落ちる型）ので収録不可ではなく
    //   **パーサ手当て待ち**（`prevBlankAsZero` の当年度版が要る）。⚠ **理由を特定できているので
    //   `unrecordable.ts` に `parser-unsupported` で記録した**（高知 R7/R6 と同じ扱い）。
    // ⚠ 採ってはいけないページ: 同じ冊子の「第１表 歳入歳出予算」（前年度列が無い。当てると throw する）と
    //   「一般会計当初予算資料」の予算概要（科目単位・前年度が肉付後の年度がある）。
    //   ⚠ ランディングに並ぶ「印刷用データ」の同名 PDF は**物理ページが -1 ずれる**ので使わない。
    // 前年度列は**当初**（12年の鎖を款単位で突合し、差は新設・廃止款だけ）。
    // ⚠⚠ **R7 は骨格予算なので R8 に `prevNote` が要る**（§11k）。総括表の前年度列は R7 の**当初**
    //   51,323,032（肉付後 52,100,011 ではない）ことを鎖で確かめたが、**前年度列が当初額であることと、
    //   その当初額が骨格であることは別**（弘前 §13-25 でレビューに指摘された取り違え）。発行元自身が
    //   「令和8年度（2026年度）一般会計当初予算資料」（`attachment/107763.pdf` の**印字 p.1＝物理 p.2**）で
    //   「※前年度予算については、当初予算が骨格予算のため、肉付後予算額（補正予算第１号）としています」
    //   と書き、対前年 +8.1% を出している。画面は当初どうしの +9.7% になるので、その差を prevNote で開示する。
    // ⚠ 年度ページの URL 規則は破れている（`reiwa8yosansyo` … `31yosansyo` … `heisei26nendo`）。ファイル ID も連番でない。
    // ライセンス: 「免責事項・著作権」（/site/userguide/copyright.html・更新 2018-04-01・確認日 2026-09-10）。「無断で複製・転用」で permission-required。
    //   オープンデータは埼玉県ポータルに登載（朝霞市17件）だが予算は0件・決算3件のみ → 及ばない（§9g。県規約の語を license に書かない）。
    //   リンクは「原則として自由」＝依頼形 → `noDeepLink` は立てない。
    // [年度, ファイル ID, 年度ページのスラグ, 歳入p, 歳出p]
    ["R8", "107761", "reiwa8yosansyo", 18, 19],
    ["R7", "100680", "reiwa7yosansyo", 18, 19],
    ["R6", "92144", "reiwa6yosansyo", 18, 19],
    ["R5", "84206", "reiwa5yosansyo", 18, 19],
    ["R4", "76413", "reiwa4yosansyo", 18, 19],
    ["R3", "69083", "reiwa3yosansyo", 20, 21],
    ["H31", "53490", "31yosansyo", 15, 16],
    ["H30", "47063", "30yosansyo", 15, 16],
    ["H29", "39148", "29yosansyo", 15, 16],
    ["H28", "32355", "28yosansyo", 15, 16],
    ["H27", "25672", "27yosansyo", 15, 16],
    ["H26", "19453", "heisei26nendo", 15, 16],
  ] as const).map(([fy, fileId, slug, rp, ep]) => ({
    id: `asaka-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 朝霞市一般会計予算及び予算説明書（歳入歳出予算事項別明細書 総括表・款別＋前年度当初比較）`,
    publisher: "朝霞市",
    url: `https://www.city.asaka.lg.jp/uploaded/attachment/${fileId}.pdf`,
    landingPage: `https://www.city.asaka.lg.jp/soshiki/7/${slug}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "朝霞市（一般会計・団体コード112275）",
    license:
      "朝霞市ホームページに掲載している内容（写真・イラスト・音声・動画及び記事、その他のデータ）の著作権は朝霞市に帰属します。（一部の画像等の著作権は、原著作者が所有しています）朝霞市ホームページの内容について、「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。使用許諾は、各ページ内に記載されたお問い合わせ先へ、事前にご相談ください。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      ...
      revenueTotalLabel: "歳入合計",
      expenditureTotalLabel: "歳出合計",
      revenueHeaderExtra: "総括表|割合",
      ...(fy === "H31" ? { prevBlankAsZero: { revenue: [9] } } : {}),
      ...(fy === "R8"
        ? {
            prevNote:
              "前年度（令和7年度）の当初予算は骨格予算（発行元の「令和8年度（2026年度）一般会計当初予算資料」p.1 に" +
              "「※前年度予算については、当初予算が骨格予算のため、肉付後予算額（補正予算第１号）としています」と明記）。" +
              "ここでの前年度額はその骨格予算の当初額（51,323,032千円）で、肉付け後の額（52,100,011千円）ではない。" +
              "発行元が公表している対前年比は肉付け後に対する +8.1% です。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 武蔵野市（東京都・団体コード 132039。⚠ 武蔵村山市 **132233**・西東京市 132292 とは別。132110 は小平市）。
    //   「○年度予算参考資料」（財務部財政課）の「○年度一般会計予算前年度比較」歳入／歳出（連番の別ページ）。
    //   千円・列 `[本年度, 前年度, 比較, 増減率, 構成比×2]`。合計ラベルは両側とも `計` 単独。
    // ⚠ 見出しが非対称 — 歳入ページだけ節タイトル「○年度一般会計予算前年度比較」を持ち、歳出は `(歳出)` だけ。
    //   側を入れ替えると見出し不在で throw する（無言の入替は起きない）。
    // ⚠⚠ **R6 は同じ様式の PDF が2本ある** — `R6sankou.pdf`（初版）と `R6sankou-2.pdf`（**修正後**）。
    //   令和6年3月26日の予算修正可決で 衛生費 7,053,640→6,959,092・予備費 100,000→194,548。
    //   **初版も Σ4系統とも差0 で完走する**（内部整合しているため）＝**検証ゲートでは見分けられない**。
    //   R7 の前年度列が修正後の値なので、**年度間クロスチェックだけが正版を決める** → 正は `R6sankou-2.pdf`。
    //   ⚠ R8 の `-2` は改訂ではなくただのファイル名（`R8sankou.pdf` は 404）。
    // ⚠ 同じ冊子の隣接ページ（目的別性質別構成・性質別前年度比較・主な増減理由）はいずれも当てると throw する。
    // ⚠ 歳出の款6 は原典が **`農業費`**（農林水産業費でも農林費でもない）＝寄せない。
    // ⚠ 印字ノンブルと物理ページはズレる（R6〜R8 と H27〜R5 は −3・H21〜H26 は −1）。物理ページで指定する。
    // 前年度列は**当初**（H21→R8 の18年度が1本の鎖で全款一致。R6 は修正後の値で R7 と一致）。骨格予算は0件。
    // ⚠ H20 は歳入・歳出が同一 A3 ページに横並びで、末尾に款番号を持たない廃止款
    //   `特別地方消費税交付金`（当年度0・前年度1）があるのに**この資料には「皆減」の語彙欄が無い**ため
    //   前年度 Σ が −1 で割れる（error で止まる型）＝**パーサ手当て待ち**。⚠ **理由を特定できているので
    //   `unrecordable.ts` に `parser-unsupported` で記録した**。
    // ⚠ 年度ページ ID は連番でない（H28〜H20 が 1010206→1010214 の**降順**という反転がある）＝機械生成しない。
    // ライセンス: 「このサイトについて／著作権について」（/about/1000036.html・更新 2025-08-25・確認日 2026-09-10）。
    //   「無断転載はご遠慮ください」で permission-required。「引用・転載・複製を行うことができます」という
    //   許容が先に立つ書き方だが**開ける側へ倒さない**（伊勢崎・草加・宝塚・厚木と同じ判断）。
    //   オープンデータ規約（政府標準利用規約2.0準拠）は自ら範囲を「本サイト上で公開しているオープンデータ」に限り、
    //   市カタログ240件・東京都カタログの t132039 24件のどちらにも予算・決算・財政は0件 → 及ばない（§9g）。
    //   リンクは「悪意によるものを除き、フリー」→ `noDeepLink` は立てない。
    // [年度, 年度ページ ID, ファイル名, 歳入の物理ページ]
    ["R8", "1053506", "R8sankou-2.pdf", 6],
    ["R7", "1049201", "R7sankou.pdf", 6],
    ["R6", "1046438", "R6sankou-2.pdf", 6],
    ["R5", "1041473", "R5sankou.pdf", 8],
    ["R4", "1035700", "R4sankou.pdf", 8],
    ["R3", "1031386", "R3sankou.pdf", 8],
    ["R2", "1026441", "R2sankou.pdf", 8],
    ["H31", "1022646", "31yosansankou.pdf", 8],
    ["H30", "1018458", "30yosansankou.pdf", 8],
    ["H29", "1015330", "29sannkou.pdf", 8],
    ["H28", "1010206", "28sannkou.pdf", 8],
    ["H27", "1010207", "sankou27.pdf", 8],
    ["H26", "1010208", "26-2.pdf", 7],
    ["H25", "1010209", "123456.pdf", 7],
    ["H24", "1010210", "24yosannsannkousiryou.pdf", 7],
    ["H23", "1010211", "16051-2.pdf", 7],
    ["H22", "1010212", "13249-2.pdf", 7],
    ["H21", "1010213", "10479-2.pdf", 7],
  ] as const).map(([fy, pageId, file, rp]) => ({
    id: `musashino-yosan-sankou-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 武蔵野市予算参考資料（一般会計予算前年度比較・款別＋前年度当初比較）`,
    publisher: "武蔵野市",
    url: `https://www.city.musashino.lg.jp/_res/projects/default_project/_page_/001/${pageId.slice(1, 4)}/${pageId.slice(4)}/${file}`,
    landingPage: `https://www.city.musashino.lg.jp/shiseijoho/zaisei/zaiseijokyo/yosangaiyou/${pageId}.html`,
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "武蔵野市（一般会計・団体コード132039）",
    license:
      "武蔵野市公式ホームページに掲載されている個々の情報(文字、写真、イラスト等)は著作権の対象となっています。また、武蔵野市公式ホームページ全体も編集著作物として、著作権の対象となっています。著作権は日本国著作権法および国際条約により保護されています。本サイトの内容の全部または一部については適宜の方法により出所を明示することにより、引用・転載・複製を行うことができますが、無断転載はご遠慮ください。また商用目的で複製する場合も予め武蔵野市秘書広報課までご連絡ください。本サイトの内容の全部または一部について、武蔵野市に無断で改変を行うことはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: rp + 1,
      revenueHeading: "一般会計予算前年度比較",
      expenditureHeading: "(歳出)",
      revenueTotalLabel: "計",
      expenditureTotalLabel: "計",
    },
  ...
  ...([
    // 各務原市（岐阜県・団体コード 212130）。「○年度 わかりやすい予算書・後半【主要事業・計数資料】」
    //   （R5 以前は「○年度予算の概要【別冊】」）巻末の計数資料にある
    //   「一般会計予算の款別比較（歳入）」「一般会計予算の歳出款別比較」。千円・
    //   列 `[当年度, 構成比, 前年度, 構成比, 増減額, 増減率]`（R6〜R8 は右端に「主な増減要因」欄が付く）。
    // ⚠⚠ **R8・R7・R6 は CropX が必須** — 右端の「主な増減要因」の文言に `（皆減）` が入る行があり、
    //   切らないと当年度が 0 に潰れる（Σ が歳入 −243,442／歳出 −8,462,207 で割れる＝大声で落ちる型）。
    //   境界は側でも年度でも違う（R8/R7 は歳入 557・歳出 588、R6 は 595/628）＝**流用しない**。
    // ⚠ 款「国有提供施設等所在市町村助成交付金」は款名が上下2行に割れる（全年度）→ `kanNameContinues`。
    //   R5 以前は株式等譲渡所得割交付金・ゴルフ場利用税交付金・交通安全対策特別交付金も同型で、
    //   **対象の款番号が年度で動く**（R8〜R6 は 10 のみ・R5〜R2 は 5/8/10/13・H31 は 5/7/8/10/13）。
    // ⚠ R2 の歳入は末尾に款番号の無い廃止款「自動車取得税交付金」（当年度 `－`・前年度 71,000・皆減）→ `kanNoless`。
    // ⚠⚠ **別冊子「予算書【附 予算説明書】」の「歳入歳出予算事項別明細書 １．総括」を採ってはいけない** —
    //   歳出 p.38 は **Σ4系統とも差0 のまま通るのに款11「災害復旧費」（0円）が原典から省かれて13款になる**
    //   （計数資料は14款）。Σ も汚染ゲートも捕まえない＝狭山（§13-29）・刈谷（§13-28）と同族の
    //   「同じ発行元に通る表と静かに壊れる表が両方ある」型の**3例目**。歳入 p.37 は左右2段組で Σ が割れる。
    // ⚠ 同じ冊子の隣「一般会計予算の歳出性質別比較」は款番号が無いので当てると throw する。
    // ⚠ R6/R5 のあいだで冊子の構成と款別表の様式が変わる（主な増減要因の追加）＝ opts を流用しない。
    // ⚠ 年度ページ ID もファイル名も規則が無い（R8 は `r8wakayo1`=後半なのに R7 は `r7wakayo5`=後半と番号の向きが逆）。
    // 前年度列は**当初**（冊子前半が「対前年度当初比」と明記・7鎖を款単位で突合し差は新設/廃止款のみ）。骨格予算0件。
    // ⚠ 8ファイル中7つが 1 MiB を超える → 魚拓の照合で §9b の打ち切りに当たらないか必ず sha256 を見ること。
    // ライセンス: 「著作権・リンク・免責事項」（/about/1005264.html・確認日 2026-09-10）。「無断で複製・転用」で permission-required。
    //   各務原市に自前のオープンデータページは無く（/opendata/ は404）、岐阜県カタログの各務原市17件に予算・決算は0件。
    //   県ポータルの規約も「本サイト」に自ら範囲を限る → 及ばない（§9g）。
    //   リンクは「原則自由」＋「トップページに設定してください」＝港区・北区と同じ型 → `noDeepLink` は立てない。
    // [年度, ファイル URL, ランディング URL, 歳入p, 歳出p]
    ["R8", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/026/747/r8wakayo1.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1026746/1026747.html", 51, 52],
    ["R7", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/023/886/r7wakayo5.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1023884/1023886.html", 52, 53],
    ["R6", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/020/324/r6wakayokouhan2.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1020323/1020324.html", 49, 50],
    ["R5", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/017/368/r5syuyou2.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1017826/1017368.html", 7, 8],
    ["R4", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/014/059/r4syuyou2.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1014059.html", 7, 8],
    ["R3", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/010/112/syuyou20210301.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1010112.html", 7, 8],
    ["R2", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/008/032/bessatsu_2020.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1008032.html", 7, 8],
    ["H31", "https://www.city.kakamigahara.lg.jp/_res/projects/default_project/_page_/001/008/034/bessatsu_2019.pdf",
      "https://www.city.kakamigahara.lg.jp/shisei/zaisei/1008030/1008034.html", 7, 8],
  ] as const).map(([fy, url, landingPage, rp, ep]) => {
    const newStyle = fy === "R8" || fy === "R7" || fy === "R6";
    const cropTo: Record<string, { rev: number; exp: number }> = {
      R8: { rev: 557, exp: 588 }, R7: { rev: 557, exp: 588 }, R6: { rev: 595, exp: 628 },
    };
    const tails: Record<string, number[]> = {
      R8: [10], R7: [10], R6: [10],
      R5: [5, 8, 10, 13], R4: [5, 8, 10, 13], R3: [5, 8, 10, 13], R2: [5, 8, 10, 13],
      H31: [5, 7, 8, 10, 13],
    };
    return {
      id: `kakamigahara-yosan-keisu-${fy.toLowerCase()}`,
      title: `${eraYear(fy)}年度 各務原市 一般会計予算の款別比較（計数資料・款別＋前年度当初比較）`,
      publisher: "各務原市",
      url,
      landingPage,
      kind: "pdf" as const,
      fiscalYear: fy,
      scope: "各務原市（一般会計・団体コード212130）",
      license:
        "各務原市公式ウェブサイトに掲載している内容（文書、画像などの各ファイルの情報）およびその内容に関する諸権利は、原則として各務原市に帰属します。各務原市公式ウェブサイトの内容について、「私的使用のための複製」や「引用」など著作権法上認められた場合を除き、無断で複製・転用することはできません。転用などをする場合は必ず各務原市へその旨をご連絡ください。",
      parser: "kofu-yosansho" as const,
      parserOptions: {
        revenuePage: rp,
        expenditurePage: ep,
        revenueHeading: newStyle ? "款別比較（歳入）" : "歳入",
        expenditureHeading: newStyle ? "歳出款別比較" : "歳出",
        revenueTotalLabel: "合計",
        expenditureTotalLabel: "合計",
        ...(newStyle
          ? { revenueCropX: { from: 0, to: cropTo[fy]!.rev }, expenditureCropX: { from: 0, to: cropTo[fy]!.exp } }
          : {}),
        kanNameContinues: { revenue: tails[fy]! },
        ...(fy === "R2" ? { kanNoless: true } : {}),
      },
    } satisfies SourceEntry;
  }),
  ...
  ...([
    // 今治市（愛媛県・団体コード 382027）。「令和○年度 今治市議会定例会（第２回）当初予算（記者発表資料）」
    //   巻末の「一般会計款別予算額調」（歳入・歳出が別ページ）。千円・
    //   列 `[当年度当初, 構成比, 前年度当初, 構成比, 増減額, 増減率]`（右端に空の備考列）。
    // ⚠ 物理ページは年度でバラバラ（R8 48/49・R7 46/47・R6 48/49・R5 34/35・R4 35/36・R3 25/26）＝外挿しない。
    // ⚠ 見出しは原典が字間スペース入り（`一 般 会 計 款 別 予 算 額 調`）だが、パーサが空白を畳んでから照合するので当たる。
    // ⚠⚠ **同じ内容が「当初予算資料【概要版】」（/zaisei/gaiyou/）にもあるが採らない** —
    //   R6〜R4 の概要版は円グラフの凡例が款名欄に食い込み、**歳出の款5 が `衛生費労働費` になるのに
    //   Σ は4系統とも差0 のまま通る**（偵察が実測）。狭山・刈谷・各務原と同族。R8/R7 の概要版は
    //   字間が割れて `24 ,0 7 3,0 00` になる別の壊れ方。**記者発表資料は無指定で素通りする**ので迷わない。
    // ⚠ 予算書（一般会計）の総括は R3 以前が OCR スキャンで、テキストは出るが数字が化ける。使わない。
    // 前年度列は**当初**（列見出しが両年度とも `当初予算(A)`／`当初予算(B)`＋5リンクを款単位で突合し全一致）。
    // ⚠⚠ **R3 が骨格予算**（記者発表 p.3「令和３年度は、骨格予算を基本に…具体的な事業は、今後の定例会等で
    //   予算化を行っていく」＝自分で原文を読んだ）。当年度側が骨格の R3 には書かず、**前年度側が骨格になる R4 に
    //   `prevNote`** を置く（§11k・大垣 §13-27 と同じ扱い。弘前 §13-25 でレビューに指摘された取り違えを繰り返さない）。
    // ⚠ 歳出の款数が年で違う（R8/R7 は14款＝13 諸支出金・14 予備費／R6〜R3 は13款＝諸支出金なし）。
    // ⚠ **R2〜H26 は別資料「財政の概要」（Wayback）で通せることを偵察が実測したが採っていない** —
    //   歳入表が法定の款順でなく「自主財源／依存財源」の類型別で**款番号が印字されない**うえ、
    //   `小計` 行を落とすのに `revenueHeaderExtra` を本来の用途外で流用することになるため。
    //   H24 は歳出ページの二重描画で Σ が桁違いに割れる。いずれも判断待ちで unrecordable には記録しない。
    // ⚠ R5 以前のランディングは `yosan/back.html`。**年度が変わると R6 がそちらへ落ちる**ので次年度に見直す。
    // ライセンス: 「今治市公式サイトについて」（/about/about.html・確認日 2026-09-10）。「無断転載」で permission-required。
    //   ⚠ 文意は「出所明示すれば引用・転載・複製は可、無断転載はご遠慮ください」＝依頼形だが**開ける側へ倒さない**。
    //   オープンデータ規約（CC BY 2.1 JP）は範囲を「本ページで公開しているデータ」に自ら限り、
    //   カタログ25件に予算 PDF は無い → 及ばない（§9g）。リンクは「悪意によるものを除き、自由」→ `noDeepLink` なし。
    // [年度, ファイルパス, 歳入p, 歳出p, ランディングが back.html か]
    ["R8", "R8/R8_tosho_siryo02.pdf", 48, 49, false],
    ["R7", "R7/R07_tosho_siryo.pdf", 46, 47, false],
    ["R6", "R6/R06_tosho_siryo.pdf", 48, 49, false],
    ["R5", "R5/R05_tosho_siryo.pdf", 34, 35, true],
    ["R4", "R4/R04_tosho_siryo02.pdf", 35, 36, true],
    ["R3", "R3/R0303_tokubetu_siryo2.pdf", 25, 26, true],
  ] as const).map(([fy, file, rp, ep, back]) => ({
    id: `imabari-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 今治市当初予算（記者発表資料）一般会計款別予算額調（款別＋前年度当初比較）`,
    publisher: "今治市",
    url: `https://www.city.imabari.ehime.jp/zaisei/yosan/${file}`,
    landingPage: back
      ? "https://www.city.imabari.ehime.jp/zaisei/yosan/back.html"
      : "https://www.city.imabari.ehime.jp/zaisei/yosan/",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "今治市（一般会計・団体コード382027）",
    license:
      "今治市公式ホームページに掲載されている個々の情報（文字、写真、イラスト等）は著作権の対象となっています。また、今治市公式ホームページ全体も編集著作物として、著作権の対象となっています。著作権は日本国著作権法および国際条約により保護されています。本サイトの内容の全部または一部については適宜の方法により出所を明示することにより、引用・転載・複製を行うことができますが、無断転載はご遠慮ください。また商用目的で複製する場合も予め今治市秘書広報課までご連絡ください。本サイトの内容の全部または一部について、今治市に無断で改変を行うことはできません。",
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
      revenueHeading: "一般会計款別予算額調",
      expenditureHeading: "一般会計款別予算額調",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
      ...(fy === "R4"
        ? {
            prevNote:
              "前年度（令和3年度）の当初予算は骨格予算（発行元の「令和3年度 今治市議会定例会（第2回）当初予算」" +
              "記者発表資料 p.3 に「令和３年度は、骨格予算を基本に…具体的な事業は、今後の定例会等で予算化を行っていく」と明記）。" +
              "ここでの前年度額はその骨格予算の当初額（74,170,000千円）で、肉付け後の額ではない。",
          }
        : {}),
    },
  } satisfies SourceEntry)),
  ...
  ...([
    // 多摩市（東京都・団体コード 132241。⚠ 奥多摩町 133086 とは別）。「○年度 一般会計予算書」の
    //   「歳入歳出予算事項別明細書 １ 総括」。千円・列 `[本年度予算額, 前年度予算額, 比較]`。歳入23款／歳出14款で不変。
    // ⚠ 物理ページは R8〜R5 が 23/24、**R4 だけ 25/26**（R4 の予算書は2分冊で総括は分冊1）。印字ノンブルとのズレは無い。
    // ⚠ **R8・R7 のファイル名は `-1` 付き**（`08ippan.pdf` は 404・`08ippan-1.pdf` が実体）＝年度から外挿しない。
    // ⚠ 同じ冊子の「第１表 歳入歳出予算」（p.12〜16）は款と項が同じ表に並び前年度列が無い。当てると throw する。
    //   多摩には「Σ 差0 のまま壊れる第2の表」は見つからなかった（総括の歳出側に財源内訳ヘッダが無い）。
    // ⚠ 歳出の款6 は原典が **`農林業費`**（農林水産業費ではない）＝寄せない。款11 が災害復旧費・13 が諸支出金・14 が予備費。
    // 骨格予算: 収録6本（**予算書5本 R8〜R4 ＋ R3 の予算の概要**）を `骨格|肉付` で全文検索して **0件**（自分で実測）。
    //   ⚠ `市長選` は R8 に10件・R7 に19件・R5 に1件・R4 に4件出るが、**すべて款項目の `4 市長選挙費`
    //   `162 市長選挙執行経費` そのもの**で骨格の記述ではない。
    //   ⚠ 他市の前例（弘前=5月補正概要・大垣=当初予算の概要・今治=記者発表・朝霞=当初予算資料）では
    //   **骨格の明記は予算書の外にある**。**R8〜R4 の「予算の概要」は未確認**（R3 の概要は収録済みで0件）。
    // 前年度列は**当初**（資料に明記が無いので隣接5ペアを款単位で突合して確定。加えて CC BY のオープンデータ CSV を
    //   款集計した R7 の 68,550,000千円 が予算書の当年度列と全款一致することでも裏付けた）。
    // ⚠⚠ **R2 以前は回収できない**（偵察が実測）: 発行元から削除済み・**総括が載る分冊1 だけ Wayback に捕捉が無い**
    //   （分冊2〜4 は有る）・**WARP は多摩市（waid/10931）が館内限定**で pywb が 403
    //   「Available only within the facilities of the National Diet Library」を返す。⇒ この自治体は WARP 経由で回収できない。
    // ライセンス: 「多摩市ウェブサイトの考え方／著作権について」（/about/1008006.html・更新 2023-03-16・確認日 2026-09-10）。
    //   「無断で複製・転用」で permission-required。オープンデータの CC BY 4.0 は
    //   「予算・決算についての資料(オープンデータ)」ページの **CSV にだけ**付いており予算書 PDF に及ばない（§9g）。
    //   ⚠ その CSV は**ページに「千円単位」と書いてあるが実データは円**（練馬型。使うときは 1000 で割る）。
    //   リンクは「公序良俗に反しない限り自由・事前の承諾は不要」→ `noDeepLink` は立てない。
    // [年度, ファイル名, 歳入p, 歳出p]
    ["R8", "08ippan-1.pdf", 23, 24],
    ["R7", "07ippan-1.pdf", 23, 24],
    ["R6", "06ippan.pdf", 23, 24],
    ["R5", "05ippankaikeiyosansyo.pdf", 23, 24],
    ["R4", "0401_ippan_p1-287.pdf", 25, 26],
  ] as const).map(([fy, file, rp, ep]) => ({
    id: `tama-yosansho-${fy.toLowerCase()}`,
    title: `${eraYear(fy)}年度 多摩市一般会計予算書（歳入歳出予算事項別明細書 総括・款別＋前年度当初比較）`,
    publisher: "多摩市",
    url: `https://www.city.tama.lg.jp/_res/projects/default_project/_page_/001/004/897/${file}`,
    landingPage: "https://www.city.tama.lg.jp/shisei/zaisei/yosan/1004897.html",
    kind: "pdf" as const,
    fiscalYear: fy,
    scope: "多摩市（一般会計・団体コード132241）",
    license: TAMA_LICENSE,
    parser: "kofu-yosansho" as const,
    parserOptions: {
      revenuePage: rp,
      expenditurePage: ep,
  ...
  {
    // 多摩市 R3 — 予算書は発行元から消えている（分冊1 は Wayback にも WARP にも無い）ので、
    //   現行サイトに生きている「令和3年度 予算の概要 第2章」の「款別予算額【歳入】」p.4／「【歳出】」p.12 から採る。
    // ⚠ 款10「国有提供施設等所在市町村助成交付金」は款名が中央寄せ3行に割れるが既定で連結できる。
    // ⚠ 原典は歳出の前年度（R2）災害復旧費を `17522` とカンマ無しで印字するが、増減額 △17,518 と整合する。
    // 前年度列は当初（R4 予算書の前年度列＝本表の当年度列が23款/14款すべて一致）。
    id: "tama-yosan-gaiyou-r3",
    title: "令和3年度 多摩市予算の概要 第2章（一般会計 款別予算額・款別＋前年度当初比較）",
    publisher: "多摩市",
    url: "https://www.city.tama.lg.jp/_res/projects/default_project/_page_/001/004/900/03yosannnogaiyou2.pdf",
    landingPage: "https://www.city.tama.lg.jp/shisei/zaisei/yosan/1004900.html",
    kind: "pdf" as const,
    fiscalYear: "R3",
    scope: "多摩市（一般会計・団体コード132241）",
    license: TAMA_LICENSE,
    ...
    parserOptions: {
      revenuePage: 4,
      expenditurePage: 12,
      revenueHeading: "款別予算額【歳入】",
      expenditureHeading: "款別予算額【歳出】",
      revenueTotalLabel: "合計",
      expenditureTotalLabel: "合計",
    },
  },
