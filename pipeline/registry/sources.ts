...
export const SOURCES: SourceEntry[] = [
  ...
  ...([
    ...
    //   `HeaderExtra` に `^助成交付金$` を足して断片を捨て、**款10 は `国有提供施設等所在市町村` に切れたまま**にしている
    //   （八尾 §13-13 と同型の既知の切れ。パーサを1行変えれば完全形が取れる見込みだが、この巡では見送った）。
    ...
    parserOptions: {
      ...
      ...(fy === "R7"
        ...
        : {}),
      ...(fy === "R3"
        ? { prevNote: "前年度（令和2年度）の当初予算は、市長選挙を控えた骨格予算として編成されている（原典「令和2年度当初予算 主要事務事業説明資料」p.1）。前年当初比はその骨格予算との比較。" }
        : {}),
      ...(fy === "R2"
        ? { prevNote: "本年度（令和2年度）の当初予算は、市長選挙を控えた骨格予算として編成されている（原典 p.1）。" }
        : {}),
    },
  })),
  ...
  ...([
    ...
    // ⚠ **R4・H25 は歳入款9/10 の折返し下段 `助成交付金` が次の款に漏れる**（`助成交付金地方特例交付金`）。
    //   **Σ は差0 のまま**なので `revenueHeaderExtra` で断片を捨て、**款名は切れたまま**にしている
    //   （八尾 §13-13・松本 R4/R3 と同型）。⚠ **R6・R5 は同じ款が1行に収まっている**ので
    //   `助成交付金` を入れると**款行ごと消えて Σ −781,200 になる**。**年度ごとに指定を変える**。
    // ⚠ **R7 だけ歳入に `うち ○ 臨時財政対策債 … 皆減` の内訳行がある**（`皆減` で廃止款の分岐に乗り
    //   前年度Σ +500,000）。R8・R6・R5 では no-op だが全年度に入れてある。
