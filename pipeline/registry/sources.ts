...
export const SOURCES: SourceEntry[] = [
  ...
  },
  {
    ...
    //    執行額を持たない施策は `cost: []`（原典に無いものを推計しない）。**418施策中406件が執行額を持つ**。
    ...
    parserOptions: {
      ...
      corruptPages: 4,
      // ⚠ **執行額が本文ではなく下部の表に載る施策**（表の構造が施策ごとに違い、1つの正規表現では
      //   取れない）。**金額を推計で埋めるより載せないほうを採り、件数を宣言して開示する**:
      //     p.262 地域密着型社会資本整備事業（路線別の表）/ p.269 高齢者向け優良賃貸住宅供給促進事業
      //     p.320 国直轄災害復旧事業（事業名別の表）/ p.261 国直轄港湾事業（**本文だけが破損**＝
      //     名前欄は健全なので施策自体は残す。上の corruptPages は名前欄で判定するため4頁に入らない）
      execMissedAllowed: 4,
    },
  },
  {
