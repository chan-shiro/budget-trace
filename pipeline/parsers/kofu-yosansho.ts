...
/**
 * **廃止款のマーカー**（行頭に立ち、款番号の代わりに置かれる印）。
 *
 * ⚠⚠ **検出側と掃除側で必ず同じものを使う**ため1か所に置いてある。2026-08-23 に
 * **掃除側にだけ `○` があり検出側に無い**（秋田 R2 が行ごと落ちて前年度Σ −90,372）という
 * 事故を踏んだ。**片方だけ広げない**。
 *
 * ⚠ 記号は自治体・年度ごとに揺れ、**同じ市の別年度でも変わる**（葛飾 R2 は `○` U+25CB・R4 は `〇` U+3007）。
 * これまでに実物で出たもの: `廃款`（甲府）／`△` `▲`（複数）／`〇` `○`（葛飾・秋田）／
 * `(廃)` `（廃）`（市原）／**`×` ＋ ピリオド（つくば R2）**。
