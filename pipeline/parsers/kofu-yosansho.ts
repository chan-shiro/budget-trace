...
interface Options {
   ...
   *   罫線は列区切りを兼ねる字なので、消すと**列見出し行**が `本年度予算額前年度予算額` と連結する。
   *   ⚠ 款行は罫線の右に空白があるので削除しても変わらない（収録レビューで実測）— 影響は列見出し行だけ。
 ...
 * ⚠ **アスタリスク `＊`（U+FF0A）と `*`**（2026-09-03・岸和田 R2 `＊ 自動車取得税交付金 0 85,000 △85,000`）— 「皆減」の語も無く
 *   行ごと落ちて前年度 Σ が −85,000 になっていた（validate では warning 止まり）。全角・半角をまとめて入れる（§9c）。
 ...
 * ⚠ **`（廃止）` も足した**（2026-09-25・岩国 R2 `自動車取得税交付金（廃止）  －  －  79,000 … 皆減`）。
 *   行は `皆減` で廃止款の分岐に入るので Σ は合っていたが、**款名に `（廃止）` が残って H31 の同じ款と名前が割れていた**
 *   （表示専用なのでどのゲートも鳴らない）。`廃款` を足したときに `廃止` も入れておくべきだった＝§9c の「文字クラスごと広げる」。
 */
const TAIL_ABOLISHED_MARK_RE = /[（(]\s*廃(?:款|止)?\s*[）)]/;
...
function parseKanPage(
  ...
  for (let li = 0; li < allLines.length; li++) {
    ...
    // **款番号の直後が「.7%」のような小数の百分率なら款行ではない**（2026-09-10・栃木 R4）。
    // 円グラフの中心ラベル `8.7%   671 億 9 千万円` が同一行に組まれると、行頭の `8` が款番号に、
    // `671` `9` が金額に取られて**14 行目の偽の款**になる（Σ が +671／+9 ずれるのでゲートは捕まえるが
    // その年度だけ収録できない）。款番号の直後には必ず款名（日本語）が来るので、小数の百分率が
    // 続く行は款行ではないと判定してよい。⚠ 整数の百分率は `stripPercents` が別途落とす。
    if (lead && /^[.．]\d+\s*[%％]/.test(rawForLead.slice(lead[0].length))) continue;
    // **款名末尾の脚注番号 `(※1)` を金額化の前に落とす**（2026-09-10・栃木）。
    // 栃木の歳入は款名の末尾に脚注の参照番号が付く（`1 市税(※1)  22,760,423 …`）。ここで落とさないと
    // **`1` が第1金額として読まれ**、その款だけ列が1つずれる（栃木は9年度すべての歳入がこれで落ちる）。
