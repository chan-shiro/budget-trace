...
// 歳入内訳ファイルの科目。名前は資料表記のまま。年度による科目の増減・改称に備えて
// 過去年度の名称も併記する（R2: 特別地方消費税交付金あり・「地方特例交付金」に等なし）
const REVENUE_COLUMNS = [
  "地方税", "地方譲与税", "利子割交付金", "配当割交付金", "株式等譲渡所得割交付金",
  "分離課税所得割交付金", "地方消費税交付金", "ゴルフ場利用税交付金", "特別地方消費税交付金",
  "自動車取得税交付金", "軽油引取税交付金", "自動車税環境性能割交付金", "法人事業税交付金",
  "地方特例交付金等", "地方特例交付金",
...
function parseOneFile(
  ...
  // 実質公債費比率・将来負担比率は「健全化判断比率」結合セルの2段目にある
  const colOfH1 = (name: string): number => {
    const i = colOf(name);
    if (i >= 0) return i;
    for (let c = 0; c < refStart; c++) {
      if ((h1[c] ?? "") === name) return c;
    }
    return -1;
  };
  const jkhCol = colOfH1("実質公債費比率");
  const sfhCol = colOfH1("将来負担比率");
