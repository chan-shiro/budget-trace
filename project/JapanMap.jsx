...
// props: onSelectMuni(prefName, muniName|null), onSelect(prefName)(フォールバック), colorMode('カラフル'|'青系')
const SVG_URL = 'https://cdn.jsdelivr.net/gh/geolonia/japanese-prefectures@master/map-full.svg';
const MUNI_URL = (code) => `https://cdn.jsdelivr.net/gh/smartnews-smri/japan-topography@main/data/municipality/geojson/s0010/N03-21_${String(code).padStart(2, '0')}_210101.json`;
const ACCENT = '#1798D0';
...
const AVAILABLE_PREF = { '山梨県': true };
const AVAILABLE_MUNI = { '山梨県:甲府市': true };

function regionOf(code) { return REGION_DEFS.find(r => code >= r.from && code <= r.to); }
function shade(hex, f) {
  const n = parseInt(hex.slice(1), 16);
  const c = ch => Math.max(0, Math.min(255, Math.round(ch * f)));
  return '#' + ((c(n >> 16) << 16) | (c((n >> 8) & 255) << 8) | c(n & 255)).toString(16).padStart(6, '0');
}

// GeoJSON → { munis, vw, vh, inset, areas }
// ・東京23区のみ「エリア」としてまとめ、クリックでもう一段ズーム（政令市の区は直接選択）
// ・東京都の島しょ部（伊豆諸島・小笠原諸島）、鹿児島県の奄美群島は左の枠に別縮尺で区切り、これもズーム可
function buildMunis(geo, prefName) {
  ...
  // リング単位に展開＋muni単位のメタ情報（bb = 経緯度バウンディングボックス）
  const recs = [];
  const meta = {}; // code → {name, city, minLat, maxLat, ...}
  feats.forEach(f => {
  ...
  const W = 800, H = 620;
  // 島しょエリア判定（muni単位：同一自治体が本土／島に分裂しないように）
  // 島しょエリア判定（基本はmuni単位。「所属未定地」など境界をまたぐものはリング単位で判定）
  const islandAreaOf = (code, bb) => {
    const mm = meta[code];
    // 名前ベース（自治体全体が離島のもの）
  ...
  // 本土側のエリア判定（東京23区・北海道の振興局・政令市の区）
  const mainAreaOf = (code) => {
    const mm = meta[code];
    if (prefName === '東京都' && /区$/.test(mm.name)) return '特別区（23区）';
    if (mm.city && /市$/.test(mm.city) && /区$/.test(mm.name)) return mm.city; // 政令指定都市
    if (prefName === '北海道' && mm.sub) return mm.sub.replace(/総合振興局$|振興局$/, '');
    return null;
  };
  const islandRecs = [], mainRecs = [];
  recs.forEach(r => (islandAreaOf(r.code, r.bb) ? islandRecs : mainRecs).push(r));

  const byCode = {};
  const projectInto = (list, x0, y0, w, h, pad) => {
    let minLon = 999, maxLon = -999, minLat = 999, maxLat = -999;
    list.forEach(r => {
    ...
    const Xn = lon => ox + (lon - minLon) * kx * s;
    const Yn = lat => oy + (maxLat - lat) * s;
    list.forEach(rc => {
      ...
      const pts = rc.ring.map(([lon, lat]) => [Xn(lon), Yn(lat)]);
      m.d += 'M' + pts.map(p => p[0].toFixed(1) + ',' + p[1].toFixed(1)).join('L') + 'Z';
      pts.forEach(([x, y]) => {
        if (x < m.bx0) m.bx0 = x; if (x > m.bx1) m.bx1 = x;
        if (y < m.by0) m.by0 = y; if (y > m.by1) m.by1 = y;
      });
      // 最大リングの重心をラベル位置に（面積の重心）
      let a = 0, cx = 0, cy = 0;
  ...
  if (islandRecs.length) {
    ...
    const degA = r => (r.bb.maxLon - r.bb.minLon) * (r.bb.maxLat - r.bb.minLat);
    const ctr = r => [(r.bb.minLon + r.bb.maxLon) / 2, (r.bb.minLat + r.bb.maxLat) / 2];
    keys.forEach((k, i) => {
      // 町村ごとに1セル。最大の島の近傍リングのみ描画（硫黄島・沖ノ鳥島・南鳥島などの遠隔孤島は省略）
      const g = groups[k];
  ...
  const munis = Object.values(byCode).sort((a, b) => String(a.code).localeCompare(String(b.code)));
  const areasByName = {};
  munis.forEach((m, i) => {
    m.area = mainAreaOf(m.code); // 島しょ部はエリアズームなし（ラベルを直接表示）
    m.avail = !!AVAILABLE_MUNI[prefName + ':' + m.name];
    m.idx = i;
    m.fs = Math.max(9.5, Math.min(15, Math.sqrt(m.maxA) / 8)); // 面積に応じたラベルサイズ
    if (m.area) {
  ...
  const areas = Object.values(areasByName).map(a => ({ name: a.name, x: a.x0, y: a.y0, w: a.x1 - a.x0, h: a.y1 - a.y0, isInset: a.isInset }));
  return { munis, vw: W, vh: H, inset, areas };
}

function JapanMap({ onSelect, onSelectMuni, colorMode, maxWidth }) {
  const vivid = colorMode !== '青系';
  const boxRef = React.useRef(null);
  const svgRef = React.useRef(null);
  const zoomRef = React.useRef(null);
  const nodesRef = React.useRef([]);
  const muniCache = React.useRef({});
  const [status, setStatus] = React.useState('loading');
  const [query, setQuery] = React.useState('');
  const [region, setRegion] = React.useState(null);
  const [pref, setPref] = React.useState(null);          // {code, name, region}
  const [muniView, setMuniView] = React.useState(null);  // {munis, vw, vh, inset, areas} | 'loading' | 'error'
  const [muniZoom, setMuniZoom] = React.useState(null);   // {name, tx, ty, s} — エリア（政令市・23区・島しょ）への段階ズーム
  const [hover, setHover] = React.useState(null);        // {type, name, code}
  const [zoomT, setZoomT] = React.useState({ tx: 0, ty: 0, s: 1 });
  // タッチ端末：市区町村は「1回目のタップで名前表示→も2回目で決定」の2段階選択
  const isTouch = typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
  const armedRef = React.useRef(null);
  const [muniAnim, setMuniAnim] = React.useState('in'); // 'in' | 'out'
  const outTimerRef = React.useRef(null);

  const selMuni = (prefName, muniName) => {
    if (onSelectMuni) onSelectMuni(prefName, muniName);
    else if (onSelect) onSelect(prefName);
  };

  const paint = React.useCallback((reg, hov) => {
    nodesRef.current.forEach(n => {
      ...
      n.el.style.strokeWidth = sw; n.el.style.opacity = op;
  ...
  React.useEffect(() => {
    ...
    fetch(SVG_URL)
      ...
      .then(text => {
        ...
        const nodes = [];
        g.querySelectorAll('[data-code]').forEach(el => {
          ...
          el.querySelectorAll('path,polygon,rect').forEach(p => p.setAttribute('vector-effect', 'non-scaling-stroke'));
        });
        g.querySelectorAll('.boundary-line').forEach(el => {
        ...
        nodes.forEach(n => {
          ...
          // 最大の陸塊（本土）のbboxをラベル・ズーム用に求める（鹿児島・東京など離島で全体bboxが伸びる対策）
          let best = null, bestA = 0;
          n.el.querySelectorAll('path,polygon').forEach(p => {
            if (p.tagName.toLowerCase() === 'polygon') {
              // 鹿児島などは陸塊ごとに別polygon（奄美は地図右上の区切り枠内に描画済み）
              const r = p.getBoundingClientRect(); const a = r.width * r.height;
              if (a > bestA) { bestA = a; best = r; }
              return;
            }
            const subs = (p.getAttribute('d') || '').split(/(?=M)/).filter(s => s.trim());
            if (subs.length < 2) {
              const r = p.getBoundingClientRect(); const a = r.width * r.height;
              if (a > bestA) { bestA = a; best = r; }
              return;
  ...
  const leaveMuni = () => {
    // 市区町村ビューをフェードアウトさせてから破棄（全国地図のズームアウトと重ねる）
  ...
  const enterPref = (n) => {
    ...
    applyZoom(n.lbox || n.ubox, 0.9); // 県（本土側）へさらにズーム（この間に市区町村データを取得）
    ...
    Promise.all([load, new Promise(res => setTimeout(res, 720))])
      .then(([b]) => setMuniView(v => v === 'loading' ? b : v))
      .catch(() => setMuniView(v => v === 'loading' ? 'error' : v));
  };
  const backToRegion = () => {
  ...
  const handleMove = (e) => {
    ...
    setHover(region && n.region === region
      ? { type: 'pref', name: n.name, code: n.code }
      : { type: 'region', name: n.region });
  };
  ...
  const accentChip = { ...chipBase, border: '1px solid ' + ACCENT, color: ACCENT, fontWeight: 700 };

  // フリーワード検索：都道府県＋収録済市区町村＋読み込み済み県の市区町村
  const q = query.trim();
  let results = [];
  if (q) {
    ...
    Object.entries(muniCache.current).forEach(([code, b]) => {
  ...
  const zoomMuniArea = (name) => {
    if (!showMunis) return;
    const a = (muniView.areas || []).find(x => x.name === name);
    if (!a) return;
    const s = Math.min(muniView.vw / Math.max(a.w, 1), muniView.vh / Math.max(a.h, 1)) * 0.82;
    setMuniZoom({
      name,
      s,
      tx: muniView.vw / 2 - s * (a.x + a.w / 2),
      ty: muniView.vh / 2 - s * (a.y + a.h / 2),
    });
    setHover(null); armedRef.current = null;
  };
  // エリア所属で未ズームならクリックはエリアズーム
  const needsAreaZoom = (m) => m.area && (!muniZoom || muniZoom.name !== m.area);

  // 地域名（全国表示時）・都道府県名（地域ズーム時）のラベル
  let mapLabels = null;
  if (status === 'ready' && (!pref || muniLeaving) && svgRef.current) {
    ...
    if (!region) {
      REGION_DEFS.forEach(r => {
        ...
        const use = cs.length ? cs : all; // 沖縄など単独地域
      ...
      prefs.forEach(n => {
        const b = n.lbox || n.ubox; // 本土側のbboxにラベルを置く
        const rel = Math.sqrt((b.w * b.h) / maxA); // 面積比でラベルサイズを調整
  ...
  if (pref) {
    chips = [
      ...
      ...(showMunis ? muniView.munis.map(m => (
          ...
          {REGION_DEFS.map(r => (
            <div key={r.name}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {Object.entries(PREFS).filter(([c]) => c >= r.from && c <= r.to).map(([c, nm]) => (
  ...
  return (
    <div style={{ width: '100%', maxWidth: maxWidth || 820, margin: '0 auto' }}>
          ...
          <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: '#FFFFFF', border: '1px solid #DFE7EC', borderRadius: 12, boxShadow: '0 8px 24px rgba(20,24,28,0.12)', overflow: 'hidden' }}>
            ...
            {results.map((r, i) => (
              <button key={i} onClick={() => pickResult(r)}
                ...
                onMouseEnter={e => { e.currentTarget.style.background = '#F1F6F9'; }}
            ...
            <svg viewBox={`0 0 ${muniView.vw} ${muniView.vh}`} style={{ width: '100%', height: '100%', display: 'block' }}>
              ...
              {muniView.munis.map(m => (
                <path key={m.code} d={m.d} vectorEffect="non-scaling-stroke"
                  ...
                  onTouchEnd={e => {
                    e.preventDefault(); // 合成clickを抑止し2段階タップに
              ...
              {[...muniView.munis].filter(m => (muniZoom ? m.area === muniZoom.name : !m.area)).sort((a, b) => {
                const h = m => hover && hover.type === 'muni' && hover.code === m.code ? 1 : 0;
                return h(a) - h(b); // ホバー中のラベルを最前面に
              }).map(m => {
      ...
      <style>{'@keyframes jmFade { from { opacity: 0; } to { opacity: 1; } } @keyframes jmMuniIn { from { opacity: 0; transform: scale(0.82); } to { opacity: 1; transform: scale(1); } } @keyframes jmMuniOut { from { opacity: 1; transform: scale(1); } to { opacity: 0; transform: scale(0.84); } }'}</style>
    </div>
  );
}
