...
export const ROADMAP_PLAN: RoadmapItem[] = [
  {
    ...
    "status": "now",
    "why": "予算 → 執行 → 成果 の鎖の最後。甲府では公表サンプル5件しか無く全事業は情報公開請求が要ったが、川崎の572事業を収録して請求なしで鎖が閉じた。札幌（H24〜R7 の14年分）と横浜（R3〜R7・款項目つき）も全量公開しているので、同じことができる。",
    "needs": "様式が3市3様でパーサが要る（川崎=事務事業評価シート・収録済み／札幌=1事業1PDF／横浜=事業評価書）。札幌は R7 だけで666件が1事業1PDF なので、局別一覧から辿る収集器の設計も要る。評価体系も市ごとに違う（甲府=A〜F／川崎=達成度1〜5＋方向性Ⅰ〜Ⅴ）ので、丸めずに出し分ける。",
